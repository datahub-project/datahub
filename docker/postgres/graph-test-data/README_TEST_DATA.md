...
2. **Graph Schema**: Tables created via SqlSetup `sqlsetup/pggraph/migrations`
3. **Graph Functions**: Applied by the repeatable connected-components migration
...
```bash
cd docker/postgres/graph-test-data
...
1. **"Graph schema not found"**
   ...
   - Apply SqlSetup pgGraph migrations (`sqlsetup/pggraph/migrations`) first

2. **"Graph functions not found"**

   - Ensure the repeatable connected-components migration (`R__connected_components.sql`) ran
