...
function create_user() {
  ...
  curl -XPUT "$ELASTICSEARCH_PROTOCOL://$ELASTICSEARCH_HOST_URL:$ELASTICSEARCH_PORT/_opendistro/_security/api/roles/${ROLE}" -H 'Content-Type: application/json' \
    -d "{\"cluster_permissions\":[\"indices:data/write/bulk\", \"indices:data/write/reindex\", \"indices:data/read/scroll\"], \"index_permissions\":[{\"index_patterns\":[\"${INDEX_PREFIX}_*\"], \"allowed_actions\":[\"indices_all\"]}]}"
