...
if [ "$DATAHUB_EXECUTOR_MODE" = "coordinator" ] || [ -z "$DATAHUB_EXECUTOR_MODE" ]; then
    ...
    watchmedo auto-restart -d /datahub-executor/ -p '*.py' -R -- celery -- -A datahub_executor.worker.celery_sqs.app -- worker -Q ${DATAHUB_EXECUTOR_WORKER_ID} -P threads --loglevel=info
fi
