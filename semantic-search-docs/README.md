...
> **Note:** In production, embeddings are generated by the ingestion connector at ingestion time. 
> The `backfill_document_embeddings.py` script is provided for local development and testing only.

For local testing (generates embeddings for existing documents):
```bash
AWS_PROFILE=your-profile uv run python backfill_document_embeddings.py
```

### 4. Verify Semantic Search

```bash
AWS_PROFILE=your-profile uv run python test_document_semantic_search.py
```
...
These scripts are provided for **local development and testing only**. In production, embeddings are generated by the ingestion connector.

| File | Description |
|------|-------------|
| `ingest_sample_documents.py` | Ingest sample documents for testing |
| `backfill_document_embeddings.py` | Generate embeddings for test documents (dev only) |
| `test_document_semantic_search.py` | End-to-end test suite |
...
| File | Description |
|------|-------------|
| `embedding_utils.py` | Embedding provider factory and utilities |
| `embedding_cache.py` | Local file cache for embeddings |
| `embeddings_base.py` | Base interface for embedding providers |
| `litellm_embeddings.py` | LiteLLM-based embedding implementation |
