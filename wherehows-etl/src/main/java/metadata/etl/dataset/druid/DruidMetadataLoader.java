...
public class DruidMetadataLoader {
    ...
    private String LOAD_STG_DATASET_META = 		"INSERT INTO wherehows.stg_dict_dataset"
            + "(db_id, name, `schema`, schema_type, properties, fields, urn, source,"
    ...
    private String LOAD_COL_METADATA = 		"LOAD DATA LOCAL INFILE \"" + "$DRUID_COL_METADATA_CSV_FILE" + "\""
            + " INTO TABLE wherehows.druid_col_metadata_tbl "
            + "FIELDS TERMINATED BY '\\t' "
            + "LINES TERMINATED BY '\\n'"
    ...
    private String LOAD_STG_FIELD_META = 	"INSERT INTO stg_dict_field_detail"
            + "("
            + "db_id, urn, sort_id, parent_sort_id,field_name, "
            + "data_type, data_size, "
            + "is_nullable, is_indexed, is_partitioned, "
            + "namespace, last_modified"
            + ") "
            + "SELECT "
            + "$DB_ID, urn, id, 1, field_name, "
            + "data_type, data_size,"
            + "'F', 'T', 'T',"
            + "ds_name, NOW() "
            + "FROM druid_col_metadata_tbl "
            + ";\n";
    private String UPDATE_STG_FIELD_META = "UPDATE stg_dict_field_detail sf, dict_dataset d "
    ...
    private String DUMP_NON_EXIST_FIELD = "CREATE TEMPORARY TABLE IF NOT EXISTS t_deleted_fields (primary key (field_id))\n"
            ...
            + "join dict_dataset i "
            + "ON s.urn=i.urn AND s.db_id=$DB_ID "
            + "RIGHT JOIN dict_field_detail x "
            + "ON i.id=x.dataset_id AND s.field_name = x.field_name "
    ...
    private String UPDATE_FIELD_METADATA = "UPDATE dict_field_detail t join "
            + "("
            + "select x.field_id, s.*  "
            ...
            + "where s.db_id=$DB_ID "
            + "and ( "
            + "x.data_type<>s.data_type "
            + "or x.data_size<>s.data_size "
            + ") "
            + ") p "
            + "on t.field_id=p.field_id "
            + "set t.data_type=p.data_type, "
            + "t.data_size=p.data_size, "
            + "t.modified = NOW();\n";
    private String INSERT_NEW_FIELD_METADATA = 	"INSERT IGNORE INTO dict_field_detail"
            + "("
            + "dataset_id, sort_id, parent_sort_id, "
