...
public class AuthServiceClientTest {
  ...
  public void setUp() {
    ...
    systemAuthentication = new Authentication(
        systemActor,
        "Basic datahub:datahub",
  ...
  public void testConstructorWithNullParameters() {
    ...
    assertThrows(NullPointerException.class, () -> {
      new AuthServiceClient("localhost", 8080, null, false, systemAuthentication, mockHttpClient);
    });
  ...
  public void testGenerateSessionTokenForUserWithBasePath() throws Exception {
    ...
    when(mockEntity.getContent()).thenReturn(
        new ByteArrayInputStream(createTokenResponseJson(expectedToken).getBytes()));
  ...
  public void testGenerateSessionTokenForUserWithEmptyBasePath() throws Exception {
    ...
    verify(mockHttpClient).execute(argThat(request -> {
      if (request instanceof HttpPost) {
        ...
        return uri.contains("/auth/generateSessionTokenForUser") && !uri.contains("/api/v2");
      }
      return false;
    }));
  }
  ...
  public void testGenerateSessionTokenForUserWithBadResponse() throws Exception {
    ...
    when(mockEntity.getContent()).thenReturn(
        new ByteArrayInputStream("Bad Request Body".getBytes()));
    ...
    assertTrue(exception.getCause().getMessage().contains("Bad response from the Metadata Service"),
  ...
  public void testSignUpWithBasePath() throws Exception {
    ...
    when(mockResponse.getEntity()).thenReturn(mockEntity);
    when(mockEntity.getContent()).thenReturn(
        new ByteArrayInputStream(createSignUpResponseJson(true).getBytes()));

    when(mockHttpClient.execute(any(HttpPost.class))).thenReturn(mockResponse);

    // Execute the method
    boolean result = authServiceClient.signUp(userUrn, fullName, email, title, password, inviteToken);
  ...
  public void testSignUpWithNullParameters() {
    ...
    assertThrows(NullPointerException.class, () -> {
      authServiceClient.signUp(null, "Test User", "test@example.com", "Engineer", "password", "token");
    });

    assertThrows(NullPointerException.class, () -> {
      authServiceClient.signUp("urn:li:corpuser:test", null, "test@example.com", "Engineer", "password", "token");
    });

    assertThrows(NullPointerException.class, () -> {
      authServiceClient.signUp("urn:li:corpuser:test", "Test User", null, "Engineer", "password", "token");
    });

    assertThrows(NullPointerException.class, () -> {
      authServiceClient.signUp("urn:li:corpuser:test", "Test User", "test@example.com", null, "password", "token");
    });

    assertThrows(NullPointerException.class, () -> {
      authServiceClient.signUp("urn:li:corpuser:test", "Test User", "test@example.com", "Engineer", null, "token");
    });

    assertThrows(NullPointerException.class, () -> {
      authServiceClient.signUp("urn:li:corpuser:test", "Test User", "test@example.com", "Engineer", "password", null);
    });
  }
  ...
  public void testResetNativeUserCredentialsWithBasePath() throws Exception {
    ...
    when(mockResponse.getEntity()).thenReturn(mockEntity);
    when(mockEntity.getContent()).thenReturn(
        new ByteArrayInputStream(createResetCredentialsResponseJson(true).getBytes()));
  ...
  public void testVerifyNativeUserCredentialsWithBasePath() throws Exception {
    ...
    when(mockResponse.getEntity()).thenReturn(mockEntity);
    when(mockEntity.getContent()).thenReturn(
        new ByteArrayInputStream(createVerifyCredentialsResponseJson(true).getBytes()));
  ...
  public void testJsonParsingWithInvalidJson() throws Exception {
    ...
    assertTrue(exception.getCause().getMessage().contains("Failed to parse JSON received from the MetadataService"));
  }
  ...
  public void testJsonParsingWithMissingField() throws Exception {
    ...
    when(mockResponse.getEntity()).thenReturn(mockEntity);
    when(mockEntity.getContent()).thenReturn(
        new ByteArrayInputStream("{\"userId\":\"testuser\"}".getBytes()));
