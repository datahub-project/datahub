...
function new_url(original: string, filepath: string): string {
  ...
  if (
    ...
    if (
      ...
      // Machine-generated pages (docgen, modelDocGen) legitimately contain
      // site-absolute links like /docs/managed-datahub/observe/assertions and
      // relative doc links like ../hive. This branch assumes any extension-less
      // link is a path into the git repo, which is not true for them -- it
      // prepends the page's own directory and produces nonsense such as
      // sources/docs/managed-datahub/... These pages were never reached by this
      // check before, because `type: autogenerated` kept their doc ids out of
      // the sidebar; grouping them by name exposed it. Link correctness inside
      // the generated tree is docgen's responsibility, not this script's.
      !filepath.startsWith("docs/generated/") &&
