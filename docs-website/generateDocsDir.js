...
const GITHUB_EDIT_URL = "https://github.com/datahub-project/datahub/blob/master";
const GITHUB_BROWSE_URL = "https://github.com/datahub-project/datahub/blob/master";
...
const MyOctokit = rest_1.Octokit.plugin(plugin_retry_1.retry).plugin(plugin_throttling_1.throttling);
...
function actually_in_sidebar(filepath) {
    const doc_id = get_id(filepath);
    return sidebars_json.indexOf(`"${doc_id}"`) >= 0;
}
function accounted_for_in_sidebar(filepath) {
...
function list_markdown_files() {
    let all_markdown_files = (0, child_process_1.execSync)("git ls-files --full-name .. | grep '.md$'")
        .toString()
        .trim()
        .split("\n");
    let all_generated_markdown_files = (0, child_process_1.execSync)("cd .. && ls docs/generated/**/**/*.md && ls docs/generated/**/*.md")
        .toString()
        .trim()
        .split("\n");
    ...
    if (!process.env.CI) {
        ...
        const untracked_files = (0, child_process_1.execSync)("(git ls-files --full-name --others --exclude-standard .. | grep '.md$') || true")
            .toString()
            .trim()
            .split("\n")
        ...
        const deleted_files = (0, child_process_1.execSync)("(git ls-files --full-name --deleted --exclude-standard .. | grep '.md$') || true")
            .toString()
            .trim()
            .split("\n");
        if (deleted_files.length > 0) {
            console.log(`Removing deleted files from docs list: [${deleted_files}]`);
            all_markdown_files = all_markdown_files.filter((filepath) => !deleted_files.includes(filepath));
        }
    }
...
const hardcoded_descriptions = {
    ...
    "README.md": "DataHub is a data discovery application built on an extensible metadata platform that helps you tame the complexity of diverse data ecosystems.",
};
...
function markdown_guess_title(contents, filepath) {
    ...
    if (filepath in hardcoded_titles) {
        ...
        if (filepath in hardcoded_descriptions) {
            contents.data.description =
                hardcoded_descriptions[filepath];
        }
        ...
        if (!headers) {
            throw new Error(`${filepath} must have at least one h1 header for setting the title`);
        }
...
function markdown_add_edit_url(contents, filepath) {
    const editUrl = `${GITHUB_EDIT_URL}/${filepath}`;
    contents.data.custom_edit_url = editUrl;
}
function markdown_add_slug(contents, filepath) {
...
function trim_anchor_link(url) {
    return url.replace(/#.+$/, "");
}
function new_url(original, filepath) {
    ...
    if (suffix == "" ||
        ...
        if (!fs.existsSync(`../${check_path}`) &&
            ...
            // checked out repo.
            throw new Error(`broken github repo link to ${updated_path} in ${filepath}`);
        }
        ...
        const relation = path.dirname(filepath);
        const updated = path.normalize(`${"../".repeat(up_levels + 2)}/${relation}/${original}`);
...
function markdown_rewrite_urls(contents, filepath) {
    ...
    /^\[([^^\n\r]+?)\]\s*:\s*(.+?)\s*$/gm, (_, text, url) => {
        const updated = new_url(preprocess_url(url), filepath);
        return `[${text}]: ${updated}`;
    });
    contents.content = new_content;
}
function markdown_enable_specials(contents, filepath) {
    const new_content = contents.content
        .replace(/^<!--HOSTED_DOCS_ONLY$/gm, "")
        .replace(/^HOSTED_DOCS_ONLY-->$/gm, "");
    contents.content = new_content;
}
function markdown_process_inline_directives(contents, filepath) {
    const new_content = contents.content.replace(/^(\s*){{(\s*)inline\s+(\S+)(\s+)(show_path_as_comment\s+)?(\s*)}}$/gm, (_, indent, __, inline_file_path, ___, show_path_as_comment) => {
...
function markdown_process_command_output(contents, filepath) {
    const new_content = contents.content.replace(/^{{\s*command-output\s*([\s\S]*?)\s*}}$/gm, (_, command) => {
        try {
...
function markdown_sanitize_and_linkify(content) {
    // MDX escaping
    content = content.replace(/</g, "&lt;");
    // Link to user profiles.
    content = content.replace(/@([\w-]+)\b/g, "[@$1](https://github.com/$1)");
    // Link to issues/pull requests.
    content = content.replace(/#(\d+)\b/g, "[#$1](https://github.com/datahub-project/datahub/pull/$1)");
    // Prettify bare links to PRs.
    content = content.replace(/(\s+)(https:\/\/github\.com\/linkedin\/datahub\/pull\/(\d+))(\s+|$)/g, "$1[#$3]($2)$4");
    return content;
}
function pretty_format_date(datetime) {
    const d = new Date(Date.parse(datetime));
    return d.toISOString().split("T")[0];
}
function make_link_anchor(text) {
    return text.replace(/\./g, "-");
}
async function generate_releases_markdown() {
    const contents = (0, gray_matter_1.default)(`---
...
## Summary\n\n`);
    ...
    const releases_list = releases_list_full.data.filter((release) => !release.prerelease && !release.draft);
    ...
    for (const release of releases_list) {
        ...
        let row = `| **${release.tag_name}** | ${pretty_format_date(release.created_at)} |`;
    ...
    for (const release of releases_list) {
        ...
        if (release.tag_name === "v1.1.0") {
            ...
            const max_heading_level = Math.min(3, ...[...body.matchAll(heading_regex)].map((v) => v[1].length));
            body = body.replace(heading_regex, `${"#".repeat(3 - max_heading_level)}$1 `);
        }
        else {
            // Link to GitHub.
            body = `View the [release notes](${release.html_url}) for ${release.name} on GitHub.`;
        }
        const info = `## [${release.name}](${release.html_url}) {#${make_link_anchor(release.tag_name)}}

Released on ${pretty_format_date(release.created_at)} by [@${release.author.login}](${release.author.html_url}).
...
function write_markdown_file(contents, output_filepath) {
...
(async function main() {
    for (const filepath of markdown_files) {
        ...
        const contents = (0, gray_matter_1.default)(contents_string);
    ...
    for (const filepath of markdown_files) {
        if (autogenerated_sidebar_directories.some((dir) => filepath.startsWith(dir))) {
        ...
        if (!accounted_for_in_sidebar(filepath)) {
            console.warn(`File not accounted for in sidebar: ${filepath} - consider adding it to docs-website/sidebars.js or explicitly ignoring it`);
        }
    }
