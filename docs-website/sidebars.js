...
const SOURCE_CATEGORY_MANIFEST_PATH = path.join(
  __dirname,
  "..",
  "docs",
  "generated",
...
function getPushIntegrationsSidebarCategory() {
  return {
    type: "category",
    label: "Orchestration",
    collapsed: true,
...
// Canonical category order — must stay in sync with:
//   1. IntegrationsEmbed CATEGORY_ORDER (docs-website/src/components/IntegrationsEmbed)
//   2. Frontend Create Source flow (ingestV2/source/multiStepBuilder/steps/step1SelectSource/utils.ts)
// Sources within each category are sorted alphabetically by docgen.py's
// generate_source_category_manifest(). Do not manually reorder entries.
...
function getSourcesSidebarItems() {
  // Build generated category items first
  let generatedItems = [];

  if (!fs.existsSync(SOURCE_CATEGORY_MANIFEST_PATH)) {
    console.warn(
      `Missing source category manifest at ${SOURCE_CATEGORY_MANIFEST_PATH}. Falling back to autogenerated source listing.`
    );
    return [
      getPushIntegrationsSidebarCategory(),
  ...
  try {
    const manifest = JSON.parse(
      fs.readFileSync(SOURCE_CATEGORY_MANIFEST_PATH, "utf-8")
    );
    const categories = Array.isArray(manifest.categories)
      ? manifest.categories
      : [];

    // Sort categories by canonical order; unknown categories go to the end alphabetically
    const sorted = [...categories].sort((a, b) => {
      const ai = SIDEBAR_CATEGORY_ORDER.indexOf(a.category);
      const bi = SIDEBAR_CATEGORY_ORDER.indexOf(b.category);
      const aIdx = ai === -1 ? SIDEBAR_CATEGORY_ORDER.length : ai;
      const bIdx = bi === -1 ? SIDEBAR_CATEGORY_ORDER.length : bi;
    ...
    generatedItems = sorted.map((category) => ({
      ...
      items: (category.sources || []).map((source) => ({
        type: "doc",
        id: `docs/generated/ingestion/sources/${source.platform_id}`,
        label: source.platform_name,
      })),
    }));
  } catch (e) {
    console.warn(
      `Failed to parse source category manifest. Falling back to autogenerated source listing.`,
      e
    );
    return [
      getPushIntegrationsSidebarCategory(),
  ...
  // Inject Orchestration (push integrations) after ML Platforms
  const mlIdx = generatedItems.findIndex((c) => c.label === "ML Platforms");
  const insertAt = mlIdx >= 0 ? mlIdx + 1 : generatedItems.length;
  const result = [...generatedItems];
  result.splice(insertAt, 0, getPushIntegrationsSidebarCategory());
  return result;
}
