...
function extractDataHubColors() {
  try {
    const colorsFile = fs.readFileSync(
      path.resolve(__dirname, DATAHUB_COLORS_PATH),
      "utf8",
    );
...
function extractSemanticTokens() {
  try {
    const semanticFile = fs.readFileSync(
      path.resolve(__dirname, DATAHUB_SEMANTIC_TOKENS_PATH),
      "utf8",
    );

    // Extract semantic token mappings
    const tokenMatches =
      semanticFile.match(/'([^']+)':\s*colors\.([^,\s]+)/g) || [];
    const tokens = {};

    tokenMatches.forEach((match) => {
      const [, tokenName, colorPath] = match.match(
        /'([^']+)':\s*colors\.([^,\s]+)/,
      );
...
function generateCSSVariables(colors, semanticTokens) {
  ...
  const colorMappings = {
    "datahub-primary":
      colors.primary?.[500] || colors.violet?.[500] || "#533FD1",
    "datahub-primary-dark":
      colors.primary?.[600] || colors.violet?.[600] || "#4C39BE",
    "datahub-primary-light":
      colors.primary?.[400] || colors.violet?.[400] || "#7565DA",
    "datahub-primary-lightest":
      colors.primary?.[0] || colors.violet?.[0] || "#F1F3FD",
  ...
  const darkMappings = {
    ...
    "datahub-primary-dark":
      colors.primary?.[500] || colors.violet?.[500] || "#533FD1",
    "datahub-primary-light":
      colors.primary?.[300] || colors.violet?.[300] || "#8C7EE0",
    "datahub-primary-lightest":
      colors.primary?.[800] || colors.violet?.[800] || "#2E2373",
...
function updateComponentStyles(cssVariables) {
  ...
  componentDirs.forEach((componentDir) => {
    ...
    try {
      ...
      const variableRegex =
        /\/\* Auto-generated DataHub Design Tokens \*\/[\s\S]*?}\s*\n\s*\n/;
...
function main() {
  ...
  if (!colors) {
    ...
    console.log(
      "   Using fallback design tokens to ensure build continues...\n",
    );
    ...
    console.log(
      "✅ Applied fallback styling - components will use default DataHub colors",
    );
    return;
  }
  ...
  console.log(
    `   - Semantic tokens: ${semanticTokens ? Object.keys(semanticTokens).length : 0} mappings\n`,
  );
  ...
  console.log(
    "   Tutorial components now match the latest DataHub UI styling.",
  );
}
