...
const FeatureAvailability = ({ saasOnly, ossOnly }) => (
  <div className={clsx(styles.availabilityCard, "card")}>
    ...
    <div>
      <span
        className={clsx(styles.platform, !saasOnly && styles.platformAvailable)}
      >
        Self-Hosted DataHub{" "}
        {saasOnly ? <CloseCircleFilled /> : <CheckCircleFilled />}
      </span>
    </div>
    <div>
      <CloudOutlined className={styles.managedIcon} />
      <span
        className={clsx(styles.platform, !ossOnly && styles.platformAvailable)}
      >
