...
const ProcessFlow = ({ 
  ...
  const renderStep = (step, index) => (
    <div key={index} className={`${styles.step} ${animated ? styles.animated : ''}`}>
      {showNumbers && (
        ...
        {step.details && (
          <div className={styles.stepDetails}>
            {step.details.map((detail, i) => (
              <div key={i} className={styles.stepDetail}>• {detail}</div>
            ))}
          </div>
        )}
      </div>
  ...
  return (
    <div className={`${styles.processFlow} ${styles[type]} ${hasOverflow ? styles.hasOverflow : ''}`}>
    ...
    steps: [
      {
        ...
        details: ["Identify data needs", "Set success criteria", "Define scope"]
      },
      {
      ...
      },
      {
        ...
        details: ["Check permissions", "Review documentation", "Plan integration"]
      }
    ]
  },
    ...
    steps: [
      {
        ...
        details: ["Configure credentials", "Test connectivity", "Set up authentication"]
      },
      {
        title: "Discovery",
      ...
      },
      {
        ...
        details: ["Apply business rules", "Enrich with context", "Validate quality"]
      },
      {
        title: "Loading",
        description: "Store in DataHub",
        details: ["Update knowledge graph", "Index for search", "Enable discovery"]
      }
    ]
  }
};
