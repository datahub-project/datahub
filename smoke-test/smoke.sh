...
if [[ "${TEST_STRATEGY}" == "pytests" ]]; then
  ...
  pytest -rP --durations=20 -vv --continue-on-collection-errors --reruns 1 --reruns-delay 1 --junit-xml=junit.smoke-pytests.xml -k 'not test_run_cypress' -m  'not release_tests' --splits ${BATCH_COUNT} --group $((BATCH_NUMBER + 1)) --durations-path=./pytest-durations.json
