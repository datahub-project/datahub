...
if [[ "${TEST_STRATEGY}" == "pytests" ]]; then
  ...
  # Batching is now handled by conftest.py using module-level weighted bin-packing
  pytest -rP --durations=20 -vv --continue-on-collection-errors --reruns 1 --reruns-delay 1 --junit-xml=junit.smoke-pytests.xml -k 'not test_run_cypress' -m  'not release_tests'
