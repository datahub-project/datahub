...
from datahub.emitter.mce_builder import (make_dataset_urn, make_tag_urn,
                                         make_term_urn, make_user_urn, make_data_job_urn)
...
def test_datajob_ownership_patch(wait_for_healthchecks):
    ...
    with DataHubGraph(DataHubGraphConfig()) as graph:
        ...
        for patch_mcp in (
            DataJobPatchBuilder(datajob_urn).add_owner(owner_to_add).build()
        ):
        ...
        for patch_mcp in (
            DataJobPatchBuilder(datajob_urn).remove_owner(owner_to_add.owner).build()
        ):
            graph.emit_mcp(patch_mcp)
