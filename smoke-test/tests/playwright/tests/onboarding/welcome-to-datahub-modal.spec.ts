...
test.describe("WelcomeToDataHubModal", () => {
  test.beforeEach(async ({ page }: { page: Page }) => {
    ...
    await clearLocalStorage(page, SKIP_WELCOME_MODAL_KEY);
    await skipOnboardingTour(page);
    await skipIntroducePage(page);
    await enableThemeV2(page);
  });

  test("should display the modal for first-time users", async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test("should handle user interactions and track events", async ({
    page,
  }: {
    page: Page;
  }) => {
