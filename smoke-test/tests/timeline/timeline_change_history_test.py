...
# Constants
# ---------------------------------------------------------------------------
# Tags referenced in tests (not run-scoped)
TAG_PII = "urn:li:tag:PII"
TAG_CONFIDENTIAL = "urn:li:tag:Confidential"

# Run-scoped entity URNs, (re)generated by _init_unique_urns() at import and at
# the start of every setup_entities run.
UNIQUE: str
DATASET_URN: str
...
def _init_unique_urns() -> None:
    """(Re)generate all run-scoped URNs under a fresh uuid suffix.

    A structured property's Elasticsearch field mapping survives hard-delete
    (mappings are additive; removal needs a reindex), and its assignment cleanup
    runs asynchronously via PropertyDefinitionDeleteSideEffect. Reusing the same
    qualifiedName across a create/delete/recreate cycle in one process therefore
    collides on the orphaned field (and could race the cleanup). Regenerating the
    suffix per setup gives each run a brand-new property, avoiding reuse.
    """
...
def setup_entities(graph_client):
    ...
    # Fresh run-scoped URNs so this setup never reuses a prior run's (deleted)
    # structured-property qualifiedName — see _init_unique_urns().
    _init_unique_urns()
