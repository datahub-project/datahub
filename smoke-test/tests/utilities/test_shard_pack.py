...
def test_long_parallel_module_not_packed_with_long_serial_mutator() -> None:
    ...
    modules = [
        ModuleShard("tests/entity_graph_cache/test_entity_graph_cache.py", 833.0, 0.0),
        ModuleShard(
            "tests/authorization/test_domain_scoped_create_entity_auth.py",
            0.0,
            536.0,
        ),
        *[ModuleShard(f"tests/filler_{i}.py", 40.0, 0.0) for i in range(20)],
    ]
    plans = pack_module_plans(modules, batch_count=7, xdist_workers=3)
    parallel_batch = _batch_of(
        plans, "tests/entity_graph_cache/test_entity_graph_cache.py"
    )
    serial_batch = _batch_of(
        plans, "tests/authorization/test_domain_scoped_create_entity_auth.py"
    )
