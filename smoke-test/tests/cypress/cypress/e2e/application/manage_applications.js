describe("manage applications", () => {
  ...
  it("Manage Applications Page - Verify search not exists", () => {
    cy.login();
    cy.visit("/applications");
    cy.get('[data-testid="page-title"]').should("contain.text", "Manage Applications");
    
    cy.get('[data-testid="application-search-input"]', { timeout: 10000 }).should('be.visible');
    ...
    cy.get('[id="application-search-input"]').type("testtestnomatch", { force: true });
  ...
  it("Manage Applications Page - Verify create and delete", () => {
    ...
    cy.get('[data-testid="application-description-input"]').type("test new description");
