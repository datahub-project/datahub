...
describe("siblings", () => {
  it.skip("will merge metadata to non-primary sibling", () => {
  ...
  it.skip("will merge metadata to primary sibling", () => {
  ...
  it.skip("can mutate at individual node or combined node level", () => {
    cy.visitWithLogin(`/dataset/${DBT_URN}/?is_lineage_mode=false`);
    cy.get(".ant-table-row").should("be.visible");
    // navigate to the bq entity
    cy.get(`[data-testid="compact-entity-link-${BIGQUERY_URN}"`).click();
  ...
  it.skip("will combine results in search", () => {
  ...
  it.skip("will combine results in lineage", () => {
    cy.login();
    cy.visit(
  ...
  it.skip("can separate results in lineage if flag is set", () => {
    cy.login();
    cy.visit(
      "dataset/urn:li:dataset:(urn:li:dataPlatform:dbt,cypress_project.jaffle_shop.stg_orders,PROD)/?is_lineage_mode=true",
    );
