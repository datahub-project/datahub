...
class KillSwitchSweepPhase(Phase):
    ...
    def run(self, ctx: TestContext) -> PhaseResult:  # noqa: C901
        ...
        # the write path held on the OLD image — seed, sweep, kill and resume
        # alike. The window has to span the sweep, not just the seed: restoring
        # to NEW hands the fresh fixture to an armed mutator chain that drains
        # it in seconds. The consumers are passed explicitly because they only
        # live inside the GMS process on the embedded profiles — on a split
        # topology a GMS-only swap leaves them armed. See old_image_window.
