"""Suite A sweep-invariant validators (TC-324..031).

These scenarios share the non-blocking sweep phase with Suite A's per-URN
aspect-migration scenarios, so they live in Suite A. Their
...
_VALIDATORS: dict[int, Callable[[ZDUTestScenario, TestContext], ValidationResult]] = {
    ...
    # were folded into Suite A's range because both groups exercise the
