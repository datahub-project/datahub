...
@with_test_retry()
def test_marketplace_flag_exposed(graph_client):
    result = graph_client.execute_graphql(
        "{ appConfig { featureFlags { dataProductMarketplaceEnabled } } }"
    )
    assert "appConfig" in result
    # Flag may be on or off depending on env; just ensure the field resolves after deploy.
    assert isinstance(
        result["appConfig"]["featureFlags"]["dataProductMarketplaceEnabled"], bool
    )
