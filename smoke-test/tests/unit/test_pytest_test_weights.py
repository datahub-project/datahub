...
CLASS_JUNIT_KEY = (
    "tests.e2e.authorization.test_domain_scoped_create_entity_auth"
    f".TestDomainScopedCreateEntityAuth::{PARAM_NAME}"
)
...
def test_missing_id_uses_default() -> None:
    weight, used_default = lookup_test_weight(
        "tests/e2e/foo.py::TestBar::test_new", {}, DEFAULT
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("tests/e2e/foo.py::test_it", "tests/e2e/foo.py"),
        ("tests/e2e/foo.py::test_it[param]", "tests/e2e/foo.py"),
        ("tests/e2e/foo.py::TestCls::test_it", "tests/e2e/foo.py::TestCls"),
    ],
)
