...
from tests.e2e.utils import ingest_file_via_rest, wait_for_writes_to_sync
...
def test_setup(auth_session, graph_client):
    ...
    ingested_dataset_run_id = ingest_file_via_rest(
        auth_session, "tests/e2e/cli/cli_test_data.json"
    ).config.run_id
...
def test_rollback_editable(auth_session, graph_client):
    ...
    ingested_editable_run_id = ingest_file_via_rest(
        auth_session, "tests/e2e/cli/cli_editable_test_data.json"
    ).config.run_id
