...
from utilities.file_emitter import FileEmitter
...
def test_structured_properties_yaml_load_with_bad_entity_type(
    ...
    with pytest.raises(
        ...
        StructuredProperties.create(
            "tests/e2e/structured_properties/bad_entity_type.yaml",
            graph=graph_client,
        )
...
def test_dataset_yaml_loader(ingest_cleanup_data, graph_client):
    StructuredProperties.create(
        "tests/e2e/structured_properties/test_structured_properties.yaml",
        graph=graph_client,
    )

    for dataset in Dataset.from_yaml(
        "tests/e2e/structured_properties/test_dataset.yaml"
    ):
...
def test_structured_properties_list(ingest_cleanup_data, graph_client, caplog):
    ...
    # Hydrate only URNs created above; global list() fails on orphan properties from other tests.e2e.
