...
from tests.e2e.privileges.utils import assign_user_to_group, remove_group
from tests.e2e.utils import execute_graphql, get_frontend_url
...
def _create_group_with_id(auth_session, group_id: str) -> str:
    # create_group() in tests.e2e.privileges.utils omits `id`, so the resolver mints a random
