...
from tests.e2e.entity_graph_cache.helpers import (
...
class TestDomain:
    @pytest.mark.p0
    def test_domain_parent_domains_hierarchy(self, auth_session, graph_client):
    ...
    def test_domain_sync_move_reflects_in_parent_domains(
    ...
    def test_domain_cache_metrics_when_isolated(self, auth_session, graph_client):
        ...
        try:
            ...
            counter_before = prometheus_counter_total(
                metrics_before, METRIC_SEARCH_SCROLL
            )
...
class TestGlossary:
    @pytest.mark.p0
    def test_glossary_parent_nodes_hierarchy(self, auth_session, graph_client):
    ...
    def test_glossary_sync_reparent_reflects_in_parent_nodes(
        self, auth_session, graph_client
    ):
    ...
    def test_glossary_deep_hierarchy_within_bundled_max_depth(
        self, auth_session, graph_client
    ):
    ...
    def test_glossary_cache_metrics_when_isolated(self, auth_session, graph_client):
        if not prometheus_isolated(auth_session):
            pytest.skip(
                "Prometheus cache metrics require isolated run (management URL resolvable "
                "from auth_session, BATCH_COUNT=1, no pytest-xdist worker)"
...
class TestContainer:
    @pytest.mark.p0
    def test_container_parent_containers_hierarchy(self, auth_session, graph_client):
        ...
        try:
            ...
            parent_urns = query_parent_container_urns_on_container(
                auth_session, child_urn
            )
            assert len(parent_urns) >= 2
            assert parent_urns[0] == parent_urn
            assert grandparent_urn in parent_urns
    ...
    @pytest.mark.p0
    def test_container_relationships_direct_children(self, auth_session, graph_client):
        ...
        try:
            ...
            child_urns = wait_for_container_child(
                auth_session, grandparent_urn, parent_urn
            )
...
class TestMembership:
    ...
    def test_add_group_members_adds_every_user_in_one_mutation(
        ...
        first user would still report success. Guards the batched add path generally;
        it is not a reproduction of a bug fixed here.
        """
    ...
    def test_migration_converts_existing_members_when_group_becomes_native(
        ...
        The first addGroupMembers runs the inline migration before adding the requested
        user. The pre-existing legacy member must survive it and end up labeled native.
        Guards the migration end-to-end against the reordered steps dropping a member;
        the reordering's own failure mode needs an interrupted request to observe.
        """
        ...
        try:
            ...
            # Converted, not merely left on the legacy aspect.
            wait_for_session_group_membership_labels(
    ...
    def test_remove_group_members_revokes_legacy_membership(
        ...
        directly on a member holding only groupMembership - the state a member is left in when
        an earlier migration did not reach them. Stripping just nativeGroupMembership writes
        nothing and still reports success, leaving the user in the group.

        The group must NOT be origin-less here: the resolver migrates before removing, which
        would convert the member to native first and never exercise the legacy branch.
        """
        ...
        try:
            group_urn = create_native_group(auth_session, f"egc-legacy-rm-{run_id}")
            add_corp_group_membership(graph_client, user_urn, group_urn)
            wait_for_hierarchy_writes()

            # Assert the membership is live first, so the post-removal check cannot pass vacuously.
