...
from utilities.metadata_operations import get_prometheus_metrics
...
def generate_graphql_read_traffic(auth_session, *, repeat: int = 3) -> None:
    from tests.e2e.utils import execute_graphql
...
def configured_admin_urn() -> str:
    ...
    from tests.e2e.utils import get_admin_username
...
def try_mint_personal_access_token(
    ...
    from tests.e2e.utils import execute_graphql, unique_suffix
...
def make_support_actor_user(admin_session, name: str):
    ...
    from tests.e2e.utils import TestSessionWrapper, get_admin_credentials, login_as
    from utilities.multi_user import make_step_actor_user
