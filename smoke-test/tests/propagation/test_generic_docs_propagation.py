...
def create_test_action(
    ...
    recipe_file = test_resources_dir / "docs_propagation_generic_action_recipe.yaml"
    ...
    for expectation in scenario.base_expectations:
        ...
        if expectation.propagation_found:
            try:
                ...
                # Debug logging for document propagation
                print(f"[DOCS DEBUG] Entity: {expectation.schema_field_urn}")
                print(f"[DOCS DEBUG] Expected origin: {expectation.propagation_origin}")
                print(
                    f"[DOCS DEBUG] Actual origin: {first_element.attribution.sourceDetail.get('origin')}"
                )
                print(f"[DOCS DEBUG] Expected via: {expectation.propagation_via}")
                print(
                    f"[DOCS DEBUG] Actual via: {first_element.attribution.sourceDetail.get('via')}"
                )
                print(
                    f"[DOCS DEBUG] Full sourceDetail: {first_element.attribution.sourceDetail}"
                )
                ...
                logger.debug(
                    f"Failed expectation: {expectation} with error: {e} actual was {documentation_aspect}"
                )
                # breakpoint()
                raise
        else:
        ...
        for expectation in scenario.post_mutation_expectations:
            ...
            if expectation.propagation_found:
                try:
                    ...
                    # Debug logging for document propagation live
                    print(f"[DOCS LIVE DEBUG] Entity: {expectation.schema_field_urn}")
                    print(
                        f"[DOCS LIVE DEBUG] Expected origin: {expectation.propagation_origin}"
                    )
                    print(
                        f"[DOCS LIVE DEBUG] Actual origin: {first_element.attribution.sourceDetail.get('origin')}"
                    )
                    print(
                        f"[DOCS LIVE DEBUG] Expected via: {expectation.propagation_via}"
                    )
                    print(
                        f"[DOCS LIVE DEBUG] Actual via: {first_element.attribution.sourceDetail.get('via')}"
                    )
                    print(
                        f"[DOCS LIVE DEBUG] Full sourceDetail: {first_element.attribution.sourceDetail}"
                    )
