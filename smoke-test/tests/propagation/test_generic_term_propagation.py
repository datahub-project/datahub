...
def check_attribution(
    ...
    if expectation.propagation_found:
        ...
        print("\n[TERM DEBUG] =====================================")
        print(f"[TERM DEBUG] ENTITY: {expectation.schema_field_urn}")
        print("[TERM DEBUG] EXPECTED PROPERTIES:")
        print(f"[TERM DEBUG]   - propagation_found: {expectation.propagation_found}")
        print(f"[TERM DEBUG]   - propagation_source: {expectation.propagation_source}")
        print(f"[TERM DEBUG]   - propagation_via: {expectation.propagation_via}")
        print(f"[TERM DEBUG]   - propagation_origin: {expectation.propagation_origin}")
        print("[TERM DEBUG] ACTUAL PROPERTIES:")
        print(f"[TERM DEBUG]   - attribution.source: {attribution.source}")
        print(
            f"[TERM DEBUG]   - attribution.sourceDetail.origin: {attribution.sourceDetail.get('origin')}"
        )
        print(
            f"[TERM DEBUG]   - attribution.sourceDetail.via: {attribution.sourceDetail.get('via')}"
        )
        print(
            f"[TERM DEBUG]   - attribution.sourceDetail.propagation_depth: {attribution.sourceDetail.get('propagation_depth')}"
        )
        print(
            f"[TERM DEBUG]   - attribution.sourceDetail.propagation_direction: {attribution.sourceDetail.get('propagation_direction')}"
        )
        print(
            f"[TERM DEBUG]   - attribution.sourceDetail.propagated: {attribution.sourceDetail.get('propagated')}"
        )
        print("[TERM DEBUG] FULL ATTRIBUTION:")
        print(f"[TERM DEBUG]   - Full sourceDetail: {attribution.sourceDetail}")
        print("[TERM DEBUG] =====================================")
...
def check_expectation(
    ...
    if isinstance(expectation, TermPropagationExpectation):
        ...
        if editable_schema_metadata_aspect:
            ...
            if field_info:
                assert field_info.glossaryTerms
                ...
                print(f"[DEBUG] All terms on field: {[x.urn for x in terms]}")
                print(
                    f"[DEBUG] Terms propagated from action: {[x.urn for x in terms_propagated_from_action]}"
                )
                print(f"[DEBUG] Expected term: {expectation.propagated_term}")
                if terms_propagated_from_action:
                    if not rollback:
                        ...
                        assert (
                            term_propagated_from_action.urn
                            == expectation.propagated_term
                        )
                        check_attribution(
                            action_urn,
