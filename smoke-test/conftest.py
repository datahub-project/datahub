...
@pytest.fixture(scope="session")
def admin_privileges_ready(auth_session):
    """
    Wait for DataHub bootstrap to complete and admin privileges to be available.
    
    This fixture ensures the system is fully initialized before running privilege-dependent tests.
    It waits up to 60 seconds for the admin user to have the required privileges (managePolicies, manageIdentities).
    
    Returns:
        dict: User info with privileges once ready
        
    Raises:
        TimeoutError: If admin privileges aren't ready within 60 seconds
    """
    import time
    import sys
    from tests.utils import get_admin_credentials, login_as
    from tests.privileges.utils import get_current_user_info
    
    print("🔄 Waiting for DataHub admin privileges to be ready...", file=sys.stderr)
    ...
    while time.time() - start_time < timeout_seconds:
        ...
        try:
            ...
            if user_info:
                has_manage_policies = user_info['privileges'].get('managePolicies', False)
                has_manage_identities = user_info['privileges'].get('manageIdentities', False)
                
                if has_manage_policies and has_manage_identities:
                    print(f"✅ Admin privileges ready after {elapsed:.1f} seconds (attempt {attempt})", file=sys.stderr)
                    print(f"   User: {user_info['username']} ({user_info['email']})", file=sys.stderr)
                    print(f"   Key privileges: managePolicies={has_manage_policies}, manageIdentities={has_manage_identities}", file=sys.stderr)
                    return user_info
                else:
                    print(f"⏳ Admin privileges not ready yet after {elapsed:.1f}s (attempt {attempt})", file=sys.stderr)
                    print(f"   managePolicies: {has_manage_policies}, manageIdentities: {has_manage_identities}", file=sys.stderr)
            else:
                print(f"⏳ Could not retrieve user info after {elapsed:.1f}s (attempt {attempt})", file=sys.stderr)
                
        except Exception as e:
            print(f"⏳ Error checking admin privileges after {elapsed:.1f}s (attempt {attempt}): {e}", file=sys.stderr)
    ...
    raise TimeoutError(
        f"❌ Timeout after {final_elapsed:.1f} seconds waiting for admin privileges to be ready. "
        f"This suggests DataHub bootstrap process is taking longer than expected or failed to complete. "
        f"Check DataHub logs for bootstrap errors."
    )
