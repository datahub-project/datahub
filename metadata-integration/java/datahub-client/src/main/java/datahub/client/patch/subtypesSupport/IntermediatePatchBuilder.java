...
/**
 * Used for supporting intermediate subtypes when constructing a patch for an aspect that includes complex objects.
...
public interface IntermediatePatchBuilder<T extends AbstractMultiFieldPatchBuilder<T>> {
  ...
  /**
   * Exposes subpath values to parent patch builder in Op, Path, Value triples. Should
   * usually only be called by the parent patch builder class when constructing the path values.
   */
