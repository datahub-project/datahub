...
public class UpstreamLineagePatchBuilder extends AbstractMultiFieldPatchBuilder<UpstreamLineagePatchBuilder> {
   ...
   * @param confidenceScore optional, confidence score for the lineage edge. Defaults to 1.0 for full confidence
   * @param transformationOperation string operation type that describes the transformation operation happening in the lineage edge
   * @return this builder
   */
  public UpstreamLineagePatchBuilder addFineGrainedUpstreamDataset(@Nonnull DatasetUrn datasetUrn, @Nullable Float confidenceScore,
      @Nonnull String transformationOperation) {
    Float finalConfidenceScore = getConfidenceScoreOrDefault(confidenceScore);

    pathValues.add(ImmutableTriple.of(PatchOperationType.ADD.getValue(), FINE_GRAINED_PATH_START
        + transformationOperation + "/" +  "upstreamType" + "/" +  "DATASET" + "/" + datasetUrn,
   ...
   * @param schemaFieldUrn a schema field to be marked as upstream, format: urn:li:schemaField(DATASET_URN, COLUMN NAME)
   * @param confidenceScore optional, confidence score for the lineage edge. Defaults to 1.0 for full confidence
   * @param transformationOperation string operation type that describes the transformation operation happening in the lineage edge
   * @param type the upstream lineage type, either Field or Field Set
   * @return this builder
   */
  public UpstreamLineagePatchBuilder addFineGrainedUpstreamField(@Nonnull Urn schemaFieldUrn,
      @Nullable Float confidenceScore, @Nonnull String transformationOperation, @Nullable FineGrainedLineageUpstreamType type) {
    Float finalConfidenceScore = getConfidenceScoreOrDefault(confidenceScore);
    String finalType;
    if (type == null) {
    ...
    pathValues.add(ImmutableTriple.of(PatchOperationType.ADD.getValue(),FINE_GRAINED_PATH_START
        + transformationOperation + "/" + "upstreamType" + "/" + finalType + "/" + schemaFieldUrn,
        instance.numberNode(finalConfidenceScore)));
   ...
   * @param schemaFieldUrn a schema field to be marked as downstream, format: urn:li:schemaField(DATASET_URN, COLUMN NAME)
   * @param confidenceScore optional, confidence score for the lineage edge. Defaults to 1.0 for full confidence
   * @param transformationOperation string operation type that describes the transformation operation happening in the lineage edge
   * @param type the downstream lineage type, either Field or Field Set
   * @return this builder
   */
  public UpstreamLineagePatchBuilder addFineGrainedDownstreamField(@Nonnull Urn schemaFieldUrn,
      @Nullable Float confidenceScore, @Nonnull String transformationOperation, @Nullable FineGrainedLineageDownstreamType type) {
    Float finalConfidenceScore = getConfidenceScoreOrDefault(confidenceScore);
    String finalType;
    if (type == null) {
      // Default to set of fields if not explicitly a single field
    ...
    pathValues.add(ImmutableTriple.of(PatchOperationType.ADD.getValue(),FINE_GRAINED_PATH_START
         + transformationOperation + "/" + "downstreamType" + "/" + finalType + "/" + schemaFieldUrn,
        instance.numberNode(finalConfidenceScore)));
    return this;
  }
   ...
   * @return this builder
   */
  public UpstreamLineagePatchBuilder removeFineGrainedUpstreamField(@Nonnull Urn schemaFieldUrn,
      @Nonnull String transformationOperation, @Nullable FineGrainedLineageUpstreamType type) {
    String finalType;
    if (type == null) {
      // Default to set of fields if not explicitly a single field
      finalType = FineGrainedLineageUpstreamType.FIELD_SET.toString();
    ...
    pathValues.add(ImmutableTriple.of(PatchOperationType.REMOVE.getValue(),FINE_GRAINED_PATH_START
            + transformationOperation + "/" + "upstreamType" + "/" + finalType + "/" + schemaFieldUrn, null));

    return this;
  }

  public UpstreamLineagePatchBuilder removeFineGrainedUpstreamDataset(@Nonnull DatasetUrn datasetUrn, @Nonnull String transformationOperation) {

    pathValues.add(ImmutableTriple.of(PatchOperationType.REMOVE.getValue(),FINE_GRAINED_PATH_START
        +  transformationOperation + "/" +  "upstreamType" + "/" +  "DATASET" + "/" + datasetUrn, null));
    return this;
  }
   ...
   * @return this builder
   */
  public UpstreamLineagePatchBuilder removeFineGrainedDownstreamField(@Nonnull Urn schemaFieldUrn,
      @Nonnull String transformationOperation, @Nullable FineGrainedLineageDownstreamType type) {
    String finalType;
    if (type == null) {
      // Default to set of fields if not explicitly a single field
      finalType = FineGrainedLineageDownstreamType.FIELD_SET.toString();
