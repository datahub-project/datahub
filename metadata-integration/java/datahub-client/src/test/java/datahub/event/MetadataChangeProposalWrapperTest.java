...
public class MetadataChangeProposalWrapperTest {
  ...
  public void testBuilderExceptions() {
    try {
      MetadataChangeProposalWrapper.create(b -> b
          .entityUrn("foo") // bad urn should throw exception
          .entityType("dataset"));
      Assert.fail("Should throw an exception");
    } catch (EventValidationException e) {
      Assert.assertTrue("Underlying exception should be a URI syntax issue", e.getCause() instanceof URISyntaxException);
    } catch (Exception e) {
  ...
  public void testAspectInferenceSuccess() throws EventValidationException {
    MetadataChangeProposalWrapper mcpw = MetadataChangeProposalWrapper.create(b -> b
        .entityType("dataset")
        .entityUrn("urn:li:dataset:(foo,bar,PROD)")
        .aspect(new DatasetProperties()));
    Assert.assertEquals(mcpw.aspectName, "datasetProperties");
  }
   ...
   */
  @Test(expected = EventValidationException.class)
  public void testAspectInferenceFailureLambda() throws URISyntaxException, EventValidationException {
