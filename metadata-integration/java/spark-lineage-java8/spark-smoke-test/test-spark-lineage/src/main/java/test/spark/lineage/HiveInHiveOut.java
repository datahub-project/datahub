...
public class HiveInHiveOut {
	...
	public static void main(String[] args) {
		...
		spark.sql("create table " + Utils.tbl(TEST_NAME, "foo5") + " as "
		    + "(select v1.a, v1.b, v2.c, v2.d from v1 join v2 on v1.id = v2.id)");

		// CreateHiveTableAsSelectCommand
		spark.sql("create table " + Utils.tbl(TEST_NAME, "hivetab") + " as " + "(select * from "
		    + Utils.tbl(TEST_NAME, "foo5") + ")");

		// InsertIntoHiveTable
		spark.sql(
		    "insert into " + Utils.tbl(TEST_NAME, "hivetab") + " (select * from " + Utils.tbl(TEST_NAME, "foo5") + ")");
