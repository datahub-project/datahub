...
public class ProtobufFieldTest {
    ...
    public void nestedTypeReservedFieldsTest() throws IOException {
        ...
        SchemaField msg3_13_field = testMetadata.getFields().stream().filter(v -> v.getFieldPath()
                        .equals("[version=2.0].[type=extended_protobuf_MyMsg].[type=extended_protobuf_MyMsg_Msg3].field3.[type=google_protobuf_StringValue].msg3_13"))
                .findFirst()
                .orElseThrow();

        assertEquals("test comment 13", msg3_13_field.getDescription());

        SchemaField msg3_14_field = testMetadata.getFields().stream().filter(v -> v.getFieldPath()
                        .equals("[version=2.0].[type=extended_protobuf_MyMsg].[type=extended_protobuf_MyMsg_Msg3].field3.[type=google_protobuf_StringValue].msg3_14"))
                .findFirst()
                .orElseThrow();

        assertEquals("test comment 14", msg3_14_field.getDescription());
