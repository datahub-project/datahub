...
public class Proto2DataHub {
    ...
    private static final Option OPTION_DATAHUB_PLATFORM =
        Option.builder()
            .longOpt("platform")
            .hasArg()
            .desc("[Optional] The data platform to produce schemas for. e.g. kafka, snowflake, etc. (defaults to kafka)")
            .build();

    private static final Option OPTION_DATAHUB_API = Option.builder()
        .longOpt("datahub_api")
        .hasArg()
        .desc("[Optional] The API endpoint for DataHub GMS. (defaults to https://localhost:8080)")
        .build();

    private static final Option OPTION_DATAHUB_TOKEN = Option.builder()
        .longOpt("datahub_token")
        .hasArg()
        .desc("[Optional] The authentication token for DataHub API access. (defaults to empty)")
        .build();

    private static final Option OPTION_DESCRIPTOR = Option.builder()
        .longOpt("descriptor")
        .hasArg()
        .desc("[Required] The generated protobuf descriptor file. "
            + "Typically a single .dsc file for the repo or a .protoc file (1:1 with each src file)")
        .required()
        .build();

    private static final Option OPTION_FILE = Option.builder()
                .longOpt("file")
                .hasArg()
                .desc("[Optional if using --directory] The protobuf source file. Typically a .proto file.")
                .build();

    private static final Option OPTION_MESSAGE_NAME = Option.builder()
            .longOpt("message_name")
            .hasArg()
            .desc("[Optional if using --directory] The protobuf message name to read from.")
            .build();

    private static final Option OPTION_DIR = Option.builder()
        .longOpt("directory")
        .hasArg()
        .desc("[Optional if using --file] The root directory containing protobuf source files.")
        .build();

    private static final Option OPTION_EXCLUDE_PATTERN = Option.builder()
        .longOpt("exclude")
        .valueSeparator(',')
        .hasArgs()
        .desc("[Optional] Exclude patterns to avoid processing all source files, separated by ,. Typically used with --directory option. "
    ...
    private static final Option OPTION_DATAHUB_USER = Option.builder()
    ...
    private static final Option OPTION_ENV = Option.builder()
        .longOpt("env")
        .hasArg()
        .desc("[Optional] The environment to attach all entities to. Typically, DEV, PROD etc. (defaults to DEV)")
        .build();

    private static final Option OPTION_GITHUB_ORG = Option.builder()
        .longOpt("github_org")
        .hasArg()
        .desc("[Optional] The GitHub organization that this schema repository belongs to. "
    ...
    private static final Option OPTION_SLACK_ID = Option.builder()
        .longOpt("slack_id")
        .hasArg()
        .desc("[Optional] The Slack team id if your protobuf files contain comments with references to channel names. "
    ...
    private static final Option OPTION_TRANSPORT = Option.builder()
        .longOpt("transport")
        .hasArg()
        .desc("[Optional] What transport to use to communicate with DataHub. Options are: rest (default), kafka and file.")
        .build();

    private static final Option OPTION_FILENAME = Option.builder()
        .longOpt("filename")
        .hasArg()
    ...
    private static final Option OPTION_SUBTYPE = Option.builder()
        .longOpt("subtype")
        .desc("[Optional] A custom subtype to attach to all entities produced. e.g. event, schema, topic etc."
            + "(Default is schema)")
        .build();
    ...
    static class AppConfig {
        ...
        AppConfig(CommandLine cli) {
            ...
            datahubAPI = cli.getOptionValue(OPTION_DATAHUB_API, env.getOrDefault("DATAHUB_API", "http://localhost:8080"));
            datahubToken = cli.getOptionValue(OPTION_DATAHUB_TOKEN, env.getOrDefault("DATAHUB_TOKEN", ""));
            datahubUser = cli.getOptionValue(OPTION_DATAHUB_USER, env.getOrDefault("DATAHUB_USER", "datahub"));
            fabricType = FabricType.valueOf(
                cli.getOptionValue(OPTION_ENV, env.getOrDefault("DATAHUB_ENV", "DEV")).toUpperCase(Locale.ROOT));
            githubOrg = cli.getOptionValue(OPTION_GITHUB_ORG, env.getOrDefault("DATAHUB_GITHUBORG", null));
            ...
            transport = TransportOptions.valueOf(cli.getOptionValue(OPTION_TRANSPORT, "rest").toUpperCase(Locale.ROOT));
        ...
        private AppConfig validate() throws Exception {
            ...
            if (this.slackId != null) {
                if (!this.slackId.startsWith("T")) {
                    throw new Exception(String.format("Slack team id %s should start with the letter T. "
                        ...
                        + "In this case, the team-id is TUMKD5EGJ.", this.slackId));
                }
            }
            return this;
        }
        ...
        options.addOption(OPTION_DATAHUB_PLATFORM)
        ...
        Options firstPassOptions = new Options()
            .addOption(OPTION_HELP);
        ...
        switch (config.transport) {
            case REST: {
                emitter = RestEmitter
                    .create(b -> b.server(config.datahubAPI).token(config.datahubToken));
            } break;
            case KAFKA: {
            ...
            default: {
                throw new UnsupportedOperationException(String
                    .format("%s transport is not supported yet.", config.transport));
            }
        }
        ...
        try {
            filePathStream.forEach(filePath -> {
                ...
                try {
                    ...
                    dataset.getAllMetadataChangeProposals().flatMap(Collection::stream).forEach(mcpw -> {
                        try {
                    ...
                    if (e.getMessage() != null && e.getMessage().equals("Cannot autodetect protobuf Message.")) {
                        System.err.printf("WARN: Top-level schema not found in %s, no dataset emitted%n", args[1]);
                    } else {
                        e.printStackTrace();
                        System.err.println(String.format("‼️ Failed to emit to DataHub over %s. Num events emitted so far %d",
    ...
    if (exitCode.get() == 0) {
        System.out.println(
            String.format("✅ Successfully emitted %d events for %d files to DataHub %s", totalEvents.get(), totalFiles.get(), config.transport));
    } else {
        System.out.println(
            String.format("‼️ Emitted %d events for %d files to DataHub %s", totalEvents.get(), totalFiles.get(), config.transport));
    }
