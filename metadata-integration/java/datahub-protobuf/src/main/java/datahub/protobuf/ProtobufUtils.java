...
public class ProtobufUtils {
    ...
    try {
      ...
      options.addAll(
          ((Map<Descriptors.FieldDescriptor, Object>)
                  FIELD_OPT_EXT_FIELDS_METHOD.invoke(fieldProto.getOptions()))
              .entrySet().stream()
      ...
      options.addAll(
          ((Map<Descriptors.FieldDescriptor, Object>)
                  FIELD_OPT_ALL_FIELD_METHOD.invoke(fieldProto.getOptions(), false))
              .entrySet().stream()
                  .map(e -> Pair.of(e.getKey(), e.getValue()))
                  .collect(Collectors.toList()));
  ...
  public static List<Pair<Descriptors.FieldDescriptor, Object>> getMessageOptions(
    ...
    try {
      ...
      options.addAll(
          ((Map<Descriptors.FieldDescriptor, Object>)
                  MSG_OPT_EXT_FIELDS_METHOD.invoke(messageProto.getOptions()))
              .entrySet().stream()
                  .map(e -> Pair.of(e.getKey(), e.getValue()))
                  .collect(Collectors.toList()));
      ...
      options.addAll(
          ((Map<Descriptors.FieldDescriptor, Object>)
                  MSG_OPT_ALL_FIELD_METHOD.invoke(messageProto.getOptions(), false))
              .entrySet().stream()
                  .map(e -> Pair.of(e.getKey(), e.getValue()))
                  .collect(Collectors.toList()));
  ...
  public static ExtensionRegistry buildRegistry(DescriptorProtos.FileDescriptorSet fileSet) {
    ...
    Map<String, DescriptorProtos.FileDescriptorProto> descriptorProtoMap =
            ...
            fdp -> {
              try {
                Descriptors.FileDescriptor file =
                    descriptorFromProto(fdp, descriptorProtoMap, descriptorCache);
                Stream.concat(
                        file.getExtensions().stream(),
                        file.getMessageTypes().stream()
                            .flatMap(msg -> msg.getExtensions().stream()))
   ...
   * a {@link Descriptors.FileDescriptor} for the supplied proto itself. For maximal efficiency,
   * reuse the descriptorCache argument across calls.
   */
  private static Descriptors.FileDescriptor descriptorFromProto(
      ...
      Map<String, Descriptors.FileDescriptor> descriptorCache)
    ...
    for (String dependencyName : descriptorProto.getDependencyList()) {
      ...
      DescriptorProtos.FileDescriptorProto dependencyProto =
          descriptorProtoIndex.get(dependencyName);
    ...
    Descriptors.FileDescriptor descript =
        Descriptors.FileDescriptor.buildFrom(
            descriptorProto, dependencies.build().toArray(empty), false);
