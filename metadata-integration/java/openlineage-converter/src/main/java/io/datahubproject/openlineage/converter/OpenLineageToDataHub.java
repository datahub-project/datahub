...
public class OpenLineageToDataHub {
  ...
  private static void convertJobToDataJob(
    ...
    // If this is a MERGE INTO command, try to extract the target table name
    if (isMergeIntoCommand) {
      log.info("Detected MERGE INTO command in job: {}", job.getName());
    ...
    if (isMergeIntoCommand && tableName != null) {
