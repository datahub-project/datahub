...
public class SparkStreamingLineageIntegration {
    ...
    public static DatahubEventEmitter setupLineageCapturing(
        ...
        try {
            ...
            SparkOpenLineageConfig openLineageConfig = 
                ArgumentParser.parse(sc.getConf());
    ...
    /**
     * Verifies that the DataHub lineage components are properly installed.
     * This can be used to check if the log interceptors and streaming listeners
     * are properly registered after setup.
     * 
    ...
    public static boolean verifyLineageComponents(SparkSession spark) {
        ...
        try {
            ...
            try {
                ...
                boolean microBatchAppenderFound = checkForDataHubAppender(logger, "DataHubMicroBatchAppender");
            ...
            try {
                Class<?> loggerClass = Class.forName("org.slf4j.LoggerFactory");
                Object logger = loggerClass.getMethod("getLogger", String.class)
                        .invoke(null, "org.apache.spark.sql.execution.streaming.continuous.ContinuousExecution");
                
                // Try to get the root logger and check appenders
                boolean continuousAppenderFound = checkForDataHubAppender(logger, "DataHubContinuousExecAppender");
            ...
            try {
                ...
                log.info("Cannot directly verify streaming query listeners - assume they're installed if no errors occurred during setup");
            } catch (Exception e) {
        ...
        try {
            ...
            try {
                ...
                java.util.Enumeration<?> appenders = 
                    (java.util.Enumeration<?>) log4jLogger.getClass().getMethod("getAllAppenders").invoke(log4jLogger);
            ...
            try {
                ...
                java.util.Iterator<?> appenderIt = 
                    (java.util.Iterator<?>) logbackLogger.getClass().getMethod("iteratorForAppenders").invoke(logbackLogger);
    ...
    /**
     * Test method to generate some log messages from MicroBatchExecution and ContinuousExecution
     * classes to verify that the log interceptors are working.
     * This should only be used for testing, as it uses reflection to access private classes.
     */
    public static void generateTestLogMessages() {
        try {
            log.info("Generating test log messages for DataHub lineage interceptors");
            
            // Try to generate a log message from MicroBatchExecution
            try {
                Class<?> loggerClass = Class.forName("org.slf4j.LoggerFactory");
                Object logger = loggerClass.getMethod("getLogger", String.class)
                        .invoke(null, "org.apache.spark.sql.execution.streaming.MicroBatchExecution");
                
                // Call info method on the logger
                logger.getClass().getMethod("info", String.class).invoke(
                    logger, "TEST Starting microbatch for queryId=test-query-id with sink=test-sink");
                
                logger.getClass().getMethod("info", String.class).invoke(
                    logger, "TEST Committing microbatch for queryId=test-query-id with sink=test-sink");
                
                logger.getClass().getMethod("info", String.class).invoke(
                    logger, "TEST Writing job for queryId=test-query-id dataframe with path=test-path");
                
                log.info("Generated test MicroBatchExecution log messages");
            } catch (Exception e) {
                log.warn("Failed to generate test MicroBatchExecution log messages: {}", e.getMessage(), e);
            }
            
            // Try to generate a log message from ContinuousExecution
            try {
                Class<?> loggerClass = Class.forName("org.slf4j.LoggerFactory");
                Object logger = loggerClass.getMethod("getLogger", String.class)
                        .invoke(null, "org.apache.spark.sql.execution.streaming.continuous.ContinuousExecution");
                ...
                // Call info method on the logger
                logger.getClass().getMethod("info", String.class).invoke(
                    logger, "TEST Starting continuous processing for queryId=test-query-id");
                
                logger.getClass().getMethod("info", String.class).invoke(
                    logger, "TEST Committing epoch for queryId=test-query-id with sink=test-sink");
                
                logger.getClass().getMethod("info", String.class).invoke(
                    logger, "TEST Writing sink for queryId=test-query-id with path=test-path");
                
                log.info("Generated test ContinuousExecution log messages");
            } catch (Exception e) {
                log.warn("Failed to generate test ContinuousExecution log messages: {}", e.getMessage(), e);
            }
            
            log.info("Test log message generation complete");
        } catch (Exception e) {
            log.error("Error generating test log messages", e);
        }
    }
