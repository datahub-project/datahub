 ...
 * 
 * This class creates a log interceptor for Spark's ContinuousExecution engine logs.
 * When a relevant log is captured, it forwards it to the DatahubEventEmitter for processing.
 */
@Slf4j
public class ContinuousExecutionLogInterceptor {
    private static final String LOGGER_NAME = "org.apache.spark.sql.execution.streaming.continuous.ContinuousExecution";
    ...
    public static boolean install(DatahubEventEmitter eventEmitter) {
        ...
        try {
            ...
            // Try to install log interceptor using reflection to avoid direct dependency on log frameworks
            try {
    ...
    private static boolean installLog4jAppender(Object logger) {
        try {
            ...
            log4jLogger.getClass().getMethod("addAppender", appenderClass)
                    .invoke(log4jLogger, appender);
    ...
    protected static void processLogMessage(String message, String level) {
        if (emitter == null) {
            log.warn("No emitter registered for ContinuousExecutionLogInterceptor, can't process log: {}", message);
            return;
        }
