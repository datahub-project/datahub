...
| spark.datahub.captureQueryEntities                                              |          | false                   | Set this to true to emit a Query entity for each job's captured SQL statement and reference it from the corresponding fine-grained lineage, making the statement a searchable entity with its own page, listed on the Queries tab of each dataset the job touches. This does not change the table-level lineage graph, where the Spark task remains the intermediate node. Only takes effect when Spark captures an OpenLineage SQL job facet for the operation — this happens for `spark.sql(...)` calls, but **not** for DataFrame-API operations (`withColumn`, `concat`, etc.), which emit no SQL facet at all. With no SQL to capture, no Query entity is emitted for those jobs even with this flag on. By default, it is disabled. |
...
With `spark.datahub.captureQueryEntities=true`, a job's captured SQL statement is emitted as a DataHub **Query entity** and referenced from its fine-grained lineage. That makes the statement a first-class object: searchable, with its own page, and listed on the **Queries** tab of each dataset it reads or writes. Identical statements are de-duplicated, so the same SQL emitted by this listener, by the Airflow plugin, or through the Python SDK resolves to one entity.

**What this does not change:** the table-level lineage graph is unchanged. The Spark task remains the intermediate node between datasets — this flag does not put a query node between two tables. That rendering is driven by a query reference on a dataset's own `upstreamLineage`, which this listener does not write (it emits lineage on the job, not on datasets). Selecting a column does attribute the column-level transformation to the query rather than to the task.

This depends entirely on Spark having captured SQL text for the operation in the first place, which only happens for `spark.sql(...)` calls. DataFrame-API operations (`withColumn`, `concat`, and similar) produce no SQL — OpenLineage's job facet for them is limited to a type tag such as `DIRECT:TRANSFORMATION` — so there is nothing to capture and no transformation node appears for those jobs, regardless of this flag. The `transformOperation` field that already drives the Logic panel today is populated the same way in both cases, so nothing changes for DataFrame-only pipelines or for anyone who leaves this flag off.
