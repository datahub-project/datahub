...
| spark.datahub.captureQueryEntities                                              |          | false                   | Set this to true to emit a Query entity for each job's captured SQL statement and reference it from the corresponding fine-grained lineage, so the lineage graph shows a transformation node with the statement in its Logic panel instead of attributing the edge to the task. Only takes effect when Spark captures an OpenLineage SQL job facet for the operation — this happens for `spark.sql(...)` calls, but **not** for DataFrame-API operations (`withColumn`, `concat`, etc.), which emit no SQL facet at all. With no SQL to capture, no Query entity is emitted for those jobs even with this flag on. By default, it is disabled.                                         |
...
With `spark.datahub.captureQueryEntities=true`, a job's captured SQL statement is emitted as a DataHub Query entity and referenced from its fine-grained lineage, so the lineage graph renders a transformation node whose Logic panel shows the statement instead of attributing the edge to the task.
