...
| spark.datahub.column_lineage.include_indirect                    |          | false                   | By default in acryl-spark-lineage, only DIRECT (value-producing) column upstreams are emitted, matching DataHub's Python SQL parser behaviour and avoiding the every-column-depends-on-every-join-key fan-out in the lineage panel. Set this to true to additionally emit INDIRECT upstreams (JOIN keys, WHERE filters, GROUP BY columns) as a separate `fineGrainedLineage` entry, so each downstream column gets two edges (one DIRECT, one INDIRECT) that the UI can render or filter independently. Columns with no DIRECT upstream (e.g. `COUNT(*)` driven only by a GROUP BY column) get no column-level lineage when the flag is off, but the source datasets still appear in table-level lineage. |
