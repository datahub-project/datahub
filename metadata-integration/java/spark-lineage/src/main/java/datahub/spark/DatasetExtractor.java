...
import datahub.spark.model.dataset.BigqueryDataset;
...
public class DatasetExtractor {
  ...
  static {
    ...
    PLAN_TO_DATASET.put(InsertIntoHadoopFsRelationCommand.class, (p, ctx) -> {
      ...
      try {
        List<SparkDataset> datasets = Collections
            .singletonList(HdfsPathDataset.create(cmd.outputPath()));
    ...
    REL_TO_DATASET.put(HadoopFsRelation.class, (r, ctx) -> {
      ...
      try {
        List<SparkDataset> datasets = Collections
            .singletonList(HdfsPathDataset.create(res.get(0)));
        return Optional.of(datasets);
      } catch (InstantiationException e) {
        log.error("Hdfs based dataset could not be created.", e);
        return Optional.empty();
    ...
    REL_TO_DATASET.put(BigQueryRelation.class, (r, ctx) -> {
      BigQueryRelation rel = (BigQueryRelation) r;
      String tbl = rel.tableName();
      if (tbl.isEmpty()) {
        return Optional.empty();
      }
      return Optional.of(Collections.singletonList(new BigqueryDataset(tbl)));
    });
    
    REL_TO_DATASET.put(DirectBigQueryRelation.class, (r, ctx) -> {
      DirectBigQueryRelation rel = (DirectBigQueryRelation) r;
      String tbl = rel.tableName();
      if (tbl.isEmpty()) {
        return Optional.empty();
      }
