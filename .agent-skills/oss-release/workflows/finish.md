...
| `4`  | Failed.                | First check **why**: if the failing jobs died in the install step with `No solution found when resolving dependencies`, that is the known pypi-propagation release race, not a code failure — tell the user to **re-run the failed connector-test jobs** and re-run `finish` once they're green (see `known-flaky-tests.md`). Otherwise do NOT offer to re-dispatch — the same SHA will fail the same way. Show the run URL, suggest cutting a new RC with a fix, and stop.                                                                                                                                                                                                                                                   |
