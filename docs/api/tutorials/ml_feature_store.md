...
```python
# Inlined from /metadata-ingestion/examples/library/mlfeature_create.py
import os
...
```python
# Inlined from /metadata-ingestion/examples/library/mlfeature_table_create.py
import os
...
# Emit metadata!
emitter.emit_mcp(metadata_change_proposal)
print(f"Created ML feature table: {feature_table_urn}")
...
```python
# Inlined from /metadata-ingestion/examples/library/mlmodel_read.py
