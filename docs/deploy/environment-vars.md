...
| Environment Variable                                            | Default                          | Description                                                                                                                                                          | Components         |
| --------------------------------------------------------------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| `ELASTICSEARCH_BUILD_INDICES_ALLOW_DOC_COUNT_MISMATCH`          | `false`                          | When `true` (requires `ELASTICSEARCH_BUILD_INDICES_CLONE_INDICES=true`), allows the reindex poll loop to exit successfully when the destination doc count reaches the pre-reindex source snapshot and the reindex task is not observably still running. Prevents an infinite poll loop on live-write sources where the exact source/destination equality can never be reached. The original index is preserved as a fallback via `cloneIndices`. | System Update      |
