...
| Environment Variable                                  | Default                               | Description                                                                                                                                                                                                                                                     | Components                 |
| ----------------------------------------------------- | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| `TIMESERIES_ASPECT_SERVICE_IMPLEMENTATION`            | `elasticsearch`                       | Timeseries store for reads: `elasticsearch`, `opensearch`, or `postgres` (requires `DATAHUB_PGTIMESERIES_ENABLED=true` + SqlSetup). Design: [pgTimeseries](../pgtimeseries-design.md)                                                                           | GMS, MAE Consumer          |
| `DATAHUB_PGTIMESERIES_ENABLED`                        | `false`                               | Enable pgTimeseries SqlSetup + dedicated pool / dual-write sink (required when `TIMESERIES_ASPECT_SERVICE_IMPLEMENTATION=postgres`)                                                                                                                             | GMS, MAE Consumer, Upgrade |
...
| `DATAHUB_PGTIMESERIES_DUAL_WRITE_FAIL_ON_ERROR`       | `false`                               | When true, dual-write upsert/delete SQL errors fail the caller (migration fail-loud); default logs + metric only                                                                                                                                                | GMS, MAE Consumer          |
...
| Environment Variable                                          | Default | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Components    |
| ------------------------------------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| `SYSTEM_UPDATE_SCHEMA_FIELDS_FROM_SCHEMA_METADATA_ENABLED`    | `false` | Enable the non-blocking system-update step that re-ingests dataset `schemaMetadata`/`status` to materialize schemaField key/aliases/status (and optionally backfill or clean up mirrored `domains`/`ownership` based on the current MCP flags). **When a mirror flag is off, cleanup deletes that aspect from every field under each scanned dataset — including manually written field aspects.** See [Updating DataHub](../how/updating-datahub.md).                                                                                                                                  | System Update |
...
| `MCP_SIDE_EFFECTS_SCHEMA_FIELD_DOMAIN_ENABLED`    | `false`    | When schema-field side effects are on, mirror dataset `domains` onto each schemaField (upsert and cascade delete). Changing this flag updates the `schemaFieldsFromSchemaMetadata` upgrade fingerprint so the next SystemUpdateNonBlocking run backfills (enable) or deletes field `domains` (disable). **Disable cleanup removes `domains` from every field under each scanned dataset, including manually written field aspects.** Set `SYSTEM_UPDATE_SCHEMA_FIELDS_FROM_SCHEMA_METADATA_ENABLED=true` for that run. | GMS, MCE Consumer, System Update |
| `MCP_SIDE_EFFECTS_SCHEMA_FIELD_OWNERSHIP_ENABLED` | `false`    | When schema-field side effects are on, mirror dataset `ownership` onto each schemaField (upsert and cascade delete). Same fingerprint / backfill-or-cleanup behavior as the domain sub-flag (including non-mirror-aware delete of all field `ownership` when disabling).                                                                                                                                                                                                                                               | GMS, MCE Consumer, System Update |
