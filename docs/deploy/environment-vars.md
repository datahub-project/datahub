...
| `ENABLE_STRUCTURED_PROPERTIES_TYPE_MISMATCH_REINDEX`             | `true`        | Reindex entity search indices when structured-property Elasticsearch field types disagree with definition-driven targets (e.g. dynamic `float`/`long` vs `double` for NUMBER). Loads SP definitions into system-update mapping targets when enabled. | GMS, System Update      |
