...
| Environment Variable                                  | Default         | Description                                                                                                          | Components |
| ----------------------------------------------------- | --------------- | -------------------------------------------------------------------------------------------------------------------- | ---------- |
| `TIMESERIES_ASPECT_SERVICE_IMPLEMENTATION`            | `elasticsearch` | Timeseries store for reads: `elasticsearch`, `opensearch`, or `postgres` (requires `postgres.pgTimeseries` SqlSetup) | GMS        |
...
| `DATAHUB_USAGE_EVENTS_IMPLEMENTATION`                | `elasticsearch`                                                                                                                                                                             | Persist usage events in OpenSearch/ES (`elasticsearch`) or partitioned PostgreSQL (`postgres`)                    | GMS, MAE Consumer                                      |
| `DATAHUB_USAGE_EVENTS_RETENTION_MONTHS`              | `18`                                                                                                                                                                                        | Months of usage-event history kept (PostgreSQL drops older partitions)                                            | GMS (partition maintenance when enabled), MAE (writes) |
| `DATAHUB_USAGE_EVENTS_PARTITIONS_AHEAD`              | `3`                                                                                                                                                                                         | Empty monthly partitions to pre-create ahead of UTC “now” (PostgreSQL)                                            | GMS                                                    |
| `DATAHUB_USAGE_EVENTS_RECOMMENDATION_LOOKBACK_DAYS`  | `90`                                                                                                                                                                                        | Bounds home-page recommendation queries against PostgreSQL usage events                                           | GMS                                                    |
| `DATAHUB_USAGE_EVENTS_PARTITION_MAINTENANCE_ENABLED` | `false`                                                                                                                                                                                     | Run scheduled PostgreSQL partition create/drop inside GMS (recommended false if you run upgrades/cron externally) | GMS                                                    |
| `DATAHUB_USAGE_EVENTS_PARTITION_MAINTENANCE_CRON`    | `0 0 3 * * *`                                                                                                                                                                               | Spring cron for scheduled partition maintenance                                                                   | GMS                                                    |
...
| `DATAHUB_PGQUEUE_PAYLOAD_COMPRESSION`             | `SNAPPY`           | When `DATAHUB_MESSAGING_TRANSPORT=pgqueue`, application-layer codec for enqueued payloads (`NONE` or `SNAPPY`). Stored per row in `message.payload_compression`. Set to `NONE` if any peer cannot decode Snappy. | GMS, System Update               |
| `DATAHUB_PGQUEUE_TOPIC_DEFAULT_CONTENT_TYPE_MIME` | `application/avro` | MIME string resolved to `topic.default_content_type_id` via the pgQueue `*_content_type` lookup table. Messages omit `content_type_id` when it matches this topic default (storage optimization).                | GMS, System Update               |
...
| `DATAHUB_PGQUEUE_TOPIC_DEFAULT_CONSUMER_CONCURRENCY` | `1`                               | When `DATAHUB_MESSAGING_TRANSPORT=pgqueue`, default same-JVM poll threads per topic (partition `p` → thread `p % c`). Capped to `postgres.pgQueue.topicDefaults.partitionCount` (and per-topic to that topic's partition count). Override per logical topic under `postgres.pgQueue.topics.*.consumerConcurrency`. Cross-JVM dedup still uses Postgres group leases. | MAE Consumer, MCE Consumer, PE Consumer                   |
...
When **`DATAHUB_MESSAGING_TRANSPORT`** is **`pgqueue`**, MAE/MCE/PE consumers read from Postgres (`consumer_offset` per consumer group and `*_message_group_lease` for in-flight work) instead of Kafka. Each logical hook group still maps to its own consumer group string (same as Kafka mode). Offsets are **not** exposed via Kafka Admin REST; use DB queries or application metrics for lag. Topic retention (age / row / payload limits) applies regardless of how far consumers have caught up. **MCL batch mode** (`metadataChangeLog.consumer.batch.enabled=true`) is supported with pgQueue via `PgQueueBatchAccumulator`, which flushes on message count, cumulative payload bytes, or linger age thresholds.
...
| Environment Variable                           | Default | Description                                                                                                                                                                                                                                                                                                   | Components                                   |
| ---------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `management.metrics.enable.cache`              | `true`  | Spring Boot [per-meter enable flag](https://docs.spring.io/spring-boot/reference/actuator/metrics.html#actuator.metrics.customizing.per-meter): when `false`, Micrometer drops meters whose IDs start with `cache.` (including Spring `CacheManager` cache metrics). YAML: `management.metrics.enable.cache`. | GMS, MAE Consumer, MCE Consumer, PE Consumer |
