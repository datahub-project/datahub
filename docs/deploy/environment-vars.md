...
| `VIEW_UNRESTRICTED_ENTITY_TYPES`                        | see `application.yaml` | Comma-separated entity types that bypass view authorization when `VIEW_AUTHORIZATION_ENABLED=true`. Default is backwards-compatible: all `entity-registry.yml` entities except the former `VIEW_RESTRICTED_ENTITY_TYPES` allowlist. Trim with `_REMOVE` (e.g. `schemaField,document`). Shared GMS config for Cloud [Search Access Controls](../features/feature-guides/search-access-controls.md) and OSS entity-page VBAC — query-time search pushdown remains Cloud-only | GMS        |
| `VIEW_UNRESTRICTED_ENTITY_TYPES_ADD`                    | _(empty)_              | Comma-separated registry names to append to the view-unrestricted entity-type list                                                                                                                                                                                                                                                                                                                                                                                         | GMS        |
| `VIEW_UNRESTRICTED_ENTITY_TYPES_REMOVE`                 | _(empty)_              | Comma-separated registry names to remove from the view-unrestricted entity-type list                                                                                                                                                                                                                                                                                                                                                                                       | GMS        |
