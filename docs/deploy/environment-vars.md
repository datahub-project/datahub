...
| `ES_BULK_ITEM_REQUEUE_ENABLED`                               | `true`    | Requeue failed bulk items (version conflicts, 429/503, etc.) onto the same processor with a bound. Graph edge ops also apply an in-process write-version fence: stale requeues are declined when a newer `graphWriteVersion` for the same edge docId was already submitted (see [graph write semantics](../what/graph.md#graphwriteversion-fencing)). | GMS, MAE Consumer |
...
| `GRAPH_SERVICE_DIFF_MODE_ENABLED`       | `true`  | Enable pairwise edge diff for graph writes (add/remove/update vs previous aspect). See [graph write semantics](../what/graph.md#how-graph-edges-are-written-es--opensearch). | GMS        |
