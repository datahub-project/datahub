...
| `DATAHUB_PGTIMESERIES_TABLE_PREFIX`                   | `metadata_timeseries`          | Table prefix for `{prefix}_aspect`                                                                                                                                                                     | GMS, MAE Consumer, Upgrade |
...
| `DATAHUB_PGTIMESERIES_RETENTION_MAX_AGE_SECONDS`      | `7776000`                      | Hard ceiling (seconds) for partman partition drop (default 90 days). `0` clears `part_config.retention` (stops drops). Shorter per-aspect TTLs use `datahub-gc` / `truncateTimeseriesAspect`.          | Upgrade                    |
| `DATAHUB_PGTIMESERIES_MAINTENANCE_CRON_ENABLED`       | `false`                        | Enable pg_cron maintenance job for pgTimeseries partitions. Requires `postgres.pgCron.admin` JDBC (see pg_cron admin vars below).                                                                      | Upgrade                    |
| `DATAHUB_PGTIMESERIES_MAINTENANCE_INTERVAL_SECONDS`   | `3600`                         | Maintenance cron interval when enabled                                                                                                                                                                 | Upgrade                    |
| `DATAHUB_PGTIMESERIES_DATASOURCE_URL`                 | `${ebean.url}`                 | JDBC URL for the dedicated pgTimeseries pool (falls through to Ebean)                                                                                                                                  | GMS, MAE Consumer, Upgrade |
