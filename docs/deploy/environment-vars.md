...
| `QUERY_ENTITY_AUTHORIZATION_REQUIRE_ALL_SUBJECTS`       | `false`                | `true`, `false`, or `COMPAT`. `true`: reading a Query requires `VIEW_ENTITY_QUERIES` on every dataset in its `querySubjects` (or `VIEW_ALL_QUERIES`) everywhere, instead of any single one. `false`: any single subject dataset suffices everywhere. `COMPAT`: require-all on the dataset view page's Queries tab, any-subject everywhere else (direct Query entity reads, REST/OpenAPI, `topSqlQueries`)                                                                                                                         | GMS        |
