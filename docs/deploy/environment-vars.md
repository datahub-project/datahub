...
| `INGESTION_VERSION_MATRIX_URL`              | _(empty)_ | Location of a per-connector CLI version matrix JSON document, overriding `defaultCliVersion` per connector without a GMS redeploy. The scheme selects the backend: `s3://bucket/key`, `gs://bucket/key`, `file:///path`, or `https://host/path`. Leave empty to disable — connectors then use `defaultCliVersion`. An unreadable or unsupported URI degrades to the same no-op rather than blocking GMS startup. | GMS, MAE Consumer |
| `INGESTION_VERSION_MATRIX_REFRESH_SECONDS`  | `600`     | Poll interval for re-fetching the CLI version matrix document                                                                                                                                                                                                                                                                                                                                                    | GMS, MAE Consumer |
| `INGESTION_VERSION_MATRIX_AUTH_TOKEN`       | _(empty)_ | `Authorization` header value for `https://` matrix URLs (e.g. `token ghp_xxx`, `Bearer eyJ...`). Not used for `s3://`/`gs://` (ambient cloud credentials) or `file://`.                                                                                                                                                                                                                                          | GMS, MAE Consumer |
