...
| `VIEW_UNRESTRICTED_ENTITY_TYPES`                        | _(empty)_              | Optional full override of view-unrestricted entity types when `VIEW_AUTHORIZATION_ENABLED=true`. Empty keeps the effective defaults from `entity-registry.yml` plus `application.yaml`. Shared GMS config for Cloud [Search Access Controls](../features/feature-guides/search-access-controls.md) and OSS entity-page VBAC — query-time search pushdown remains Cloud-only | GMS        |
| `VIEW_UNRESTRICTED_ENTITY_TYPES_DEFAULT`                | see `application.yaml` | Comma-separated application defaults appended to the registry `viewUnrestricted` baseline. Overrides the stock YAML CSV (does not replace the registry baseline). Empty keeps only the registry baseline before `_ADD` / `_REMOVE`.                                                                                                                                     | GMS        |
| `VIEW_UNRESTRICTED_ENTITY_TYPES_ADD`                    | _(empty)_              | Comma-separated registry names to append to the effective defaults (or to `VIEW_UNRESTRICTED_ENTITY_TYPES` when set). Stock defaults exclude `document`, `schemaField`, and `container`; setting `_ADD` mutates rather than replaces those defaults                                                                                                                     | GMS        |
