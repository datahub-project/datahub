...
| `DATAHUB_EXECUTOR_CUSTOMER_ID`              | _(empty)_ | This deployment's id, matched against `cohorts[].deployments` in the [CLI version matrix](../docker/ingestion-cli-version-matrix.md). Unset on single-tenant installs — cohort matching is then always a no-op.                                                                                                                                                                                                  | GMS, MAE Consumer |
