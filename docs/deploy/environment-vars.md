...
| `ELASTICSEARCH_INDEX_BUILDER_ENTITY_SETTINGS_OVERRIDES`         | `null`                           | Entity settings overrides applied at index creation only. To raise `mapping.total_fields.limit` on an already-existing index, use `ELASTICSEARCH_INDEX_ENTITYMAPPINGLIMITS_<ENTITY>_TOTALFIELDS` instead.                                                                                                                | System Update      |
| `ELASTICSEARCH_INDEX_ENTITYMAPPINGLIMITS_<ENTITY>_TOTALFIELDS`  | `null`                           | Sets the maximum number of mapped fields (`mapping.total_fields.limit`) for an entity's index, applied both when the index is created and to existing indices on every system update; use entity name `DEFAULT` to cover unlisted entities. Example: `ELASTICSEARCH_INDEX_ENTITYMAPPINGLIMITS_DATASET_TOTALFIELDS=2500`. | System Update      |
