...
A Metadata Audit Event captures changes made to one or multiple metadata [aspects](aspect.md) associated with a particular [entity](entity.md), in the form of a metadata [snapshot](/docs/what/snapshot.md) (deprecated) before the change, and a metadata snapshot after the change.
