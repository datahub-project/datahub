...
Options:
  ...
  --urn TEXT              Urn of recipe to update. If not specified, creates a new ingestion source
  --executor-id TEXT      Executor id to route execution requests to. Defaults to "default"
  --cli-version TEXT      Custom CLI version to use for ingestion. Uses server default if not specified
  --schedule TEXT         Cron definition for schedule. If none provided, recipe will not be scheduled
  --time-zone TEXT        Timezone for schedule in 'America/New_York' format. Uses UTC by default
  --debug BOOLEAN         Enable debug mode for ingestion execution
  --extra-pip TEXT        Extra pip packages to install, e.g. ["memray"]
```
...
```shell
datahub ingest deploy --urn "urn:li:dataHubIngestionSource:my-existing-source" --schedule "0 6 * * *" -c updated_recipe.yaml
```
...
#### Using `deployment` section to avoid CLI args
...
**Note:** CLI options will override corresponding values in the deployment section.
