...
Snowflake's `emit_semantic_model_entities` control is tri-state, as is the dbt source's flag of the same name — see [From dbt](#from-dbt):
...
source:
  ...
  config:
    ...
    emit_semantic_model_entities: true # tri-state; unset follows the server
```
...
Because the default follows the server, a Cloud deployment already ingesting dbt semantic models as datasets will switch to the new URNs on its next run — unless the server fails a check, in which case it stays on datasets and reports why: a version string it cannot parse, or a Metrics kill-switch it cannot read, both fail closed. The two shapes use different URNs, so read the migration note in the [dbt connector docs](../../generated/ingestion/sources/dbt.md#migrating-between-the-modes) first — or pin `emit_semantic_model_entities: false` until you are ready.
