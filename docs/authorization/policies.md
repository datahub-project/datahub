...
`QUERY_ENTITY_AUTHORIZATION_REQUIRE_ALL_SUBJECTS` also accepts `COMPAT` (the default), a middle ground between the table's two modes: require-all applies only to the one query-read path actually gated by `VIEW_AUTHORIZATION_ENABLED` — the shared entity-VIEW evaluator's Query-entity branch, used for search-result masking and related-entity visibility, which is unreachable when that switch is off. Every read in the table above — direct Query reads, `listQueries`, REST/OpenAPI, `topSqlQueries` — is gated by the independent `QUERY_ENTITY_AUTHORIZATION_ENABLED` flag instead and stays any-subject under COMPAT, same as the `false` mode.
...
| Entity       | Privilege                                 | Description                                                                                                                                                                                                                                                                                                                                            |
| ------------ | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Dataset      | View Dataset Usage                        | Allow actor to access dataset usage information (numeric statistics). When `QUERY_ENTITY_AUTHORIZATION_REQUIRE_ALL_SUBJECTS` is set, holding **View Entity Queries** on the dataset is not sufficient to see the SQL text of the dataset's most frequently run queries — **View All Queries** is also required. See [Query entities](#query-entities). |
