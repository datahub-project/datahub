...
When Domain-Based Authorization is enabled, users must have appropriate privileges on the domain(s) that an entity belongs to in order to perform write operations on that entity. This ensures that metadata changes are scoped to the domains a user is authorized for.

**Configuration:**

To enable domain-based authorization, set the environment variable:

```
DOMAIN_BASED_AUTHORIZATION_ENABLED=true
```
