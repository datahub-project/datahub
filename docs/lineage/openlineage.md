...
The REST endpoint accepts `RunEvent`, `JobEvent`, and `DatasetEvent` objects and selects the event type from the required root fields rather than from an exact `schemaURL`. OpenLineage 2-0-2 events include URI-valued `schemaURL` metadata, while compatibility events may use an older or noncanonical URL or omit it. DataHub never fetches request-provided schema URLs.

Validation rejects malformed JSON, duplicate keys, trailing content, root combinations that match zero or multiple official event variants, missing required typed fields, invalid event types, and malformed standard-facet fields that DataHub consumes. Structural dispatch follows the official OpenLineage `oneOf`: unknown root properties do not reclassify an otherwise valid event. Standard facets are selected by their key and attachment point; their `_schemaURL` does not have to equal one pinned value. Unknown and producer-specific facet objects remain opaque at the request boundary, including fields not represented by DataHub's local compatibility schemas. Supported historical Airflow and Spark mappings are listed in [OpenLineage custom-facet compatibility](openlineage-custom-facet-compatibility.md).

Timezone-less OpenLineage timestamps are interpreted in the GMS host's default time zone. Their resulting instant therefore depends on the host configuration; producers should include an offset when they require an unambiguous instant. This behavior applies to root and facet timestamp fields. Legacy non-UUID root run identifiers are converted to a stable UUID from the job namespace, job name, and reported identifier.

The conformance corpus vendors all 37 JSON fixtures from OpenLineage 1.45.0 `spec/tests`, all 74 pinned OpenLineage compatibility-test events, and all 15 pinned Marquez OpenLineage events. Standalone events must pass validation and submit metadata for the entity families implied by their event type. OpenLineage facet fragments are attached to minimal events at their official attachment points and must pass the same request, deserialization, conversion, and ingestion path.

The endpoint returns `400 Bad Request` for invalid events, `401 Unauthorized` when authentication is missing, and `415 Unsupported Media Type` for non-JSON requests. The event-level `producer` remains a free-form identifier. DataHub resolves DataFlow platforms from the configured override, typed facets, known producer mappings, or the `unknown` fallback.

The DataFlow platform resolution order is: `DATAHUB_OPENLINEAGE_ORCHESTRATOR`, `JobTypeJobFacet.integration`, `ProcessingEngineRunFacet.name`, explicit mappings for known producer URIs, then `unknown`. Dataset platforms are inferred separately from dataset namespaces and facets, with `unknown` used when no platform can be inferred.
...
- **Custom facets**: Raw OpenLineage payloads are not retained. Unknown facet objects and unsupported producer/schema combinations remain opaque and are not mapped. Supported historical Airflow and Spark mappings are listed in [OpenLineage custom-facet compatibility](openlineage-custom-facet-compatibility.md).
