...
| datajob_lineage_dag_filter_str     | { "allow": [".*"] }  | AllowDenyPattern (JSON string) to control which DAGs emit DataJob input/output lineage. Use to disable Airflow-derived lineage for specific DAGs (e.g. Cosmos/dbt DAGs that already have lineage from dbt ingestion) while keeping it for other DAGs. Only applies when `enable_datajob_lineage` is true. |
