...
| test-coverage         | Improve test coverage (token refresh, OAuth errors, resolvers, deletes)   | complete | 4     |
...
| 2026-02-18 | **4.1 Test coverage implemented.** Three areas: Python smoke tests for GraphQL API lifecycle (10 tests), token refresh edge cases (11 tests), OAuth error handling (6 tests). Cascading delete tests deferred to 4.4 (requires deletion hook infrastructure). Used Python smoke tests instead of Cypress — all tests are API/GraphQL-level, no browser UI needed. Dropped thread-based concurrency tests (fragile timing); StripedLock already has dedicated tests in `test_striped_lock.py`.                                                                                                                                                                                                                                                                                                                                        |
| 2026-02-19 | **Renamed workflow mutations to `upsertAiPlugin`/`deleteAiPlugin`.** The convention is `Upsert<Entity>` for pure entity upserts, but these mutations do a complex workflow (Service entity + AiPluginConfig + optional OAuthAuthorizationServer). Renaming reserves `upsertService`/`deleteService` for future pure Service entity operations. Removed the old `deleteAiPlugin(id: String!)` from `app.graphql` (config-only delete, superseded by the full workflow delete) and gave the name to `deleteService(urn)` → `deleteAiPlugin(urn)`. Frontend delete now calls the full-cleanup mutation instead of the partial config-only one. Java resolvers renamed: `UpsertServiceResolver` → `UpsertAiPluginResolver`, `DeleteServiceResolver` → `DeleteAiPluginResolver`. Old `resolvers/settings/DeleteAiPluginResolver` removed. |
| 2026-02-20 | **Split backend schema: `aiPlugin.graphql` created.** Moved `upsertAiPlugin`/`deleteAiPlugin` mutations and `UpsertAiPluginInput` type from `service.graphql` to new `aiPlugin.graphql`. `service.graphql` now contains only Service/OAuth entity types and pure entity queries. Registered in `Constants.java` and `GmsGraphQLEngine.java`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2026-02-20 | **Privilege fix: added `manageConnections` to `PlatformPrivileges`.** The old `deleteAiPlugin` resolver checked `MANAGE_GLOBAL_SETTINGS`, but all new resolvers use `MANAGE_CONNECTIONS`. Added `manageConnections` field to the GraphQL `PlatformPrivileges` type, populated in `MeResolver`, queried in `me.graphql`. Frontend AI settings page now shows if user has either `manageGlobalSettings` OR `manageConnections`.                                                                                                                                                                                                                                                                                                                                                                                                        |
| 2026-02-20 | **Improved plugin connection error messages.** `PluginConnectionError` details (e.g., "API key not set", "token refresh failed") were logged but not shown to users. Now appended as a Markdown blockquote in the chat error message so users can see the root cause without checking logs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-20 | **Expanded smoke tests to 16 tests.** Added: OAuth plugin enable without connection, OAuth initiate connect URL validation, admin test-connection endpoint, API key save/disconnect lifecycle via REST, OAuth server edit two-mutation flow, public client without secret, inline OAuth server ID rejection, missing mcpServerProperties rejection.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 2026-02-20 | **Added `usePluginActions` frontend tests.** 11 tests covering `buildToggleInput`, `handleToggleEnabled` (success, missing URN, mutation failure), and `handleDelete` (success with analytics, mutation failure). Coverage: 100% statements/lines/functions.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `smoke-test/tests/ai_plugins/__init__.py`             | complete | Empty init for pytest discovery                                                                                                                                                                                                                                                              |
| `smoke-test/tests/ai_plugins/test_ai_plugins.py`      | complete | 16 smoke tests: CRUD for all 4 auth types, auth type switching, OAuth edit two-mutation flow, GlobalSettings consistency, inline OAuth server, user plugin settings (NONE + OAuth), OAuth connect URL, admin test-connection, API key save/disconnect, validation constraints, list services |
| `datahub-graphql-core/.../resources/aiPlugin.graphql` | complete | New schema file for AI plugin workflow mutations (`upsertAiPlugin`, `deleteAiPlugin`, `UpsertAiPluginInput`)                                                                                                                                                                                 |
...
| `tests/unit/oauth/test_router.py`                                | complete | Added 24 endpoint tests (4.2), then 6 more for token exchange edge cases + callback error scenarios (4.1). Total: 93        |
| `tests/unit/oauth/test_credential_store.py`                      | complete | Added 11 tests: refresh token rotation (2), 5-min buffer edge cases (6), on_refresh callback (2), expired refresh token (1) |
| `chat/chat_session_manager.py`                                   | complete | Plugin connection error messages now include root cause detail as Markdown blockquote                                       |
| `datahub-graphql-core/.../resources/service.graphql`             | complete | Moved AI plugin mutations to `aiPlugin.graphql`; kept Service/OAuth entity types and queries                                |
| `datahub-graphql-core/.../resources/app.graphql`                 | complete | Removed old `deleteAiPlugin(id)` mutation; added `manageConnections` to `PlatformPrivileges`                                |
| `datahub-graphql-core/.../GmsGraphQLEngine.java`                 | complete | Updated wiring for renamed resolvers + new `aiPlugin.graphql` schema; removed old `deleteAiPlugin` wiring                   |
| `datahub-graphql-core/.../Constants.java`                        | complete | Added `AI_PLUGIN_SCHEMA_FILE` constant                                                                                      |
| `datahub-graphql-core/.../MeResolver.java`                       | complete | Populates `manageConnections` privilege                                                                                     |
| `datahub-web-react/.../me.graphql`                               | complete | Added `manageConnections` to `getMe` query                                                                                  |
| `datahub-web-react/.../SettingsPage.tsx`                         | complete | AI settings page visible with either `manageGlobalSettings` or `manageConnections`                                          |
| `datahub-web-react/.../hooks/usePluginActions.ts`                | complete | Updated to use `useUpsertAiPluginMutation`; delete now passes `urn` not `id`                                                |
| `datahub-web-react/.../hooks/__tests__/usePluginActions.test.ts` | complete | 11 tests: buildToggleInput (4), handleToggleEnabled (4), handleDelete (3)                                                   |

#### Remaining Work

| Task                             | Status   | Notes                                                                                                               |
| -------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------- |
| 4.1 Improve test coverage        | complete | Python smoke tests, token refresh edge cases, OAuth error handling. Cascading delete tests moved to 4.4             |
...
| 4.4 Cascading deletes rework     | pending  | Deletion hook or GC, `@Relationship` annotations, searchable reference fields, cascading delete tests               |
...
| `oauth/router.py`                                             | Added `test-oauth-connect` endpoint, `McpDiscoveryFlow` branch in callback, `_handle_test_mcp_discovery_callback` helper, `_create_test_result_popup` with XSS escaping, XSS fix in existing `_create_popup_response`, reordered disconnect to remove settings before credentials |
...
| `datahub-web-react/.../aiConnections/useOAuthConnect.ts`      | Safari popup blocker fix: synchronous `window.open('about:blank')` + navigate after fetch. Interval leak fix. Switched from antd `message` to alchemy `notification`                                                                                                              |
| `datahub-web-react/.../myIntegrations/ManageMyAiSettings.tsx` | Changed fetchPolicy from `cache-and-network` to `network-only` to prevent stale data after disconnect                                                                                                                                                                             |
