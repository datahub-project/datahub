---
description: "What's new in DataHub Cloud Action Workflows — quorum policies, conditional step branching, dynamic actor resolution, context-aware form pickers, and an expanded category model on top of Data Access Workflows."
---
...
Workflow side-effects are implemented as [DataHub Actions](https://docs.datahub.com/docs/actions) subscribed to the workflow lifecycle events, exactly as in Data Access Workflows. The events and subscription pattern are unchanged; what shifts in practice is that v2 customers commonly drive _internal_ catalog side-effects alongside external-system provisioning — flipping a structured property to mark a dataset Certified, attaching a glossary term to record the outcome, or propagating metadata downstream when the workflow completes.
