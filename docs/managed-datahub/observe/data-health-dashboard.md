...
1. **Define a search predicate** — specify the datasets you want to monitor using filters such as DataHub Domain, data platform, schema, tags, or any combination of search criteria. For a Container filter, **Within** selects tables under the chosen database (including nested containers), while **Equals** matches that container only. For a Domain filter, **Within** includes nested subdomains while **Equals** matches that domain only.
