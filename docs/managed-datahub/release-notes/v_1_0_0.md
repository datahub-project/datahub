...
- **CLI/SDK**: 1.5.0.19
- **Remote Executor**: v1.0.3-cloud
- **Helm**: 1.6.141

#### Product

- **Schema field drawer — column statistics**: Restored column statistics in the Statistics drawer by switching to case-insensitive field path matching, so stats render correctly for datasets whose column paths differ in case from the underlying schema.
- **Observe - Assertions**: Fixed an issue where assertions checking against the value 0 (ex. Row count is 0) would encounter a runtime error. Fix requires Remote Executor upgrade to v1.0.3-cloud.
- **Action Workflows — Task Center assignees**: Fixed an issue where approval tasks for workflow form requests could disappear from assignees' Task Center after a periodic owner-sync. Workflow form requests now retain the assignees set by their workflow step definition.

#### Platform

- **Ebean — autoCommit on connection pool**: Added `autoCommit` configuration on the Ebean DataSource connection pool by default. Will enable gradually to eliminate tens of thousands of redundant `COMMIT` statements per second from read-only queries and reducing InnoDB group-commit contention under high connection counts. Set `EBEAN_DATASOURCE_AUTOCOMMIT` to modify the behavior.
- **Elasticsearch**: Fixes for Elasticsearch 8 and its re-index on every upgrade/install even when the index settings/mappings were not modified.
...
- **Observe — Assertion notification chart**: Slack assertion notifications now include a chart showing recent assertion results and downstream impact context, with performance improvements for smart-assertion charts.
- **Observe — Assertion notification redesign**: Backported Slack assertion notification redesign with downstream impact summary.
- **Observe — Push assertion errors to alerts**: Surface assertion error details directly in the alert health message UI so on-call engineers see the underlying failure reason without drilling into the assertion.
- **Observe — Volume assertion decimal thresholds**: Volume row-count change assertions now accept decimal values for percentage-based thresholds (absolute row counts remain integer-only).
- **Observe - UTC timezone always available**: Ensure UTC is present in the timezone selector for Assertions
- **Schema field drawer**: Column statistics are now shown in full inline on the About tab of the Schema Field Drawer.
- **Dataset sidebar Edit/Generate actions**: Fixed missing Edit/Generate actions in the dataset sidebar.

#### AI / Ask DataHub

- **Slack bot — respond to bot mentions safely**: Ask DataHub no longer replays other bots' messages as its own assistant history, and now correctly pivots to the last human in a thread when another bot @-mentions it. Self-mentions are dropped, and a friendly refusal is posted when no human can be attributed.
- **Slack bot — analytics on bot-triggered traffic**: `triggered_by_bot`, `triggering_bot_id`, and `effective_asker_user_id` added to `ChatbotInteractionEvent` for monitoring and debugging bot-to-bot interactions.
- **MCP tools — custom ownership types**: `add_owners` / `remove_owners` MCP tools now accept custom ownership types by name (case-insensitive), matching what users see in the UI, in addition to the built-in `OwnershipType` enum values.

#### Platform

- **Zero-downtime upgrade (ZDU) — prevent overwrites during background sweep**: Background aspect schema-version migration sweep no longer overwrites concurrently-updated aspects.
- **System update — protect env vars during scale-down**: Fixed an issue where the system update path could incorrectly patch container environment variables when the consolidated upgrade went through scale-down.
- **Ingestion source resolution**: GraphQL `IngestionResolver` now resolves the correct originating source.
- **Dashboard stats**: Fixed `NullPointerException` in `DashboardUsageStatsUtils` when `DashboardUserUsageCounts.usageCount` is null (e.g. some Superset usage paths), restoring dashboard usage summaries.

#### Ingestion / Executor

- **Snowflake executor — preserve mixed-case identifiers**: All identifiers are now quoted in the Snowflake executor source so assertion and observability flows correctly handle mixed-case object names.
- **Redshift executor**: Removed the Redshift connection-closed pre-check that was causing spurious failures.

#### Security / Dependencies

- **Spring Boot 4.0.6**: Bumped Spring Boot from 4.0.5 to 4.0.6 across all 27 Gradle modules to address CVE-2026-22751 and related advisories.
- **pycurl**: Bumped minimum `pycurl` version to satisfy CVE minimums.
...
- Observe: BigQuery, Databricks, and Redshift assertions that perform live warehouse queries will now time out after 10 minutes to avoid resource hogging. These limits can be adjusted via the `DATAHUB_EXECUTOR_BIGQUERY_TIMEOUT`, `DATAHUB_EXECUTOR_DATABRICKS_TIMEOUT`, and `DATAHUB_EXECUTOR_REDSHIFT_TIMEOUT` environment variables respectively. Snowflake is already bounded by this limit, which is controlled via the `DATAHUB_EXECUTOR_SNOWFLAKE_TIMEOUT` variable.
