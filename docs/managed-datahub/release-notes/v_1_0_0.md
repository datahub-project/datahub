...
- **Ebean — autoCommit on connection pool**: Enabled `autoCommit=true` on the Ebean DataSource connection pool by default, eliminating tens of thousands of redundant `COMMIT` statements per second from read-only queries and reducing InnoDB group-commit contention under high connection counts. Set `EBEAN_DATASOURCE_AUTOCOMMIT=false` to restore the prior behavior.
