...
- **Assertions & data health UI**: Filter assertions by **tags** and **custom assertion types**; **remote executor** setup surfaces pool/executor version and health; fixes for bucketed assertions, smart prediction bounds, smart **volume** backfill, and **Databricks** freshness validation; monitor **error messages** are truncated to a safe maximum length before storage so oversized failures cannot break writes.
