...
- **Security Fix** The default signing key and salt values present in application.yaml used in [Metadata Service Authentication](https://docs.datahub.com/docs/authentication/introducing-metadata-service-authentication#configuring-metadata-service-authentication) have been removed. The application now requires these values to be provided via env variables DATAHUB_TOKEN_SERVICE_SIGNING_KEY and DATAHUB_TOKEN_SERVICE_SALT to the GMS deployment, consumers and other related services and jobs.
  IMPACT: Due to the change in the signing key and salt values existing PATs will be invalidated and will have to be regenerated. See notes on backward compatibility below.
