...
Patch release on top of **v0.3.17.8-acryl**.
...
- **CLI/SDK**: 1.5.0.13
- **Remote Executor**: v0.3.17.9-acryl
- **Helm**: 1.6.108

#### Bug Fixes

- **Observe SQL Assertion Validation:** Allow read-only SQL set operations and subqueries in SQL assertions

### v0.3.17.8

Patch release on top of **v0.3.17.6-acryl**.
...
- **CLI/SDK**: 1.5.0.13
- **Remote Executor**: v0.3.17.58-acryl
- **Helm**: 1.6.108

#### Improvements

- **Page Load Performance:** Optimized page load times on various parts of the product.

### v0.3.17.7

This release was immediately subsumed in v0.3.17.8

### v0.3.17.6

Patch release on top of **v0.3.17.5-acryl**.
...
- **CLI/SDK**: 1.5.0.13
- **Remote Executor**: v0.3.17.6-acryl
- **Helm**: 1.6.108

#### Bug Fixes

- **Ask DataHub:** Fixed ability to response to mentions in Slack.
- **Authorization:** Fixed missing proposals in Task Center when View Authorization is turned on
...
- **CLI/SDK**: 1.5.0.13
- **Remote Executor**: v0.3.17.5-acryl
- **Helm**: 1.6.108

#### Bug Fixes

- **Executor Dependency Bump:** `acryl-datahub` dependency bumped to `1.5.0.13` to address transient assertion "source connection" errors during concurrent evaluation.

### v0.3.17.4

Patch release on top of **v0.3.17.3-acryl**.
...
- **CLI/SDK**: 1.5.0.13
- **Remote Executor**: v0.3.17.4-acryl
- **Helm**: 1.6.107

#### SDK / Patch APIs

- **Python & Java patch clients:** New `add_documentation` / `remove_documentation` patch helpers (documentation patch template is now available on most asset types).

#### Slack integration

- **Dynamic optional scopes:** `channel history`, `group history`, and `im history` scopes are now registered as **optional** unless the "store conversations from Slack" feature flag is enabled, aligning with Slack's new [optional scopes model](https://docs.slack.dev/changelog/2026/03/16/optional-scopes/) so installs no longer require these scopes by default.

#### Bug Fixes

- **Remote executor (Redshift assertions):** Fixed `SOURCE_CONNECTION_ERROR: 'Connection' object has no attribute 'closed'` for Redshift assertions that were querying the warehouse.

### v0.3.17.3

Patch release on top of **v0.3.17.2-acryl**.
...
- **CLI/SDK**: 1.5.0.8
- **Remote Executor**: v0.3.17.3-acryl
- **Helm**: 1.6.88

#### Security

- **Security and dependency hygiene:** Raised Python dependency floors and refreshed lockfiles for the remote executor and integrations service, including **LiteLLM** (≥1.83.0) for CVE-2026-35030, **pyarrow** for CVE-2026-25087, plus **tornado**, **pillow**, and related alignment with bundled images. Bumped **Jetty** and refreshed Gradle lockfiles.
- **Authentication:** Removed use of the deprecated `corpUserInfo.active` flag for session eligibility and cleaned up related login behavior for inactive users (CUS-8505).
- **Ingestion:** Upgraded **sqlglot** to v30.0.3 with v30 API compatibility fixes.
- **Integrations service:** Bumped **mlflow** to 3.11.1 with more robust ML tracing teardown, plus dependency updates (**fastmcp**, **observe-models**, and related integration service bumps).

### v0.3.17.2

Patch release on top of **v0.3.17.1-acryl**.
...
- **CLI/SDK**: 1.5.0.8
- **Remote Executor**: v0.3.17.2-acryl
- **Helm**: 1.6.64

#### Bug Fixes

- **Remote executor**: Bundled **acryl-executor** is **0.3.5.1** (was 0.3.5). Capability checks for ingestion features no longer use a fixed subprocess timeout, so slow cold starts are less likely to fail those checks.
...
- **Microsoft Teams**: When enabled via feature flag, the Teams integration page can offer a **downloadable Teams app package** (.zip) with a link to the [managed DataHub Teams setup guide](/docs/managed-datahub/teams/saas-teams-setup).
...
- **Security Fix** The default signing key and salt values present in application.yaml used in [Metadata Service Authentication](/docs/authentication/introducing-metadata-service-authentication#configuring-metadata-service-authentication) have been removed. The application now requires these values to be provided via env variables DATAHUB_TOKEN_SERVICE_SIGNING_KEY and DATAHUB_TOKEN_SERVICE_SALT to the GMS deployment, consumers and other related services and jobs. IMPACT:
...
- [NEW] **Ask DataHub Plugins**: Private beta support for external MCP plugins to enable Ask DataHub to interact with Snowflake, GitHub, Databricks, BigQuery, and more. Includes support for OAuth discovery, Dynamic Client Registration, and redesigned plugin configuration flow. Read more [here](/docs/features/feature-guides/ask-datahub-plugins/overview).
...
- [NEW] **Service Accounts**: Introducing support for creating named service accounts and generating access tokens. Read more [here](/docs/features/feature-guides/service-accounts).
