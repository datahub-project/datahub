...
- **CLI/SDK**: 1.6.0.4
- **Remote Executor**: v2.0.4-cloud, v2.0.3-cloud, v2.0.2-cloud, v2.0.1-cloud, v1.1.3-cloud, v1.0.3-cloud
- **On-Prem Versions**:
  - **Helm**: 1.6.189
  - **API Gateway**: 0.7.3
...
- **ZDU stability**: Semantic search un-aliased indices are no longer deleted during orphan cleanup, preventing accidental index loss during zero-downtime upgrades.

#### Bug Fixes

- Fixed hidden documents incorrectly inflating search UI entity counts causing platform counts to appear much larger than what they are in reality.
- Fixed metadata propagation not matching target columns by simple field name, causing propagation to silently skip columns with nested field paths.
- Fixed inability to add multiple datasets to an existing incident entity via the API.
- Fixed ES8 kNN semantic search queries failing due to un-normalized `adjust_pure_negative` parameter.
- Fixed Search V2.5 rescore phase duplicating or dropping results.

### v2.0.3

This release rolls up hotfixes on top of v2.0.2.
...
- **CLI/SDK**: 1.6.0.4
- **Remote Executor**: v2.0.3-cloud, v2.0.2-cloud, v2.0.1-cloud, v1.1.3-cloud, v1.0.3-cloud
- **On-Prem Versions**:
  - **Helm**: 1.6.189
  - **API Gateway**: 0.7.3
...
- Fixed an issue with resolving Slack integration connections, which caused the UI falsely indicate that Slack was not connected.
...
- **CLI/SDK**: 1.6.0.4
- **Remote Executor**: v2.0.2-cloud, v2.0.1-cloud, v1.1.3-cloud, v1.0.3-cloud
- **On-Prem Versions**:
  - **Helm**: 1.6.189
  - **API Gateway**: 0.7.3
...
- Stability improvements for ZDU upgrade when doing upgrades to `2.0.x` version.
...
- **Anomaly Detection Assertion Improvements** — Volume assertions that use Anomaly Detection will now see more accurate predictions, reducing false alarms. This improvement is most pronounced on datasets with large row counts. This improvement requires upgrading your remote executor.
- **Assertion ownership** — assertions now support owners. Ownership is viewable on both the Data health dashboard and the dataset quality tab. Monitoring rules will reflect the rule owner as the generated assertion owner.
- **Assertion SQL Query Replay** - You can now see the exact queries issued by assertions against your warehouse, enabling you to replay failures to find failing records.
...
- **Assertion failure severity configuration SDK** — programmatic configuration of assertion failure severity behavior.
- **Severity-escalation broadcast in Slack** - changes in failure severity will be broadcasted to the main thread in Slack notifications.
...
- **Document entity: CRUD privileges plus `MANAGE_DOCUMENTS`.** Document CREATE accepts standard entity create privileges (`CREATE_ENTITY` or `EDIT_ENTITY`) or `MANAGE_DOCUMENTS`. UPDATE / DELETE still accept `EDIT_ENTITY` / `DELETE_ENTITY` (or `MANAGE_DOCUMENTS`). Bridge documents inherit VIEW from the source entity; writes do not inherit.
...
- Fixed an issue where falsy assertion parameters (ex. row count = 0) caused assertion errors.
