...
- **Observe - Anomaly Detection:** Several improvements made to assertion anomaly detection algorithm, primarily improving volume assertions. This should greatly reduce false positives and poor predictions.
- **Observe - Assertions:** Assertions with falsy parameter values (e.g. `0`) now evaluate correctly.
- **Observe - Assertions (`_NATIVE_` operator):** The Quality/Assertions tab keeps rendering for
  `_NATIVE_` operators, and `_NATIVE_` is now rejected on FieldValues/FieldMetric assertions.
- **Action Workflows:** Skip `WORKFLOW_FORM_REQUEST` in the action-request owner source.
- **Generate Documentation:** The AI documentation-generation flow is now available on the
  Dataset Summary page.

#### AI / MCP

- **Ask DataHub:** Raised UI and default response truncation limits to 100k characters so longer answers are no longer cut off.

#### Platform

- **Search (Elasticsearch 8):** Fixed a settings reindex loop and removed range-query
  deprecation warnings.

#### Ingestion / Executor

- **Remote Executor:** Added the `ssh` binary to the executor image (enables SSH-based connections during ingestion/assertion runs).

### v1.1.0

Includes all upstream OSS changes through DataHub Core [v1.6.0](https://github.com/datahub-project/datahub/releases/tag/v1.6.0) — see [Updating DataHub](/docs/how/updating-datahub) for breaking changes.
...
- **Assertion Failure Severity** — Assertion failures now include a `severity` classification that indicates how significant the failure is. Thresholds can be manually configured through [Severity Assignment Rules](/docs/managed-datahub/observe/assertions#assertion-failure-severity), or automatically determined by DataHub using signals like deviation from expected bounds, asset importance, and potential downstream impact. [SDK tutorial](/docs/api/tutorials/assertions#configure-failure-severity).
...
v1.1.0 inherits breaking changes from upstream OSS v1.6.0. Scan the categories below to see if any apply to your deployment — full details, exact configs, and migration actions are in [Updating DataHub — v1.6.0](/docs/how/updating-datahub#v160).
...
- **Databricks Improvements** — `TABLE_STATISTICS` metric source added. This runs [ANALYZE TABLE ... COMPUTE STATISTICS](https://docs.databricks.com/sql/language-manual/sql-ref-syntax-aux-analyze-table.html) followed by DESCRIBE TABLE EXTENDED, which reads the cached numRows from the catalog rather than scanning data. On Delta tables this is a metadata-only operation, making it significantly cheaper than a COUNT(\*) query on large tables. [Read more](/docs/managed-datahub/observe/volume-assertions#3-volume-source).
- **BigQuery Improvements** — New `PLATFORM_API` metric source added. Uses the [tables.get](https://cloud.google.com/bigquery/docs/reference/rest/v2/tables/get) API to retrieve the current row count for a Table. This is the most cost-effective option as it does not run any SQL queries or scan any data — the row count is retrieved via a free API call. [Read more](/docs/managed-datahub/observe/volume-assertions#3-volume-source).
...
- **Java 25 LTS** runtime in official Docker images; java 25 build toolchain.
