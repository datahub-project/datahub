...
This release rolls up hotfixes on top of v2.1.6. There are no breaking changes or deprecations.

:::caution
Remote Executor **v2.1.6-cloud** is withdrawn: SQS-channel workers on it never receive messages (Kafka-channel workers are unaffected).
:::
...
- **CLI/SDK**: 1.6.0.16
- **Remote Executor**: v2.1.7-cloud, v2.1.5-cloud, v2.1.4-cloud, v2.1.3-cloud, v2.1.2-cloud, v2.1.1-cloud, v2.1.0-cloud, v2.0.4-cloud, v2.0.3-cloud, v2.0.2-cloud, v2.0.1-cloud, v1.1.4-cloud
- **On-Prem Versions**:
  - **Helm**: 1.6.233
  - **API Gateway**: v0.7.3
...
- **AWS IRSA credential providers (MSK / JDBC / Iceberg)** — remaining IRSA `StsAssumeRoleWithWebIdentityCredentialsProvider` leaks after v2.1.6 are closed. GMS owns a single process-wide `DefaultCredentialsProvider` (closed on shutdown) and shares it with S3, STS, SQS, Bedrock, MSK IAM callbacks, AWS JDBC IAM token minting, Iceberg warehouse credential vending, and S3 backup restore. AWS SDK Java v2 is pinned to **2.54.16**, `aws-msk-iam-auth` to **2.3.8**, and `aws-advanced-jdbc-wrapper` to **4.4.0**. **Action:** none; IRSA token-refresh task counts should stay near-flat after upgrade.

#### Bug Fixes

- Fixed Remote Executor workers on the **SQS channel** appearing healthy (`Connected to sqs`, `ready`, passing probes) but never receiving messages after v2.1.6 Wolfi images shipped a post-quantum `openssl.cnf` that the PyPI `pycurl` wheel's bundled OpenSSL 3.4 cannot parse. The executor now builds `pycurl` against the image OpenSSL. Kafka-channel workers were unaffected. **Action:** upgrade SQS-channel Remote Executors to **v2.1.7-cloud**.
