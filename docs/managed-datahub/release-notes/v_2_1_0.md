...
- **Expanded internationalization** — multi-language support is now on by default, and DataHub follows the browser locale where a translation exists.
...
- **Data Product Lineage** - Trace relationships between data products and the data assets that make them up. Lineage now supports visibility from one data product to another through upstream and downstream dependencies. It’s easier to assess downstream impact before changing or decommissioning assets and to identify which data products may be affected during incident response. 
