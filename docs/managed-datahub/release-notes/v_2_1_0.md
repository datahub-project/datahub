...
- **Multi-provider LLM support.** A single `DATAHUB_LLM_PROVIDER` setting selects the provider (Bedrock, Anthropic, OpenAI, or Vertex AI) whose recommended defaults apply across every LLM use case. When unset, the provider is auto-detected from the environment (Vertex AI on GCP, otherwise Bedrock).
