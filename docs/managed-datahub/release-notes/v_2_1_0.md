...
- **GMS usage-metric semantics (opt-in).** With `USAGE_AGGREGATION_ENABLED=true`, `datahub_request_count` is emitted from the aggregation flush (not per-request) and the actor tag becomes `actor_class`. New byte and active-identity metrics are added. Aggregation is off by default; legacy JMX metrics are unaffected.
- **GMS rate-limit configuration renamed.** `defaultRetryAfterSeconds` / `RATE_LIMITS_DEFAULT_RETRY_AFTER` are renamed to `minRetryAfterSeconds` / `RATE_LIMITS_MIN_RETRY_AFTER` (now a minimum floor), and a retry-after jitter percentage is added. An external rate-limit policy file is now loaded by Spring: `RATE_LIMITS_CONFIG_FILE_ENABLED` and `RATE_LIMITS_CONFIG_JSON` are removed, `RATE_LIMITS_CONFIG_FILE` must be a scheme-qualified URI (for example `file:/etc/datahub/rate-limits.yaml`), and a mounted policy file must be rooted under `datahub.gms.rateLimits:`. Migrate these before upgrading if you set them.
...
- **Smart-assertion inference selector split.** `DATAHUB_USE_OBSERVE_MODELS=true` no longer implies the V2 trainer. Any executor relying on it for V2 must also set `DATAHUB_USE_INFERENCE_V2=true`, or it falls back to the V1 trainer after upgrade.
...
- **Slack experience.** The connect prompt now leads with what DataHub does and offers a one-click "I've connected" button that replays the user's original question. Slack and Microsoft Teams connection state is now read from dedicated GraphQL status queries (a single source of truth) rather than decoding the credential blob, fixing settings pages that could show a working integration as disconnected.
- **Custom agents billed separately from Ask DataHub.** Custom agents now report distinct usage telemetry and no longer consume the Ask DataHub message-credit pool. Core Ask DataHub modes, the Ingestion Troubleshooter, and the Data Observability agent remain classified as Ask DataHub usage. No customer-facing behavior change — this only affects usage categorization.
...
- **Smart-assertion inference V2.** Smart-assertion training can route to a V2 trainer pipeline, selected by the new `DATAHUB_USE_INFERENCE_V2` flag (default `false`); this is now decoupled from `DATAHUB_USE_OBSERVE_MODELS`.
- **Delta-space bounds gating.** The V1 trainer only explores and persists differenced (delta-space) prediction bounds when `DATAHUB_EXECUTOR_ENABLE_DELTA_BOUNDS=true` (default `false`), protecting mixed-fleet rollouts from misreading delta bounds as absolute.
- **Lower-footprint executor builds.** With `DATAHUB_USE_OBSERVE_MODELS=false`, the V1 trainer no longer imports the observe-models package (PyTorch/Darts) at module load, so slim and low-RAM executor builds no longer risk an import-time crash loop.
...
- **Multi-dimensional GMS rate limiting.** Rate limiting gains adaptive capacity, a per-actor / per-class / global scoped chain, and per-resolver gates, building on the distributed rate limiting introduced in v2.0.0.
- **Usage aggregation and metering.** GMS can aggregate request and byte usage in memory and emit it on a flush window (opt-in via `USAGE_AGGREGATION_ENABLED`), with new Micrometer metrics for input/output bytes and active identities, plus a report-driven metering path. A SaaS billing overlay consumes these totals.
...
- **Periodic analytics roll-ups.** Two opt-in, system-managed ingestion sources compact usage events into hourly, daily, and monthly roll-up layers and compute auditable calendar-month usage totals. Both are inert by default.
