...
- **Usage aggregation (Micrometer / Prometheus).** GMS can aggregate request and byte usage in memory and export it on a flush window (opt-in via `USAGE_AGGREGATION_ENABLED`), with Micrometer metrics for input/output bytes and active identities. Off by default; legacy JMX `requestContext_*` metrics are unaffected. Optionally set `USAGE_AGGREGATION_INCLUDE_AGENT_NAME=true` to roll up by client `agent_name` (default `false` — unbounded client-controlled values can multiply metric cardinality).
- **Entity count gauges (Micrometer / Prometheus).** GMS periodically refreshes key-aspect entity inventory counts and exports them as Micrometer gauges (`datahub.entity.count`, tagged by `entity_type` and `removal_status` for active vs soft-deleted), plus refresh duration/error/last-success metrics. On by default (`ENTITY_COUNT_METRICS_ENABLED=true`); refresh interval defaults to hourly after a 60s startup delay.
- **OpenAPI entity counts.** `GET /openapi/v1/entities/counts` and `GET /openapi/v1/entities/{entityType}/count` return active and soft-deleted counts from the system-metadata key-aspect index (optional `types`, `includeTotal`, and `skipCache` query params). Requires entity READ authorization.
- **Primary storage read pool.** Optional second Ebean connection pool or Cassandra session for non-locking entity-aspect reads (`EBEAN_READ_POOL_ENABLED` / `CASSANDRA_READ_POOL_ENABLED`, default off). Point `EBEAN_READ_POOL_URL` or `CASSANDRA_READ_POOL_HOSTS` at a replica to offload reads, or omit for same-DB split-pool isolation. Writes and `FOR UPDATE` reads stay on primary. See [Primary storage read pool](../../deploy/primary-storage-read-pool.md).
