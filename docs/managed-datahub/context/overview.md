...
An Admin must assign users to the [Editor role](/docs/authorization/roles), or grant the individual privileges below, so that expert reviewers and SMEs can review generated context documents.

### 3. Configure your data source connectors

Configure [data source connectors](/docs/metadata-ingestion) with **query ingestion enabled**. Supported sources in Private Beta: **Snowflake, Databricks, BigQuery, and Redshift**.
...
Configure [Domains](/docs/domains) with owners (users or groups) and assets. Context generation is scoped by domain, and domain owners are who receive context proposals for review.
