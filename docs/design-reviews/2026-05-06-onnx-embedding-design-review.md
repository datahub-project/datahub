...
**Status: Resolved (variant images).** ONNX/DJL are now `compileOnlyApi` in `metadata-io`, so they are off the runtimeClasspath of every default image (GMS, `datahub-upgrade`, mce/mae-consumer) — ~71MB removed. They are re-added only in a second boot archive (`onnxBootJar` → `war-onnx.war` / `datahub-upgrade-onnx.jar`) whose `BOOT-INF/lib` carries them, built into the dedicated `datahub-gms-onnx` + `datahub-upgrade-onnx` images (the default Dockerfiles select the archive via a `WAR_FILE`/`UPGRADE_JAR` build arg). No `COPY`-jars hack is used — the jars live inside the variant archive as required: both GMS launch modes resolve from the archive (the default `-jar war.war`, or the deterministic classpath built from `BOOT-INF/classpath.idx` when `EXTRACT_JAR_ENABLED=true`), and `datahub-upgrade`'s `-Dloader.path` is inert (JarLauncher ignores it), so a loose `COPY` would never land on the classpath. `OnnxEmbeddingProvider` loads only in the factory `"onnx"` arm, so a default image with `EMBEDDING_PROVIDER_TYPE=onnx` fails there rather than at startup.
