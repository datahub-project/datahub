...
1. **System-update (datahub-upgrade)**: The default retention policies are ingested when you run the **non-blocking** system-update (`IngestRetentionPolicies` step). The default YAML is bundled in the datahub-upgrade module (`boot/retention.yaml`). On each system-update run, policies are compared to the stored config at each exact `(entity, aspect)` key; when the desired config differs and the stored policy is system-managed (or you opt in to overwrite), the policy is updated. When policies change, a batch apply trims historical aspect versions (enabled by default via `ENTITY_SERVICE_RETENTION_APPLY_ON_POLICY_CHANGE=true`). Set `ENTITY_SERVICE_APPLY_RETENTION_BOOTSTRAP=true` to force a full-database batch apply even when no policy ingest changed. Batch apply pages candidate `(urn, aspect)` pairs with keyset pagination (ordered by `urn`, `aspect`) and only considers pairs that have historical versions (`version > 0`), avoiding deep `OFFSET` scans on large `metadata_aspect_v2` tables.
...
Resolved policies (after `entity/aspect` → `entity/*` → `*/aspect` → `*/*` fallback) are read through the GMS `SystemEntityClient`'s process-local entity/aspect cache, keyed by the `dataHubRetention`/`dataHubRetentionConfig` entity-aspect pair (see `cache.client.entityClient.entityAspectTTLSeconds.dataHubRetention.dataHubRetentionConfig` in `application.yaml`, default 6 hours). This avoids a second primary-storage read on every aspect write, reusing the same cache that already backs other entity/aspect reads rather than a retention-specific one.

Policy writes via the API or system-update invalidate the cached entry for the affected retention urn in that process. Other pods keep serving the previous resolved policy — including `maxVersions` used to decide whether an aspect write inserts a history row — until the TTL expires.
