...
Resolved policies (after `entity/aspect` → `entity/*` → `*/aspect` → `*/*` fallback) are cached for 10 minutes (`CACHE_RETENTION_POLICY_TTL_SECONDS`, default `600`), matching the shared Caffeine `cache.primary` TTL and max size (10k). The working set is one entry per `(entity type, aspect)` pair (~680 in the OSS registry, not per URN). The cache is process-local Caffeine by default and is promoted to the shared Hazelcast map when `SEARCH_SERVICE_CACHE_IMPLEMENTATION=hazelcast`. Policy writes via the API or system-update invalidate the cache in that process; other pods pick up changes on TTL expiry. Set `CACHE_RETENTION_POLICY_ENABLED=false` to disable.
