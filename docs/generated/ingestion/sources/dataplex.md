...
- **`read_export`** — submits no jobs and instead ingests the output of metadata exports you run yourself (Cloud Scheduler, Workflows, a separate pipeline, etc.). Requires the `read_export_config` section and only storage read access (see below).
...
Notes on `read_export` mode (`read_export_config.export_paths`):

- Map each location to the `gs://bucket[/prefix]` output path of a metadata export you orchestrate yourself. The connector submits no jobs and only reads those paths, so the service account needs just `roles/storage.objectViewer` on the buckets — no Dataplex job-submission roles.
...
- The entries stage reads exactly the locations in this mapping; `entries_locations` applies to the other stages (lineage, glossaries).
...
For `extraction_method: read_export`, none of the grants above are needed — the connector only requires [`roles/storage.objectViewer`](https://cloud.google.com/storage/docs/access-control/iam-roles) on the bucket(s) holding the pre-existing export output.
...
source:
  ...
  config:
    ...
    # Optional: Extraction method (default: api)
    # Use 'export' for central-catalog / federated setups where assets surface
    # in a catalog project via Dataplex catalog linking and list_entries returns
    # nothing. One metadata export job runs per entries location; each location
    # must resolve to a GCS bucket.
    # extraction_method: export
    # export_config:
    #   export_job_runner_project: "my-runner-project" # needs roles/dataplex.metadataJobOwner
    #   bucket_base_name: "my-export-bucket" # bucket per location: {base}-{location}
    #   # export_bucket_config: # explicit per-location buckets (take priority)
    #   #   us: "my-export-bucket-us"
    #   # prefix: "dataplex-exports" # optional folder prefix inside each bucket (max 128 chars)
    #   # export_poll_seconds: 15
    #   # export_timeout_seconds: 3600

    # Or: ingest exports you orchestrate yourself instead of submitting jobs
    # (only needs roles/storage.objectViewer on the buckets).
    # extraction_method: read_export
    # read_export_config:
    #   export_paths:
    #     us: "gs://my-export-bucket-us/dataplex-exports"
...
| <div className="path-line"><span className="path-main">extraction_method</span></div> <div className="type-name-line"><span className="type-name">Enum</span></div> | One of: "api", "export", "read_export" <div className="default-line default-line-with-docs">Default: <span className="default-value">api</span></div> |
...
| <div className="path-line"><span className="path-main">export_config</span></div> <div className="type-name-line"><span className="type-name">One of DataplexExportJobConfig, null</span></div> | Settings for extraction_method 'export' (job runner project, GCS buckets, polling). Required for 'export', not allowed otherwise. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
| <div className="path-line"><span className="path-prefix">export_config.</span><span className="path-main">export_job_runner_project</span>&nbsp;<abbr title="Required if export_config is set">❓</abbr></div> <div className="type-name-line"><span className="type-name">string</span></div> | GCP project that runs the Dataplex metadata export jobs. The service account needs roles/dataplex.metadataJobOwner on this project.  |
| <div className="path-line"><span className="path-prefix">export_config.</span><span className="path-main">bucket_base_name</span></div> <div className="type-name-line"><span className="type-name">One of string, null</span></div> | Fallback base GCS bucket name used when a location is not listed in 'export_bucket_config'. The bucket name is derived as '{bucket_base_name}-{location}'. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
| <div className="path-line"><span className="path-prefix">export_config.</span><span className="path-main">export_bucket_config</span></div> <div className="type-name-line"><span className="type-name">map(str,string)</span></div> |   |
| <div className="path-line"><span className="path-prefix">export_config.</span><span className="path-main">export_poll_seconds</span></div> <div className="type-name-line"><span className="type-name">integer</span></div> | Polling interval (seconds) while waiting for export jobs to finish. <div className="default-line default-line-with-docs">Default: <span className="default-value">15</span></div> |
| <div className="path-line"><span className="path-prefix">export_config.</span><span className="path-main">export_timeout_seconds</span></div> <div className="type-name-line"><span className="type-name">integer</span></div> | Total wait timeout (seconds) for all export jobs to finish. <div className="default-line default-line-with-docs">Default: <span className="default-value">3600</span></div> |
| <div className="path-line"><span className="path-prefix">export_config.</span><span className="path-main">prefix</span></div> <div className="type-name-line"><span className="type-name">One of string, null</span></div> | Optional folder prefix inside each export bucket. The Dataplex Metadata Export API limits the custom prefix to 128 characters. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
...
| <div className="path-line"><span className="path-prefix">filter_config.entries.</span><span className="path-main">fqn_pattern</span></div> <div className="type-name-line"><span className="type-name">AllowDenyPattern</span></div> | A class to store allow deny regexes. <br />  <br /> Patterns are matched against the start of the string only, not the entire <br /> string - a pattern does not need to match to the end to be considered a match. <br /> For example, the pattern "prod" matches "prod", "prod_east", and "production". <br /> To require an exact match, anchor your pattern explicitly, e.g. "^prod$".  |
| <div className="path-line"><span className="path-prefix">filter_config.entries.fqn_pattern.</span><span className="path-main">ignoreCase</span></div> <div className="type-name-line"><span className="type-name">One of boolean, null</span></div> | Whether to ignore case sensitivity during pattern matching. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
| <div className="path-line"><span className="path-prefix">filter_config.entries.</span><span className="path-main">pattern</span></div> <div className="type-name-line"><span className="type-name">AllowDenyPattern</span></div> | A class to store allow deny regexes. <br />  <br /> Patterns are matched against the start of the string only, not the entire <br /> string - a pattern does not need to match to the end to be considered a match. <br /> For example, the pattern "prod" matches "prod", "prod_east", and "production". <br /> To require an exact match, anchor your pattern explicitly, e.g. "^prod$".  |
...
| <div className="path-line"><span className="path-prefix">filter_config.entry_groups.</span><span className="path-main">pattern</span></div> <div className="type-name-line"><span className="type-name">AllowDenyPattern</span></div> | A class to store allow deny regexes. <br />  <br /> Patterns are matched against the start of the string only, not the entire <br /> string - a pattern does not need to match to the end to be considered a match. <br /> For example, the pattern "prod" matches "prod", "prod_east", and "production". <br /> To require an exact match, anchor your pattern explicitly, e.g. "^prod$".  |
...
| <div className="path-line"><span className="path-main">read_export_config</span></div> <div className="type-name-line"><span className="type-name">One of DataplexReadExportConfig, null</span></div> | Settings for extraction_method 'read_export' (paths of pre-existing export output). Required for 'read_export', not allowed otherwise. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
| <div className="path-line"><span className="path-prefix">read_export_config.</span><span className="path-main">export_paths</span>&nbsp;<abbr title="Required if read_export_config is set">❓</abbr></div> <div className="type-name-line"><span className="type-name">map(str,string)</span></div> |   |
...
```javascript
{
  "$defs": {
    ...
    "DataplexExportJobConfig": {
      ...
      "properties": {
        ...
        "export_bucket_config": {
          ...
          "description": "Explicit mapping of GCP location to GCS bucket name. Example: {us: my-bucket-us, us-east5: my-bucket-east5}. Entries here take priority over 'bucket_base_name'.",
          "title": "Export Bucket Config",
          "type": "object"
        },
        "bucket_base_name": {
          "anyOf": [
          ...
          "default": null,
          "description": "Fallback base GCS bucket name used when a location is not listed in 'export_bucket_config'. The bucket name is derived as '{bucket_base_name}-{location}'.",
          "title": "Bucket Base Name"
        },
        "prefix": {
          "anyOf": [
            {
          ...
          "default": null,
          "description": "Optional folder prefix inside each export bucket. The Dataplex Metadata Export API limits the custom prefix to 128 characters.",
          "title": "Prefix"
        },
        "export_poll_seconds": {
          "default": 15,
    ...
    "DataplexReadExportConfig": {
      "additionalProperties": false,
      "description": "Configuration for ``extraction_method: read_export``.\n\nIngests the output of Dataplex metadata exports produced outside DataHub\n(Cloud Scheduler, Workflows, a separate pipeline, etc.). No export jobs are\nsubmitted, so the service account only needs roles/storage.objectViewer on\nthe buckets \u2014 no Dataplex job-submission roles.",
      "properties": {
        "export_paths": {
          "additionalProperties": {
            "type": "string"
          },
          "description": "Mapping of entries location to the 'gs://bucket[/prefix]' output path of an already-completed Dataplex metadata export, e.g. {us: 'gs://my-bucket-us/exports'}. If a path contains output from several export jobs (multiple 'job=<id>' partitions), only the most recently written partition is read; point the path at a specific '.../job=<id>' folder to pin an exact run. The entries stage reads exactly the locations in this mapping ('entries_locations' applies to the other stages: lineage, glossaries).",
          "title": "Export Paths",
          "type": "object"
        }
      },
      ...
      "title": "DataplexReadExportConfig",
      "type": "object"
    },
    "EntriesFilterConfig": {
      "additionalProperties": false,
  ...
  "properties": {
    ...
    "extraction_method": {
      "default": "api",
      "description": "How entries are fetched from the Universal Catalog. 'api' (default) lists entries per project via list_entry_groups / list_entries / get_entry \u2014 this only sees entries physically created in the configured projects. 'export' submits a Dataplex metadata EXPORT job per entries location (scoped to the configured projects) that writes JSONL to a GCS bucket, then reads entries from that bucket; use it for central-catalog / federated architectures where assets surface in a catalog project via Dataplex catalog linking and are invisible to list_entries. Requires 'export_config'. 'read_export' ingests the output of export jobs you run outside DataHub \u2014 no jobs are submitted and only storage read access is needed. Requires 'read_export_config'.",
      "enum": [
        "api",
        "export",
        "read_export"
    ...
    "export_config": {
      "anyOf": [
        {
          "$ref": "#/$defs/DataplexExportJobConfig"
        },
        {
          "type": "null"
        }
      ...
      "default": null,
      "description": "Settings for extraction_method 'export' (job runner project, GCS buckets, polling). Required for 'export', not allowed otherwise."
    },
    "read_export_config": {
      "anyOf": [
        {
          "$ref": "#/$defs/DataplexReadExportConfig"
        },
        {
          "type": "null"
        }
      ...
      "default": null,
      "description": "Settings for extraction_method 'read_export' (paths of pre-existing export output). Required for 'read_export', not allowed otherwise."
    },
    "entries_locations": {
    ...
    "aspect_type_pattern": {
      ...
      "description": "Regex allow/deny patterns matched against Dataplex aspect type names to decide which aspects are flattened into DataHub custom properties. Defaults to denying DataHub-authored aspects (those prefixed 'datahub-' that the DataHub sync-back writes, e.g. 'datahub-tags' / 'datahub-properties') so synced-back metadata does not return as custom-property noise."
    },
    "include_schema": {
      "default": true,
