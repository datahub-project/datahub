...
Trino is a data platform used to store and query analytical or operational data. Learn more in the [official Trino documentation](https://trino.io/).

The DataHub integration for Trino covers core metadata entities such as datasets/tables/views, schema fields, and containers. It also captures table- and column-level lineage, data profiling, and stateful deletion detection.
...
| Column-level Lineage | ✅ | Column-level lineage when emitting lineage to connector sources (e.g. Iceberg, Hive). Supported for types - Table, View. |
...
| <div className="path-line"><span className="path-main">include_column_lineage</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | When emitting upstreamLineage to connector sources (e.g. Iceberg, Hive), include column-level (fine-grained) lineage. Requires schema to be available. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
...
```javascript
{
  ...
  "properties": {
    ...
    "include_column_lineage": {
      "default": true,
      "description": "When emitting upstreamLineage to connector sources (e.g. Iceberg, Hive), include column-level (fine-grained) lineage. Requires schema to be available.",
...
If you've got any questions on configuring ingestion for Trino, feel free to ping us on [our Slack](https://datahub.com/slack).
:::
