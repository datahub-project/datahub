...
| Extract Tags | ✅ | Optionally enabled via `extract_table_tags` to extract dynamoDB table tags as DataHub tags. |
...
source:
  ...
  config:
    ...
    # Number of items to sample from each table for schema inference (default: 100)
...
| <div className="path-line"><span className="path-main">extract_table_tags</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | When enabled, tags associated with DynamoDB tables in AWS will be emitted as DataHub tags. Tags are applied using OVERWRITE mode, meaning any existing tags including tags manually added or modified from the UI will be replaced. Use with caution. <div className="default-line default-line-with-docs">Default: <span className="default-value">False</span></div> |
...
| <div className="path-line"><span className="path-main">schema_sampling_size</span></div> <div className="type-name-line"><span className="type-name">integer</span></div> | Number of items to sample from each table for schema inference. This controls how many items are scanned to determine the table's schema structure. <div className="default-line default-line-with-docs">Default: <span className="default-value">100</span></div> |
...
```javascript
{
  ...
  "properties": {
    ...
    "extract_table_tags": {
      "default": false,
      "description": "When enabled, tags associated with DynamoDB tables in AWS will be emitted as DataHub tags. Tags are applied using OVERWRITE mode, meaning any existing tags including tags manually added or modified from the UI will be replaced. Use with caution.",
      "title": "Extract Table Tags",
      "type": "boolean"
    }
  },
