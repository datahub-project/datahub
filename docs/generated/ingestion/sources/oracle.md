...
Oracle is a data platform used to store and query analytical or operational data. Learn more in the [official Oracle documentation](https://www.oracle.com/database/).

The DataHub integration for Oracle covers core metadata entities such as datasets/tables/views, schema fields, and containers. It also captures table- and column-level lineage, usage statistics, and stateful deletion detection.
...
| Dataset Usage | ✅ | Optionally enabled via `include_query_usage` to extract from V$SQL, or via `include_usage_stats` for view/procedure lineage. |
| Descriptions | ✅ | Enabled by default. |
| [Detect Deleted Entities](../../../../metadata-ingestion/docs/dev_guides/stateful.md#stale-entity-removal) | ✅ | Enabled by default via stateful ingestion. |
| [Domains](../../../domains.md) | ✅ | Enabled by default. |
| [Operation Capture](../../../api/tutorials/operations.md) | ✅ | Optionally enabled via `include_query_usage` and `include_operational_stats`. |
...
| <div className="path-line"><span className="path-main">add_database_name_to_urn</span></div> <div className="type-name-line"><span className="type-name">One of boolean, null</span></div> | Include database name in URNs. Default is False (schema.table format). Set to True for database.schema.table format when ingesting from multiple Oracle databases. <div className="default-line default-line-with-docs">Default: <span className="default-value">False</span></div> |
...
| <div className="path-line"><span className="path-main">data_dictionary_mode</span></div> <div className="type-name-line"><span className="type-name">Enum</span></div> | One of: "ALL", "DBA"  |
...
| <div className="path-line"><span className="path-main">include_lineage</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | Enable lineage extraction for stored procedures. When enabled, SQL code in procedures/functions is parsed to extract table dependencies. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
| <div className="path-line"><span className="path-main">include_materialized_views</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | Include materialized views in ingestion. Requires access to DBA_MVIEWS or ALL_MVIEWS. If permission errors occur, you can disable this feature or grant the required permissions. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
| <div className="path-line"><span className="path-main">include_operational_stats</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | Generate operation statistics from audit trail data (CREATE, INSERT, UPDATE, DELETE operations). <div className="default-line default-line-with-docs">Default: <span className="default-value">False</span></div> |
| <div className="path-line"><span className="path-main">include_query_usage</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | Extract actual query usage from V$SQL for usage statistics. Requires SELECT privilege on V$SQL (or SELECT_CATALOG_ROLE). When enabled, usage statistics will be based on actual queries executed in Oracle. <div className="default-line default-line-with-docs">Default: <span className="default-value">False</span></div> |
...
| <div className="path-line"><span className="path-main">include_stored_procedures</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | Include ingest of stored procedures, functions, and packages. Requires access to DBA_PROCEDURES or ALL_PROCEDURES. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
...
| <div className="path-line"><span className="path-main">include_usage_stats</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | Generate usage statistics via SQL aggregator. Requires observed queries to be processed. <div className="default-line default-line-with-docs">Default: <span className="default-value">False</span></div> |
...
| <div className="path-line"><span className="path-main">lazy_schema_resolver</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | If enabled, skips the upfront bulk fetch of all known schemas from DataHub when resolving lineage. Useful on large DataHub instances where the bulk fetch causes memory or performance issues. <div className="default-line default-line-with-docs">Default: <span className="default-value">False</span></div> |
| <div className="path-line"><span className="path-main">max_queries_to_extract</span></div> <div className="type-name-line"><span className="type-name">integer</span></div> | Maximum number of queries to extract from V$SQL for usage statistics. Queries are ordered by recency (most recent first). <div className="default-line default-line-with-docs">Default: <span className="default-value">1000</span></div> |
...
| <div className="path-line"><span className="path-main">start_time</span></div> <div className="type-name-line"><span className="type-name">One of string(date-time), string</span></div> | Earliest date of lineage/usage to consider. Default: Last full day in UTC (or hour, depending on `bucket_duration`). You can also specify relative time with respect to end_time such as '-7 days' Or '-7d'. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
| <div className="path-line"><span className="path-main">thick_mode_lib_dir</span></div> <div className="type-name-line"><span className="type-name">One of string, null</span></div> | Path to the directory containing the Oracle Instant Client libraries. Required on Windows and Mac when enable_thick_mode is true. Optional on Linux: when set, the connector preloads the client libraries from this directory before initializing python-oracledb, which makes thick mode work without needing ldconfig or LD_LIBRARY_PATH to be set (see https://github.com/oracle/python-oracledb/issues/578). When unset on Linux, the standard ldconfig / LD_LIBRARY_PATH search is used. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
...
| <div className="path-line"><span className="path-main">urn_db_name</span></div> <div className="type-name-line"><span className="type-name">One of string, null</span></div> | Override the database name used in URN construction. Only relevant when add_database_name_to_urn is true and service_name is used (i.e. database is not set). The connector auto-detects the name by querying sys_context('USERENV','CON_NAME') (the PDB name in multitenant setups) with a fallback to DB_NAME. Set this explicitly if auto-detection returns the wrong value — for example if your service_name does not route directly to the target PDB. Do not set this alongside database; only one should be used. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
...
| <div className="path-line"><span className="path-main">procedure_pattern</span></div> <div className="type-name-line"><span className="type-name">AllowDenyPattern</span></div> | A class to store allow deny regexes. <br />  <br /> Patterns are matched against the start of the string only, not the entire <br /> string - a pattern does not need to match to the end to be considered a match. <br /> For example, the pattern "prod" matches "prod", "prod_east", and "production". <br /> To require an exact match, anchor your pattern explicitly, e.g. "^prod$".  |
...
| <div className="path-line"><span className="path-main">query_exclude_patterns</span></div> <div className="type-name-line"><span className="type-name">One of array, null</span></div> | Regex patterns for SQL statements to exclude from usage statistics. e.g., ['^SELECT.*FROM SYS\..*', '^BEGIN.*END;'] to exclude system queries and PL/SQL blocks. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
...
| <div className="path-line"><span className="path-main">user_email_pattern</span></div> <div className="type-name-line"><span className="type-name">AllowDenyPattern</span></div> | A class to store allow deny regexes. <br />  <br /> Patterns are matched against the start of the string only, not the entire <br /> string - a pattern does not need to match to the end to be considered a match. <br /> For example, the pattern "prod" matches "prod", "prod_east", and "production". <br /> To require an exact match, anchor your pattern explicitly, e.g. "^prod$".  |
...
| <div className="path-line"><span className="path-prefix">classification.</span><span className="path-main">column_pattern</span></div> <div className="type-name-line"><span className="type-name">AllowDenyPattern</span></div> | A class to store allow deny regexes. <br />  <br /> Patterns are matched against the start of the string only, not the entire <br /> string - a pattern does not need to match to the end to be considered a match. <br /> For example, the pattern "prod" matches "prod", "prod_east", and "production". <br /> To require an exact match, anchor your pattern explicitly, e.g. "^prod$".  |
...
| <div className="path-line"><span className="path-main">profiling</span></div> <div className="type-name-line"><span className="type-name">ProfilingConfig</span></div> |   |
...
| <div className="path-line"><span className="path-prefix">profiling.</span><span className="path-main">max_distinct_per_statement</span></div> <div className="type-name-line"><span className="type-name">integer</span></div> | Only used when `query_combiner_flatten_enabled` is on. Maximum COUNT(DISTINCT) columns allowed in one flattened statement. Each one builds a distinct-value tree in server memory, so merging too many trades a scan problem for a memory problem. The default is a starting point, not a measured optimum — raise it if your server has headroom and unique counts dominate profiling time, lower it if profiling causes memory pressure. <div className="default-line default-line-with-docs">Default: <span className="default-value">5</span></div> |
...
| <div className="path-line"><span className="path-prefix">profiling.</span><span className="path-main">profiling_isolation_level</span></div> <div className="type-name-line"><span className="type-name">One of Enum, null</span></div> | Isolation level for the profiling connection. Defaults to unset, so the connection keeps the driver default and one transaction spans the whole table profile. Set AUTOCOMMIT if profiling is holding Postgres idle-in-transaction (blocking VACUUM) or pinning an InnoDB read view and growing the undo log on MySQL; each profiling SELECT then runs on its own. Under AUTOCOMMIT or READ_COMMITTED, metrics come from different snapshots, so a profile can be internally inconsistent on a concurrently-written table (e.g. uniqueCount > rowCount). On MySQL InnoDB, REPEATABLE_READ keeps a consistent snapshot; on Postgres, READ COMMITTED already takes a fresh snapshot per statement, so AUTOCOMMIT loses no consistency there. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
...
| <div className="path-line"><span className="path-prefix">profiling.</span><span className="path-main">query_combiner_flatten_enabled</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | Flattens same-shape aggregate queries into one flat SELECT per FROM group to reduce full table scans on row stores (e.g. MySQL). Requires `query_combiner_enabled`; has no effect on its own. Off by default. COUNT(DISTINCT) columns are capped per statement to bound server memory. <div className="default-line default-line-with-docs">Default: <span className="default-value">False</span></div> |
...
```javascript
{
  "$defs": {
    ...
    "DataDictionaryMode": {
      "description": "Oracle data dictionary access modes.",
      "enum": [
        "ALL",
        "DBA"
      ],
      "title": "DataDictionaryMode",
      "type": "string"
    },
    ...
    "ProfilingConfig": {
      ...
      "properties": {
        ...
        "query_combiner_flatten_enabled": {
          ...
          "title": "Query Combiner Flatten Enabled",
          "type": "boolean"
        },
        "max_distinct_per_statement": {
          "default": 5,
          "description": "Only used when `query_combiner_flatten_enabled` is on. Maximum COUNT(DISTINCT) columns allowed in one flattened statement. Each one builds a distinct-value tree in server memory, so merging too many trades a scan problem for a memory problem. The default is a starting point, not a measured optimum \u2014 raise it if your server has headroom and unique counts dominate profiling time, lower it if profiling causes memory pressure.",
          "exclusiveMinimum": 0,
          "title": "Max Distinct Per Statement",
          "type": "integer"
        },
        "catch_exceptions": {
          "default": true,
        ...
        "profiling_isolation_level": {
          "anyOf": [
            {
              "$ref": "#/$defs/ProfilingIsolationLevel"
            },
            {
              "type": "null"
            }
          ...
          "default": null,
          "description": "Isolation level for the profiling connection. Defaults to unset, so the connection keeps the driver default and one transaction spans the whole table profile. Set AUTOCOMMIT if profiling is holding Postgres idle-in-transaction (blocking VACUUM) or pinning an InnoDB read view and growing the undo log on MySQL; each profiling SELECT then runs on its own. Under AUTOCOMMIT or READ_COMMITTED, metrics come from different snapshots, so a profile can be internally inconsistent on a concurrently-written table (e.g. uniqueCount > rowCount). On MySQL InnoDB, REPEATABLE_READ keeps a consistent snapshot; on Postgres, READ COMMITTED already takes a fresh snapshot per statement, so AUTOCOMMIT loses no consistency there.",
          "schema_extra": {
            "supported_sources": [
              "mysql",
              "postgres"
  ...
  "properties": {
    ...
    "start_time": {
      "anyOf": [
        ...
        {
          "pattern": "^\\s*-\\s*\\d+(\\.\\d+)?\\s*(ms|milliseconds?|s|secs?|seconds?|m|mins?|minutes?|h|hrs?|hours?|d|days?|w|weeks?|y|years?)\\s*$",
          "type": "string"
        }
      ],
      "default": null,
    ...
    "profiling": {
      ...
      "default": {
        ...
        "query_combiner_flatten_enabled": false,
        "max_distinct_per_statement": 5,
        "catch_exceptions": true,
        "profiling_isolation_level": null,
    ...
    "urn_db_name": {
      ...
      "default": null,
      "description": "Override the database name used in URN construction. Only relevant when add_database_name_to_urn is true and service_name is used (i.e. database is not set). The connector auto-detects the name by querying sys_context('USERENV','CON_NAME') (the PDB name in multitenant setups) with a fallback to DB_NAME. Set this explicitly if auto-detection returns the wrong value \u2014 for example if your service_name does not route directly to the target PDB. Do not set this alongside database; only one should be used.",
      "title": "Urn Db Name"
    },
    "add_database_name_to_urn": {
      ...
      "default": false,
      "description": "Include database name in URNs. Default is False (schema.table format). Set to True for database.schema.table format when ingesting from multiple Oracle databases.",
      "title": "Add Database Name To Urn"
    },
    "data_dictionary_mode": {
      "$ref": "#/$defs/DataDictionaryMode",
      "default": "ALL",
    ...
    "thick_mode_lib_dir": {
      ...
      "default": null,
      "description": "Path to the directory containing the Oracle Instant Client libraries. Required on Windows and Mac when enable_thick_mode is true. Optional on Linux: when set, the connector preloads the client libraries from this directory before initializing python-oracledb, which makes thick mode work without needing ldconfig or LD_LIBRARY_PATH to be set (see https://github.com/oracle/python-oracledb/issues/578). When unset on Linux, the standard ldconfig / LD_LIBRARY_PATH search is used.",
    ...
    "procedure_pattern": {
      ...
      "description": "Regex patterns for stored procedures to filter in ingestion. Specify regex to match the entire procedure name in database.schema.procedure_name format. e.g. to match all procedures starting with customer in HR schema, use the regex 'ORCL.HR.CUSTOMER.*'"
    },
    "include_lineage": {
      "default": true,
      "description": "Enable lineage extraction for stored procedures. When enabled, SQL code in procedures/functions is parsed to extract table dependencies.",
      "title": "Include Lineage",
      "type": "boolean"
    },
    "include_materialized_views": {
      "default": true,
      "description": "Include materialized views in ingestion. Requires access to DBA_MVIEWS or ALL_MVIEWS. If permission errors occur, you can disable this feature or grant the required permissions.",
    ...
    "lazy_schema_resolver": {
      "default": false,
      "description": "If enabled, skips the upfront bulk fetch of all known schemas from DataHub when resolving lineage. Useful on large DataHub instances where the bulk fetch causes memory or performance issues.",
      "title": "Lazy Schema Resolver",
      "type": "boolean"
    },
    "include_query_usage": {
      "default": false,
      "description": "Extract actual query usage from V$SQL for usage statistics. Requires SELECT privilege on V$SQL (or SELECT_CATALOG_ROLE). When enabled, usage statistics will be based on actual queries executed in Oracle.",
      "title": "Include Query Usage",
      "type": "boolean"
    },
    "max_queries_to_extract": {
      "default": 1000,
      "description": "Maximum number of queries to extract from V$SQL for usage statistics. Queries are ordered by recency (most recent first).",
    ...
    "query_exclude_patterns": {
      ...
      "default": null,
      "description": "Regex patterns for SQL statements to exclude from usage statistics. e.g., ['^SELECT.*FROM SYS\\..*', '^BEGIN.*END;'] to exclude system queries and PL/SQL blocks.",
