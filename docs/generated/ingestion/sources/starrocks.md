...
| ---------- | ------ | ----- |
| Asset Containers | ✅ | Enabled by default. Supported for types - Catalog, Database. |
...
### Install the Plugin
```shell
pip install 'acryl-datahub[starrocks]'
```
...
source:
  ...
  config:
    ...
    # Optional: Ingest specific database (catalog.database format)
    # database: "default_catalog.my_database"

    # Multi-catalog discovery: Include external catalogs (Hive, Iceberg, Hudi, Delta Lake)
    # include_external_catalogs: true

    # Filter catalogs to ingest
    # catalog_pattern:
    #   allow:
    #     - "^default_catalog$"
    #     - "^iceberg_.*"

    # Filter databases within catalogs (StarRocks databases, not schemas)
    # schema_pattern:
    #   allow:
    #     - "^production_.*"
    #     - "^analytics$"
...
| <div className="path-line"><span className="path-main">database</span></div> <div className="type-name-line"><span className="type-name">One of string, null</span></div> | Database in catalog.database format (e.g., 'default_catalog.quickstart'). If specified, only this database will be ingested. Leave unset for multi-catalog discovery. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
| <div className="path-line"><span className="path-main">host_port</span></div> <div className="type-name-line"><span className="type-name">string</span></div> | StarRocks FE host URL. <div className="default-line default-line-with-docs">Default: <span className="default-value">localhost:9030</span></div> |
| <div className="path-line"><span className="path-main">include_external_catalogs</span></div> <div className="type-name-line"><span className="type-name">boolean</span></div> | Whether to include external catalogs (Hive, Iceberg, etc.) in ingestion. <div className="default-line default-line-with-docs">Default: <span className="default-value">True</span></div> |
...
```javascript
{
  ...
  "properties": {
    "schema_pattern": {
      ...
      "description": "Regex patterns for databases to filter in ingestion. Note: In StarRocks three-tier hierarchy (Catalog -> Database -> Table), this filters databases within catalogs. Specify regex to match the database name only (e.g., 'quickstart', not 'default_catalog.quickstart')."
    },
    "table_pattern": {
      "$ref": "#/$defs/AllowDenyPattern",
      "default": {
    ...
    "database": {
      ...
      "default": null,
      "description": "Database in catalog.database format (e.g., 'default_catalog.quickstart'). If specified, only this database will be ingested. Leave unset for multi-catalog discovery.",
      "title": "Database"
    },
    "sqlalchemy_uri": {
      "anyOf": [
...
### Code Coordinates
- Class Name: `datahub.ingestion.source.sql.starrocks.StarRocksSource`
- Browse on [GitHub](https://github.com/datahub-project/datahub/blob/master/metadata-ingestion/src/datahub/ingestion/source/sql/starrocks.py)


:::tip Questions?

If you've got any questions on configuring ingestion for StarRocks, feel free to ping us on [our Slack](https://datahub.com/slack).
:::
