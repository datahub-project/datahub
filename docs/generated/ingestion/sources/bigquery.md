...
| <div className="path-line"><span className="path-main">profiling</span></div> <div className="type-name-line"><span className="type-name">BigQueryProfilingConfig</span></div> |   |
...
| <div className="path-line"><span className="path-prefix">profiling.</span><span className="path-main">fallback_partition_values</span></div> <div className="type-name-line"><span className="type-name">One of map(str,union), map(str,union), map(str,union)</span></div> |   |
...
| <div className="path-line"><span className="path-prefix">profiling.</span><span className="path-main">partition_fetch_max_bytes_billed</span></div> <div className="type-name-line"><span className="type-name">One of integer, null</span></div> | Optional ceiling (in bytes) on the data scanned by each partition-value fetch query. These probes group by a partition column across the table, so on a very large table they can scan a lot of data. Set this to fail such a probe fast instead of billing for a full-column scan; on failure the table is treated as having no discoverable partition. Left unset (no cap) by default because a low ceiling would spuriously fail discovery on legitimately large tables; `partition_fetch_timeout` already bounds runaway probes by time. <div className="default-line default-line-with-docs">Default: <span className="default-value">None</span></div> |
| <div className="path-line"><span className="path-prefix">profiling.</span><span className="path-main">partition_fetch_timeout</span></div> <div className="type-name-line"><span className="type-name">integer</span></div> | Timeout in seconds for each partition value fetch query. On timeout the table is treated as having no discoverable partition, so it is either skipped or (for require_partition_filter=false tables) profiled without a partition filter. <div className="default-line default-line-with-docs">Default: <span className="default-value">30</span></div> |
...
```javascript
{
    ...
    "BigQueryProfilingConfig": {
      ...
      "properties": {
        ...
        "fallback_partition_values": {
          ...
          "description": "Fallback values for partition columns when partition discovery fails. Keys are column names, values are the fallback values to use (string, int, or float). For non-date columns, the values are used directly. Example: {'batch': 'default', 'region': 'us-east-1'}",
          "title": "Fallback Partition Values",
          "type": "object"
        },
        "partition_fetch_timeout": {
          "default": 30,
          "description": "Timeout in seconds for each partition value fetch query. On timeout the table is treated as having no discoverable partition, so it is either skipped or (for require_partition_filter=false tables) profiled without a partition filter.",
          "exclusiveMinimum": 0,
          "title": "Partition Fetch Timeout",
          "type": "integer"
        },
        "partition_fetch_max_bytes_billed": {
          "anyOf": [
            {
              "exclusiveMinimum": 0,
              "type": "integer"
          ...
          "default": null,
          "description": "Optional ceiling (in bytes) on the data scanned by each partition-value fetch query. These probes group by a partition column across the table, so on a very large table they can scan a lot of data. Set this to fail such a probe fast instead of billing for a full-column scan; on failure the table is treated as having no discoverable partition. Left unset (no cap) by default because a low ceiling would spuriously fail discovery on legitimately large tables; `partition_fetch_timeout` already bounds runaway probes by time.",
          "title": "Partition Fetch Max Bytes Billed"
        }
      },
      "title": "BigQueryProfilingConfig",
      "type": "object"
    },
    "BigQueryUsageConfig": {
      "additionalProperties": false,
  ...
  "properties": {
    ...
    "profiling": {
      "$ref": "#/$defs/BigQueryProfilingConfig",
    ...
    "usage": {
      ...
      "default": {
        "bucket_duration": "DAY",
        "end_time": "2026-09-26T00:28:14.587818Z",
