...
- [integrations-service] `DATAHUB_TEAMS_ENABLED` (default: false) - Whether to enable Microsoft Teams integration. Requires additional Teams-specific configuration.
- [integrations-service] `DATAHUB_TEAMS_APP_ID`, `DATAHUB_TEAMS_APP_PASSWORD`, `DATAHUB_TEAMS_TENANT_ID` - Customer-specific Azure AD credentials for Teams integration. See internal Teams setup documentation.
