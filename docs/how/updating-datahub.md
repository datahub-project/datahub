...
- **(Ingestion / BigQuery)** BigQuery now sources row count, size, and last-altered time from the supported `INFORMATION_SCHEMA.PARTITIONS` view instead of the deprecated `__TABLES__` Legacy SQL construct, which Google deprecated in 2021 and may stop working without notice. `PARTITIONS` is dataset-scoped and free but covers base tables only, so the default stats change per entity type:
