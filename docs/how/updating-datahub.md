...
- #16134: Java 21 Runtime Required - DataHub now compiles to Java 21 bytecode (previously Java 11). All Docker images already ship with Java 21 runtime. Self-hosted users must ensure their runtime environment uses Java 21+. The Maven artifact name remains `datahub-client-java8` for backward compatibility, but now requires Java 21+ at runtime. This change also includes:
  - Spark integration upgraded from 3.0.3 to 3.3.4 (minimum version required for Java 21 support)
  - Hadoop upgraded from 2.7.2 to 3.3.6 (addresses Hadoop CVEs, bundled with Spark 3.3+)
  - Note: If you're a self-hosted user still running Java 11, you must upgrade to Java 21 before deploying this release. Spark lineage users must upgrade to Spark 3.3.0+.
...
- Build requires JDK21 (Runtime Java 21)
