...
- **(Spark lineage / Databricks Unity Catalog)** `acryl-spark-lineage` now bundles OpenLineage 1.53. When Databricks Unity Catalog is enabled (`spark.databricks.unityCatalog.enabled=true`) and DataHub symlink resolution remains enabled (`spark.datahub.disableSymlinkResolution=false`, the default), table symlinks now use a stable `unity-catalog` namespace and the fully qualified `<catalog>.<schema>.<table>` name. With the default `spark.datahub.metadata.dataset.hivePlatformAlias=hive`, the logical dataset URN therefore changes from `urn:li:dataset:(urn:li:dataPlatform:hive,<schema>.<table>,<env>)` to `urn:li:dataset:(urn:li:dataPlatform:hive,<catalog>.<schema>.<table>,<env>)`; a configured platform instance or different Hive platform alias is preserved in the corresponding URN. Existing lineage and dataset references against the old URN are not migrated automatically. **Action:** after deploying the upgraded agent, re-run affected Spark workloads to emit lineage against the catalog-qualified URNs, verify the new edges, then remove stale edges or old logical dataset entities using your normal lineage and entity cleanup process. Jobs with symlink resolution disabled continue to use the physical storage identity and are unaffected.
