...
- [#19947](https://github.com/datahub-project/datahub/pull/19947) **(Ingestion / Sigma)** A Sigma chart URN is built from the element id alone, and duplicating a workbook reuses those ids, so two workbooks can write the same chart's `inputFields`. Page ids collide the same way, and a page becomes a dashboard. The connector now keeps whichever chart or page aspect resolved more columns instead of letting the workbook processed last win. **Action:** none, but if you preferred the copy that resolved less, that entity's column lineage changes. Three caveats: this does not make the URNs unique -- two genuinely different charts still share one entity; only `inputFields` is chosen this way, so `chartInfo`, `dashboardInfo` and the browse path on a colliding chart or page still come from whichever workbook wrote last -- and `chartInfo` carries the chart's table-level lineage (`inputs` / `inputEdges`), not just its title and link, so a colliding chart can take its column lineage from one workbook and its table lineage from another; and the comparison is within one run, so duplicates ingested by separate recipes are not protected. `input_fields_regressive_emission_skipped` in the report counts the refusals and `input_fields_regressive_emission_samples` names a sample of them, with the workbook kept and the workbook refused on each one.
