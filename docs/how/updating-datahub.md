...
- #15436: (Ingestion) The Trino ingestion source now supports connector lineage and column-level lineage to upstream connectors by default. The new `ingest_lineage_to_connectors` (default: `true`) config enables automatic lineage from Trino datasets to their upstream connector sources (e.g., `trino:hive_catalog.schema.table` → `hive:schema.table`). Column-level lineage is also enabled by default via `include_column_lineage: true`, creating 1:1 column mappings between Trino and upstream sources. **Performance note**: Column lineage makes additional database queries (approximately +2-5 minutes per 1,000 tables). For large deployments (5,000+ tables), consider setting `include_column_lineage: false` or using `schema_pattern` to limit scope. These features are **not breaking changes** - they are additive and can be disabled if needed. See the [Trino source documentation](https://datahubproject.io/docs/generated/ingestion/sources/trino) for configuration details.
