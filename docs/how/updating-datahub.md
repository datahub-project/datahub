...
- **(Ingestion / dbt)** A dbt semantic model is a logical structure defined in YAML in your dbt project, not a table in the warehouse, and the connector was handling it as a physical one. Two things change as a result:

  - **The dataset urn changes.** A semantic model was named `<database>.<schema>.<semantic_model_name>`, using the database and schema of the dbt model it sits on. It is now named from dbt's own unique id, `semantic_model.<project>.<semantic_model_name>`, the same way exposures are already named. This also fixes a silent data-loss bug: when a semantic model has the same name as the model it is built on - which is dbt's own documented convention (`- name: orders` with `model: ref('orders')`) - the two produced an identical urn, and the semantic model, emitted last, overwrote the model's real warehouse schema, subtype and properties with no warning. Where that was happening, the model's metadata was already being lost on every run.
  - **The warehouse-side entities disappear.** A semantic model no longer produces a dataset on the target platform, a sibling relationship, or lineage into one. Those entities described tables that never existed.

  Both the old dbt-side datasets and the warehouse-side phantoms are orphaned by this change. **Action:** re-ingest with `stateful_ingestion.enabled: true`, which soft-deletes both; without it they are left behind with no owner. Governance authored on the old urns - owners, tags, glossary terms, documentation, and column-level tags and terms - does not move to the new urn, and no automated migration is provided; re-apply it if you need it. Separately, `materialized_node_pattern` no longer filters semantic models, since they have no materialized location; use `node_name_pattern`, which matches the dbt unique id.
...
- **(Ingestion / dbt)** dbt semantic models and metrics can now be described with first-class `semanticModel` and `metric` entities, controlled by the new `emit_semantic_model_entities` setting. When it is on, the connector emits one `semanticModel` per dbt project holding the join relationships MetricFlow derives from matching entity names, one `metric` per measure with `create_metric: true` and per top-level `metrics:` entry (dbt Core only - the dbt Cloud Discovery API does not expose the `metrics:` block), and adds semantic field annotations plus a link to the parent semantic model **onto the datasets the connector already emits**. Nothing existing changes: the dataset urns, their `datasetProperties`, schema, subtype, tags, owners and lineage are all exactly as before, so there is nothing to migrate and no re-ingest needed to keep what you have. The setting is three-valued: left unset it follows the server, turning on against DataHub Cloud new enough to register these entity types with the Metrics feature enabled, and staying off on OSS, older Cloud, and runs with no server connection (for example a file sink); `true` asks for it and is refused with a reason in the ingestion report where the server cannot accept it; `false` keeps the old behavior. The new entities are keyed by dbt project name, read from `manifest.metadata.project_name` for dbt Core and from the semantic models' package name for dbt Cloud - set `semantic_model_project_name` to pin it, since it is part of their identity. **Action:** none. To keep a DataHub Cloud run emitting only the datasets, set `emit_semantic_model_entities: false`.
