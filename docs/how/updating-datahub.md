...
- **(Ingestion / Fabric OneLake)** View and `queryinsights` lineage now resolves cross-item (`item.schema.table`) and cross-workspace (`workspace.item.schema.table`) references to the GUID-keyed dataset URNs the connector emits. Previously these produced dangling upstream, usage and operation URNs named after the display name (for example `urn:li:dataset:(urn:li:dataPlatform:fabric-onelake,silver_lh.dbo.customers,PROD)`); references that cannot be resolved are now skipped with an `Unresolved Cross-Item SQL Reference` warning instead. A lakehouse or warehouse that fails mid-processing is reported as `Failed to Process Lakehouse` / `Failed to Process Warehouse` (previously under `Failed to List Lakehouses` / `Failed to List Warehouses`) and no longer skips the remaining items of its workspace. **Action:** none for new deployments. With stateful ingestion, display-name datasets that previously received usage or operation aspects are soft-deleted as stale on the next run; without it, remove them manually (for example `datahub delete --urn "<urn>" --soft`). A view whose only upstreams are unresolved gets no new `upstreamLineage`, so an old dangling edge on it stays until removed.
