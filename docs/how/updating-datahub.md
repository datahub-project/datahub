...
- #19211 **(Ingestion / SQL lineage)** SQL lineage now recovers the identifier casing a query was written with when the usual URN candidates fail to resolve. Identifier case is folded during parsing, before any URN is built, and `SchemaResolver` only retried variants applying one uniform rule to the whole name (as parsed, all-lowercased, all-lowercased-except-platform-instance). Sources build URNs per identifier, so a real URN is often not uniform — an Oracle table created as `CREATE TABLE "MixedCase"` under schema `MY_SCHEMA` is ingested as `my_schema.MixedCase`, a lowercased schema beside a verbatim table, which no uniform rule produces. Lineage into such a dataset resolved to a URN that did not exist, and its column names were lowercased too, since column-case restoration depends on the table's schema having been found. This affects platforms that emit non-lowercased URNs (`convert_urns_to_lowercase` defaults to False everywhere except Snowflake) whose catalog also preserves identifier case: SQL Server and Fabric by default, and Oracle or HANA for identifiers created with quoted DDL. The retry is only consulted after the existing candidates miss and only adopts a candidate that actually resolved, so a genuine miss is unchanged. **Action:** re-run affected ingestion to emit corrected lineage. Previously emitted lowercase dataset and schemaField URNs are not cleaned up automatically and may linger as stale, metadata-less entities; soft-delete them if you want them gone.
