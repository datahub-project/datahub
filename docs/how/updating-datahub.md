...
- **(Ingestion / Sigma)** Sigma no longer emits Workspace Containers that have no name, which the UI rendered as a raw `urn:li:container:<guid>` in place of the top-level folder name. Two changes get there. First, ingestion now adopts the workspace id Sigma answers with: on tenants where a file's `path` does not start at its workspace, the id derived by walking `parentId` lands on a folder, and Sigma resolves that to the owning workspace. **On those tenants, datasets, workbooks and data models move from the folder-derived Container URN to the real workspace Container URN** — the old URN was the unnamed one, but this is a re-parent on existing deployments rather than a pure addition, and the remapped ids are listed in the report under `workspace_ids_remapped`. Second, any workspace still referenced without metadata (`/workspaces/{id}` returns 403, or the call fails) now gets a Container named from the entity path, listed under `workspaces_without_metadata`; non-permission failures are reported separately as `Unable to fetch workspace`. No entity URNs change — dataset, dashboard, chart and data model URNs are derived from their own ids, not from the workspace — so only which Container an entity is parented under moves. That move happens only on tenants hitting the first case: there, the folder-derived Container is left behind empty, because it never had any aspects emitted for it (that is the bug being fixed), so stateful ingestion never tracked it and will not soft-delete it. It has no name and nothing points at it, so it does not render, but it does remain in the graph until removed by hand. In the second case nothing is left behind at all — the Container URN is unchanged and simply gains the properties it was missing. **Action:** none required. To replace a path-derived name with the real workspace name, owner and timestamps, add the user associated with the Client ID/Secret to the workspaces named in the report. If you had manually added documentation, tags or owners to a raw-URN folder in the UI, that metadata stays on the old Container and does not follow the entities.
