...
- **(Ingestion / Fabric OneLake)** System objects (`sys.*`, `INFORMATION_SCHEMA.*`, `queryinsights.*`) referenced by `queryinsights` queries or views no longer produce datasets, lineage, usage, or operations (previously, e.g., `<warehouse>.sys.databases` datasets appeared from the ODBC driver's catalog queries). `queryinsights` rows that are T-SQL control-of-flow / session statements (`SET`, `IF`, `EXEC`, ...) are skipped before parsing, and the new `usage.skip_ingestion_identity_queries` option (default `true`) skips rows issued by the ingestion identity itself. **Action:** set `usage.skip_ingestion_identity_queries: false` if the identity used for ingestion also runs workloads whose usage you want captured. With stateful ingestion, previously emitted system-object datasets are soft-deleted as stale; without it, remove them manually (for example `datahub delete --urn "<urn>" --soft`).
- **(Ingestion / Fabric OneLake)** Warehouse tables are now discovered through the Warehouse's SQL Analytics Endpoint (`INFORMATION_SCHEMA.TABLES`). The Fabric REST Tables API the connector used before is Lakehouse-only and returns 404 for Warehouses, so T-SQL-created Warehouse tables were never ingested and lineage from Warehouse views pointed at datasets without schema metadata. **Action:** none; with the default `sql_endpoint.enabled: true`, Warehouse tables appear on the next run. With the endpoint disabled, a `Warehouse Tables Not Discovered` warning is reported instead.
- **(Ingestion / Fabric OneLake)** View and `queryinsights` lineage now resolves cross-item (`item.schema.table`, same workspace) references to the GUID-keyed dataset URNs the connector emits. Previously these produced dangling upstream, usage and operation URNs named after the display name (for example `urn:li:dataset:(urn:li:dataPlatform:fabric-onelake,silver_lh.dbo.customers,PROD)`); references that cannot be resolved (including 4-part names, which Fabric does not support) are now skipped with an `Unresolved Cross-Item SQL Reference` warning instead. A lakehouse or warehouse that fails mid-processing is reported as `Failed to Process Lakehouse` / `Failed to Process Warehouse` (previously under `Failed to List Lakehouses` / `Failed to List Warehouses`) and no longer skips the remaining items of its workspace. **Action:** none for new deployments. With stateful ingestion, display-name datasets that previously received usage or operation aspects are soft-deleted as stale on the next run; without it, remove them manually (for example `datahub delete --urn "<urn>" --soft`). A view whose only upstreams are unresolved gets no new `upstreamLineage`, so an old dangling edge on it stays until removed.
