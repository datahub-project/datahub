...
- **(Ingestion / Sigma) Sigma Data Model ingestion is opt-in.** New `ingest_data_models` flag (default `False`) emits Sigma Data Models as `Container`s with one `Dataset` per element, plus `SchemaMetadata` and `UpstreamLineage` (intra-DM, cross-DM, external, and workbook→DM). Element URNs are keyed by the immutable Data Model UUID so attachments survive Sigma slug rotation. Additive — no existing URNs change. Disabling the flag after enabling will soft-delete the emitted entities via stateful stale-entity removal (also cleans up entities from the reverted flat-Dataset shape in #17109). To opt in: set `ingest_data_models: true` and optionally use `data_model_pattern` / `ingest_shared_entities` to scope.
