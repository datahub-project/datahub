...
- `ENTITY_WRITE_LOCK_BACKEND` (default `none`) — `none` | `hazelcast`: per-URN write gate that serializes concurrent writers on the same entity off the DB connection pool. Requires optimistic locking; independent of scoped retry.
