...
- **(Ingestion / Sigma) Sigma Data Model ingestion is opt-in.** New `ingest_data_models` flag (default `False`) emits each Sigma Data Model as a `Container` with one `Dataset` per element. Each element Dataset carries `SchemaMetadata` (column types default to `NullType` / `nativeDataType: "unknown"` — Sigma does not return per-column types yet) and, when `extract_lineage: true`, `UpstreamLineage` for intra-DM references and external upstreams (Sigma Datasets ingested in the same run). The `/lineage` call is gated on `extract_lineage` so users who opt out at the workbook surface also opt out of DM lineage. Element URNs are keyed by the immutable Data Model UUID so attachments survive Sigma slug rotation. The report splits unresolved upstreams into `data_model_element_upstreams_unresolved_external` (filtered-out Sigma Datasets) and `data_model_element_upstreams_unknown_shape` (unparsed source_id shapes); the aggregate `data_model_element_upstreams_unresolved` is retained. Additive — no existing URNs change. Cross-DM lineage, personal-space DM discovery, and workbook-to-DM element linking are coming in a follow-up PR.
