...
- **(Metadata Model / Semantic Model)** `semanticModelInfo.datasets` is deprecated. The field remains in the aspect for backwards-read of already-ingested data, but `@Relationship` / `@Searchable` annotations have been stripped so it no longer writes graph or search edges. Membership is member-side only: datasets use `semanticModelProperties.semanticModel` (`IsPartOf`), metrics use `metricInfo.semanticModel` (`ModeledBy`, non-lineage). Canonical metric lineage is `Metric → Semantic Model Dataset → Physical Dataset` via `metricUpstreams.datasetUpstreams` plus each logical dataset's `upstreamLineage`. **Action:** After upgrade, customers who previously ingested the metrics-catalog shape (lineage via `Contains` / `ModeledBy` rather than `metricUpstreams`) should run `datahub migrate semantic-model-container` to backfill `metricUpstreams.datasetUpstreams` from stored `semanticModelInfo.datasets`. New writes must not populate `semanticModelInfo.datasets`. Legacy Snowflake "Semantic View" datasets still use `datahub migrate snowflake-semantic-views` for governance. See [Semantic Model](../generated/metamodel/entities/semanticModel.md) and [Metric](../generated/metamodel/entities/metric.md).
