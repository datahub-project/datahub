...
- **(Ingestion / Sigma) Sigma Data Model ingestion is opt-in.** New `ingest_data_models` flag (default `False`) emits Sigma Data Models as `Container`s with one `Dataset` per element, plus `SchemaMetadata` and `UpstreamLineage` (intra-DM, cross-DM, external, and workbook-to-DM). Element URNs are keyed by the immutable Data Model UUID so attachments survive Sigma slug rotation. Additive — no existing URNs change. Disabling the flag after enabling will soft-delete the emitted entities only if stateful stale-entity removal is enabled (`stateful_ingestion.remove_stale_metadata: true`); otherwise the Containers / Datasets remain in the graph and must be cleaned up manually. To opt in: set `ingest_data_models: true` and optionally use `data_model_pattern` / `ingest_shared_entities` to scope.
...
- #17086 (Ingestion / Sigma) Intra-workbook element-to-element lineage is now emitted via `ChartInfo.inputEdges`. On the next ingestion run, new chart-to-chart edges will appear in the lineage graph for any Sigma workbook where one element feeds another. This is additive — no existing dataset or warehouse lineage edges are removed.
