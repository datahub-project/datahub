...
- **(OpenAPI)** Removed previously deprecated OpenAPI v1 endpoints `/openapi/entities/v1` and `/openapi/relationships/v1`. These endpoints (and OpenAPI v1 generally) were announced as deprecated in **0.14.0.2** (mid-2024, with a stated minimum of 6 months) and are removed after roughly **2 years** of notice. **Migration:** use `/openapi/v3/entity` and `/openapi/v3/relationship`. For by-entity relationship traversal (the former v1 relationships API), use `GET /openapi/v3/relationship/{entityName}/{entityUrn}?direction=INCOMING|OUTGOING` (walker-relative direction). `POST /openapi/v3/relationship/scroll` also accepts optional `entityUrn` for the same walker semantics; without `entityUrn`, scroll keeps its existing filter field-remap `direction` semantics. Timeline is not removed: prefer **`/openapi/v1/timeline`** (aligned with other `/openapi/v1/...` ops APIs). Other `/openapi/v1/...` operational endpoints that were never deprecated (files, events, registry, tracing, rate-limits, system-info, etc.) also remain available. See the [OpenAPI usage guide](../api/openapi/openapi-usage-guide.md). Java `RestEmitter.emit(List<UpsertAspectRequest>)` now posts to `/openapi/v2/platform/entities/v1/`; the Python graph client's `get_related_entities` uses the OpenAPI v3 by-entity relationship GET.
...
- **(OpenAPI)** OpenAPI v2 entity and relationship APIs (`/openapi/v2/entity`, `/openapi/v2/relationship`, including generated typed entity endpoints under `/openapi/v2/entity`) are deprecated in favor of `/openapi/v3/entity` and `/openapi/v3/relationship`. They remain available; plan removal in a future release after a deprecation notice period. Timeseries, platform entity ingest, and analytics under v2 are **not** removed by this change.

- **(OpenAPI / Timeline)** Prefer **`/openapi/v1/timeline`**. The awkward paths `/openapi/timeline/v1` and `/openapi/v2/timeline/v1` remain available but are **deprecated**. A future cutover to `/openapi/v2/timeline` is not part of this change.
