...
- #17449 **(Ingestion / Patch API) Wire format change for `globalTags` JSON Patch:** The path layout for unattributed tag additions switched from `/tags/<urn>/` (tag-first, trailing empty source component) to `/tags//<urn>` (source-first, empty source in the middle), and the `arrayPrimaryKeys` order changed from `["tag", "source"]` to `["source", "tag"]`. Unattributed removes continue to use `["tag", "source"]` ordering and are now emitted as a separate patch MCP when mixed with add operations. This fixes a server-side `jakarta.json.JsonException` thrown by Parsson when patching tags on entities with no pre-existing `globalTags` aspect (Parsson rejects an `add` operation whose final JSON Pointer component is an empty string). External consumers comparing patch payloads against fixtures must regenerate those fixtures.
