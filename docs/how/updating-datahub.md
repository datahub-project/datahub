...
- #18547 **(Python SDK / OAuth)** `DATAHUB_AUTH_TYPE` env OAuth now applies to any client built with an explicit GMS host and no token — the Airflow REST hook and lineage listener, the Great Expectations and Prefect plugins, and `DataHubGraph(DatahubClientConfig(server=..., token=None))` — where previously only the internal `__from_env__` path honoured it. Those surfaces silently built unauthenticated clients before. One side effect: a malformed or unrecognised `DATAHUB_AUTH_TYPE` now raises `ConfigurationError` when such a client is constructed, instead of being ignored. **Action:** none if `DATAHUB_AUTH_TYPE` is unset. If it is set, check its required variables are present, and unset it for processes that should not authenticate with OAuth.
