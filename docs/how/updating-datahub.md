...
- **(Ingestion / Sigma) Sigma Data Model ingestion is opt-in.** New `ingest_data_models` flag (default `False`) emits each Sigma Data Model as a `Container` with one `Dataset` per element. Each element Dataset carries `SchemaMetadata` (column types default to `NullType` / `nativeDataType: "unknown"` — Sigma does not return per-column types yet) and, when `extract_lineage: true`, `UpstreamLineage`; lineage edges cover intra-DM element-to-element, cross-DM element-to-element, external upstreams (warehouse tables and Sigma Datasets), and workbook-to-DM element references. The `/lineage` call is gated on `extract_lineage` so users who opt out at the workbook surface also opt out of DM lineage. Element URNs are keyed by the immutable Data Model UUID so attachments survive Sigma slug rotation. The report splits unresolved upstreams into `data_model_element_upstreams_unresolved_external` (filtered-out Sigma Datasets) and `data_model_element_upstreams_unknown_shape` (unparsed source_id shapes); the aggregate `data_model_element_upstreams_unresolved` is retained. Additive — no existing URNs change. To opt in: set `ingest_data_models: true` and optionally use `data_model_pattern` / `ingest_shared_entities` to scope.
- **(Ingestion / Sigma) Report shape change for null `DatasetUpstream.name`.** Sigma occasionally returns `name: null` on workbook dataset upstream nodes. Pre-release this raised a Pydantic `ValidationError` that surfaced as a `SourceReport.warnings` entry. It is now tolerated at parse time; drops are counted under the new `chart_dataset_upstream_name_missing` report field instead. Lineage semantics are unchanged (the chart-to-Sigma-dataset edge cannot be SQL-correlated without a name and is skipped in both releases). Dashboards/alerts keyed on the previous warning text should migrate to the new counter.
