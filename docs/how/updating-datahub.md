...
- #19096 **(GMS / MCP ingest)** Invalid MetadataChangeProposals that fail item construction (bad URNs, unknown entity/aspect, etc.) are no longer a silent no-op when they are the **only** proposals in a request: GMS now returns a validation error instead of HTTP 200 with nothing written ([#19086](https://github.com/datahub-project/datahub/issues/19086)). Status codes vary by API surface (e.g. Rest.li often **422**, OpenAPI often **400**). **Mixed** batches still soft-skip invalid items and process valids (#11187). Kafka / pgQueue **consumers** also soft-skip invalid siblings, and now emit **Failed MCP (FMCP)** for each construction failure so poison pills are visible. **Action:** fix client-generated URNs (e.g. GUID assertion ids instead of embedding dataset URNs); treat all-invalid API responses as failed writes; monitor FMCP for consumer-side construction failures.
