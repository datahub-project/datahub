...
- #19375 **(Operations / logging)** Conditional blocks in `logback.xml` are evaluated again. Logback 1.5.37+ stopped honouring the `condition` attribute on `<if>`: it logs a deprecation warning and then skips both the `<then>` and the `<else>` branch, so the failure is silent. If you mount your own `logback.xml` containing `<if condition="...">`, those blocks are being ignored on 1.5.37+ — move the expression into a `<condition>` element placed immediately **before** the `<if>`, and note that a `<condition>` cannot be nested inside an `<appender>`, `<logger>` or `<root>` element. See the [logback conditional configuration docs](https://logback.qos.ch/manual/configuration-conditional.html) for the supported syntax.
