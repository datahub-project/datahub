...
- #6544: Ingestor Clickhouse: It is now possible to specify an arbitrary number of options in SqlAlchemy URL query parameters - `uri_opts`.
  In order not to confuse the user, the `secure` and protocol options has now been moved to `uri_opts`.
