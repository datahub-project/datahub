...
- **(Ingestion / Data Lake)** Data lake profiling (S3, GCS, Azure Blob, Delta Lake sources) now uses **DuckDB** instead of Apache Spark + PyDeequ. Java and Spark are no longer required to profile data lake sources. The data-lake profiler was the last component that used PySpark, so the `pyspark` and `pydeequ` dependencies have been dropped entirely — including from the `databricks` extra (Unity Catalog never imported PySpark; it uses `databricks-sql-connector` and SQL parsing), so `acryl-datahub[databricks]` no longer pulls PySpark either. The `spark_driver_memory` and `spark_config` profiling config options have been removed (supplying either now emits a removed-field warning and is otherwise ignored). Profiled metric values (e.g. distinct counts, quantiles, histograms) may differ slightly from previous runs because DuckDB uses different (approximate) algorithms; row, column, and null counts are unchanged. DuckDB loads the `httpfs` (remote reads), `avro` (Avro files), and `aws` (instance-profile/role-based S3 credentials) extensions at runtime; by default these download from DuckDB's extension repository on first use. Air-gapped deployments can pre-stage the extension binaries and set the new `profiling.duckdb_extension_directory` config so a correctly staged binary loads with no network access (this is not a hard offline switch — a missing or version-mismatched binary is still downloaded, and the affected table is skipped with a warning only if that download also fails). Because DuckDB is a lightweight (JVM-free) dependency, the **slim** `datahub-ingestion` image now ships data-lake profiling as well (previously only the full image could profile); the `s3-slim` / `gcs-slim` / `abs-slim` install extras remain available for metadata-only installs and for platforms without a DuckDB wheel (e.g. `armv7l`).
