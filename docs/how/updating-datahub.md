...
- #15719: Kafka ingestion source migrated to MCP API (API v2), resulting in:
  - Only emits MCPs (no more MCEs/DatasetSnapshots)
  - Workunit count changes (~1 MCE per topic → ~5-6 MCPs per topic). Users with custom monitoring based on workunit counts should adjust thresholds accordingly.
