...
- **(GMS / MCE / System Update) Optional optimistic locking for aspect writes.** Set `OPTIMISTIC_LOCKING_ENABLED=true` independently on each process to replace `SELECT FOR UPDATE` with compare-and-set updates on `SystemMetadata.version` for Ebean storage. Legacy rows without a version use plain updates until a write stamps one; Cassandra ignores the flag. Optimistic conflicts use the existing configurable Ebean retry/backoff policy and `Retry-After` behavior. Transaction retry state is now cleared after a later attempt successfully commits or intentionally rolls back, preventing a recovered conflict from being reported as retry exhaustion. **Action:** none; the flag defaults to `false`.
