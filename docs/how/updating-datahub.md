...
- **(Ingestion / Operations)** `Operation` aspects now carry the time the write actually happened in `timestampMillis`, instead of the time ingestion ran, and a `messageId` identifying the operation. This covers the Snowflake, BigQuery and Redshift usage paths and the shared SQL parsing aggregator (which those three connectors use for query-based operations). `timestampMillis` is documented in `TimeseriesAspectBase` as "the event timestamp", and `Operation.lastUpdatedTimestamp` as "the time at which the operation occurred", but only the latter was being populated that way. Two consequences: any 30-day window over operations now means the last 30 days of **writes** rather than the last 30 days of **ingestion**, which is what "recently updated" indicators are meant to express; and because `timestampMillis` is hashed into the timeseries document id, re-ingesting an overlapping window now overwrites the same operation idempotently instead of creating a second document for it. Ingestion time is unchanged and still recorded in `systemMetadata.lastObserved`. **Action:** none. Operations ingested before the upgrade keep their old ingestion-time stamps, so operations in the first post-upgrade window can be counted twice — once under the old stamp and once under the event-time stamp — until those older documents age out of the window.

- **(Ingestion / Redshift)** Redshift emits more operations than before, because the window that suppresses repeated ones now actually closes. The connector deliberately coalesces a burst of inserts on the same table by the same user into a single operation, over a 10-second window measured in event time. The watermark driving that window started at zero and never advanced, so the window never expired and **every** later repeat on a table was dropped for the whole run, not just the burst — three inserts an hour apart produced one operation. **Action:** none, but expect operation counts and write-based freshness signals for Redshift tables to rise to their true values on the next run.
