...
- **(Ingestion / MicroStrategy)** The MicroStrategy connector now resolves MicroStrategy's own "Shared Reports" folder label (via `GET /api/folders/preDefined`) instead of showing that folder's raw internal metadata name ("Reports", nested under "Public Objects"). This is enabled by default (`use_predefined_folder_names: true`) and changes that folder container's URN for any environment where it's part of the ingested browse hierarchy — the old, raw-named container becomes stale (soft-deleted automatically if stateful ingestion is enabled; otherwise left behind until manually removed). **Action:** if you'd rather keep prior container URNs on upgrade, set `use_predefined_folder_names: false` in your recipe.
