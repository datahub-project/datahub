...
- #19096 **(GMS / MCP ingest)** Invalid MetadataChangeProposals that fail URN/proposal construction (`IllegalArgumentException`, e.g. reserved characters under `STRICT_URN_VALIDATION_ENABLED`) are no longer silently dropped on **API** paths (Rest.li / OpenAPI / GraphQL, including `async=true` accept). Those requests now fail with a validation error (Rest.li/OpenAPI **422**) instead of HTTP 200 with nothing written ([#19086](https://github.com/datahub-project/datahub/issues/19086)). Kafka / pgQueue **consumer** paths keep #11187 best-effort behavior: invalid MCPs in a mixed batch are skipped so valid siblings still ingest. **Action:** fix client-generated URNs (e.g. GUID assertion ids instead of embedding dataset URNs); treat API validation errors as failed writes.
