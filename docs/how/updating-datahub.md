...
- #17370: **(Ingestion / Sigma) Two behavior changes for warehouse-backed chart column lineage and DM element schema.** When `extract_lineage: true` (default): (1) For charts whose column IDs carry an `inode-{urlId}/{NATIVE_NAME}` format, `InputFields.fields[].schemaField.fieldPath` now reflects the warehouse-native column name (e.g. `customer_id`) instead of the Sigma display name (e.g. `Customer Id`). Downstream consumers that match on the exact `fieldPath` string by display name will silently miss those fields after the next ingestion run. (2) A `(elementId, columnId)` dedup fix in DM element schema extraction may surface columns that were previously suppressed when sibling DM elements shared the same `columnId`. On the next run, affected DM element Datasets may gain new schema columns; column-count alerts and soft-delete policies scoped to those Datasets may fire. No configuration change is required.
