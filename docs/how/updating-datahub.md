...
- #18395 **(Ingestion / Snowflake)** Snowflake Semantic Views (`semantic_views.enabled`) are ingested as **Dataset** entities with subtype "Semantic View" by default — no action required. The `semantic_views.emit_semantic_model_entities` flag switches them to a **semanticModel** entity (definition, logical dimensions/facts, and lineage to the underlying tables) plus one **metric** entity per metric column. On DataHub Cloud ≥ 2.1.0 with Metrics enabled this is on by default; on OSS it is opt-in via the flag. Either entity mode requires a GMS release that registers the `semanticModel`/`metric` entities (older servers reject those aspects). **When this mode is active:** repoint any saved links, dashboards, tag/glossary assignments, or automations from the old `urn:li:dataset:(...)` semantic-view URNs to the new `urn:li:semanticModel:(...)` / `urn:li:metric:(...)` URNs. With stateful ingestion enabled, the old dataset entities are soft-deleted automatically on the next run; without it they are left behind for manual cleanup. Semantic views no longer appear in their database/schema container pages — find them via search, lineage, and the metric/semantic-model views. `semantic_views.include_usage` is ignored in this mode (semanticModel entities carry no usage statistics; usage is emitted only in the default dataset mode); `semantic_views.include_queries` still emits query entities, now attributed to the semanticModel. Separately, `include_queries: true` now emits query entities even when `include_usage` is `false` (previously that combination emitted nothing) — in both modes. **Known limitation:** a dataset or dashboard whose SQL selects `FROM SEMANTIC_VIEW(...)` cannot yet declare the semanticModel as an upstream, since dataset `upstreamLineage` accepts only dataset URNs. **Caveat:** the `semanticModel`/`metric` URN encodes `platform_instance` but not `env`, so ingesting the same Snowflake account into two different environments produces colliding URNs — use a distinct `platform_instance` per environment to keep them separate.
