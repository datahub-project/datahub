...
- #19162 **(Ingestion / Hex)** The Hex source's `queriedTables` lineage path now builds upstream dataset URNs through the same schema resolver as the SQL-cell path. Tier-1 and Tier-2 URNs agree by construction, so column lineage is no longer dropped on case mismatches with a Snowflake warehouse ingested using `convert_urns_to_lowercase: true`. BigQuery date-shard suffixes (`events_20240101` → `events_yyyymmdd`) are also folded. **Known limitation and action:** the resolver dialect-normalizes the `tableName` before probing DataHub, so on case-folding dialects (mssql, postgres, redshift) the author casing is lost before the probe runs. Snowflake and BigQuery are unaffected — Snowflake defaults to lowercase, BigQuery preserves case through the resolver. For mssql/postgres/redshift warehouses ingested with their source defaults (which preserve case), the Hex tier-1 edge is emitted lowercased (`mydb.dbo.orders`) while the warehouse URN kept its author casing (`MyDb.dbo.Orders`) — the edge dangles, and re-ingesting the warehouse does not fix it. There is no per-connection override yet; re-ingest the warehouse with `convert_urns_to_lowercase: true` to align them. The report exposes `queried_tables_unresolved_in_datahub` (count) and `queried_tables_unresolved_sample` (URNs) so the miss is diagnosable when `datahub-api` is configured (without it the counter stays at 0); `Column lineage dropped` warnings also fire when the SQL-cell URNs disagree with the tier-1 result.
