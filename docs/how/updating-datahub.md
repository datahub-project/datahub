...
- [#19899](https://github.com/datahub-project/datahub/pull/19899) **(Sigma ingestion)** A Sigma run now **fails** when one of its entity listings (workspaces, workbooks, datasets, data models, or the `/files` metadata behind workbooks and datasets) cannot be read. Previously such a run succeeded with a warning and then soft-deleted everything that listing would have returned, as if it had been removed from Sigma. **Action:** treat a newly failing run as a real problem, not new noise — the report names the failing listing and what to do about it. The likeliest cause is `/v2/datasets`, since Sigma ended dataset support on 2026-09-15; once that endpoint returns 404/410 for you, set `ingest_datasets: false`, which stops ingesting Sigma Datasets and soft-deletes the ones you already have. A token without Data Model access also starts failing, on a 403 from `/v2/dataModels` that used to be only a warning: grant it that scope, or set `ingest_data_models: false`.
