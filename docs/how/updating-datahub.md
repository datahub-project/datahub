...
- **(Ingestion / Azure Data Factory)** A Copy activity observed reading from more than one distinct source table **and** writing to more than one distinct sink table across execution history (e.g. a `ForEach`-looped Copy activity that visits many table pairs) now gets one sibling `DataJob` per real (source, sink) pair, instead of a single shared `DataJob` whose inputs and outputs were all unioned together. The previous unioned lineage implied every observed source fed every observed sink, which showed up as incorrect phantom upstream/downstream lineage in the main Lineage graph. Sibling `DataJob`s are capped per activity by the new `max_dynamic_lineage_pairs_per_activity` config (default 50); pairs beyond the cap fall back to the previous unioned-onto-the-parent-job behavior instead of being dropped. This does not affect activities where only one side (source or sink) varies across runs — those keep the existing unioned behavior, which is already correct. **Action:** none required — lineage becomes more precise automatically, and no config change is needed to opt in. Enable `stateful_ingestion` on ADF recipes with `ForEach`-style Copy activities so that sibling `DataJob`s for pairs that stop appearing get soft-deleted automatically; without it, sibling jobs for pairs that stop appearing will accumulate indefinitely.
...
- **(Ingestion / Azure Data Factory)** Lineage to a Databricks linked service now automatically carries a `platform_instance` (the workspace ID parsed from the linked service's workspace URL, e.g. `adb-1234567890123456`) whenever `platform_instance_map` doesn't already set one for it — matching the same convention DataHub's own Unity Catalog source uses when deriving its `platform_instance` from `workspace_url`. Previously, ADF's Databricks lineage URNs never carried a `platform_instance` at all. Two new options, `databricks_default_catalog` and `databricks_catalog_map`, let you fully qualify the Databricks catalog (and Unity Catalog metastore) name too, since neither is exposed anywhere in ADF's own API. **Action:** if your Databricks ingestion recipe also leaves `platform_instance` unset (the common case, matching the Unity Catalog source's own default), no action is needed — ADF lineage will now correctly resolve to your existing Databricks datasets instead of creating disconnected duplicates. If your Databricks recipe sets an explicit `platform_instance`, add a matching entry to `platform_instance_map` for that ADF linked service. Set `databricks_catalog_map` (or the simpler `databricks_default_catalog`) if you want fully-qualified `catalog.schema.table` (or `metastore.catalog.schema.table`) references instead of `schema.table`.
