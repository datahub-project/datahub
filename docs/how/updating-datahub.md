...
- **(Ingestion / MySQL profiling defaults)** MySQL and MariaDB profiling defaults changed: `profiling.max_workers` drops from `5 * cpu_count` (~40 on an 8-core box) to `5`. MySQL is a single-primary row store — extra concurrency adds contention and multiplies peak memory (each worker may hold multi-GB `COUNT(DISTINCT)` structures) rather than increasing throughput, and the ~40 concurrent full-scanning connections were a leading OOM suspect. `5` is a conservative starting value pending measurement; tune per workload. `profiling.profile_table_row_limit` and `profiling.profile_table_size_limit` stay `null` on MySQL (rather than inheriting the shared 5M-row / 5GB defaults) — the shared defaults are tuned to Snowflake/BigQuery billing, not InnoDB, and silently dropping profiles would quietly break the Volume/Column Metric assertions built on them; set them explicitly to guardrail large tables. Finally, MySQL and MariaDB now emit a post-run `report.info` entry naming the few tables that took the longest to profile, with the limits to set — a discoverability mechanism for the opt-in guardrails. (MariaDB inherits these defaults as a MySQL fork.)
