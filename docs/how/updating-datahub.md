...
- #20000 **(Sigma ingestion)** A chart column's formula reference to another chart or a Data Model element now becomes column lineage only when that upstream has the referenced column; references to warehouse tables, and to upstreams whose columns are unknown, are unchanged. References to columns the upstream does not have, and three-part references such as `[Element/Relationship/Column]`, no longer produce lineage edges; a column whose formula has no other resolvable reference points to itself instead. Element and column names now match case-insensitively, as Sigma does. The `chart_input_fields_case_mismatch` report counter now counts only references that match several names differing only in case. **Action:** none. Expect column-lineage counts on Sigma charts to drop on the next run as the incorrect edges are removed.
