...
- **(Security / Dependencies / Frontend SSO)** pac4j (`pac4j-core` / `pac4j-oidc`) bumped from 6.4.2 to **6.5.8** and `play-pac4j` from 13.0.2 to **13.0.3** for CVE-2026-82461 (Keycloak access-token roles trusted without signature/issuer/audience/expiry validation), CVE-2026-82462 (OIDC callback accepted an access token with no authorization code or ID token, allowing session forgery via token substitution), CVE-2026-82463 (reversed `CheckProfileTypeAuthorizer` check), CVE-2026-82464 (backslash-prefixed logout open redirect) and CVE-2026-82465 (unsigned SAML `LogoutRequest` could destroy a session); 6.5.8 also rejects logout redirect URLs containing control characters or spaces. Transitively bumps `nimbus-jose-jwt` to 10.9.1, `oauth2-oidc-sdk` to 11.38.2 and `json-smart` to 2.6.0. **Action:** none for operators; rebuild/redeploy picks up the new JARs. The OIDC callback now requires an authorization code or ID token, which is already the case for the standard authorization-code flow DataHub uses.
