...
- #18063 **(GMS / Authorization)** Fixed `Authorizer.authorizedActors()` (via `PolicyEngine.getMatchingActors`) reporting the wrong wildcard-group flag: the returned `allGroups` was derived from the policy's "all users" setting instead of "all groups". A policy granting a privilege to **all groups** now correctly reports `allGroups = true`, and a policy granting it to **all users** no longer falsely reports `allGroups = true`. This only affected the informational "who has access" API surface (and any plugins/UI built on it); live authorization decisions (`authorize()`) were never affected and required no change. **No action required.**
