...
- **(Python SDK / Ingestion)** The REST and Kafka emitters now reject an `entityUrn` that `Urn.from_string` cannot parse, raising `InvalidUrnError` before the write is sent. Previously such a URN was accepted by GMS — which only rejects reserved characters inside URN components when `STRICT_URN_VALIDATION_ENABLED` is enabled, and that defaults to false — then persisted, after which every typed-URN read of that entity failed forever with nothing surfaced at write time. The guard parses with `Urn.from_string`, so it accepts exactly what a reader accepts and can only reject URNs that were already unreadable. Two shapes are affected in practice: reserved characters (`(`, `)`, `,`, `␟`) left unencoded inside an id, and structurally malformed URNs such as a dataset URN whose platform component is not a `dataPlatform` URN. Reading, and constructing a `MetadataChangeProposalWrapper` from, an already-persisted bad URN is deliberately unaffected, so migration and cleanup tooling can still handle existing entities. **Action:** build URNs with the typed classes (`DatasetUrn(...)`, `DashboardUrn(...)`) or the `make_*_urn` builders, which URL-encode reserved characters for you, rather than assembling URN strings by hand.
