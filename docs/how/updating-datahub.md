...
- #19554 **(CLI / Python)** Optional extras that pull `unstructured` / `unstructured-ingest` (`notion`, `confluence`, `unstructured`, `datahub-documents`) now pin patched releases that fix CVE-2026-71428 (`unstructured==0.24.1`, `unstructured-ingest==1.4.28`). Those libraries require **Python 3.11+**. The v1.6.0 LTS hotfix line already requires Python 3.11, so these extras remain part of `acryl-datahub[all]`. **Action:** upgrade CLI/venvs that install those extras to the patched pins; no extra install is needed on the full ingestion image beyond a rebuild.

- **(CLI / Python)** `urllib3` is floored at **>=2.5.0,<3.0** in `acryl-datahub` (lock / Docker images use **>=2.7.0**). `unstructured` 0.24.1 needs urllib3 2.x (`NameResolutionError`). `acryl-datahub[elasticsearch]` now depends on `opensearch-py>=3.0.0,<4.0.0` instead of `elasticsearch==7.13.4`. The `profiling-ge` extra (acryl-great-expectations, urllib3 1.x) is no longer part of `[all]` or the generated pyproject extras. **Action:** rebuild ingestion venvs/images. Elasticsearch source `api_key` must be a `[id, api_key]` list or a base64-encoded string. To keep GE SQL profiling, install `acryl-great-expectations` separately on a urllib3 1.x venv; it cannot coexist with this urllib3 2.x floor. Installing `acryl-datahub` together with **Airflow 2.x constraint files** (urllib3 1.26.x) is unsatisfiable; plugin CI now tests Airflow 3.1 only (same as master after #17439 / #19056). Use a venv that does not apply Airflow's urllib3 1.26 pin, or Airflow 3.x.

- #19873 **(CLI / Python)** `confluent-kafka` is raised to **>=2.15.1,<3.0.0** (was capped below 2.13.0). Schema Registry clients must tolerate optional `schema_str` / schema-reference fields. **Action:** rebuild ingestion and actions images; recreate venvs so they pick up the new wheel.

- #19601 **(Ingestion / Dependencies)** `acryl-datahub` no longer caps `setuptools`, so it installs alongside `setuptools>=83` (CVE-2026-59890). `setuptools>=82` dropped `pkg_resources`, which the `redshift`/`cockroachdb` SQLAlchemy dialects still import at load, so ingestion installs a small `pkg_resources` compatibility shim at runtime. `stopit` is vendored into `datahub.utilities`, so the `powerbi` extra no longer depends on it. **Action:** none. Airflow constraint files that pin an older `setuptools` remain satisfiable; the `>=83` floor is applied at lock time and in the `datahub-actions` image.

- **(GMS / Asset summary settings writes)** The `updateAssetSettings` GraphQL mutation and any write to the `assetSettings` aspect (OpenAPI, Rest.li, ingestion MCPs) now require **Edit Entity** or **Manage Asset Summary** on the target asset, enforced at the aspect layer by `AssetSettingsAuthorizationValidator`. Previously the GraphQL mutation performed no authorization check, so any authenticated user could change the asset-summary template of any asset. Admins, Editors, and asset owners hold **Manage Asset Summary** by default, and REST writes already required **Edit Entity**, so legitimately privileged callers are unaffected. Note that OpenAPI and Rest.li still authorize the write as a generic entity update, so **Manage Asset Summary** alone is not sufficient there; only the UI / GraphQL path honors the narrower privilege. System writes are exempt. **Action:** grant **Manage Asset Summary** (or **Edit Entity**) on the relevant assets to any automation that writes `assetSettings` outside the UI. Toggle via `metadataChangeProposal.validation.aspectAuthorization.assetSettings.enabled` (default `true`).
...
- **(CLI / Python)** `snowflake-connector-python` floored at **>=4.7.1** (lock **4.7.5**) for CVE-2026-15925 (TLS hostname verification skipped a name match). **Action:** rebuild ingestion venvs/images.
...
- **(CLI / Python, v1.6.0.x hotfixes only)** Python 3.10 support is dropped on the **v1.6.0 LTS hotfix line** so customers can move onto ZDU-enabled releases that ship security-hardened images. `acryl-datahub`, plugin packages, `acryl-datahub-actions`, and `datahub-agent-context` on this line require **Python 3.11+**. This does **not** remove Python 3.10 support from `master`. **Action:** Upgrade to Python 3.11 or later before installing or upgrading v1.6.0.x packages.
