...
- **(Metadata Model / Data Products)** `dataProductProperties` now includes an optional `parentDataProduct` URN so Data Products can nest in a parent-child taxonomy (mirroring Domains' `parentDomain`). The field is additive; existing Data Products keep a null parent. **Existing search documents must be backfilled** so `hasParentDataProduct` is present — otherwise `getRootDataProducts` (which filters `hasParentDataProduct=false`) returns no results. On upgrade, the non-blocking system-update step `BackfillDataProductIndices` re-emits MCLs for `dataProductProperties` (`BOOTSTRAP_SYSTEM_UPDATE_DATA_PRODUCT_INDICES_ENABLED`, default `true`). Disable with that env var if needed; bump the step id to re-run after a successful marker.
