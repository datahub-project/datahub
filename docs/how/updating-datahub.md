...
- **(Ingestion / Iceberg)** The iceberg source now depends on `pyiceberg >=0.12,<0.13` (was 0.11.x; the transitive `pyarrow` floor is now `>=18`, already satisfied by DataHub's `pyarrow>14,<24` range) and fully supports Iceberg table format version 3: V3 tables are ingested with their `format-version` surfaced as a dataset property, nanosecond timestamps (`timestamp_ns`/`timestamptz_ns`), `unknown`, `geometry`, and `geography` columns are mapped to schema fields (ns timestamps degrade to `timestamp-micros` precision; the original type is kept as the field's native data type), V3 column defaults (`initial-default`/`write-default`) appear in field descriptions as `Field default value: <value>` (and `Initial default value: <value>` when they differ), and profiling statistics now exclude position/equality delete files. **Action:** none if you install via `acryl-datahub[iceberg]`. If you pin `pyiceberg` or `pyarrow` yourself, use `pyiceberg >=0.12.0,<0.13.0` and `pyarrow >=18`.
