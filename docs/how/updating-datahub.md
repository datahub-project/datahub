...
- **(OpenLineage endpoint)** The endpoint now accepts all three OpenLineage event types and exposes the converter's remaining configuration. **(1) `JobEvent` and `DatasetEvent` are supported.** Previously every payload was parsed as a `RunEvent`, so the spec's static-lineage events — a producer describing a job or a table it did not just execute or write — deserialized with a null run and returned `500`. A `JobEvent` now creates the DataFlow, DataJob and lineage edges without a DataProcessInstance, and a `DatasetEvent` creates dataset aspects alone. The event type is taken from `schemaURL`, falling back to the event's shape when a producer omits it. **(2) Nine configuration options that the converter supported but the endpoint never passed through are now settable**: `path-specs`, `connections` (per-connection platform instance and env), `lower-case-dataset-urns`, `hive-platform-alias`, `disable-symlink-resolution`, `remove-legacy-lineage`, `include-indirect-column-lineage`, `enhanced-merge-into-extraction`, and `pipeline-name`. `pipeline-name` was previously declared but never wired, so `DATAHUB_OPENLINEAGE_PIPELINE_NAME` silently did nothing. `path-specs` and `connections` take structured values and must be set in `application.yml` rather than through environment variables, because their keys contain characters environment variable names cannot express. **Action:** none — every new option keeps its previous effective default. Deployments spanning multiple accounts, regions or hosts should set `connections` so OpenLineage URNs match what each platform's own connector stamps.

- **(OpenLineage / Spark lineage)** Column-level lineage now records each source column's **own** transformation rather than a transformation shared across every column feeding the same output field. OpenLineage states transformations per (output column, input column) pair, but the converter built one `fineGrainedLineages` entry per output field and gave every input column in it the union of them all. A column that only appeared in a `GROUP BY` was therefore recorded as `DIRECT:IDENTITY` whenever a sibling column produced the value, which reads in the UI as though the output came from that column. The same set of column-to-column edges is emitted as before — only the transformation recorded against each one narrows to what the producer actually declared for it. Note that `transformOperation` is part of the key identifying a fine-grained lineage entry, so entries written by earlier versions are keyed on the old combined value and are **not** updated in place by new events; they remain alongside the new ones until the aspect is rewritten. `include-indirect-column-lineage` is unaffected: when indirect-only columns are excluded there is no entry left to carry their role, so those transformations continue to be folded into the columns that remain. **Action:** none for new lineage. If stale duplicates from before the upgrade matter to you, re-emit the affected jobs' events, or set `DATAHUB_OPENLINEAGE_USE_PATCH=false` (endpoint) so the aspect is replaced wholesale rather than patched.

- **(OpenLineage)** The converter now reads ten OpenLineage facets it previously discarded, so more metadata arrives from the same events with no recipe change. On **output datasets**, `outputStatistics` and `lifecycleStateChange` become an `Operation` aspect carrying row counts and the operation type, which gives datasets a write history and a freshness signal from OpenLineage alone. On **any dataset**, `dataQualityMetrics` becomes a `DatasetProfile` (row count, size, per-column null and distinct counts), `tags` and `ownership` become the dataset's `GlobalTags` and `Ownership`, and `documentation`, `storage`, `datasetType` and `version` become `DatasetProperties`. On the **run**, `nominalTime` (which distinguishes a backfill from a scheduled run), `externalQuery` (the warehouse-side query id, which joins an OpenLineage run to the warehouse's own query history) and `extractionError` become `DataProcessInstance` custom properties; an `extractionError` also logs a warning, because it is the producer reporting that its own lineage is incomplete. On the **job**, `jobType` becomes custom properties (so `BATCH` vs `STREAMING` is finally recorded) and `sourceCodeLocation` becomes an `InstitutionalMemory` link. **Action:** none — the change is additive, and an event carrying none of these facets produces exactly the same output as before. Dataset-level aspects follow the existing schema gating, so they are emitted only when dataset materialization is enabled.

- **(OpenLineage endpoint)** The `/openapi/openlineage/api/v1/lineage` endpoint now enforces authorization, changes its success status code, and applies lineage additively. **(1) Authorization is enforced.** The endpoint previously accepted any authenticated token and wrote lineage without checking privileges. It now requires `Edit Entity` and `Edit Lineage` on the entity types an event touches (DataFlow, DataJob, DataProcessInstance, Dataset) and returns `403` otherwise. **Action:** if your OpenLineage producer authenticates with a non-admin personal access token, grant that user a policy covering those privileges, or it will start receiving `403`. **(2) Status codes changed.** Success is now `201` instead of `200`, a malformed payload returns `400` instead of `500`, and an event that is well-formed but cannot be converted returns `422` instead of `500`. Error responses now carry a JSON body with an `error` field. The OpenLineage HTTP transport and Marquez-compatible clients already treat `201` as success. **Action:** none unless you assert on an exact `200`. **(3) Lineage is applied additively.** OpenLineage producers split a run's metadata across `START`, `RUNNING` and a terminal event; the endpoint previously let the last event overwrite the earlier ones, so a producer declaring inputs at `START` and outputs at `COMPLETE` kept only the outputs. Both are now retained. As a consequence job lineage is append-only: an input a job no longer reads keeps its edge until removed by hand. **Action:** set `DATAHUB_OPENLINEAGE_USE_PATCH=false` to restore the previous last-event-wins behaviour. **(4) Event payloads are no longer logged at INFO.** Request bodies, deserialized events and generated proposals moved to `DEBUG`, so table names, column names and SQL text no longer reach shipped logs by default.
