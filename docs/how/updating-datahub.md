...
- **(GMS / timeseries)** Timeseries read path is selected with `timeseriesAspectService.implementation` (`elasticsearch`, `opensearch`, or `postgres`) and `TIMESERIES_ASPECT_SERVICE_IMPLEMENTATION`, not `postgres.pgTimeseries.read.enabled` / `DATAHUB_PGTIMESERIES_READ_ENABLED` (removed). For PostgreSQL reads, set `implementation: postgres` and keep `postgres.pgTimeseries.enabled: true` with SqlSetup.
