...
- **(Ingestion / Snowflake, Unity Catalog, Glue)** Object storage dataset URNs generated as **lineage upstreams** now match what the S3, GCS, and Azure Blob Storage sources themselves emit for the same object, so the edges join instead of dangling:

  - **S3** (Snowflake, Unity Catalog, Glue): URN-reserved characters (`,`, `(`, `)`) in object keys are now escaped. Previously a key like `reports/report (1).csv` produced `...,s3,my-bucket/reports/report (1).csv,PROD)` and a key containing a comma produced a malformed URN.
  - **Azure Blob Storage** (Snowflake external stages): file extensions are no longer rewritten — `data.parquet` stays `data.parquet` instead of becoming `data_parquet` — and reserved characters are escaped. The ABS source never mangled extensions, so these upstreams previously failed to join for every file-like path.
  - **GCS** (Snowflake external stages): leading and trailing slashes are now stripped the way the GCS source strips them; previously only a single trailing slash was removed.

  Snowflake additionally applies `platform_instance_map` (keys `s3`, `gcs`, `abs`) to these upstreams, so storage ingested under a `platform_instance` now joins — see the [Snowflake connector docs](https://docs.datahub.com/docs/generated/ingestion/sources/snowflake#external-storage-upstreams-and-platform-instances). A key outside that set now fails at startup rather than being silently ignored. GCS and Azure Blob Storage locations also now produce lineage on the `COPY` history, external-table DDL, and external-lineage paths, which previously emitted S3 edges only; relatedly, `num_external_table_edges_scanned` counts edges actually emitted rather than external-table rows seen. **Unity Catalog and Glue** accept `platform_instance_map` for the same purpose, with `s3` as the only key either one reads (any other key fails at startup), so their S3 lineage upstreams now join buckets ingested under a `platform_instance` too. **Action:** set `platform_instance_map: {s3: <instance>}` on a Glue or Unity Catalog recipe whose S3 data is ingested under a platform instance; without it those upstreams stay unprefixed as before.

  **Action:** Only affects buckets and containers whose paths hit the cases above, and Snowflake sources reading from GCS or Azure Blob Storage. Lineage edges pointing at the old URNs will no longer resolve and will need to be re-ingested; the newly emitted URNs now correctly join the datasets produced by the storage sources. If that storage is ingested under a `platform_instance`, set `platform_instance_map` on the Snowflake recipe — without it these upstreams still will not join. Datasets emitted by the S3, GCS, and ABS sources themselves are unchanged, as are Glue/Athena/SageMaker _table_ URNs (only the lineage-upstream helpers changed).
