...
- **(Ingestion / Sigma — Opt-in, additive) Sigma Data Model ingestion.** New `ingest_data_models` flag (default `False`) emits each Sigma Data Model as a `Container` with one `Dataset` per element. Nothing on an existing deployment's emitted aspects changes until you opt in — no URNs rotate, no existing workbook / Sigma Dataset shape is altered. Each element Dataset carries `SchemaMetadata` (column types default to `NullType` / `nativeDataType: "unknown"` — Sigma does not return per-column types yet) and, when `extract_lineage: true`, `UpstreamLineage`; lineage edges cover intra-DM element-to-element, cross-DM element-to-element, external upstreams (warehouse tables and Sigma Datasets), and workbook-to-DM element references. The `/lineage` call is gated on `extract_lineage` so users who opt out at the workbook surface also opt out of DM lineage. Element URNs are keyed by the immutable Data Model UUID so attachments survive Sigma slug rotation. The report splits unresolved upstreams into `data_model_element_upstreams_unresolved_external` (filtered-out Sigma Datasets) and `data_model_element_upstreams_unknown_shape` (unparsed source_id shapes); the aggregate `data_model_element_upstreams_unresolved` is retained. **⚠ Side effect when paired with workbook ingestion — check your cardinality alerts before opting in:** when a workbook chart references a DM element, `ChartInfo.inputs` gains new entries (the DM element Dataset URN). This is a **one-time, permanent step-up** on first opt-in ingest. Any downstream dashboards, PagerDuty rules, or alerting policies keyed on `ChartInfo.inputs` cardinality (fan-in count) will fire on the next ingestion run after opting in. Silence or raise those thresholds before enabling `ingest_data_models`. To opt in: set `ingest_data_models: true` and optionally use `data_model_pattern` / `ingest_shared_entities` to scope. A safety cap `max_personal_dm_discovery_rounds` (default `20`) bounds personal-space DM discovery passes; the loop terminates naturally under a well-behaved API, the cap is defensive.
- **(Ingestion / Sigma — Action required) Report shape change for null `DatasetUpstream.name`.** Sigma occasionally returns `name: null` on workbook dataset upstream nodes. Pre-release this raised a Pydantic `ValidationError` that surfaced as a `SourceReport.warnings` entry. It is now tolerated at parse time; drops are counted under the new `chart_dataset_upstream_name_missing` report field and continue to emit a `SourceReport.warning` (with `node=…, element=…, workbook=…` context, auto-truncated by `LossyList`) so production triage is unchanged. Lineage semantics are unchanged (the chart-to-Sigma-dataset edge cannot be SQL-correlated without a name and is skipped in both releases). **Action:** dashboards / alerts keyed on the previous warning _text_ (e.g. Pydantic `ValidationError` patterns) will go dark — migrate to the `chart_dataset_upstream_name_missing` counter or to the new warning title `Sigma workbook dataset upstream dropped (name missing)`.
