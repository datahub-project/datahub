...
- #19144 **(Airflow plugin / dataset URNs for warehouses)** Dataset URNs derived from Airflow Assets and from OpenLineage facets now follow each platform's own naming convention, so they match what that platform's DataHub connector emits. Two changes, both corrections:

  1. **Airflow Asset URIs for `snowflake://`, `postgresql://`, `mysql://`, `bigquery://`** no longer fold the URI authority into the dataset name or keep its slashes. `Asset("snowflake://myacct/MY_DB/MY_SCHEMA/EVENTS")` produced `snowflake,myacct/MY_DB/MY_SCHEMA/EVENTS` — matching nothing the Snowflake connector emits, and showing up beside the real dataset as a duplicate. It now produces `snowflake,my_db.my_schema.events`: the authority is dropped as a connection identifier (kept for BigQuery, where it is the project), segments are joined with dots, and casing follows the same per-platform rule DataHub's SQL parser uses (`PLATFORMS_WITH_CASE_SENSITIVE_TABLES`): snowflake, postgres and mysql are lowercased, bigquery keeps its case.
  2. **OpenLineage-derived names** get the same per-platform casing. OpenLineage producers report Snowflake names in upper case while the Snowflake source lowercases them, so `MY_DB.MY_SCHEMA.EVENTS` was previously a second dataset alongside the ingested `my_db.my_schema.events`. Snowflake, postgres and mysql names from this path are now lowercased; bigquery keeps its case.

  No lineage is dropped and no configuration is required — these URIs previously produced entities that could not join the lineage graph. **Action:** none for most deployments, though the corrected URNs differ from the old ones, so the previously-created mismatched entities are now orphaned and can be soft-deleted. If your warehouse ingestion sets `platform_instance`, add a `[datahub] asset_connections` entry so the prefix matches, for example `asset_connections = {"snowflake://myacct": {"platform_instance": "prod_acct"}}` — one entry now also applies it to SQL-parsed lineage, which previously had no way to express a platform instance. Object-store and `@asset` URIs (`s3://`, `gs://`, `file://`, `hdfs://`, `abfs://`, plain names) are unchanged. See [Airflow integration](../lineage/airflow.md#native-airflow-assetsdatasets).
