...
- (Ingestion / Sigma) Sigma Data Models are now ingested by default. Each Data Model emits one `Container` entity (subType `Sigma Data Model`) plus one `Dataset` entity per element (subType `Sigma Data Model Element`) with per-element schemaMetadata and `UpstreamLineage` covering three shapes: intra-DM element→element edges, external upstreams (warehouse tables and Sigma Datasets), and **cross-Data-Model element→element edges** (one DM imports a table from another — resolved via a name-based bridge; reported through `data_model_element_cross_dm_upstreams_resolved` / `_ambiguous` / `_name_unmatched_but_dm_known` / `_dm_unknown`). Workbook chart elements that reference a Data Model resolve via name match to the specific element Dataset URN; if the element name cannot be matched no lineage edge is emitted (the occurrence is surfaced through the `element_dm_edge_name_unmatched_but_dm_known` and `element_dm_edge_unresolved` report counters). Element Dataset URNs are keyed by the immutable Data Model UUID (`urn:li:dataset:(sigma,<dataModelId>.<elementId>,env)`) so DataHub-side attachments survive Sigma slug rotation. Column-level lineage (`FineGrainedLineage`) is **not** emitted on Data Model element `UpstreamLineage` in this release; each element Dataset already carries its own `SchemaMetadata`, so CLL can be added in a follow-up without re-ingestion. Expect a net-new set of Container/Dataset URNs on the next ingestion run proportional to the number of Data Models in the source; this is additive and no existing Workbook/Dataset URNs change. To disable the new emission, set `ingest_data_models: false` in the Sigma recipe; use `data_model_pattern` to allow/deny individual Data Models by name. Also adds on-demand discovery of personal-space Data Models referenced from ingested Data Models (reported through `data_model_external_references_discovered` / `data_model_external_reference_unresolved`); discovered personal-space DMs emit as full Container + element Dataset entities flagged `isPersonalDataModel = "true"` in customProperties. Stateful ingestion with stale-entity removal will treat any previously emitted DM entity under the old (reverted) flat-Dataset URN shape as stale and clean it up on the next run.
