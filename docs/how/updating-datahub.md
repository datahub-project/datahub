...
- **(GMS / Structured Properties)** Creating a structured property is now rejected when its `qualifiedName` would collide with another property's Elasticsearch field name. Field names are derived by replacing `.` with `_`, so `certification.status` and `certification_status` both map to the field `certification_status`. Previously both could be created and would silently share (and clobber) the same search field; now `PropertyDefinitionValidator` fails the write both within a single batch and when the normalized field already exists in active entity-index mappings. The check looks at index mappings (not the primary-store definition catalog), so a definition that exists but is not yet mapped — for example during UpdateIndices/MAE lag, or when `ENABLE_STRUCTURED_PROPERTIES_HOOK=false` — may not be detected until the mapping appears; soft-deleted properties whose fields remain mapped still block colliding creates. The same applies after a **hard delete**: the definition and assignments are removed, but the search-field mapping remains until reindex, so recreating the exact same `qualifiedName` is rejected until mappings are cleaned up. That is intentional — reusing the field while leftover index state may still exist would pollute what should be a fully purged property. Prefer soft delete when you may need the name again; otherwise use a different `qualifiedName` or reindex after assignment cleanup. Existing colliding properties are not retroactively rejected — at index-mapping time, colliding fields with identical mappings keep the property with the lexicographically smallest URN (logged as a warning), while colliding fields with different mappings are omitted entirely (logged as an error) to avoid an incorrect index type. `qualifiedName` remains immutable, so upserts to an existing property skip this check. **Action:** Choose `qualifiedName` values that remain distinct after `.`→`_` normalization. If you already have colliding properties, rename or remove all but one so the intended property is indexed. See [Why the same name cannot be recreated after hard delete](../api/tutorials/structured-properties.md#why-the-same-name-cannot-be-recreated-after-hard-delete).
