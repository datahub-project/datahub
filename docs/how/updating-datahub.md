...
- **(Ingestion / BigQuery)** Date-suffixed tables that sit beside an un-suffixed table of the same name — `my_table` next to `my_table_20251120` and `my_table_20251121` — are now skipped instead of being treated as shards of `my_table`. A genuine sharded set has no un-suffixed member, so when one exists the dated tables are separate physical tables (a dated copy, a backup) rather than shards. Previously all of them collapsed onto the base table's URN, and whichever happened to be written last overwrote the real table's schema and profile, so row counts and column counts could be wildly wrong and could change between runs with no other change. Genuine sharded sets still collapse to the base name as before. Skipped tables are reported as a `Date-suffixed tables skipped` warning naming the base table, and counted in the new `num_sharded_tables_shadowed_by_base_table` report field. **Action:** none — the affected base tables get correct stats on the next run. Rename the dated copies if you want them ingested as their own datasets.
