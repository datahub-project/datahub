...
- **(GMS / Performance) Latest-aspect cache for timeseries reads.** GMS now ships an opt-in in-memory cache that fronts `TimeseriesAspectService` for the common "give me the latest version" query — used heavily by entity profile pages and search list views over timeseries aspects like `datasetProfile` and `datasetUsageStatistics`. The cache is **disabled by default**; enable it per-deployment via the env vars below.
  - `TIMESERIES_CACHE_ENABLED` (default `false`) — set to `true` to wrap the underlying ES-backed timeseries service with the cache. With this off, behavior is identical to previous releases.
  - `TIMESERIES_LATEST_CACHED_ASPECTS` (default `datasetProfile`) — comma-separated list of aspect names eligible for caching. Only "latest version" reads (`limit=1`, no time bounds, no filter, no sort) are cached. Other aspects pass through untouched. Example: `TIMESERIES_LATEST_CACHED_ASPECTS=datasetProfile,datasetUsageStatistics`.
  - `TIMESERIES_CACHE_TTL_HOURS` (default `48`) — base TTL for cached entries.
  - `TIMESERIES_CACHE_TTL_JITTER_MINUTES` (default `60`) — random jitter applied to each entry's TTL to avoid expiry stampedes. Honored on Hazelcast (per-entry TTL); on single-node Caffeine deployments the jittered value is ignored and entries inherit the cache's `expireAfterWrite` of `TIMESERIES_CACHE_TTL_HOURS`.
  - `TIMESERIES_CACHE_MAX_SIZE` (default `100000`) — eviction-size budget for the latest-aspect cache. This is independent of `cache.primary.maxSize` (which sizes other GMS caches).
  - **Backend.** Cache backend follows `searchService.cacheImplementation` — `caffeine` for single-node dev, `hazelcast` for clustered deployments. The Hazelcast path uses an `EntryProcessor` for atomic compare-and-set across replicas; out-of-order or replayed upserts cannot clobber a newer cached value.
  - **Failure modes are logged, not fatal.** If `TIMESERIES_CACHE_ENABLED=true` but `TIMESERIES_LATEST_CACHED_ASPECTS` resolves to an empty set (e.g., misconfigured env var), GMS logs `Timeseries cache enabled but no aspects configured via TIMESERIES_LATEST_CACHED_ASPECTS` and falls back to the uncached service. No request errors.
