...
- **(Ingestion / Dependencies)** The `setuptools<82` cap has been removed from `acryl-datahub`, so it can now be installed alongside `setuptools>=83` (which fixes CVE-2026-59890). The unmaintained `stopit` dependency (previously pulled by the `powerbi` extra) was replaced with an in-repo timeout utility. The `redshift` and `cockroachdb` connectors install a small `pkg_resources` compatibility shim at runtime so their SQLAlchemy dialects keep working under `setuptools>=82` (which removed `pkg_resources`). **Action:** none. Installs under Airflow constraint files that pin an older `setuptools` are unaffected; the `>=83` floor is applied at lock time and in the `datahub-actions` image, and the `datahub-ingestion` image resolves setuptools to the latest (>=83) transitively.
