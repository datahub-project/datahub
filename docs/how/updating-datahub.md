...
- #19383 **(Ingestion / openapi)** The `get_token` and `forced_examples` recipe fields are now validated. `get_token` is a typed object (`request_type: get|post`, `url_complement`) instead of a free-form dict — an unrecognized key, a missing `request_type`/`url_complement`, or a `request_type: get` `url_complement` missing the `{username}`/`{password}` placeholders now fails recipe validation instead of failing later (or silently) at ingestion time. `forced_examples` values must be lists of scalars (str/int/float/bool); a `null` or object entry is now rejected instead of being silently stringified as `"None"` in the composed URL. `schema_resolution_max_depth` is now bounded to `1..100` (previously any integer was accepted); `0` or a value above `100` now fails validation. **Action:** none if your recipe already used `get_token: {request_type: ..., url_complement: ...}` and scalar `forced_examples` values (the common case), and left `schema_resolution_max_depth` at its default or within `1..100`. Fix any recipe that previously relied on a malformed `get_token`/`forced_examples` value being silently ignored, or that set `schema_resolution_max_depth` to `0` or above `100`.
