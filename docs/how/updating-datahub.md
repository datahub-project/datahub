...
- **(GMS / Authorization)** New platform privilege `VIEW_SYSTEM_STATUS` grants read access to non-sensitive system status: messaging consumer lag, messaging transport, registered consumers, and Spring component status (`/openapi/v1/system-info` and `/spring-components`). It does **not** include full system configuration (`/system-info/properties`), raw index access, throttle/rate-limit controls, or other operational mutations. `MANAGE_SYSTEM_OPERATIONS` remains a superset. The privilege is **not** on the default Admin or root boot policies; grant it explicitly (for example to a smoke/release-test PAT). Smoke `wait_for_writes_to_sync` retries lag-API 401/403 for 20s then fails with a grant hint; set `USE_STATIC_SLEEP=true` if ops routes are unreachable.
