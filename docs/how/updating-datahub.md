...
- **(Python SDK / Metric)** `Metric.set_upstream_datasets(...)` is deprecated in favor of `Metric.set_upstreams(datasets=..., fields=...)`. The alias still works and is equivalent to `set_upstreams(datasets=...)`, which writes `fieldUpstreams=[]`. The constructor kwarg `upstream_datasets=` is not deprecated. **Action:** switch scripts to `set_upstreams`. After `client.entities.get(...)`, pass `fields=` (for example `fields=metric.upstream_fields`) if you intend to keep column lineage — `metricUpstreams` is a snapshot, not a tables-only patch.
