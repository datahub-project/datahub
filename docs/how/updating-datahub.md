...
- **(GMS / Search V3 document IDs)** Search V3 Elasticsearch/OpenSearch documents now use a SHA-256 hex of the entity URN as `_id` instead of the URL-encoded URN. The full URN remains a document field (`urn`). V2 indices are unchanged. **Action:** none if Search V3 write was never enabled. If an environment already wrote V3 documents under URL-encoded `_id`s, hashed IDs are not an in-place rewrite — run a fill/reindex (ZDU) so existing V3 docs are rewritten; otherwise updates create a new document and leave the old one.
