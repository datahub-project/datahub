...
- **(GMS / Search indexing)** GMS refuses to start when it is consuming MCLs (`MAE_CONSUMER_ENABLED` or `MCL_CONSUMER_ENABLED`) and both UI indexing paths are off (`PRE_PROCESS_HOOKS_UI_ENABLED=false` and `PRE_PROCESS_HOOKS_REPROCESS_ENABLED` unset/false). That combination stored UI-created entities without writing search documents (ingestion sources, execution requests, and other GraphQL creates never appeared in the UI). **Action:** leave `PRE_PROCESS_HOOKS_UI_ENABLED` at its default `true`, or set `PRE_PROCESS_HOOKS_REPROCESS_ENABLED=true` if you intentionally disabled the GMS fast path. Kubernetes system-update scale-down is unchanged: it also sets `MAE_CONSUMER_ENABLED=false`, so GMS still starts. See [#19119](https://github.com/datahub-project/datahub/issues/19119).
