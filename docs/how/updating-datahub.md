...
- #14014: Merging this PR has implicit consequences for `iceberg` ingestion source. If it is run from CLI and `datahub`
  CLI has been installed with all extras (`acryl-datahub[all]`), then `pyiceberg` has been kept at `0.4.0` version
  in such environment. After this PR is merged, `pyiceberg` will be installed in the newest available version
  (`0.10.0` - at the time of writing) - the same version has been used for some time by Managed Ingestion. There
  have been changes to the catalog connection configuration details - especially for AWS-based catalogs and warehouses,
  the properties `profile_name`, `region_name`, `aws_access_key_id`, `aws_secret_access_key`, and `aws_session_token`
  were deprecated and removed in version `0.8.0`. To check whether your configuration will work, consult
  https://py.iceberg.apache.org/configuration/#catalogs. **There are no changes needed for iceberg recipes orchestrated
  via the Managed Ingestion UI**
