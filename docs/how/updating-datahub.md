...
- **(Airflow plugin / native Airflow Assets)** Airflow Assets whose URI points at a warehouse (`snowflake://`, `postgresql://`, `mysql://`, `bigquery://`) are no longer turned into DataHub URNs automatically. Previously the plugin folded the URI authority into the dataset name and kept the slashes, so `Asset("snowflake://myacct/MY_DB/MY_SCHEMA/EVENTS")` produced `snowflake,myacct/MY_DB/MY_SCHEMA/EVENTS` — a dataset that matched nothing the Snowflake connector emits, appearing alongside the real one as a duplicate. A URI alone cannot supply the `platform_instance` your warehouse recipe uses or that platform's casing, so these Assets are now **skipped with a warning** unless you map their connection. **Action:** add a `[datahub] asset_connections` entry per warehouse connection, for example `asset_connections = {"snowflake://myacct": {"platform_instance": "prod_acct"}}`. The same mapping also aligns the OpenLineage path, which reports Snowflake names in upper case while the Snowflake source lowercases them. Object-store and `@asset` URIs (`s3://`, `gs://`, `file://`, `hdfs://`, `abfs://`, plain names) are unaffected — for those the URI already is DataHub's naming. If you were relying on the old URNs, mapping the connection is the migration path; note that the old and new URNs differ, so previously-created duplicate entities can be soft-deleted. See [Airflow integration](../lineage/airflow.md#connection-mapping).
