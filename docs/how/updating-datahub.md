...
- **(Ingestion / Unity Catalog)** The Unity Catalog usage/lineage extractor no longer falls back to a PySpark-based SQL-plan parser; query parsing is handled entirely by the (Java-free) sqlglot parser using the Databricks dialect. With this and the data-lake change above, **`pyspark` is no longer a dependency of any DataHub ingestion extra** and has been removed entirely, eliminating the JVM/Java requirement for the `unity-catalog`/`databricks` extras.
