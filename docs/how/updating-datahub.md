...
- #19856 **(Ingestion / Remote Executor)** Dynamic ingestion virtual environments are now reused across runs instead of being rebuilt and deleted every time. A venv whose contents are fully determined by its name is kept in a node-local cache at `<tmp_dir>/_venv_cache` (by default `/tmp/datahub/ingest/_venv_cache`), which mostly speeds up short tasks such as **test connection**, where rebuilding the venv dominated the run. This applies to DataHub Cloud remote executors and to OSS `datahub-actions`. Two behaviours change by default. **Disk:** venv disk is no longer released as each run finishes; the cache keeps at most `DATAHUB_VENV_CACHE_MAX_ENTRIES` entries (default `10`) and drops any unused for `DATAHUB_VENV_CACHE_MAX_AGE_HOURS` (default `24`). It is bounded by count rather than size, so plan for one entry more than the limit, times your largest connector's venv, and note that space is reclaimed when a new venv is built rather than on a timer. A venv in use by a running ingestion is never removed. **Freshness:** a venv built from `version: latest`, or an `extra_pip_requirements` entry that is not pinned to an exact version is re-resolved once it is older than `DATAHUB_VENV_CACHE_LATEST_TTL_HOURS` (default `24`), so a newly published connector fix can take up to that long to reach a long-lived executor. A venv whose version and every requirement are exactly pinned never expires. Venvs built from a dev-build wheel URL are not cached at all, because their install bypasses `uv`'s package cache and so costs its full size on disk rather than mostly hardlinks. **Action:** re-check executor ephemeral-storage sizing, and lower `DATAHUB_VENV_CACHE_MAX_ENTRIES` if the volume is tight. Lower the TTL if you need new releases picked up sooner, or pin `version` for full control. Set `DATAHUB_VENV_CACHE_ENABLED=false` to restore a fresh venv per run; that stops new cache entries but does not delete existing ones, so remove the cache directory by hand to reclaim the space. See [Reusing venvs between runs](../docker/bundled-ingestion-venvs.md#reusing-venvs-between-runs).
...
- `DATAHUB_VENV_CACHE_ENABLED` (default `true`) — Reuse dynamic ingestion venvs from a node-local cache. See Other Notable Changes above and [Reusing venvs between runs](../docker/bundled-ingestion-venvs.md#reusing-venvs-between-runs).
- `DATAHUB_VENV_CACHE_PATH` (default `<tmp_dir>/_venv_cache`) — Cache root.
- `DATAHUB_VENV_CACHE_MAX_ENTRIES` (default `10`) — Most venvs the cache keeps; least-recently-used are removed first.
- `DATAHUB_VENV_CACHE_MAX_AGE_HOURS` (default `24`) — Remove a cached venv nothing has used in this long, whatever the count.
- `DATAHUB_VENV_CACHE_LATEST_TTL_HOURS` (default `24`) — How long a venv built from a moving version (`latest`, a wheel URL, or unpinned `extra_pip_requirements`) is reused before being re-resolved.
