...
- #19162 **(Ingestion / Hex)** The Hex source's `queriedTables` lineage path now builds upstream dataset URNs through the same schema resolver as the SQL-cell path: the table name is dialect-normalized and resolved against DataHub, with casing following the upstream warehouse (lowercase for snowflake/postgres/redshift/mssql/…, preserve for bigquery/db2) and BigQuery date-shard suffixes folded (`events_20240101` → `events_yyyymmdd`). Tier-1 and Tier-2 URNs now agree by construction, so column lineage is no longer dropped on case mismatches with a warehouse ingested using `convert_urns_to_lowercase`. **Action:** if you set a per-connection `convert_urns_to_lowercase` on `HexConnectionDetail` in a pre-release build, remove it — it is now a no-op and casing is automatic. If a run drops column lineage for an entity, the logs now surface a `Column lineage dropped` warning naming the entity — inspect the lineage builder's mismatch sample to confirm the `queriedTables` and SQL-cell URNs refer to the same tables.
