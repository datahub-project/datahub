...
- **(Ingestion / Sigma) Sigma Data Model ingestion is opt-in.** New `ingest_data_models` flag (default `False`) emits each Sigma Data Model as a `Container` with one `Dataset` per element. Each element Dataset carries `SchemaMetadata` and `UpstreamLineage` for intra-DM references and external upstreams (Sigma Datasets ingested in the same run). Element URNs are keyed by the immutable Data Model UUID so attachments survive Sigma slug rotation. Additive — no existing URNs change. Cross-DM lineage, personal-space DM discovery, and workbook-to-DM element linking are coming in a follow-up PR.
