...
- **(Airflow / Dagster / Prefect plugins)** The default emit mode changed from synchronous to **ASYNC** to reduce GMS/MySQL load at high volume. With async, `emit()` no longer raises on a rejected write and there's no read-after-write guarantee. The Python SDK, CLI, and DataHub Actions defaults are unchanged. **Action:** to restore the previous behavior (`SYNC_PRIMARY`), set `[datahub] emit_mode = SYNC_PRIMARY` in `airflow.cfg` (Airflow), `emit_mode: SYNC_PRIMARY` on the Dagster source config, or `emit_mode=EmitMode.SYNC_PRIMARY` on the Prefect `DatahubEmitter` block.
