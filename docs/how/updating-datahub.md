...
- **(SAP Analytics Cloud ingestion)** SAC model datasets are no longer placed under the meaningless `t` > `4` browse tree. SAC exposes no folder for a model (most are private/embedded models created by a data import), so GMS previously derived a browse path by splitting the dotted internal dataset name (`t.4.<hash>:<hash>`), producing a `t` > `4` tree. Models now browse under a single canonical `Models` root, mirroring SAC's own built-in Models folder. **Action:** none required; re-ingest to move existing models out of the old `t`/`4` tree. Update any saved browse-path filter or bookmarked browse URL that pointed at the old model tree.
