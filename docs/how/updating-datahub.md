...
- #19162 **(Ingestion / Hex)** The Hex source's `queriedTables` lineage path now builds upstream dataset URNs through the same schema resolver as the SQL-cell path: the table name is dialect-normalized and resolved against DataHub, matching the casing the upstream table actually landed with when it is already ingested, and otherwise falling back to a per-platform default (lowercase for snowflake/postgres/redshift/mssql/…, case-preserving for bigquery). BigQuery date-shard suffixes are folded (`events_20240101` → `events_yyyymmdd`). Tier-1 and Tier-2 URNs now agree by construction, so column lineage is no longer dropped on case mismatches with a warehouse ingested using `convert_urns_to_lowercase`. **Action:** none required. If a run drops column lineage for an entity, the logs now surface a `Column lineage dropped` warning naming the entity — inspect the lineage builder's mismatch sample to confirm the `queriedTables` and SQL-cell URNs refer to the same tables.
