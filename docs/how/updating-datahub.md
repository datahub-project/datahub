...
- #17443 **(Ingestion / MongoDB)** AWS DocumentDB can now be ingested as its own data platform. The MongoDB source has a new opt-in config flag `emit_as_documentdb` (default `false`). When set to `true` and `hostingEnvironment: AWS_DOCUMENTDB`, the connector emits entities under the new `documentdb` data platform URN instead of `mongodb`. The default is unchanged, so existing recipes continue to emit `mongodb` URNs even when pointed at AWS DocumentDB. If enabled on an existing recipe the dataset URN will change and stale `mongodb` URNs will need to be cleaned up via stateful ingestion (if enabled) or manual soft-delete.
### Known Issues
