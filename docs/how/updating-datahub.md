...
- #19934 **(GMS / Search V3 semantic reads)** Semantic search can now be served from the Search V3 document index. Set `ELASTICSEARCH_ENTITY_INDEX_V3_SEMANTIC_READ_ENABLED=true` (requires `ELASTICSEARCH_ENTITY_INDEX_V3_ENABLED=true`) once the V3 document index holds your document embeddings; it is independent of `ELASTICSEARCH_ENTITY_INDEX_V3_KEYWORD_READ_ENABLED`. With Search V2 turned off, semantic search reads V3 on its own. Only entity types with their own V3 index and embeddings are searched (`document` by default). **Action:** none for most deployments; semantic search keeps reading the existing semantic indices. If Search V2 is turned off (`ELASTICSEARCH_ENTITY_INDEX_V2_ENABLED=false`), semantic search switches to V3 on upgrade, so make sure the V3 document index was created with semantic search enabled for `document` and holds your document embeddings.
