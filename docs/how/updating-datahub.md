...
- #19601 **(Ingestion / Dependencies)** `acryl-datahub` no longer caps `setuptools`, so it installs alongside `setuptools>=83` (which fixes CVE-2026-59890). `setuptools>=82` dropped `pkg_resources`, and a few dependencies still import it at load: `stopit` (via the `powerbi` extra) and the `redshift`/`cockroachdb` SQLAlchemy dialects. Ingestion installs a small `pkg_resources` compatibility shim at runtime before those load. **Action:** none. Installs under Airflow constraint files that pin an older `setuptools` are unaffected; the `>=83` floor is applied at lock time and in the `datahub-actions` image, while the `datahub-ingestion` image resolves setuptools to the latest (>=83) transitively.
