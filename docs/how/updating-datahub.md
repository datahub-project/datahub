...
- #19815 **(Sigma ingestion)** Sigma ended support for datasets as a data source on 2026-09-15, and a dataset-backed workbook element no longer returns SQL. The connector now recovers a Sigma Dataset's warehouse table from Sigma's connection metadata instead, so these lineage URNs are built from `connection_to_platform_map` rather than `chart_sources_platform_mapping`. Two things can change:

  - **`env` and `platform_instance`.** Without a `connection_to_platform_map` entry the URN uses the recipe's own `env` and no platform instance, where the SQL route used the mapping's. Note `env` defaults to `PROD` on both settings, so this bites even if you never set it.
  - **Identifier casing on non-Snowflake platforms.** The SQL route lower-cased every platform except BigQuery and DB2; this route lower-cases only Snowflake, so a Postgres table Sigma reports as `Analytics.Public.Orders` is emitted with that casing rather than `analytics.public.orders`.

  **Action:** add the Sigma `connectionId` to `connection_to_platform_map`, setting `env` and `platform_instance` to match your warehouse connector, plus `convert_urns_to_lowercase: true` if you want the old lower-cased spelling (honoured on any platform when set explicitly, for both table and column identifiers). The connector warns when it detects a mapping whose `env` or `platform_instance` no longer applies.
