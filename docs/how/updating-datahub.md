...
- **(Ingestion / MicroStrategy)** The MicroStrategy connector now reshapes its folder hierarchy to match Strategy Web (via `GET /api/folders/preDefined`): the "Shared Reports" folder shows its MicroStrategy-assigned label instead of its raw internal name ("Reports"), and two system containers Strategy Web never displays — the project root folder (which rendered as a duplicate project level) and "Public Objects" — are omitted, with their children re-parented upward. This is enabled by default (`use_predefined_folder_names: true`) and changes folder container URNs for any environment that already ingested these folders — the old containers become stale (soft-deleted automatically if stateful ingestion is enabled; otherwise left behind until manually removed). **Action:** if you'd rather keep prior container URNs on upgrade, set `use_predefined_folder_names: false` in your recipe.

- **(Ingestion / MicroStrategy)** Personal folders (each user's profile folder tree under the project's "Profiles" system folder: "My Reports", "My Objects", ...) are now excluded by default. Dashboards, documents and reports filed there are skipped and their folder containers are no longer emitted; previously they were ingested, and a `folder_pattern` deny on "My Reports" only removed the folder container while re-parenting its contents. With stateful ingestion the previously ingested personal content is soft-deleted on the next run. **Action:** set `include_personal_folders: true` in your recipe to keep ingesting them.
...
- **(Ingestion / MicroStrategy)** Compound-grid dossier visualizations now resolve dataset lineage per column group (using the page name, group name, and member object IDs), where previously such visualizations could resolve to no datasets at all. Visualization-local derived metrics (grid columns marked `derived: true`, which have no formula retrievable from MicroStrategy's REST API) now appear as schema fields on the dataset backing their column group, tagged `Measure` + `Derived`, and each grid's column groups are recorded on the chart's `microstrategyColumnGroups` custom property. Both are enabled by default and purely additive; set `extract_derived_metrics: false` to skip the new fields. A new opt-in `extract_metric_formula_lineage` flag (default `false`) emits field-to-field lineage between a catalog metric and the sibling fields its formula references.

- **(Ingestion / MicroStrategy)** Dataset schema fields are now emitted in report object order (metrics, then attributes and forms, then derived metrics, as the report or cube definition lists them) instead of alphabetically. Field paths and URNs are unchanged. Set `dataset_field_order: alphabetical` to keep the previous order.
