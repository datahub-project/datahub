...
- #18548 **(Python SDK / Circuit breakers)** `datahub.api.graphql.BaseApi` and its `Assertion` / `Operation` subclasses now query GMS through `DataHubGraph` instead of a `gql` client, so they support OAuth, PAT, system auth and mTLS via `DatahubClientConfig`. The `transport=` constructor argument is replaced by `graph=` (a `DataHubGraph`), the `.client` attribute by `.graph`, GraphQL errors raise `GraphError` instead of gql's `TransportQueryError`, and passing both `datahub_token` and `datahub_auth` is rejected. Requests also pick up the SDK client's request defaults: with `timeout` unset they now time out after 30 seconds and retry transient failures up to 4 times, where the `gql` transport waited indefinitely and never retried. **Action:** none for the Airflow circuit-breaker operators/sensors. Code that constructed these classes with a custom `gql` transport must pass a `DataHubGraph` instead. The `acryl-datahub[circuit-breaker]` extra is now empty (it no longer pulls in `gql`); installing it still works, so no recipe or install change is required. Set `timeout` (or `timeout_sec` in an Airflow connection extra) explicitly if a poke must wait longer than 30 seconds on a slow GMS.
