...
- **(Ingestion / dbt)** dbt semantic models can now be emitted as first-class `semanticModel`, `Semantic Model Dataset`, and `metric` entities, instead of a single dataset with subtype `Semantic Model`. Opt in with `emit_semantic_model_entities: true` on the `dbt` or `dbt-cloud` source. Off by default, so existing dataset URNs are unchanged. This also ingests dbt's top-level `metrics:` block and measures marked `create_metric: true`, which were previously parsed and discarded. **Action:** none unless you opt in. The two modes use different URNs (`<database>.<schema>.<name>` versus `<project>.<name>`), so turning the flag on is a replacement rather than an in-place upgrade — run with `stateful_ingestion.enabled: true` so the previous datasets are soft-deleted, and use `datahub migrate dbt-semantic-models` to copy owners, tags, terms, and documentation across first. The command works in both directions, so the flag stays reversible. Note that the project-level Semantic Model is not a governance destination: it is shared by every semantic model in the project, so governance moves to the corresponding Semantic Model Dataset. Requires a server new enough to have `semanticModel`/`metric` in its entity registry (DataHub Cloud >= 2.1.0, or DataHub Core >= v1.7.0). On Cloud the version is checked and ingestion falls back to the previous behaviour reporting why; on OSS there is no check, so confirm the server version first. `METRICS_ENABLED=true` is separate and only controls visibility in the Metrics UI and search — ingestion succeeds without it. Top-level `metrics:` definitions are dbt Core only — the dbt Cloud Discovery API does not expose them. See [dbt semantic models and metrics](../generated/ingestion/sources/dbt.md#semantic-models-and-metrics).
