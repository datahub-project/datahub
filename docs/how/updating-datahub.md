...
- **(Ingestion / Sigma) Sigma Data Model ingestion is now opt-in.** A new `ingest_data_models` flag (default `False`) gates the emission of Sigma Data Models as `Container` entities (subType `Sigma Data Model`) with one `Dataset` per element (subType `Sigma Data Model Element`). When enabled, ingestion emits per-element `SchemaMetadata` and `UpstreamLineage` covering four shapes: intra-DM element→element edges, external upstreams (warehouse tables and Sigma Datasets ingested in the same run), cross-Data-Model element→element edges (one DM imports a table from another — resolved via a name-based bridge; reported through `data_model_element_cross_dm_upstreams_resolved` / `_ambiguous` / `_name_unmatched_but_dm_known` / `_dm_unknown`), and workbook chart → DM element lineage via name match (reported through `element_dm_edges_resolved` / `element_dm_edge_name_unmatched_but_dm_known` / `element_dm_edge_unresolved`). Element Dataset URNs are keyed by the immutable Data Model UUID (`urn:li:dataset:(sigma,<dataModelId>.<elementId>,env)`) so DataHub-side attachments survive Sigma slug rotation. The source also performs on-demand discovery of personal-space Data Models referenced from ingested Data Models (reported through `data_model_external_references_discovered` / `data_model_external_reference_unresolved`); personal-space DMs are themselves gated by `ingest_shared_entities` (default `False`) and `data_model_pattern`, and are flagged `isPersonalDataModel = "true"` in customProperties (lowercase, matching the JSON boolean convention used elsewhere in the Sigma source). Column-level lineage (`FineGrainedLineage`) is **not** emitted on Data Model element `UpstreamLineage` in this release; each element Dataset already carries its own `SchemaMetadata`, so CLL can be added in a follow-up without re-ingestion. **Enabling this flag introduces a new entity class to the graph** (new Container and Dataset URNs proportional to the number of Data Models in the source); this is additive and no existing Workbook/Dataset URNs change. Disabling the flag after entities are emitted will cause stateful ingestion with stale-entity removal to soft-delete them on the next run. To opt in, set `ingest_data_models: true` in the Sigma recipe and use `data_model_pattern` to allow/deny individual Data Models by name. Stateful ingestion with stale-entity removal will also treat any previously emitted DM entity under the old (reverted) flat-Dataset URN shape from #17109 as stale and clean it up on the next run.
