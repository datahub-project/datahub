...
- #19930 **(Ingestion / MSSQL dataJob lineage)** Three changes. (1) With `incremental_lineage: true`, `dataJobInputOutput` is emitted as a **patch** instead of a full upsert, so lineage a user added by hand survives re-ingestion. A patch only adds, so stale auto-detected lineage is no longer pruned, and it writes the `*Edges` fields rather than the deprecated plain arrays -- entries left in those arrays by earlier runs persist, because the server-side patch template cannot reach them. If ingestion parses the same upstream a user added, the edge loses its UI stamps and stops being reported as manually added; the lineage itself is unaffected. A SQL Agent job step's lineage aspect is always empty, so it is now dropped rather than re-stated as an empty upsert. (2) Regardless of the flag, a procedure's lineage aspect is kept when it carries lineage in any field rather than only in the dataset arrays, so a body that just calls other procedures keeps its `inputDatajobs`. (3) Regardless of the flag, a stored procedure that cannot be processed no longer aborts the rest of its schema; it is still reported as a failure, so stale-entity soft-deletion stays suppressed for that run. **Action:** none on the default `incremental_lineage: false` -- procedure-to-procedure lineage appears without it. If the flag is already on (it is shared with view and query lineage, so it may be on for unrelated reasons), procedure lineage switches to patch mode on upgrade with the consequences above. Either way, lineage from earlier runs remains in the plain arrays; the UI's lineage removal clears both and can be used if the duplication matters.
