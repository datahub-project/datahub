...
- **(GMS / Search)** Query time canonicalization is now **on by default**. Time windows that DataHub derives from the wall clock — dataset usage stats, dataset operations stats, the dataset and dashboard usage summaries, search relevance ranking by usage, the analytics highlights and MCP tool-audit pages, the default lineage end time, and DataHub usage-event defaults — are rounded to a 5-minute bucket, so repeated requests for the same thing produce an identical Elasticsearch/OpenSearch query and can reuse the shard request cache. Rounding widens the window rather than narrowing it (lower bounds round down, upper bounds round up), so results are a superset of the exact window and no data is hidden; usage figures may include up to 5 extra minutes at the window edge, and a `date_histogram` can gain a bucket at the boundary if documents fall in that sliver. Timestamps you supply explicitly (an API `startTime`, a lineage date range, a point-in-time bound) are never rounded. This is not a cache TTL — it only makes requests eligible to reuse a cache entry. **Action:** none for most deployments. To restore exact per-request timestamps set `SEARCH_QUERY_CANONICALIZATION_ENABLED=false`; to change the bucket set `SEARCH_QUERY_CANONICALIZATION_TIME_BUCKET_SIZE` and `SEARCH_QUERY_CANONICALIZATION_TIME_BUCKET_UNIT` (e.g. `1` and `MINUTES`). Three counters report what the feature is doing: `datahub.search.canonicalization.applied`, `.changed` (bounds actually moved) and `.skipped` (tagged `reason=disabled|invalid_config`).
