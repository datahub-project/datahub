...
- #19210 **(Ingestion / SQL lineage for MySQL)** SQL lineage for MySQL, MariaDB and TiDB now preserves identifier casing while parsing instead of folding everything to lowercase. Previously every table and column identifier in a parsed query was lowercased before any URN was built, so lineage into a MySQL table that DataHub holds with its real casing (for example `my_db.my_schema.MixedCaseTable`) pointed at a lowercase URN that did not exist and the edge never connected to the real dataset. This affected every connector that parses SQL against a MySQL-family platform — Grafana, Superset, Metabase, Redash, Mode, dbt, and MySQL's own query lineage — whenever the query did not quote its identifiers. MySQL view definitions quote every identifier and were therefore unaffected. Column names are now matched case-insensitively and reported using the casing recorded in the schema; where no schema is available to recover the real casing from, the casing written in the query is kept rather than a lowercased guess, so emitted `schemaField` URNs keep the spelling the MySQL source reports. Because the match is made on the sqlglot dialect class, this also applies to StarRocks, Doris and SingleStore, whose dialects derive from MySQL. **Action:** re-run affected ingestion to emit corrected lineage. Previously emitted lowercase dataset and schemaField URNs are not cleaned up automatically and may linger as stale, metadata-less entities; soft-delete them if you want them gone.
