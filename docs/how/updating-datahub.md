...
- #18116 **(GMS / system-update)** The `restore-indices` steps for dbt siblings, column lineage, form info, and glossary were re-implemented on the shared `AbstractMCLStep` base and given new step IDs (e.g. `dataset-upstream-lineage-v1`, `glossary-term-info-v1`). Because completion is now tracked under these new IDs, deployments that already ran the old steps and still have them **enabled** will re-run each affected reindex **once** after upgrading, then skip on subsequent restarts. The reindex only regenerates search/graph documents (idempotent, no primary-data changes). **No action required;** to avoid the one-time re-run, disable the steps you don't need (e.g. `RESTORE_COLUMN_LINEAGE_INDICES_ENABLED=false`).
