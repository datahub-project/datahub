...
- #18175 **(Spark lineage / OpenLineage endpoint)** Several fixes to the shared OpenLineage→DataHub converter (used by both the Spark agent and the `/openapi/openlineage/api/v1/lineage` REST endpoint). **New metadata now captured:** the standard `TagsJobFacet` and `OwnershipJobFacet` are emitted on the DataJob (#14458 — previously tags were read only from the Airflow run facet and ownership was computed but never emitted); nested schema struct fields are now emitted (as dotted `fieldPath`s, or v2 `[version=2.0]…` fieldPaths when the schema contains `array`/`union` types, matching DataHub's convention); uppercase OpenLineage column types (e.g. `STRING`, `INT` from Trino/JDBC producers) now resolve to the correct DataHub type instead of `NullType`; and `ErrorMessageRunFacet` diagnostics (message / programmingLanguage / stackTrace) are captured as `DataProcessInstance` custom properties on failed runs. **Behavior changes:** (1) the job's `DocumentationJobFacet` description is now written only to the DataJob, not also to the parent DataFlow — a job-level description does not describe the flow, and multi-job flows previously overwrote each other. For Airflow, the DataFlow (DAG) description is instead sourced from the DAG's own description in the Airflow run facet (`run.facets.airflow.dag.description`); non-Airflow producers carry no flow-level description, so their DataFlow entities no longer receive one from a (mis-placed) job facet. (2) a `RunEvent` with `eventType=OTHER` (or no `eventType`) no longer emits a `dataProcessInstanceRunEvent` — it previously produced a `RunResultType.$UNKNOWN` aspect that failed validation and was silently dropped. **Known limitation:** schema extraction is opt-in in the Spark agent (both `spark.datahub.metadata.dataset.materialize` and `spark.datahub.metadata.dataset.include_schema_metadata` must be enabled — off by default), so default users are unaffected by the schema changes. For users who do enable it, a dataset that contains an `array` or `union` column now emits its schema with v2 (`[version=2.0]…`) `fieldPath`s while column-level lineage still references flat field paths, so column-level lineage on those specific datasets may not line up with the schema until CLL is migrated to v2 `fieldPath`s (planned follow-up); flat and struct-only schemas are unaffected. **Action:** none required for most users — the new metadata is additive and recipes need no changes; only note that if you relied on the OpenLineage job description appearing on the parent DataFlow entity, it now appears only on the DataJob.
