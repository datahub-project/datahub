...
- **(CLI / `datahub init --sso`)** SSO login drives the browser the operating system defaults to for `https` instead of a bundled Chromium, and reuses a per-instance profile under `~/.datahub/sso-browser-profiles` (0700), so a still-valid identity provider session skips the login form on later runs. Login now confirms who signed in with the instance rather than trusting the `actor` cookie. Three new flags, each rejected without `--sso`: `--fresh-login` discards the saved profiles and session to sign in as somebody else; `--seed-profile DIR` copies an existing browser profile in, but only while the CLI's own profile directory is still empty — anything an earlier attempt left behind skips the seed until `--fresh-login` clears it; `--remember-session`, **on by default**, stores the login cookies in `~/.datahub/sso-sessions` (directory 0700, files 0600) and replays them, which is what carries a session cookie no browser writes to disk; the next login is then tried headlessly first and falls back to a visible browser if that does not authenticate. **Action:** none to keep the repeated login away. Note that `--sso` now writes identity provider session cookies under `~/.datahub`; pass `--no-remember-session` to keep them in the browser profile only, at the cost of signing in whenever the provider's session does not survive a browser restart.
