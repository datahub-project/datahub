...
- **(Ingestion / Fabric Data Factory)** Copy activities now emit column-level lineage (`fineGrainedLineages` on the activity's `dataJobInputOutput`), on by default through the new `include_column_lineage` option. Explicit translator mappings are always used; default by-name mappings are emitted only when both source and destination columns are known (from the activity's imported schema or from `schemaMetadata` already in DataHub), except for `autoCreate` destinations. Setting `include_column_lineage: true` explicitly while `include_lineage: false` is now a config error. Three Copy dataset-resolution fixes also change emitted lineage URNs: Fabric Warehouse sinks and sources whose pipeline JSON uses the `DataWarehouse` linked-service type now resolve to the `fabric-onelake` URN instead of a `DataWarehouse`-platform URN; OneLake items whose pipeline definition carries the all-zero `00000000-0000-0000-0000-000000000000` workspace ID placeholder now resolve to the pipeline's workspace instead of an all-zero workspace (Fabric saves such definitions, although the activities fail at runtime until a real workspace is set); and Salesforce (including `SalesforceV2` / `SalesforceServiceCloudV2`) sources now resolve to the salesforce connector's URN via `objectApiName`. **Action:** none. The next run replaces the affected `dataJobInputOutput` edges; the old URNs were never emitted by another connector. Set `include_column_lineage: false` to keep dataset-level lineage only.
- **(Ingestion / Azure Data Factory)** Salesforce-family datasets (`SalesforceObject`, `SalesforceV2Object`, `SalesforceServiceCloud(V2)Object`) now resolve to `urn:li:dataset:(urn:li:dataPlatform:salesforce,<objectApiName>,<env>)`, matching the salesforce connector, instead of using the ADF dataset name; `SalesforceV2` and `SalesforceServiceCloudV2` linked services are now mapped to the `salesforce` platform. Copy activity column lineage now also parses the legacy string form of `columnMappings` and hierarchical `path` mappings, and no longer infers by-name (identity) column lineage when a translator has explicit mappings that are not name-based (e.g. ordinal-only). **Action:** none. Lineage edges to the previous ADF-dataset-name Salesforce URNs are replaced on the next run.
