...
- **(GMS / Ingestion execute)** `ApiOperation.EXECUTE` is **ingestion-source-only** across GraphQL, OpenAPI, and Rest.li (via `PoliciesConfig.API_ENTITY_PRIVILEGE_MAP`). Default entity and structured-property EXECUTE mappings were removed. `MANAGE_INGESTION` authorizes full CRUD + execute + exists on ingestion sources; `EXECUTE_ENTITY` authorizes execute, exists, and rollback — **not** recipe READ. GraphQL run/cancel/rollback and Rest.li `runs/rollback` use the same EXECUTE check as `ExecuteIngestionAuthValidator`. Test-connection still requires `MANAGE_INGESTION` (client-supplied recipe). Reading an ingestion source (including recipe) allows `MANAGE_INGESTION`, Edit, or standard entity read privileges. Admin all-resource policies may still include `EXECUTE_ENTITY`; it only takes effect for ingestion source URNs. **Action:** none for admins with `MANAGE_INGESTION`. Callers that relied on EXECUTE against non-ingestion entity types will be denied.
