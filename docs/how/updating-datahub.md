...
- #19099 **(Ingestion / sql-queries)** A run where nothing at all succeeds — every input line unparseable, or every query failing — now fails and produces no metadata, instead of completing successfully with nothing to show for it. Emitting after a total failure was silently replacing complete lineage in DataHub with partial lineage, so the run now stops instead. Individual bad rows are still skipped and counted as before. The `enable_lazy_schema_loading` option has also been removed (it was never read), and two report fields were renamed: `num_queries_processed_sequential` is now `num_queries_processed`, and `num_temp_tables_detected` is now `num_temp_table_matches`. **Action:** remove `enable_lazy_schema_loading` from your recipe.
