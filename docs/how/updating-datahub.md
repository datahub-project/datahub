...
- **(Sigma ingestion)** Sigma ended support for datasets as a data source on 2026-09-15, and a dataset-backed workbook element no longer returns SQL. The connector now recovers a Sigma Dataset's warehouse table from Sigma's connection metadata instead of from that SQL, which means the emitted warehouse URN takes its `env` and `platform_instance` from `connection_to_platform_map` rather than from `chart_sources_platform_mapping`. **Action:** if you set `env` or `platform_instance` under `chart_sources_platform_mapping`, copy those values into `connection_to_platform_map` for the relevant Sigma `connectionId`. Otherwise these lineage edges will point at URNs built from the recipe's own `env` with no platform instance, which may not match what your warehouse connector emitted. The connector logs a warning when it detects this. Recipes that do not set either field are unaffected.
