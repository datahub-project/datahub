...
- #18727 **(Ingestion / Airbyte)** Stream namespaces are now recovered for connections read through Airbyte's Public API, which omits them from the connection payload. Previously every stream in such a connection fell back to the source connector's default schema, so datasets for streams in other schemas pointed at the wrong table. DataHub now reads the namespace from Airbyte's `/streams` endpoint (Airbyte 1.8+) and only uses it when it is unambiguous; where several schemas expose a same-named stream and Airbyte does not say which is which, the namespace is left alone and a warning names the candidates. **Action:** none required, but affected datasets change URN — they move from `<database>.<default_schema>.<table>` to `<database>.<actual_schema>.<table>`. Expect new dataset entities on the next run, and soft-delete the stale ones (or enable stateful ingestion so they are marked removed automatically). On Airbyte below 1.8 behaviour is unchanged, and the run now warns that stream metadata is unavailable.
