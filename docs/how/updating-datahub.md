...
- **(GMS / analytics)** Optional PostgreSQL analytics store (`postgres.pgAnalytics` / `DATAHUB_PGANALYTICS_*`) holds product usage events, api_usage flush rollups, and system inventory gauges with progressive hour→day→month compaction. Enable with `DATAHUB_PGANALYTICS_ENABLED=true` and SqlSetup. Switch product usage-events SoT with `DATAHUB_USAGE_EVENTS_IMPLEMENTATION=postgres` (requires pgAnalytics enabled). Compaction is driven by SYSTEM source `datahub-analytics-compaction` (default hourly) calling `POST /openapi/operations/analytics/compact` — not an in-process GMS cron. The `datahub-actions` image bundles a venv for that source. Docker Compose Postgres profiles default to exclusive pgAnalytics SoT and enable api_usage flush + entity-count sinks. Switching an existing deployment to postgres SoT does **not** backfill Elasticsearch history into Postgres — keep `DATAHUB_USAGE_EVENTS_IMPLEMENTATION=elasticsearch` if you need prior chart history, or accept a new Postgres timeline. See [pgAnalytics design](../pganalytics-design.md).
