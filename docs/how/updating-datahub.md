...
- **(GMS / View Authorization)** When `VIEW_AUTHORIZATION_ENABLED=true`, entity types are now **restricted by default**. The previous `VIEW_RESTRICTED_ENTITY_TYPES` allowlist is replaced by `VIEW_UNRESTRICTED_ENTITY_TYPES` (plus `_ADD` / `_REMOVE`), which lists types that bypass view checks. The default unrestricted list is **backwards-compatible**: every entity in `entity-registry.yml` that was not in the former restricted allowlist (so types that previously fail-opened — including `schemaField` and `document` — stay unrestricted until you trim). The former restricted types (`dataset`, `dashboard`, `chart`, ML entities, pipelines, glossary, `domain`, `dataProduct`, `notebook`, `dataProcessInstance`, `dataHubView`, `query`, `tag`) remain restricted. **Action:** To stop those fail-open types from bypassing view checks, set `VIEW_UNRESTRICTED_ENTITY_TYPES_REMOVE=schemaField,document` (or replace the full list with `VIEW_UNRESTRICTED_ENTITY_TYPES`). This applies whenever view authorization is enabled — on DataHub Cloud with [Search Access Controls](../features/feature-guides/search-access-controls.md) (query-time search filtering, Cloud-only) and on self-hosted OSS (entity-page gating; OSS does **not** get Cloud search pushdown).
...
- `VIEW_UNRESTRICTED_ENTITY_TYPES` / `_ADD` / `_REMOVE` — Replaces `VIEW_RESTRICTED_ENTITY_TYPES`. Lists entity types that bypass view checks when `VIEW_AUTHORIZATION_ENABLED=true` (shared GMS config for Cloud Search Access Controls and OSS entity-page VBAC). Query-time search filtering remains DataHub Cloud–only. See Breaking Changes above and [Environment Variables](../deploy/environment-vars.md).
