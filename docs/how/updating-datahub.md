...
- [#19430](https://github.com/datahub-project/datahub/pull/19430) **(Ingestion / BigQuery)** `enable_legacy_sharded_table_support: false` now takes effect reliably. The flag is applied by overwriting process-wide state on `BigqueryTableIdentifier`, but only the `true` branch ever assigned it, so in a process that had already constructed a BigQuery source with the flag enabled (the default) — for example an executor running several recipes — a later source setting it to `false` was silently ignored and kept collapsing sharded tables to the base name. **Action:** if you set `enable_legacy_sharded_table_support: false` and saw it apply inconsistently, it will now apply on every run; expect sharded tables to move to their `_yyyymmdd` URNs on the runs where it previously failed to take effect.
...
- [#19430](https://github.com/datahub-project/datahub/pull/19430) **(Ingestion / BigQuery)** Date-suffixed tables that sit beside an un-suffixed table of the same name — `my_table` next to `my_table_20251120` and `my_table_20251121` — are no longer treated as shards of `my_table`. A genuine sharded set has no un-suffixed member, so when one exists the dated tables are separate physical tables (a dated copy, a backup) rather than shards. Previously all of them collapsed onto the base table's URN, and whichever happened to be written last overwrote the real table's schema and profile, so row counts and column counts could be wildly wrong and could change between runs with no other change. With the default `enable_legacy_sharded_table_support: true` the dated tables are now skipped, reported as a `Date-suffixed tables skipped` warning naming the base table, and counted one per table in the new `num_sharded_tables_shadowed_by_base_table` report field; with `enable_legacy_sharded_table_support: false` they keep a distinct `_yyyymmdd` URN and are ingested as before. Genuine sharded sets are unaffected either way — each set still collapses to one dataset built from its newest shard, at `proj.ds.foo` with the default and at `proj.ds.foo_yyyymmdd` when the flag is disabled, exactly as each does today. **Action:** none — the affected base tables get correct stats on the next run. Rename the dated copies, or set `enable_legacy_sharded_table_support: false`, if you want them ingested as their own datasets.
