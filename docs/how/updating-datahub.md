...
- #20017 **(Sigma ingestion)** Sigma chart columns gain column lineage in three cases that previously pointed to themselves: a column computed only from other columns of the same chart and parameters (for example `Sum([Amount])`) now inherits their upstreams; a reference through a Data Model join (`[Join Element/Joined Table/Column]`) now resolves to the joined table's column when every named table is one the join element's lineage lists as a source, so relationship references stay unresolved; and a reference to a Data Model element the workbook loads, but that Sigma does not list as the chart's upstream, resolves when exactly one element of that name has the column and the chart has no dataset or warehouse source of that name. **Action:** none. Expect column-lineage counts on Sigma charts to rise on the next run.

- #20000 **(Sigma ingestion)** A chart column's formula reference to another chart or a Data Model element now becomes column lineage only when that upstream has the referenced column; references to upstreams whose columns are unknown are unchanged. References to columns the upstream does not have no longer produce lineage edges, and a three-part reference such as `[Element/Relationship/Column]` resolves only when the upstream's known columns include the rest as one column name (for example a column named `Rev/Cost`); a column whose formula has no other resolvable reference points to itself instead. Element names now match case-insensitively, as Sigma does, so some references that were skipped now resolve, including to warehouse tables. The `chart_input_fields_case_mismatch` report counter now counts only references that match several names differing only in case. **Action:** none. Expect column-lineage counts on Sigma charts to change on the next run, mostly dropping as incorrect edges are removed.
