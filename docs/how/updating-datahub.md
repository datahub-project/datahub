...
- **(OpenLineage / Spark lineage)** New opt-in mapping of Microsoft Fabric OneLake table paths (`abfss://<ws>@onelake.dfs.fabric.microsoft.com/<item>/Tables/[<schema>/]<table>`) to `fabric-onelake` dataset URNs (`<workspaceGUID>.<itemGUID>.<schema>.<table>`), so runtime lineage from Fabric Spark attaches to the tables the Fabric OneLake source ingests. Enable it with `spark.datahub.metadata.dataset.fabricOneLake.enabled=true` (Spark agent) or per request on the GMS OpenLineage endpoint with `?fabricOneLake=true` / header `X-DataHub-Fabric-OneLake: true`; related options are `convertUrnsToLowercase` / `platformInstance` / `itemIds`, plus `fabricNotebookFlowNames` for one DataFlow per Fabric notebook. It is **off by default, so existing URNs are unchanged**. **Action:** none unless you opt in (per producer on the endpoint, so producers that don't pass the option keep their URNs). Enabling it re-keys OneLake table lineage: tables previously captured as `abs` path datasets, or as `hive.<lakehouse>.<table>` through the Spark catalog symlink, move to `fabric-onelake` URNs, and when enabled the mapping also takes precedence over `path_spec_list` for OneLake `Tables/` paths. The old `abs` / `hive` entities and their lineage are not migrated or removed; soft-delete them individually (`datahub delete --urn "<old urn>"`) if you no longer need them; don't delete by `--platform abs`, which would also remove non-OneLake ADLS datasets. Set `platformInstance` / `convertUrnsToLowercase` to match the Fabric OneLake source recipe; the global dataset platform instance is not applied to `fabric-onelake` URNs. OpenLineage schema facets are not written as `schemaMetadata` on `fabric-onelake` datasets (the Fabric OneLake source owns their schema).
