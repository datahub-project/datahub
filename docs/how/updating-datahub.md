...
- #19440 **(Ingestion / Cube)** Cube views are now ingested with subtype `Semantic Model` instead of `View`, matching dbt semantic-layer models. Cube entities themselves are unchanged (`Cube`). Dataset URNs are unchanged. This is a dataset subtype only — Cube is not yet emitted as first-class `semanticModel` / `metric` entities. **Action:** re-ingest Cube to update the subtype. Saved searches or filters that look for Cube assets with type `View` should use `Semantic Model`.
