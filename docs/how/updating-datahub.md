...
- #18690 **(Ingestion / SQL Profiling)** The SQLAlchemy profiler now runs MySQL and Postgres profiling under `AUTOCOMMIT` instead of holding one implicit transaction open across every table's profiling queries. This avoids the long-lived transaction that pinned InnoDB read views (MySQL) and held Postgres idle-in-transaction, blocking VACUUM. Profile **values can now differ** for concurrently-written tables — `min`, `max`, `COUNT(*)`, `uniqueCount`, quantiles, histograms, and sample values each come from a different snapshot, so a single profile may be internally inconsistent (e.g. `uniqueCount` > `rowCount`). To turn AUTOCOMMIT OFF and restore the prior per-table transactional behavior, set `profiling.profiling_isolation_level: TRANSACTIONAL` on the source (per-source), or set the `DATAHUB_PROFILING_ISOLATION_LEVEL=TRANSACTIONAL` environment variable on the ingestion executor for a fleet-wide kill switch (the recipe field takes precedence over the env var; the env var is global and affects every SQL source whose profiler runs, including platforms whose adapter does not opt in — a non-`TRANSACTIONAL` env value on such a platform emits a warning). Reverting restores the pre-change profile output with no migration, state, or cache to clean up. An invalid level fails the run loudly (every table fails at `execution_options` before the first result returns) rather than being swallowed per-table into a warning; the error message names the input that supplied the level (recipe field or env var).
