...
- **(Ingestion framework)** Oversized query metadata no longer fails ingestion runs. Two fixes work together: (1) the SQL parsing aggregator now caps the text of synthetic "composite" queries (built by merging a temp-table chain into one query) at 5MB, configurable via `DATAHUB_MAX_COMPOSITE_QUERY_STATEMENT_CHARS`; and (2) the `EnsureAspectSizeProcessor` size guard for `queryProperties` was fixed to measure serialized (JSON) size correctly instead of comparing a raw character count against a byte-based target — the previous unit mismatch caused the guard to give up and emit oversized aspects, which GMS then rejected with a `400 Cannot parse request entity`, failing the run after enough rejections. This most often affected Snowflake sources with `include_queries` enabled against accounts with heavy temp-table query flows (e.g. Hightouch), but applies to any SQL connector that emits query entities. **No action required;** oversized statements are now truncated (with a warning and a report counter) rather than failing the run.
