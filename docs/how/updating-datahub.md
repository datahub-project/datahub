...
- #12825 **(Ingestion / BigQuery)** Profiling now skips tables that haven't been modified within the last year by default. The new `profiling.skip_stale_tables` flag defaults to `true` with `profiling.staleness_threshold_days: 365`, so after upgrading, long-idle tables (judged by BigQuery's `last_modified_time`) are no longer profiled. Skipped tables are now surfaced in the ingestion report under the warning "Profiling skipped for stale table" (previously this was only logged). **Action:** set `profiling.skip_stale_tables: false` to profile tables regardless of age, or raise/lower `profiling.staleness_threshold_days`.
...
- #12825 **(Ingestion / BigQuery)** Profiling of partitioned tables now restricts the scan to a recent window by default. `profiling.partition_datetime_window_days` defaults to `30`, so only partitions within 30 days of the selected partition date are profiled. **Action:** set `profiling.partition_datetime_window_days: null` to disable windowing and profile the full selected partition, or raise/lower the value.
