...
- **(Ingestion / MySQL family profiling defaults)** `profiling.profile_table_row_limit` and `profiling.profile_table_size_limit` stay `null` on MySQL, MariaDB, Doris, and TiDB (rather than inheriting the shared 5M-row / 5GB defaults) — the shared defaults are tuned to Snowflake/BigQuery billing, not InnoDB, and silently dropping profiles would quietly break the Volume/Column Metric assertions built on them; set them explicitly to guardrail large tables. **Action for MySQL, MariaDB, Doris, and TiDB users:** these two limit fields were previously accepted by the config but never enforced (MySQL raised `NotImplementedError` in `generate_profile_candidates`); on upgrade they start being enforced, so any value already set on a recipe begins filtering profiles — re-check it before upgrading. (For Doris and TiDB specifically, enforcement is against `information_schema.tables` statistics whose semantics those engines do not share with InnoDB, so re-check with extra care.) MySQL and MariaDB now emit a post-run `report.info` entry naming the few tables that took the longest to profile, with the limits to set — a discoverability mechanism for the opt-in guardrails. MySQL deployments seeing high memory use during profiling can lower `profiling.max_workers` (5 is a reasonable starting point): concurrent full scans on a single-primary row store multiply peak memory rather than increasing throughput. This also caps the profiling engine's `options.max_overflow` unless `options.max_overflow` is set explicitly. (MariaDB inherits these defaults as a MySQL fork; Doris and TiDB inherit the limit fields but keep `report_expensive_tables` off, since its advice targets single-primary row stores.)
