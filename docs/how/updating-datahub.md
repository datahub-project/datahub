...
- **(Ingestion / MySQL family profiling defaults)** MySQL, MariaDB, Doris, and TiDB profiling defaults changed: `profiling.max_workers` drops from `5 * cpu_count` (~40 on an 8-core box) to `5` on MySQL and MariaDB. MySQL is a single-primary row store — extra concurrency adds contention and multiplies peak memory (each worker may hold multi-GB `COUNT(DISTINCT)` structures) rather than increasing throughput, and the ~40 concurrent full-scanning connections were a leading OOM suspect. `5` is a deliberately conservative starting value; tune per workload. This `max_workers` also caps the profiling engine's `max_overflow` (set `options.max_overflow` explicitly to decouple the two). `profiling.profile_table_row_limit` and `profiling.profile_table_size_limit` stay `null` on all four platforms (rather than inheriting the shared 5M-row / 5GB defaults) — the shared defaults are tuned to Snowflake/BigQuery billing, not InnoDB, and silently dropping profiles would quietly break the Volume/Column Metric assertions built on them; set them explicitly to guardrail large tables. **Action for Doris/TiDB users:** these two limit fields were previously accepted by the config but never enforced (MySQL raised `NotImplementedError` in `generate_profile_candidates`); on upgrade they start being enforced, against `information_schema.tables` statistics whose semantics those engines do not share with InnoDB. Re-check any value you have set on a Doris or TiDB recipe before upgrading. Finally, MySQL and MariaDB now emit a post-run `report.info` entry naming the few tables that took the longest to profile, with the limits to set — a discoverability mechanism for the opt-in guardrails. (MariaDB inherits these defaults as a MySQL fork; Doris and TiDB inherit the limit fields but revert `max_workers` and `report_expensive_tables` to the shared defaults.)
