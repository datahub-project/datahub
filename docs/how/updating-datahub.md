...
- **(GMS / GraphQL, View Authorization)** When view-based access control is enabled (`VIEW_AUTHORIZATION_ENABLED=true`), the GraphQL `lineage` field now returns neighbors the caller lacks **View Entity Page** on as `Restricted` entities with encrypted URNs, as the documentation has described since the feature shipped, instead of as fully typed entities. Lineage requested on an entity the caller may not view returns an empty result. The same visibility check now also applies to `parentDomains`, `parentNodes` (glossary) and `parentDataProducts`, which hide parents the caller may not view. Deployments with view authorization disabled (the default) are unaffected. **Action:** none unless a client walking the lineage graph as a restricted user assumes every neighbor is a concrete entity; handle the `Restricted` type as the UI already does.
