...
- **(Ingestion / Snowflake, Unity Catalog, Glue)** S3 dataset URNs generated as **lineage upstreams** now escape URN-reserved characters (`,`, `(`, `)`) in object keys, matching what the S3 source itself emits for the same object. Previously a key like `reports/report (1).csv` produced `...,s3,my-bucket/reports/report (1).csv,PROD)` and a key containing a comma produced a malformed URN. **Action:** Only affects buckets whose object keys contain those characters. Lineage edges pointing at the old unescaped URNs will no longer resolve and will need to be re-ingested; the newly emitted URNs now correctly join the datasets produced by the S3 source. Datasets emitted by the S3 source itself are unchanged, as are Glue/Athena/SageMaker _table_ URNs (only the lineage-upstream helper changed).
