...
- [#19521](https://github.com/datahub-project/datahub/pull/19521) **(Actions / Glossary term propagation)** The OSS `term_propagation` action now propagates glossary terms applied at the **column** level to downstream datasets. Previously a column-level application resolved lineage against the field's `schemaField` URN, matched no dataset-level lineage, and silently propagated nothing (no log line, empty `failed_events.log`). **Term removals now propagate as well**: removing a propagated term from the source removes it from downstream datasets, whereas before removals were a no-op and downstream terms accumulated and went stale. The action is now shipped in the actions image as a **default-off** pipeline, enabled with `DATAHUB_ACTIONS_TERM_PROPAGATION_ENABLED=true`; deployments that do not enable it are unaffected. **Action:** none by default. If you already run `term_propagation`, expect column-level applications and removals to now take effect on downstream datasets. A removal is only propagated once the source no longer carries the term on any field or at the dataset level; when it is propagated it clears the term from every downstream dataset regardless of whether that dataset was also given the term directly.
