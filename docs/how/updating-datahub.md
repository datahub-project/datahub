...
- #19815 **(Sigma ingestion)** Sigma ended support for datasets as a data source on 2026-09-15, and a dataset-backed workbook element no longer returns SQL. The connector now recovers a Sigma Dataset's warehouse table from Sigma's connection metadata instead of from that SQL, so the emitted warehouse URN takes its `env` and `platform_instance` from `connection_to_platform_map` rather than from `chart_sources_platform_mapping`. **Action:** if you rely on `chart_sources_platform_mapping` for these edges, add the matching Sigma `connectionId` to `connection_to_platform_map` with the same `env` and `platform_instance`. This includes recipes that never set `env` on the mapping: `chart_sources_platform_mapping` defaults it to `PROD`, so a recipe whose own `env` differs will now emit URNs under that env instead. Without an override the edge is built from the recipe's `env` with no platform instance, which may not match what your warehouse connector emitted. The connector warns when it detects a mapping whose env or platform instance no longer applies. **Identifier casing also changes on platforms other than Snowflake.** The SQL route lower-cased warehouse names for every platform except BigQuery and DB2; this route lower-cases only the platforms DataHub itself lower-cases (Snowflake), so a Postgres table Sigma reports as `Analytics.Public.Orders` is now emitted with that casing rather than `analytics.public.orders`. To get the old spelling back, set `convert_urns_to_lowercase: true` explicitly on that `connection_to_platform_map` entry — the flag is honoured on any platform when set, and applies to both table and column identifiers. Note that adding a `connection_to_platform_map` entry at all makes its `env` authoritative, and `env` defaults to `PROD`: if your recipe runs in another env, set `env` on the entry too or the URNs move to `PROD`.
