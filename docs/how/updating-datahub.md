...
- **(GMS / timeseries)** Timeseries source of truth is selected with `timeseriesAspectService.implementation` (`elasticsearch`, `opensearch`, or `postgres`) and `TIMESERIES_ASPECT_SERVICE_IMPLEMENTATION`. Dual-write to Elasticsearch and PostgreSQL is not supported. Switching `implementation` does not copy existing Elasticsearch timeseries history into PostgreSQL; backfill or re-ingest into PostgreSQL before flipping the source of truth, or historical points remain only in Elasticsearch. For PostgreSQL, set **both** `implementation: postgres` and `postgres.pgTimeseries.enabled: true` with SqlSetup. GMS and MAE fail fast at startup if `DATAHUB_PGTIMESERIES_ENABLED=true` without `TIMESERIES_ASPECT_SERVICE_IMPLEMENTATION=postgres`, or if `implementation=postgres` without `enabled`. SqlSetup can still create tables from `enabled` alone. Flat `DATAHUB_PGTIMESERIES_*` env vars configure the **default** store (unchanged single-table behavior). Optional multi-store / aspect routing uses `DATAHUB_PGTIMESERIES_CONFIG_FILE` (Spring resource URI, e.g. mounted ConfigMap) for `stores` + `routing`; SqlSetup migrates every store’s JDBC URL. Partman interval/premake are sticky after first register unless `DATAHUB_PGTIMESERIES_PARTITIONING_FORCE_OVERWRITE=true`. `DATAHUB_PGTIMESERIES_RETENTION_MAX_AGE_SECONDS=0` clears default-store partman retention (stops partition drops). Docker Compose Postgres profiles (`quickstart-postgres`, `debug-postgres`, …) default to exclusive pgTimeseries SoT (`DATAHUB_PGTIMESERIES_ENABLED=true`, `TIMESERIES_ASPECT_SERVICE_IMPLEMENTATION=postgres`). See [pgTimeseries design](../pgtimeseries-design.md).
