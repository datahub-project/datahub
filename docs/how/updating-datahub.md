...
- #17034 **OpenLineage REST:** `POST /openapi/openlineage/api/v1/lineage` now expects an OpenLineage event object (`RunEvent`, `JobEvent`, or `DatasetEvent`) with required typed envelope fields. OpenLineage 2-0-2 events include `eventTime`, `producer`, and `schemaURL`; legacy compatibility events may omit `schemaURL`. Generated clients should be regenerated from the updated OpenAPI contract if they previously sent a JSON-encoded string body. The endpoint returns `415 Unsupported Media Type` for non-JSON requests. Timezone-less timestamps use the GMS host's default time zone, so configure hosts consistently or include an explicit offset. Successful requests return `202 Accepted` after submitting the event's proposals through the standard asynchronous ingestion path; downstream aspect application remains eventually consistent. Requests now pass standard REST ingest authorization and return `403 Forbidden` when the caller lacks create or edit permission for any mapped entity. Ensure OpenLineage service-account tokens have the required entity privileges. Producer identifiers and custom facet objects remain compatible without request-time schema resolution and are not authorization identities or producer allow-lists. OpenLineage job documentation, ownership, and standard job/run tags now write to DataJob rather than the inferred DataFlow. Existing DataFlow aspects from earlier ingestion are not deleted automatically.
