...
- #19473 **(Ingestion / dbt)** Two dbt entities that would end up sharing one identity are now reported as a failure, and none of them are ingested. This applies to every dbt recipe, dbt Core and dbt Cloud alike, not only to the new multi-project `manifest_path` glob. Two cases are covered: models (or seeds, snapshots, semantic models) that resolve to the same table in the warehouse, and nodes or exposures that share a dbt `unique_id`. Previously whichever one was loaded last silently won and the other's metadata was discarded with no report entry. A single dbt project cannot normally hit this, because dbt enforces uniqueness within a project — but it can happen where the collision only appears after DataHub builds the identity, most notably with `convert_urns_to_lowercase: true`, which folds two relations differing only in letter case into one, or `include_database_name: false`, which folds relations that differ only by database into one. Note that any reported failure also stops stale-entity soft-deletion for the whole run, which is the safe direction — nothing is wrongly deleted — but does mean a persistent collision will keep soft-deletion from running. **Action:** none if your run has no collisions. If a new failure named "Duplicate model names across dbt projects" or "Duplicate dbt unique_id across projects" appears, fix the dbt projects so each name resolves to one thing. To keep the previous behaviour instead, set `fail_on_cross_project_collisions: false`, which keeps one entity deterministically and reports a warning rather than a failure.
