...
- **(GMS / graph)** Optional PostgreSQL graph store (`postgres.pgGraph` / `DATAHUB_PGGRAPH_*`) can replace Elasticsearch or Neo4j as the exclusive `GraphService` backend. Dual-write is not supported. For PostgreSQL, set **both** `GRAPH_SERVICE_IMPL=postgres` and `DATAHUB_PGGRAPH_ENABLED=true` with SqlSetup (PostGIS + pgRouting on the DataHub Postgres image). GMS, MAE, and MCE fail fast at startup if only one is set. SqlSetup can still create tables from `enabled` alone. Switching the source of truth does **not** copy existing Elasticsearch or Neo4j edges into Postgres; wipe and rebuild, re-ingest, or keep Elasticsearch/Neo4j as SoT. Docker Compose Postgres profiles default to exclusive pgGraph SoT; search remains on Elasticsearch/OpenSearch. `getImpactLineage` uses typed `pgr_breadthFirstSearch` (first destination wins) and reconstructs paths with via/lifecycleOwner, emitting via entities as graph-layer `LineageRelationship` rows. UI `getLineage` stays Java BFS and carries via/lifecycle on every hop. Connected-component tables are created with pgGraph but not populated unless `DATAHUB_PGGRAPH_CONNECTED_COMPONENTS_ENABLED=true` (default `false`). See [pgGraph design](../pggraph-design.md).
...
- **(GMS / analytics compaction)** Progressive analytics compaction runs via SYSTEM source `datahub-analytics-compaction` (default hourly cron) calling `POST /openapi/operations/analytics/compact`. Work budgets use `DATAHUB_ANALYTICS_COMPACT_*`. Requires `datahub-actions` (bundled venv) + GMS with pgAnalytics enabled; soft-skips when the backend is unavailable. See [pgAnalytics design](../pganalytics-design.md).
...
- **(GMS / analytics)** Optional PostgreSQL analytics store (`postgres.pgAnalytics` / `DATAHUB_PGANALYTICS_*`) holds product usage events, api_usage flush rollups, and system inventory gauges with progressive hour→day→month compaction. Dual-write of product usage events to Elasticsearch and PostgreSQL is not supported. For PostgreSQL, set **both** `DATAHUB_USAGE_EVENTS_IMPLEMENTATION=postgres` and `DATAHUB_PGANALYTICS_ENABLED=true` with SqlSetup. GMS, MAE, and MCE fail fast at startup if only one is set. SqlSetup can still create tables from `enabled` alone. Switching the source of truth does **not** backfill Elasticsearch history into Postgres; keep search as SoT with **both** flags off/`elasticsearch`, or accept a new Postgres timeline. Compaction is driven by SYSTEM source `datahub-analytics-compaction` (default hourly) calling `POST /openapi/operations/analytics/compact` — not an in-process GMS cron. The `datahub-actions` image bundles a venv for that source. Docker Compose Postgres profiles default to exclusive pgAnalytics SoT and enable api_usage flush + entity-count sinks. See [pgAnalytics design](../pganalytics-design.md).
- **(GMS / system metadata)** Optional PostgreSQL system-metadata store (`postgres.pgSystemMetadata` / `DATAHUB_PGSYSTEMMETADATA_*`) holds ingestion-run / aspect catalog documents (the `system_metadata_service_v1` table, matching the Elasticsearch index name). Dual-write to Elasticsearch and PostgreSQL is not supported. For PostgreSQL, set **both** `SYSTEM_METADATA_SERVICE_IMPLEMENTATION=postgres` and `DATAHUB_PGSYSTEMMETADATA_ENABLED=true` with SqlSetup. GMS, MAE, and MCE fail fast at startup if only one is set. SqlSetup can still create tables from `enabled` alone. Switching the source of truth does **not** backfill Elasticsearch history into Postgres. An existing Postgres+Elasticsearch volume that already has system-metadata in Elasticsearch is **not** copied into `system_metadata_service_v1`; wipe and rebuild, re-ingest, or keep Elasticsearch as SoT. `getTaskStatus` is unsupported on Postgres (always empty). Consistency checks use a store-agnostic scroll client. Docker Compose Postgres profiles default to exclusive pgSystemMetadata SoT. See [pgSystemMetadata design](../pgsystemmetadata-design.md).
