...
- #19835 **(Ingestion / MSSQL dataJob lineage)** With `incremental_lineage: true`, the MSSQL source now emits `dataJobInputOutput` as a **patch** rather than a full upsert, so lineage a user added by hand survives re-ingestion. This covers stored procedures and SQL Agent job steps alike; a job step's lineage aspect is always empty, so it is now dropped rather than re-stated as an empty upsert. Two consequences for procedures: stale auto-detected lineage is no longer pruned (a patch only adds), and the patch writes the `inputDatasetEdges` / `inputDatajobEdges` / `outputDatasetEdges` fields while a full upsert writes the plain `inputDatasets` / `inputDatajobs` / `outputDatasets` arrays. The server-side patch template supports only the `*Edges` paths, so entries written into the plain arrays by earlier non-patch runs cannot be removed by the patch and will persist. **Action:** none with the default `incremental_lineage: false`. When enabling it on an existing deployment, expect lineage from earlier runs to remain in the plain arrays; remove it out of band if the duplication matters.
