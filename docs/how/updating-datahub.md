...
- **(GMS / Groups)** #18982: Group membership mutations behave differently in two visible ways. **`removeGroupMembers` now actually removes members of a group that never finished migrating to native membership.** Such members hold only the legacy `groupMembership` aspect; previously the mutation stripped only `nativeGroupMembership`, reported success, and left the user in the group with their access intact. **`addGroupMembers` now rejects the whole request if any supplied user does not exist**, naming the missing URNs, instead of adding the users before the bad one and then failing — which left membership partially applied with no indication of how far it got. **Action:** none required. If you have automation that tolerated the old partial-add behaviour, it will now receive an error and no members will have been added; retry with a corrected user list.
