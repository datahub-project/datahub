...
- **(Metadata Model / Semantic Model)** Containment is now single-direction on the member. `SemanticModelInfo.datasets` and `SemanticModelInfo.metrics` have been removed; membership is expressed only via `MetricInfo.semanticModel` (`ModeledBy`, non-lineage) and `SemanticModelProperties.semanticModel` (`IsPartOf`, non-lineage). Both fields remain searchable (`hasSemanticModel`). Listing a model's members is an ES filter on that field (same pattern as Domains / Tags / Owners). Logical datasets still carry `semanticModelProperties.alias` for join edges. The SemanticModel itself is a bounding-box container in the lineage explorer, not a lineage hop — Metric → SMD lineage continues to use `metricUpstreams.datasetUpstreams`. **Action:** Re-ingest semantic models so membership is written on the member-side aspects; update any custom emitters or GraphQL clients that wrote or read the removed container-side arrays.
