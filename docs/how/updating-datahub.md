...
- **(GMS / graph index)** Graph edge documents now carry an optional `graphWriteVersion` field (aspect `SystemMetadata.version` from the producing MCL). Diff-mode writes use it for conditional upserts when the document exists and for declining stale in-process bulk requeues after a newer write to the same edge. Mapping adds are additive; existing deployments pick up the field on the next graph index mapping update / reindex. Historical ghost edges already present in the index are not auto-healed — use [restore indices](restore-indices.md) with `-a clean` if needed. See [graph write semantics](../what/graph.md#how-graph-edges-are-written-es--opensearch).
