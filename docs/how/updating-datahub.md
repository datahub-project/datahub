...
- **(Ingestion / SQL query parsing)** Temp-aware query fingerprinting collapses per-run ephemeral object names (ELT staging UUIDs, Segment/Snowpark/Great-Expectations temp names, etc.) when computing the query fingerprint, so repeated ETL/sync runs that differ only by an ephemeral table name map to a **single** Query entity instead of a new one per run. This applies both to queries that **read** an ephemeral temp table and to queries whose **output** is a temp table: the per-run variants collapse to one Query entity (carrying only their real upstream subjects — the ephemeral temp itself is not emitted as a downstream dataset). It reuses each connector's **existing `temporary_tables_pattern`** — temp tables are matched by resolved name on the parse path (no separate per-dialect text heuristics), so it is dialect-agnostic and consistent with how those tables are already excluded from lineage. This behavior is **always on** (there is no configuration flag) for **Snowflake** and **BigQuery**, and any other source that parses SQL through the `SqlParsingAggregator` benefits once it supplies temp-table name patterns. This is lineage-safe, but it **changes `query_id` and therefore Query URNs**: on the next ingestion run you get a new set of collapsed Query entities, while the old per-run ones become stale and are reclaimed by `datahub-gc`. **Action:** none required.
