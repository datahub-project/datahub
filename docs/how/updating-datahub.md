...
- (Ingestion) Dataplex connector: Filter configuration structure has been redesigned for clarity and correctness:
  **Removed**: `filter_config.entries.dataset_pattern` - this was incorrectly named since patterns apply to entry IDs, not datasets.
  **Added**: `filter_config.entry_groups.pattern` - filter entry groups by name.
  **Added**: `filter_config.entries.pattern` - filter entries by name.
  **Added**: `filter_config.entries.fqn_pattern` - filter entries by their fully qualified name (FQN) instead of entry name.
  Location configuration changed from single to multiple locations.
  **Removed**: `entries_location: str` - single location configuration
  **Added**: `entries_locations: List[str]` - list of locations to scan
