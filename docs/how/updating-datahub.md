...
- **(Ingestion / Snowflake)** Snowflake Semantic Views (`semantic_views.enabled`) continue to be ingested as **Dataset** entities with subtype "Semantic View" by default — no action required. A new opt-in flag, `semantic_views.emit_semantic_model_entities`, switches semantic views over to a **semanticModel** entity (definition, logical dimensions/facts, and lineage to the underlying tables) plus one **metric** entity per metric column defined on the view. **If you enable this flag:** update any saved links, dashboards, glossary/tag assignments, or automations that reference the old semantic-view dataset URNs to point at the new `urn:li:semanticModel:(...)` and `urn:li:metric:(...)` URNs — with stateful ingestion enabled, the old dataset entities are soft-deleted automatically on the next run via stateful ingestion's generic stale-entity handling (no special-casing for this flag; if stateful ingestion is disabled, the old dataset entities are left behind and must be cleaned up manually). Semantic views no longer appear inside their database/schema container pages; find them via search, lineage, and the metric/semantic model views instead. **Server version requirement:** the `semanticModel`/`metric` entities require a DataHub server (GMS) release with those entity registry registrations — run a server on the same release as this connector change, or the server will reject those aspects. `semantic_views.include_usage` has no effect once this flag is enabled: semanticModel entities do not carry usage statistics (per model-owner decision), so no usage statistics are emitted for semantic views in this mode — usage statistics are only emitted in the default (legacy dataset) mode; `semantic_views.include_queries` is unaffected and keeps emitting query entities, now attributed to the semanticModel. Separately, `semantic_views.include_queries: true` now emits query entities even when `semantic_views.include_usage` is `false` — previously that combination emitted nothing; this applies in both legacy and new modes. **Known limitation:** a dataset or dashboard whose SQL selects `FROM SEMANTIC_VIEW(...)` cannot yet declare the semanticModel as an upstream, since dataset `upstreamLineage` only accepts dataset URNs. **Toggling the flag back off:** if you later disable `emit_semantic_model_entities` after running with it enabled, the tag-derived `StructuredPropertyDefinition`s are re-upserted on the next run WITHOUT the `semanticModel`/`metric` entries in their `entityTypes` (GMS allows this narrowing; it does not hard-fail the write). Any structured-property values already applied to `semanticModel`/`metric` entities remain as-is but are no longer covered by the definition's declared entity types — a known, benign rollback artifact, not a bug. **Manual verification of stale-entity soft-delete:** to confirm the old dataset URNs are actually soft-deleted on the flag flip, run ingestion once with the flag off, then again with it on (both with stateful ingestion enabled), and check that the semantic view's old `urn:li:dataset:(...)` entity has `status.removed=true` (e.g. via `datahub get --urn <urn>` or the GraphQL `dataset(urn) { status { removed } }` query) while the new `urn:li:semanticModel:(...)` entity is active.
