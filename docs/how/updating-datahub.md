...
- **(GMS / schemaField domain & ownership mirroring)** Opt-in MCP side effects can copy a dataset's `domains` / `ownership` onto each of its `schemaField` entities (`MCP_SIDE_EFFECTS_SCHEMA_FIELD_ENABLED` plus `MCP_SIDE_EFFECTS_SCHEMA_FIELD_DOMAIN_ENABLED` / `MCP_SIDE_EFFECTS_SCHEMA_FIELD_OWNERSHIP_ENABLED`). While a sub-flag is on, dataset domain/ownership deletes cascade to the corresponding field aspects. **Changing a sub-flag (or the master flag) does not immediately rewrite historical fields** — ongoing writes use the new flag state, but existing inventory is reconciled by the `schemaFieldsFromSchemaMetadata` system-update step (see **Other Notable Changes**). Until that step runs after a disable, leftover field aspects can still affect DOMAIN / OWNER policy resolution and search. **Note:** toggling flags back to a previously SUCCEEDED fingerprint does not re-run the job unless you use manual reprocess (`SYSTEM_UPDATE_SCHEMA_FIELDS_FROM_SCHEMA_METADATA_REPROCESS=true`) or clear/modify that fingerprint's upgrade result — either is valid.

- **(GMS / schemaField disable cleanup is not mirror-aware)** When `GenerateSchemaFieldsFromSchemaMetadata` runs with a domain or ownership mirror flag **off**, it deletes that aspect from **every** schemaField under each scanned dataset — including aspects operators wrote directly on the field (API/UI), not only copies produced by dataset mirroring. There is no provenance filter. **Action:** Before disabling a mirror flag (or before reprocessing a fingerprint with that flag off), export or be prepared to lose any intentionally field-scoped `domains` / `ownership`. Prefer keeping the matching sub-flag on if you rely on per-column domain/owner metadata that is not inherited from the parent dataset.
...
- **(GMS / schemaField MCP side effects + system update)** Optional nested flags under `metadataChangeProposal.sideEffects.schemaField` can mirror dataset `domains` and `ownership` onto schemaField entities (`MCP_SIDE_EFFECTS_SCHEMA_FIELD_DOMAIN_ENABLED` / `MCP_SIDE_EFFECTS_SCHEMA_FIELD_OWNERSHIP_ENABLED`, both requiring `MCP_SIDE_EFFECTS_SCHEMA_FIELD_ENABLED`). Defaults remain off. Enabling increases write fan-out (one MCP per column per mirrored aspect change) and can change DOMAIN / OWNER policy matches for schemaField URNs once those aspects are present.
  ...
  - **Backfill / cleanup:** The non-blocking SystemUpdate step `GenerateSchemaFieldsFromSchemaMetadata` (`SYSTEM_UPDATE_SCHEMA_FIELDS_FROM_SCHEMA_METADATA_ENABLED`) scans dataset `schemaMetadata`/`status`, builds RESTATE MCLItems stamped with `APP_SOURCE=SYSTEM_UPDATE`, and invokes post-MCP schemaField side effects (then async `ingestProposal`) — it does **not** rely on a no-op parent upsert. Enabling a mirror flag backfills mirrored aspects; disabling deletes field `domains`/`ownership` for the off flag(s). Upgrade id is fingerprinted by the effective domain/ownership mirror flags (`schema-field-from-schema-metadata-v2-d{0|1}-o{0|1}`), so a **first-time** change to a new fingerprint schedules a fresh run on the next SystemUpdateNonBlocking. **That cleanup deletes the aspect on every field of each scanned dataset — including manually written field aspects, not only mirrored copies** (see **Known Issues**). Keep the MCP flags consistent on GMS, MCE, and the upgrade job.
