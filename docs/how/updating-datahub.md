...
- **(Ingestion / Sigma)** Sigma Workspace Containers are always named now; the UI previously rendered some as a raw `urn:li:container:<guid>`. **On tenants where the workspace id was derived from the wrong node, datasets, workbooks and data models move to the correct Workspace Container URN** — a re-parent on existing deployments, with the affected ids listed in the report under `workspace_ids_remapped`. No entity URNs change. The Container they moved off is left behind empty: it never had aspects emitted for it, so stateful ingestion does not soft-delete it, and anything manually attached to it in the UI stays there rather than following the entities. Separately, a workspace whose metadata Sigma withholds now gets a Container named from the entity path instead of no name at all, listed under `workspaces_without_metadata`. **Action:** none required. Add the user associated with the Client ID/Secret to the workspaces in that list to replace a path-derived name with the real name, owner and timestamps.
