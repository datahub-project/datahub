...
- **(GMS / Ingestion sources)** Reading `dataHubIngestionSource` by URN now requires **Manage Metadata Ingestion** (`MANAGE_INGESTION`) on GraphQL (`ingestionSource`), OpenAPI, and Rest.li — the same privilege already required to **list** ingestion sources. Previously, any principal with generic entity View/Get could fetch a source (including `config.recipe`) if they knew its URN. **Action:** Grant `MANAGE_INGESTION` to service accounts or users that call get/list ingestion source APIs; View Entity alone is no longer enough.

- **(GMS / Ingestion execute)** `ApiOperation.EXECUTE` is **ingestion-source-only** across GraphQL, OpenAPI, and Rest.li (via `PoliciesConfig.API_ENTITY_PRIVILEGE_MAP`). Default entity and structured-property EXECUTE mappings were removed. `MANAGE_INGESTION` authorizes full CRUD + execute + exists on ingestion sources; `EXECUTE_ENTITY` authorizes execute and exists only — **not** recipe READ. GraphQL run/cancel/test-connection/rollback now use the same EXECUTE check as `ExecuteIngestionAuthValidator`. Admin all-resource policies may still include `EXECUTE_ENTITY`; it only takes effect for ingestion source URNs. **Action:** none for admins with `MANAGE_INGESTION`. Callers that relied on EXECUTE against non-ingestion entity types will be denied.
