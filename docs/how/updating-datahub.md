...
- #18395 **(Ingestion / Snowflake)** Snowflake Semantic Views can be ingested as first-class **semanticModel** + **metric** entities instead of datasets (subtype "Semantic View"), via the server-aware `semantic_views.emit_semantic_model_entities` flag: unset (default) auto-resolves — enabled on DataHub Cloud ≥ 2.1.0 unless the Metrics feature is explicitly disabled, off on OSS and older Cloud (recipe-driven); `true` requests it (Cloud warns and falls back to datasets if unsupported); `false` forces datasets. Legacy dataset behavior is unchanged where the new mode is off. When it is active: repoint links, tags, and automations from the old `urn:li:dataset:(...)` URNs to the new `semanticModel`/`metric` URNs (old datasets are soft-deleted only with stateful ingestion enabled, else cleaned up manually), `include_usage` is ignored (semanticModel entities carry no usage statistics), and the URN has no `env` (use a distinct `platform_instance` per environment). Separately, `include_queries: true` now emits query entities even when `include_usage: false` (previously nothing) — in both modes.
