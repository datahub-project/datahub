...
- #19715 **(Ingestion / LookML remote dependencies)** The LookML source now reads `remote_dependency` entries from `manifest.lkml` and validates their `url` before cloning. Previously a key-name bug meant these entries were silently ignored, so no cloning happened; fixing that activates the clone path. URLs must use `https://` or SSH (`ssh://` or `git@host:path`); non-HTTPS/SSH schemes, malformed URLs, and loopback / link-local / cloud-metadata hosts (including hosts that resolve to those IPs via DNS, and alternative IP encodings such as decimal or hex) are now rejected and the dependency is skipped with a warning. A new optional config field `remote_dependency_domain_pattern` (an `AllowDenyPattern`, default allow-all) lets you restrict which Git hosts may be cloned — for example `allow: ['^github\.com$', '.*\.github\.com$']`. **Action:** none required for valid recipes; if your `manifest.lkml` lists `remote_dependency` URLs pointing at internal/private hosts that previously never cloned (because of the bug), those clones will now be attempted and may be skipped by the new validation — set `remote_dependency_domain_pattern` to the hosts you expect, and for defense in depth restrict executor network egress to those same hosts.
