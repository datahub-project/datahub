...
- [#19522](https://github.com/datahub-project/datahub/pull/19522) **(Ingestion / dbt)** A dbt **source** whose physical table name differs from its logical name (its `identifier` is set to something other than the source `name`) is now modeled on the target platform using that `identifier`, so its warehouse dataset URN — and therefore its lineage and warehouse siblings — points at the real table. Previously the connector ignored a source's `identifier` unless the deprecated `use_identifiers` flag was enabled, building the URN from the logical `name` instead. Models, seeds, and snapshots are unchanged (their physical name comes from `alias`, which was already resolved). **Action:** none for the common case where `identifier` matches `name`. If you ingest renamed sources, their source datasets will be re-created under the corrected `identifier`-based URN on the next run and the old logical-name URN will be soft-deleted by stateful ingestion — a one-time migration; update any hardcoded references to those source URNs.
