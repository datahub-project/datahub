...
- #16134: Java 21 Runtime & Compilation Required - DataHub now compiles with Java 21 JDK and produces Java 17 bytecode for compatibility. All Docker images ship with Java 21 runtime. Self-hosted users must ensure their build and runtime environments use Java 21+. The Maven artifact name remains `datahub-client-java8` for backward compatibility, but now requires Java 21+ at both compile and runtime. This change also includes:
  - Spark integration upgraded from 3.0.3 to 3.3.4 (minimum version required for Java 17 bytecode support)
  - Hadoop upgraded from 2.7.2 to 3.3.6 (addresses Hadoop CVEs, bundled with Spark 3.3+)
  - Note: If you're a self-hosted user still running Java 17 or earlier, you must upgrade to Java 21 before deploying this release. Spark lineage users must upgrade to Spark 3.3.0+.
