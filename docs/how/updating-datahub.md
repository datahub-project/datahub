...
- **(Ingestion / SQL query parsing — all sources)** Temp-aware query fingerprinting is now **on by default for every source that parses SQL through the `SqlParsingAggregator`** (Snowflake, BigQuery, Redshift, Dremio, MSSQL, Oracle, Teradata, Postgres, the generic `sql-queries` source, etc.). Per-run ephemeral object names (ELT staging UUIDs, Segment/Snowpark/Great-Expectations temp names, etc.) are collapsed when computing the query fingerprint, so repeated ETL/sync runs that differ only by an ephemeral table name now map to a **single** Query entity instead of a new one per run, and queries whose output is a temp table no longer produce a standalone Query entity. The matcher is dialect-aware (double-quote, backtick, and bracket identifier quoting). This is lineage-safe (those tables are already excluded/resolved from lineage), but it **changes `query_id` and therefore Query URNs**: on the next ingestion run you will see a new set of collapsed Query entities, while the old per-run ones become stale and are reclaimed by `datahub-gc`. **Action:** none required to benefit. Snowflake and BigQuery expose `temp_aware_query_fingerprinting: false` to opt out; other sources can pass `temp_table_patterns=None` to the aggregator if they need the previous per-run fingerprints.
