...
- **(GMS / search)** **Data Products are now a first-class search filter and facet on their member assets.** Data Product membership is stored only on the Data Product side (`dataProductProperties.assets`), so member assets previously carried no data-product field: a `dataProduct` search filter matched nothing (even combined with other filters), and the Data Product facet never appeared in search. A new asset-side `dataProducts` aspect (searchable field `dataProduct`, `addToFilters`) is now kept in sync by `DataProductAssetsSideEffect` whenever a Data Product's asset list changes, so filtering by Data Product composes with any other filter (e.g. structured properties) and the facet is returned automatically. Existing memberships are populated by a new non-blocking `system-update` step (`BackfillDataProductAssetsStep`), gated by the `BACKFILL_DATA_PRODUCT_ASSETS` env var — it emits idempotent PATCH updates and self-skips once recorded. **Action:** to make pre-existing assets filterable/facetable, run the `system-update` job once with `BACKFILL_DATA_PRODUCT_ASSETS=true`; newly-created or edited memberships are handled live with no action. New env vars: `MCP_SIDE_EFFECTS_DATA_PRODUCT_ASSETS_ENABLED` (default `true`, enables the sync side effect), `BACKFILL_DATA_PRODUCT_ASSETS` (default `false`; the Docker quickstart sets it `true`), and `BOOTSTRAP_SYSTEM_UPDATE_DATA_PRODUCT_ASSETS_ENABLED` / `BOOTSTRAP_SYSTEM_UPDATE_DATA_PRODUCT_ASSETS_BATCH_SIZE` (default `true` / `1000`).
