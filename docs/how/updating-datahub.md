...
- #19815 **(Sigma ingestion)** Sigma ended support for datasets as a data source on 2026-09-15, and a dataset-backed workbook element no longer returns SQL. The connector now recovers a Sigma Dataset's warehouse table from Sigma's connection metadata instead, so **Sigma Dataset** lineage URNs are built from `connection_to_platform_map` rather than `chart_sources_platform_mapping`. Three things to know:
  ...
  - **Identifier casing changes on platforms other than Snowflake.** The SQL route lower-cased every platform except BigQuery and DB2; this route lower-cases only Snowflake, so a Postgres table Sigma reports as `Analytics.Public.Orders` is emitted with that casing rather than `analytics.public.orders`. Setting `convert_urns_to_lowercase: true` explicitly restores the lower-cased spelling — but only for **Sigma Dataset table names**, and never for BigQuery or DB2, whose identifiers are case-sensitive. Data Model and chart lineage are unchanged, and column identifiers on this route are not folded, so the same physical table can appear lower-cased via a Sigma Dataset and case-preserved via a Data Model in one run.
