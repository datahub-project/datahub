...
- **(GMS / GraphQL patchEntity authorization)** The `patchEntity` and `patchEntities` GraphQL mutations now authorize each write with the same entity-type-aware privilege that OpenAPI and Rest.li ingest use, instead of a flat **Edit Entity** check. For most entity types nothing changes: **Edit Entity** on the target remains sufficient. For platform-managed entity types the specific management privilege is now required and Edit Entity no longer substitutes: **Manage Policies** for `dataHubPolicy`, **Manage Secrets** for `dataHubSecret`, and **Manage Global Settings** for `globalSettings`. The entity type that selects the privilege is taken from the URN; a client-supplied `entityType` that disagrees with the URN is rejected. In addition, every non-system write to the `dataHubPolicyInfo` aspect on any API now requires **Manage Policies**, enforced at the aspect layer by `PolicyWriteAuthorizationValidator`. Previously an actor holding an unscoped **Edit Entity** grant could rewrite a policy's privileges or actors through `patchEntity`. Default policies do not grant unscoped Edit Entity to non-admins, so stock deployments see no change. **Action:** grant the specific privilege (for example **Manage Policies**) to any automation that patches these entity types through GraphQL with only Edit Entity. Toggle the aspect-layer check via `metadataChangeProposal.validation.aspectAuthorization.policyWrite.enabled` (default `true`).
