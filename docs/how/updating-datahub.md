...
- **(Ingestion)** The column-level **classification** feature has been removed from `acryl-datahub`. It depended on the unmaintained `acryl-datahub-classify` package, which pinned `numpy<2` and an outdated spaCy stack and blocked dependency upgrades. Recipes that set `classification.enabled: true` (on Snowflake, BigQuery, Redshift, dynamodb, or any SQL source) now fail fast at startup with a message pointing here. Recipes that leave classification disabled (the default) are unaffected. **Action:** Remove the `classification` block from your recipe. If you still need classification, pin the last release that supports it: `pip install 'acryl-datahub==1.6.0.5'`.
