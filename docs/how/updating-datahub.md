...
- **(GMS / MCE / System Update) Optional scoped retry + per-entity write gate for write thundering-herd mitigation.** Requires optimistic locking. Set `SCOPED_RETRY_ENABLED=true` to retry only the conflicted URN's branch within the transaction on an optimistic-lock conflict, instead of re-running the whole batch. Set `ENTITY_WRITE_LOCK_BACKEND` (`none` | `db` | `hazelcast`, default `none`) to serialize concurrent writers on the same URN before they contend: `db` uses the DB advisory lock (Postgres `pg_advisory` / MySQL `GET_LOCK`); `hazelcast` uses a distributed lock that keeps lock waits off the DB connection pool (preferred when connections are the bottleneck; reuses the existing Hazelcast instance). `ENTITY_WRITE_LOCK_ACQUIRE_TIMEOUT_SECONDS` (default `10`) bounds the acquire wait before proceeding lockless (CAS still guards); `ENTITY_WRITE_LOCK_LEASE_SECONDS` (default `300`) auto-releases a lock held by a dead/hung holder. **Action:** none; all default off/`none`.
...
- `ENTITY_WRITE_LOCK_BACKEND` (default `none`) — `none` | `db` | `hazelcast`: per-URN write gate to serialize concurrent writers on the same entity (hazelcast keeps waits off the DB connection pool).
