...
- **(GMS / timeseries)** Timeseries read path is selected with `timeseriesAspectService.implementation` (`elasticsearch`, `opensearch`, or `postgres`) and `TIMESERIES_ASPECT_SERVICE_IMPLEMENTATION`. For PostgreSQL reads, set `implementation: postgres` and keep `postgres.pgTimeseries.enabled: true` with SqlSetup (`implementation=postgres` without `enabled` fails fast at startup). Flat `DATAHUB_PGTIMESERIES_*` env vars configure the **default** store (unchanged single-table behavior). Optional multi-store / aspect routing uses `DATAHUB_PGTIMESERIES_CONFIG_FILE` (Spring resource URI, e.g. mounted ConfigMap) for `stores` + `routing`; SqlSetup migrates every store’s JDBC URL. Partman interval/premake are sticky after first register unless `DATAHUB_PGTIMESERIES_PARTITIONING_FORCE_OVERWRITE=true`. `DATAHUB_PGTIMESERIES_RETENTION_MAX_AGE_SECONDS=0` clears default-store partman retention (stops partition drops). Docker Compose Postgres profiles (`quickstart-postgres`, `debug-postgres`, …) default to exclusive pgTimeseries SoT (`DATAHUB_PGTIMESERIES_ENABLED=true`, `TIMESERIES_ASPECT_SERVICE_IMPLEMENTATION=postgres`; no ES dual-write). See [pgTimeseries design](../pgtimeseries-design.md).
