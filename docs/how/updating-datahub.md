...
- **(Build / Gradle)** The Gradle properties `apacheMavenRepositoryUrl`, `mavenCentralRepositoryUrl`, `confluentMavenRepositoryUrl`, and `linkedinOpenSourceRepositoryUrl` for overriding Maven repository URLs are deprecated. Use the new `datahub.dependencies.maven.central`, `datahub.dependencies.maven.confluent`, and `datahub.dependencies.maven.linkedinOpenSource` properties instead. The old properties are still honoured for backward compatibility but will be removed in a future release. See [Custom Maven Repositories](../developers.md#maven-repositories) for configuration details.
- **(Build / Gradle)** The Gradle property `nodeDistBaseUrl` for overriding the Node.js distribution mirror is deprecated. Use `datahub.dependencies.node.distBaseUrl` instead. The old property is still honoured for backward compatibility but will be removed in a future release. See [Node.js Distribution](../developers.md#nodejs-distribution) for configuration details.
- **(Build / Gradle)** The Gradle property `githubMirrorUrl` for overriding the GitHub base URL used in Docker builds is deprecated. Use `datahub.dependencies.github.baseURL` instead. The old property is still honoured for backward compatibility but will be removed in a future release. See [GitHub Base URL](../developers.md#github-base-url) for configuration details.
