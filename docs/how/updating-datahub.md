...
- [#19880](https://github.com/datahub-project/datahub/pull/19880) **(SAP Analytics Cloud ingestion)** Stories and applications no longer browse under a language-specific root folder. SAC returns the built-in root folder in a resource's `ancestorPath` localized to the tenant/resource language (for example `Public` in English and `Öffentlich` in German), which previously split one physical root into a separate browse-path tree per language. The connector only ingests public content, so the first `ancestorPath` segment is always this root; it is now rewritten to a single canonical label (`Public` by default, configurable via `public_root_folder_name`), so all public content shares one `Public/...` tree. User-created sub-folders are unchanged. **Action:** none required. Update any saved browse-path filter or bookmarked browse URL that pointed at a localized root such as `Öffentlich` to `Public`, or set `public_root_folder_name` to `null` to preserve the previous localized behavior. Note that `folder_pattern` is matched against this canonicalized browse path, so any rule that allowed or denied content by a localized root (e.g. `Öffentlich/...`) must be rewritten against the canonical `Public/...` root.
