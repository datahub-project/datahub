...
- **(Ingestion / Snowflake queries)** Temp-aware query fingerprinting is now **on by default** (`temp_aware_query_fingerprinting`, default `true`). Per-run ephemeral object names (ELT staging UUIDs, temp tables, etc.) are collapsed when computing the query fingerprint, so repeated ETL/sync runs that differ only by an ephemeral table name now map to a **single** Query entity instead of a new one per run, and queries whose output is a temp table no longer produce a standalone Query entity. This is lineage-safe (those tables are already excluded/resolved from lineage), but it **changes `query_id` and therefore Query URNs**: on the next ingestion run you will see a new set of collapsed Query entities, while the old per-run ones become stale and are reclaimed by `datahub-gc`. **Action:** none required to benefit; set `temp_aware_query_fingerprinting: false` to retain the previous per-run fingerprints.
