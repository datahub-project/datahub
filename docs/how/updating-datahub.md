...
- **(Ingestion / Sigma) Sigma Data Model ingestion is opt-in.** New `ingest_data_models` flag (default `False`) emits each Sigma Data Model as a `Container` with one `Dataset` per element. Each element Dataset carries `SchemaMetadata` and `UpstreamLineage`; lineage edges cover intra-DM element-to-element, cross-DM element-to-element, external upstreams (warehouse tables and Sigma Datasets), and workbook-to-DM element references. Element URNs are keyed by the immutable Data Model UUID so attachments survive Sigma slug rotation. Additive — no existing URNs change. To opt in: set `ingest_data_models: true` and optionally use `data_model_pattern` / `ingest_shared_entities` to scope.
