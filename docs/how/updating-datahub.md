...
- **(System Update / Usage events)** On deployments where the first usage event was written before the usage event index template existed, `<prefix>datahub_usage_event` is a plain index whose `type` field is mapped as text, which makes audit event search return 500 and leaves analytics charts that filter by event type empty. System update now moves such an index onto the layout the template defines (a data stream on Elasticsearch, a rollover alias over `<prefix>datahub_usage_event-000001` on OpenSearch). It copies the index to `<prefix>legacy_datahub_usage_event_<timestamp>`, recreates the usage event index, copies the events back, and deletes the copy once every event is accounted for. Usage events sent during the few seconds the old index is write-blocked are dropped, and analytics history is incomplete until the copy-back finishes. Events with no timestamp at all are dropped. If the copy-back is incomplete, system update logs an error, keeps the copy, and retries on its next run. Deployments whose usage event index was created from the template are unaffected. **Action:** none. Set `SKIP_LEGACY_USAGE_EVENT_INDEX_MIGRATION=true` on the system update job to postpone the migration.
