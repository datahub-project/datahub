...
- [#19947](https://github.com/datahub-project/datahub/pull/19947) **(Ingestion / Sigma)** A Sigma chart URN is built from the element id alone, and duplicating a workbook reuses those ids, so two workbooks can write the same chart's `inputFields`. The connector now keeps whichever aspect resolved more columns instead of letting the workbook processed last win. **Action:** none, but if you preferred the copy that resolved less, that chart's column lineage changes. Two caveats: this does not make the URNs unique -- two genuinely different charts still share one entity -- and only `inputFields` is chosen this way, so `chartInfo` and the browse path on a colliding chart still come from whichever workbook wrote last. `chart_input_fields_regressive_emission_skipped` in the report counts the refusals and `chart_input_fields_regressive_emission_samples` names a sample of the charts.
