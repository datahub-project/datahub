...
- **(Ingestion / Power BI)** DirectLake tables now also emit column-level lineage to their upstream Fabric OneLake tables when `extract_column_level_lineage` is enabled (the default). An edge is emitted only when the column is found in the upstream table's schema in DataHub, or when the scan response carries an explicit `sourceColumn`. Columns renamed in the semantic model, calculated columns, and the hidden `RowNumber` column get no edge and are counted in the ingestion report. **Action:** none required. To get column-level edges, ingest Fabric OneLake before Power BI and run Power BI ingestion with a DataHub connection (`datahub-rest` sink or `datahub_api`). Setting `extract_column_level_lineage: false` keeps only table-level lineage, for M-Query tables as well.
