...
- #19653 **(Ingestion / dbt)** dbt semantic models are now emitted as first-class `semanticModel`, `Semantic Model Dataset`, and `metric` entities instead of a single dataset with subtype `Semantic Model`, controlled by `emit_semantic_model_entities` on the `dbt` or `dbt-cloud` source. Unset (default) auto-resolves — enabled on DataHub Cloud 2.1.0 or later unless the Metrics feature is switched off, and off on OSS, older Cloud, and connectionless runs (e.g. file sink) unless the recipe sets it `true`; `false` always forces the legacy dataset. On Cloud, a server that cannot accept the new entities, an unreadable Metrics probe, or an unparseable server version all fall back to the legacy dataset and report why. On OSS there is no such check — an explicit `true` is taken at face value with no fallback, so confirm the server version yourself. This also ingests dbt's top-level `metrics:` block and measures marked `create_metric: true`, which were previously parsed and discarded. **Action:** the two modes use different URNs (`<database>.<schema>.<name>` versus `<project>.semantic_layer.<name>`), so a Cloud deployment already ingesting dbt semantic models will switch URNs on its next run — pin `emit_semantic_model_entities: false` to stay on the old shape, or plan the change: run with `stateful_ingestion.enabled: true` so the previous datasets are soft-deleted, and use `datahub migrate dbt-semantic-models` to copy owners, tags, terms, and documentation across first. The command works in both directions, so the change stays reversible. Note that the project-level Semantic Model is not a governance destination: it is shared by every semantic model in the project, so governance moves to the corresponding Semantic Model Dataset. `METRICS_ENABLED=true` is separate and only controls visibility in the Metrics UI and search — ingestion succeeds without it. Top-level `metrics:` definitions are dbt Core only — the dbt Cloud Discovery API does not expose them. See [dbt semantic models and metrics](../generated/ingestion/sources/dbt.md#semantic-models-and-metrics).
