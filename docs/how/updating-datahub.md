...
- #18395 **(Ingestion / Snowflake)** Snowflake Semantic Views can be ingested as first-class **semanticModel** + **metric** + **logical-dataset** entities instead of a single dataset (subtype "Semantic View"), via the server-aware `semantic_views.emit_semantic_model_entities` flag: unset (default) auto-resolves — enabled on DataHub Cloud ≥ 2.1.0 unless the Metrics feature is explicitly disabled, off on OSS and older Cloud (recipe-driven); `true` requests it (Cloud warns and falls back to datasets if unsupported); `false` forces datasets. Legacy dataset behavior is unchanged where the new mode is off. When it is active: each logical table the view exposes becomes its own `dataset` (subtype "Semantic Model Dataset") carrying `semanticModelProperties` + `schemaMetadata` + `upstreamLineage` to its physical base table; per-field semantic metadata lives on `semanticFieldAnnotation` aspects on the logical dataset's `schemaField` entities; `METRIC` columns become `metric` entities (no `metricUpstreams` — lineage flows `Metric → SemanticModel → Logical Dataset → Physical Dataset`). Repoint links, tags, and automations from the old `urn:li:dataset:(...)` URNs to the new `semanticModel`/`metric`/logical-`dataset` URNs (old datasets are soft-deleted only with stateful ingestion enabled, else cleaned up manually), `include_usage` is ignored (semanticModel entities carry no usage statistics), and the `semanticModel` and `metric` URNs have no `env` (use a distinct `platform_instance` per environment); the logical-dataset URNs are ordinary dataset URNs and keep `env`. Separately, `include_queries: true` now emits query entities even when `include_usage: false` (previously nothing) — in both modes.
