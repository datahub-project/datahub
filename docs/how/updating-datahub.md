...
- **(Ingestion / SQL profiling)** The SQLAlchemy profiler now runs MySQL, MariaDB, and Postgres profiling under `AUTOCOMMIT` instead of holding one implicit transaction open across every profiling query for a table. This prevents the long-lived transaction that pinned InnoDB read views (MySQL) / held Postgres idle-in-transaction and blocked VACUUM. Profile **values can now differ** for concurrently-written tables: `min`, `max`, `COUNT(*)`, `uniqueCount`, quantiles, histograms, and sample values each come from a different snapshot, so a single profile may be internally inconsistent (e.g. `uniqueCount` > `rowCount`). This is an accepted trade-off (analytical profiling tolerates minor inconsistency) and is safer than the long-transaction alternative. To restore per-table transactional behavior as an advanced escape hatch — e.g. for MySQL behind a proxy that rejects the `AUTOCOMMIT` session setting — set `profiling.profiling_isolation_level: TRANSACTIONAL` on the source (a SQLAlchemy isolation level name also works, but this is an advanced knob for operators who need it, not a recommended general setting). The opt-in is per-adapter by exact platform match; other SQL sources are unchanged.
