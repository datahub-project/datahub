...
- #16748 (Ingestion / Snowflake) **Cross-account share lineage and organization metadata.** Snowflake recipes with a configured `platform_instance` now emit a `DataPlatformInstance` aspect populated with `organization_name`, `account_locator`, and (when both are available) an org-qualified `account_identifier`, plus a `share_database_mapping` mined from `ACCOUNT_USAGE.QUERY_HISTORY`. Inbound shares are auto-discovered from the `origin` field on each shared database (populated by merging `SHOW DATABASES` into the information_schema result) and emit `Siblings` + `UpstreamLineage` (COPY) to the producer URN, resolved in this order: graph lookup of the producer's `DataPlatformInstance.account_identifier` → `account_mapping` config → `account_locator_fallback` convention. The new behaviors are governed by independent flags for gradual rollout: `include_organization_metadata`, `include_show_databases_metadata`, `auto_discover_inbound_shares`, and `publish_share_database_mapping`, each defaulting to `true`. Manual `shares` config continues to take precedence — auto-discovery skips databases already covered manually, so existing recipes are unchanged. To restore the previous behavior exactly, set `auto_discover_inbound_shares: false` and `publish_share_database_mapping: false`. Requires `IMPORTED PRIVILEGES on SNOWFLAKE` for the producer-side mining (already granted for lineage).
