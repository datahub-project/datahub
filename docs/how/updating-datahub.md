...
- **(UI / Micro frontends)** Micro frontends can now be placed as **tabs on entity profile pages**, not only as pages in the left navigation. Add `placement: { slot: entity.detail.tab, entityTypes: [dataset], tabName: "..." }` to a `microFrontends[]` entry; the tab appears on matching entity pages and the remote is loaded only when the tab is opened. `mount(el, ctx)` now receives a typed context (`slot`, `version`, optional `principal`, and for entity tabs `entity: { urn, type }`) instead of an empty object. Existing entries without `placement` keep working as navigation pages. `path` and `navIcon` are only required for navigation pages. See [Micro Frontends](../micro-frontends.md).
