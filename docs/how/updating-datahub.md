...
- #17450 **(Ingestion) `add_structured_properties_to_entity_wu` now emits JSON Patch (changeType: PATCH) per-property instead of replacing the entire `structuredProperties` aspect (changeType: UPSERT).** User- or pipeline-added structured properties are now preserved across ingestion runs. Caveat: removing a property from the recipe no longer removes it from DataHub on the next run — clean up via the UI or API. Affects Snowflake schema-gen and any third-party connector calling this helper.
