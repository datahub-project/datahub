...
- **(GMS / Validation)** Aspect constraints enforced by `FieldPathValidator` (schema field paths), `FormPromptValidator` (form prompt-id uniqueness), `LifecycleStageValidator`, `PolicyFieldTypeValidator`, `LogicalParentFieldPathValidator`, and `ServiceDefinitionLargeStringValidator` now also apply to **PATCH** writes of the corresponding aspects. These still use the standard aspect validator hooks (`validateProposedAspects`); `PATCH` is included in each validator's `supportedOperations`, and the values carried by the patch's own `add`/`replace` operations are checked. Previously PATCH writes bypassed these validators entirely. Patch `remove` operations and values already stored on the entity are not re-validated. **Action:** none for valid data; a patch that now fails was writing a value an equivalent upsert would already have rejected.
...
- [#19931](https://github.com/datahub-project/datahub/pull/19931) **(GMS / all APIs)** The `corpUserCredentials` aspect on `corpuser` entities, which stores native users' password hashes, encrypted salts and password-reset tokens, now requires the **Manage User Credentials** platform privilege to read. Previously any authenticated user could read it through the GraphQL `aspects` field, OpenAPI (`/openapi/v1`, `/v2`, `/v3` entity and aspect reads) and Rest.li (`/entitiesV2`, `/entitiesVersionedV2`, `/aspects`), because read authorization was enforced per entity and `corpuser` is viewable by everyone. Requests for the aspect alone now return 403 (or an empty list in GraphQL); whole-entity reads succeed with the aspect omitted. Admins hold this privilege by default. **Action:** none unless an integration reads this aspect with a non-admin token, in which case grant that token's user **Manage User Credentials**.
...
- [#19898](https://github.com/datahub-project/datahub/pull/19898) **(Ingestion / DataHub Lite)** DataHub Lite's storage engine is now pluggable, and the default for new instances is **SQLite** rather than DuckDB. Both engines are selected by the `lite.type` config key (`sqlite` or `duckdb`) and share the same schema, CLI, and REST API. DuckDB is unchanged but has moved out of the `datahub-lite` extra into its own: `acryl-datahub[datahub-lite,duckdb]`. Because SQLite ships with Python, `acryl-datahub[datahub-lite]` now installs on platforms with no DuckDB wheel (Linux armv7l, aarch64). The default database file for new instances is `~/.datahub/lite/datahub.db`; `~/.datahub/lite/datahub.duckdb` is untouched. The two file formats are not interchangeable: a DuckDB file cannot be opened with `type: sqlite`, or the other way around. **Action:** existing DuckDB users keep their instance and config as-is, but must add the `duckdb` extra to their install; without it, `datahub lite` reports that the `duckdb` type is disabled and names the extra. Users of the `datahub-lite` **sink** who relied on the default path get a new, empty `datahub.db` unless they pin `type: duckdb` and `file: ~/.datahub/lite/datahub.duckdb` in the recipe; the CLI warns when it creates a fresh store next to an existing `*.duckdb` file. To switch an existing instance to SQLite, run `datahub lite export --file lite_export.json`, then `datahub lite init --type sqlite` and `datahub lite import --file lite_export.json`. Exact-flavor searches are portable between the two: both engines support the `->` and `->>` JSON operators (SQLite 3.38+).
...
- **(GMS / Search backend)** Elasticsearch 7.x is no longer a supported search backend. DataHub requires **Elasticsearch 8.x** or **OpenSearch 2.x or 3.x**. Elasticsearch 9.x is **not** a certified backend: auto-detect still maps a 9.x cluster onto the Elasticsearch 8 client, but there is no ES9 CI coverage and known gaps remain (for example Analytics query DSL). GMS fails startup if it detects a 7.x cluster, if `ELASTICSEARCH_SHIM_ENGINE_TYPE` is set to `ELASTICSEARCH_7` / `ES7`, or if OpenSearch is running **Elasticsearch compatibility mode** (`compatibility.override_main_response_version`, which makes `GET /` report `7.10.2`). That compatibility mode is not treated as OpenSearch 2/3. **Action:** upgrade Elasticsearch 7 clusters to 8.x (or OpenSearch 2/3) before this DataHub version; on OpenSearch, turn compatibility mode off so the cluster reports its real 2.x/3.x version. Helm values that still set engine type `ELASTICSEARCH_7` must be changed to auto-detect or a supported type.
...
- #19815 **(Sigma ingestion)** Sigma ended support for datasets as a data source on 2026-09-15, and a dataset-backed workbook element no longer returns SQL. The connector now recovers a Sigma Dataset's warehouse table from Sigma's connection metadata instead, so **Sigma Dataset** lineage URNs are built from `connection_to_platform_map` rather than `chart_sources_platform_mapping`. Three things to know:
  ...
  - **Adding an entry at all makes its `env` authoritative, and that field defaults to `PROD`.** So an entry added only to fix casing will move URNs to `PROD` unless you set `env` on it too.
  - **Identifier casing changes on platforms other than Snowflake.** The SQL route lower-cased every platform except BigQuery and DB2; this route lower-cases only Snowflake, so a Postgres table Sigma reports as `Analytics.Public.Orders` is emitted with that casing rather than `analytics.public.orders`. Setting `convert_urns_to_lowercase: true` explicitly restores the lower-cased spelling — but only for **Sigma Dataset table names**, and never for BigQuery or DB2, whose identifiers are case-sensitive. Data Model and workbook warehouse-table lineage are unchanged, and column identifiers on this route are not folded, so the same physical table can appear lower-cased via a Sigma Dataset and case-preserved via a Data Model in one run. Note a chart reading through a Sigma Dataset does pick up the flag on the table half of its `inputFields`, since that URN comes from this route.

  **Action:** add the Sigma `connectionId` to `connection_to_platform_map` with the `env` and `platform_instance` your warehouse connector used, plus `convert_urns_to_lowercase: true` if you want the old spelling for those table names. The connector warns when it detects a mapping whose `env` or `platform_instance` no longer applies.

- #18982 **(GMS / Groups)** `removeGroupMembers` now removes members of a group that never finished migrating to native membership. Such members hold only the legacy `groupMembership` aspect; previously the mutation stripped only `nativeGroupMembership`, reported success, and left the user in the group with their access intact. **Action:** none required. Automation that removed members from an unmigrated group and relied on them staying will now see them actually removed.
...
- #19673 **(pgQueue / empty-poll sleep)** Idle pgQueue consumers now back off exponentially (1s doubling to 5s) instead of a tight fixed sleep. `DATAHUB_PGQUEUE_EMPTY_POLL_SLEEP_MS` and `DATAHUB_PGQUEUE_MCL_EMPTY_POLL_SLEEP_MS` (YAML `postgres.pgQueue.consumerPoll.emptyPollSleepMillis` / `mclEmptyPollSleepMillis`) are removed and are **not** read as aliases. Use shared `DATAHUB_PGQUEUE_EMPTY_POLL_SLEEP_MIN_MS` (floor) plus per-topic ceilings `DATAHUB_PGQUEUE_MAE_USAGE_EMPTY_POLL_SLEEP_MS`, `DATAHUB_PGQUEUE_MAE_MCL_EMPTY_POLL_SLEEP_MS`, `DATAHUB_PGQUEUE_MCE_MCP_EMPTY_POLL_SLEEP_MS`, and `DATAHUB_PGQUEUE_PE_EMPTY_POLL_SLEEP_MS`. On the Python actions pgQueue event source, `poll_interval_seconds` is removed; set `empty_poll_sleep_min_millis` / `empty_poll_sleep_max_millis`. **Action:** delete the old env keys and recipe field. Set the new min/max knobs only if you need non-default idle sleep.

- #19673 **(pgQueue / maintenance cron)** `postgres.pgQueue.maintenance.intervalSeconds` must map to a constant pg_cron cadence: a minute interval that divides 60, an hour interval that divides 24, or exactly 86400 (1 day). Values such as 5 hours or 7 minutes fail SqlSetup (90 minutes and multi-day were already rejected). **Action:** use 1, 2, 3, 4, 5, 6, 10, 12, 15, 20, or 30 minutes; 1, 2, 3, 4, 6, 8, or 12 hours; or 1 day.
...
- #19554 **(CLI / Python)** Optional extras that pull `unstructured` / `unstructured-ingest` (`notion`, `confluence`, `unstructured`, `datahub-documents`) now pin patched releases that fix CVE-2026-71428. Those libraries require **Python 3.11+**, so installing those extras on Python 3.10 will fail at `pip install`. They are **no longer included in `acryl-datahub[all]`**, so `pip install 'acryl-datahub[all]'` will not install Notion, Confluence, unstructured, or documents ingestion. Core `acryl-datahub` and `[all]` remain **Python 3.10+**. Managed ingestion still installs the extra from the recipe source type. The full `acryldata/datahub-ingestion` image still ships those sources (it installs `[all,datahub-documents,notion,confluence]`); slim and locked images do not. **Action:** If you use Notion, Confluence, unstructured, or documents ingestion from a local CLI or venv, install the matching extra on Python 3.11 or later (for example `pip install 'acryl-datahub[datahub-documents]'` or `'acryl-datahub[all,notion,confluence,datahub-documents]'`). If you previously relied on `[all]` alone for those sources, add the extra explicitly. No extra install is needed on the full ingestion image.

- #19554 **(agent-context / Snowflake UDFs)** Generated Snowflake Python UDFs now set `RUNTIME_VERSION = '3.11'` instead of `'3.10'`. Re-applying the generated SQL runs `CREATE OR REPLACE FUNCTION` and replaces existing 3.10 UDFs. **Action:** Re-apply the generated UDF SQL after upgrading, and confirm the Snowflake account allows Python 3.11 UDFs.
...
- **(GMS / Role and group membership writes)** Writes to the aspects that grant a user privileges are now authorized at the aspect layer, uniformly across GraphQL, OpenAPI, and Rest.li, by `PrivilegeGrantAuthorizationValidator`. Adding a role to a user or group (`roleMembership`) requires **Manage Policies** — the privilege the Assign Role UI already demands. Adding a user to a group (`groupMembership` / `nativeGroupMembership`) requires **Edit Group Members**, and adding a corpGroup owner (`ownership`) requires **Edit Owners**; when the acting user is the one being added, both require **Manage Users & Groups** instead. Only additions are checked — removals and re-ingestion of unchanged membership always pass, and system/bootstrap writes are exempt. Previously entity-level authorization ignored the aspect name, so **Edit Entity** on a user (or on a group you own, via the Asset Owners policy) was enough to grant that user or group the Admin role. **Action:** grant **Manage Policies** to any automation that writes `roleMembership` outside the UI — ingestion recipes, scripts, or raw MCP emitters. Ingestion that only syncs group membership (LDAP, Okta, Azure AD, `datahub user upsert`) needs **Edit Group Members** or **Manage Users & Groups**, not Manage Policies. Group owners can no longer add themselves to groups they own, or grant those groups roles. Toggle via `metadataChangeProposal.validation.aspectAuthorization.privilegeGrant.enabled` (default `true`).

- **(GMS / Analytics API)** The raw analytics search endpoint `POST /openapi/v2/analytics/datahub_usage_events/_search` now requires the **Analytics API access** privilege (`GET_ANALYTICS_PRIVILEGE`) or **Manage System Operations**. Previously it accepted **View Analytics**, which every user holds through the default "All Users" policy, even though the endpoint forwards a caller-supplied search request directly to the search engine. **View Analytics** continues to grant the in-app analytics dashboard. **Analytics API access** is held by Admins by default. **Action:** grant **Analytics API access** to any service account or script that calls this endpoint.

- **(GMS / Compliance form assignment writes)** The `batchAssignForm`, `batchRemoveForm`, and `createDynamicFormAssignment` GraphQL mutations now require the **Manage Compliance Forms** platform privilege, the same privilege the create / update / delete form mutations already required. The same rule is enforced at the aspect layer by `FormAssignmentAuthorizationValidator`: changing which forms are associated with an asset (adding or removing entries in the `forms` aspect) or writing a form's `dynamicFormAssignment` aspect requires **Manage Compliance Forms**. Writes that only change prompt completion or verification state of an already-assigned form are unaffected, so assignees can still complete forms. System writes, including the dynamic assignment hook, are exempt. Previously these mutations performed no authorization check. Admins and Editors hold **Manage Compliance Forms** by default. **Action:** grant **Manage Compliance Forms** to any automation (for example `datahub forms upsert` or scripts writing the `forms` aspect) that assigns or unassigns forms outside the UI. Toggle via `metadataChangeProposal.validation.aspectAuthorization.formAssignment.enabled` (default `true`).

- #19745 **(GMS / Asset summary settings writes)** The `updateAssetSettings` GraphQL mutation and any write to the `assetSettings` aspect (OpenAPI, Rest.li, ingestion MCPs) now require **Edit Entity** or **Manage Asset Summary** on the target asset, enforced at the aspect layer by `AssetSettingsAuthorizationValidator`. Previously the GraphQL mutation performed no authorization check, so any authenticated user could change the asset-summary template of any asset. Admins, Editors, and asset owners hold **Manage Asset Summary** by default, and REST writes already required **Edit Entity**, so legitimately privileged callers are unaffected. Note that OpenAPI and Rest.li still authorize the write as a generic entity update, so **Manage Asset Summary** alone is not sufficient there; only the UI / GraphQL path honors the narrower privilege. System writes are exempt. **Action:** grant **Manage Asset Summary** (or **Edit Entity**) on the relevant assets to any automation that writes `assetSettings` outside the UI. Toggle via `metadataChangeProposal.validation.aspectAuthorization.assetSettings.enabled` (default `true`).

- #19295 **(GMS / Search indexing)** GMS refuses to start when it is consuming MCLs (`MAE_CONSUMER_ENABLED` or `MCL_CONSUMER_ENABLED`) and both UI indexing paths are off (`PRE_PROCESS_HOOKS_UI_ENABLED=false` and `PRE_PROCESS_HOOKS_REPROCESS_ENABLED` unset/false). Standalone MAE consumers apply the same check in `UpdateIndicesHook`. That combination stored UI-created entities without writing search documents (ingestion sources, execution requests, and other GraphQL creates never appeared in the UI). **Action:** leave `PRE_PROCESS_HOOKS_UI_ENABLED` at its default `true`, or set `PRE_PROCESS_HOOKS_REPROCESS_ENABLED=true` if you intentionally disabled the GMS fast path. Set the same pair on GMS and MAE (embedded GMS and standalone MAE). Helm charts that previously set both MAE env vars to false when GMS preprocess was on will fail MAE startup after this version — upgrade the chart with this GMS/MAE image. Kubernetes system-update scale-down cannot leave this invalid state: when it flips preprocess off it also sets `MAE_CONSUMER_ENABLED=false` in the same Deployment patch (and scales standalone MAE to zero). v1.7+ Helm defaults to ZDU, which skips scale-down entirely. See [#19119](https://github.com/datahub-project/datahub/issues/19119).

- **(GMS / Access Tokens)** Creating access tokens with **no expiry** (`NO_EXPIRY` / "Never") is **disabled by default**. Allowed finite durations are configured as a comma-separated ISO-8601 list (`ACCESS_TOKEN_ALLOWED_DURATIONS`, default `PT1H,P1D,P7D,P30D,P90D,P180D,P365D`). Exactly one of GraphQL `duration` (enum) or `durationIso` (ISO-8601 string) must be provided on create; use `duration: NO_EXPIRY` only when `ACCESS_TOKEN_ALLOW_NO_EXPIRY=true`. **Already-issued tokens (including never-expiring ones) keep working** — this is create-time policy only. Remote Executor token creation in Settings is shown to users with **Manage Metadata Ingestion**, independent of this never-expire policy. **Action:** To allow never-expire tokens again, set `ACCESS_TOKEN_ALLOW_NO_EXPIRY=true`. To customize available TTLs, set `ACCESS_TOKEN_ALLOWED_DURATIONS` (e.g. `PT1H,P1D,P30D,P1Y`). Month/year use fixed approximations (`P1M` = 30 days, `P1Y` = 365 days).

- **(Ingestion / Kafka Connect)** SQL Server (`mssql`) is now treated as a three-level platform (`database.schema.table`) in `has_three_level_hierarchy`. JDBC sink connectors default the schema to `dbo` when `schema.name` is unset, so sink URNs change from `database.table` to `database.dbo.table`. JDBC **source** connectors now take the three-level branch too: when the schema cannot be resolved from the connector config, the dataset is dropped (with a warning) instead of emitting a two-level URN. **Oracle JDBC sinks** now emit `schema.table` (`database_name` omitted); schema comes from `schema.name` / `db.schema` if set, otherwise `connection.user`. Previously a missing schema could produce a bare `table` URN. **Action:** none if your SQL Server assets were ingested with schema in the URN (the default). If a source connector starts warning `Could not find schema for table`, set `schema.name` (or equivalent) on the connector, or use `generic_connectors` to force the mapping. For Oracle, ingest without `add_database_name_to_urn`, or map the sink with `generic_connectors` using a `database.schema.table` `target_dataset`.

- **(Ingestion / Elasticsearch)** `acryl-datahub[elasticsearch]` now requires `opensearch-py` 3.x. The 3.x Python client still talks to OpenSearch 2 clusters; 2.x of the client is no longer installed. **Action:** none if you install via the extra. If you pin `opensearch-py` yourself, use `>=3.0.0,<4.0.0`.
...
- #19891 **(Ingestion / SQL Profiling)** The profiling config classes have been renamed to drop their Great Expectations prefix, now that the GE profiler is gone: `GEProfilingConfig` is now `ProfilingConfig` and `GEProfilingBaseConfig` is now `ProfilingBaseConfig`, both moved from `datahub.ingestion.source.ge_profiling_config` to `datahub.ingestion.source.profiling.config`. Unity Catalog's `UnityCatalogGEProfiler` is now `UnityCatalogProfiler` (moved from `datahub.ingestion.source.unity.ge_profiler` to `datahub.ingestion.source.unity.profiler`). **No recipe changes are needed** — every `profiling.*` config option keeps its name and behavior. **Action:** only if you maintain a custom ingestion source that imports these classes directly, update the import path and class name.

- #18944 **(GMS / OpenAPI entity create authorization)** OpenAPI `createEntity` / `createGenericEntities` (and aligned create/patch batch paths) now authorize **each** URN in the request after building the MCP batch, using entity existence to choose Create vs Edit privileges. A principal with only **Create Entity** can no longer overwrite an existing entity via OpenAPI create/UPSERT. Type-level create checks alone are no longer sufficient for write authorization. OpenAPI `createAspect` authorizes via the batch-item change type: default aspect create-if-not-exists (`ChangeType.CREATE`) requires **Edit Entity**; inventing a missing entity (`createIfEntityNotExists=true` → `CREATE_ENTITY`) is existence-aware Create vs Edit. **Action:** Grant **Edit Entity** (or an equivalent edit privilege) to callers that update existing entities or use default `createAspect`; keep create-only policies for true creates. See [Policies](../authorization/policies.md#domain-separated-writers-vs-elevated-ingestion).
...
- #18548 **(Python SDK / Circuit breakers)** `datahub.api.graphql.BaseApi` and its `Assertion` / `Operation` subclasses now query GMS through `DataHubGraph` instead of a `gql` client, so they support OAuth, PAT, system auth and mTLS via `DatahubClientConfig`. The `transport=` constructor argument is replaced by `graph=` (a `DataHubGraph`), the `.client` attribute by `.graph`, GraphQL errors raise `GraphError` instead of gql's `TransportQueryError`, and passing both `datahub_token` and `datahub_auth` is rejected. Four behaviours come with the shared client, all of which the `gql` transport lacked: (1) an unset `timeout` now means a 30-second connect and read timeout instead of an unbounded wait; (2) failed requests are retried up to 4 times, POST included — `report_operation` is append-only and the server stamps its timestamp, so a request that fails after the write landed can add a duplicate operation row; (3) DataHub Cloud URLs go through `fixup_gms_url` (`:8080` dropped, `http` upgraded to `https`, `/gms` appended), where the `gql` transport posted to `<host>/api/graphql` verbatim; and (4) ambient credentials are honoured — `DATAHUB_AUTH_TYPE` env OAuth, and, when neither a token nor `datahub_auth` is supplied, the `DATAHUB_SYSTEM_CLIENT_ID` / `DATAHUB_SYSTEM_CLIENT_SECRET` pair, which turns a previously unauthenticated client into one acting as the system user. `BaseApi` logs the credential source it resolved at INFO. **Action:** code that constructed these classes with a custom `gql` transport must pass a `DataHubGraph` instead. Set `timeout` on the API or circuit-breaker config (`timeout_sec` in an Airflow connection extra, which the operators forward as `timeout`) if a poke must wait longer than 30 seconds on a slow GMS, and check that a client meant to stay unauthenticated does not run somewhere the system-client environment variables are set. Declarative auth on a circuit breaker is a config-level option: the Airflow operators build their config from the connection, which cannot express an `AuthConfig`, so OAuth for them comes from the `DATAHUB_AUTH_TYPE` environment variables with an empty connection password. The `acryl-datahub[circuit-breaker]` extra is now empty (it no longer pulls in `gql`); installing it still works, so no recipe or install change is required.

- **(Ingestion SDK / Connector support status)** Connector support tiers are now **Alpha**, **Beta**, and **GA**, replacing Certified, Incubating, and Testing. Tiers are assigned from production adoption and code ownership rather than by the connector author, so some connectors moved between tiers in this release; connector docs and the [integrations directory](https://docs.datahub.com/integrations) show the new badges. The old enum names were **removed**, not aliased: a custom source decorated with `@support_status(SupportStatus.CERTIFIED)` now fails at import with `AttributeError`. They were removed deliberately — kept as aliases, `CERTIFIED` would have silently published a brand-new connector as GA. **Action:** in any ingestion source you maintain outside this repository, replace `SupportStatus.CERTIFIED` with `SupportStatus.GA`, `SupportStatus.INCUBATING` with `SupportStatus.BETA`, and `SupportStatus.TESTING` with `SupportStatus.ALPHA`. Connector registries generated by older versions are still read correctly, so no regeneration is required.
...
- **(Metadata Model / Semantic Model)** `semanticModelInfo.datasets` is deprecated. The field remains in the aspect for backwards-read of already-ingested data, but `@Relationship` / `@Searchable` annotations have been stripped so it no longer writes graph or search edges. Membership is member-side only: datasets use `semanticModelProperties.semanticModel` (`IsPartOf`), metrics use `metricInfo.semanticModel` (`ModeledBy`, non-lineage). Canonical metric lineage is `Metric → Semantic Model Dataset → Physical Dataset` via `metricUpstreams.datasetUpstreams` plus each logical dataset's `upstreamLineage`. **Action:** After upgrade, re-ingest your Semantic Models so `metricUpstreams` is populated with per-metric routing by the connector. **Metric-to-dataset lineage will be missing between the upgrade and the next ingest run.** Legacy Snowflake Semantic Views: run `datahub migrate snowflake-semantic-views` first. New writes must not populate `semanticModelInfo.datasets`. See [Semantic Model](../generated/metamodel/entities/semanticModel.md) and [Metric](../generated/metamodel/entities/metric.md).

### Other Notable Changes

- [#19957](https://github.com/datahub-project/datahub/pull/19957) **(System Update / Usage events)** On deployments where the first usage event was written before the usage event index template existed, `<prefix>datahub_usage_event` is a plain index whose `type` field is mapped as text, which makes audit event search return 500 and leaves analytics charts that filter by event type empty. System update now moves such an index onto the layout the template defines (a data stream on Elasticsearch, a rollover alias over `<prefix>datahub_usage_event-000001` on OpenSearch). It clones the index to `<prefix>legacy_datahub_usage_event_<timestamp>`, recreates the usage event index once the clone holds every event, copies the events back, and deletes the clone when the copy accounts for all of them. Usage events sent during the few seconds the old index is write-blocked are dropped, analytics history is incomplete until the copy finishes, and on Elasticsearch events with no timestamp at all are dropped. System update waits up to 5 minutes for the copy and checks on it again on its next run; a copy that was interrupted is started again as long as the index has not rolled over since. If the copy reports failures, system update logs the first one and keeps the clone without copying it again, since a second copy can duplicate events once the index has rolled over; copy the remaining events into `<prefix>datahub_usage_event` yourself (`op_type: create`), then delete the clone. If the old index is gone but a copy cannot be started, system update fails so its retries start it; a copy still running after the wait is checked again on the next run. Only one system update migrates at a time: it holds `<prefix>legacy_datahub_usage_event_lease` while it moves the old index aside and starts the copies, and takes over a lease older than 30 minutes. Cases that need you (a copy that reports failures, an index that rolled over since an unfinished copy, a write block that cannot be lifted, or credentials that may not use the lease or the tasks API) are logged and counted on the Micrometer counter `datahub.system_update.legacy_usage_event_migration` with `outcome=manual_recovery`; `outcome=skipped` counts runs that left the migration to another system update. System update needs to create and delete `<prefix>legacy_datahub_usage_event_*` indices and to use the tasks API. Deployments whose usage event index was created from the template are unaffected. **Action:** before upgrading an affected deployment, make sure the search cluster has free disk space of about the size of that index, which the copy needs until the clone is deleted. Set `SKIP_LEGACY_USAGE_EVENT_INDEX_MIGRATION=true` on the system update job to postpone the migration; if a system update was interrupted mid-migration, the old index can be left write-blocked and its lease held; the next system update waits up to 11 minutes for that run's clone or lease to go stale, then migrates or lifts the block itself, and a system update once the lease is 30 minutes old takes it over and migrates. To skip that wait, lift the block with `PUT <prefix>datahub_usage_event/_settings {"index.blocks.write": false}` and, once no system update is running, delete `<prefix>legacy_datahub_usage_event_lease` before postponing. On OpenSearch the usage event step of an affected deployment fails until its index is migrated, as it did before this change; `SKIP_CREATE_USAGE_EVENT_INDICES_STEP=true` skips that step entirely, and deployments that set it to get past that failure need to unset it for the migration to run.
...
- #19837 **(GMS / Search V3 document embeddings on OpenSearch)** Enabling semantic search for `document` after Search V3 has already created `documentindex_v3` is not an in-place mapping update. OpenSearch k-NN 2.19+ treats `index.knn` as final, so adding `knn_vector` while that setting is false is rejected. Enable semantic search for `document` **before or together with** Search V3 write so the index is created with `index.knn: true`. If the V3 document index already exists without it, set `ELASTICSEARCH_INDEX_BUILDER_SETTINGS_REINDEX=true` and run system-update (or recreate `documentindex_v3`). Elasticsearch uses field-level `dense_vector` and does not need `index.knn`. **Action:** none for new OpenSearch deploys that turn on document semantic with Search V3; existing `documentindex_v3` on OpenSearch needs a settings reindex as above.

- #19934 **(GMS / Search V3 semantic reads)** Semantic search can now be served from the Search V3 document index. Set `ELASTICSEARCH_ENTITY_INDEX_V3_SEMANTIC_READ_ENABLED=true` (requires `ELASTICSEARCH_ENTITY_INDEX_V3_ENABLED=true`) once the V3 document index holds your document embeddings (documents embedded before V3 writes were on need a re-index into V3 or a re-embed); it is independent of `ELASTICSEARCH_ENTITY_INDEX_V3_KEYWORD_READ_ENABLED`. Only entity types with their own V3 index and embeddings are searched (`document` by default). The flag needs OpenSearch 3.5+ or Elasticsearch 8.18+ on the Search V3 cluster; DataHub services given it (GMS, and system-update or the consumers if they share the setting) refuse to start on older OpenSearch. **Action:** none; semantic search keeps reading the existing semantic indices until you set the flag.

- #19799 **(GMS / Lineage timeouts)** Impact-analysis lineage traversals (`searchAcrossLineage` / `scrollAcrossLineage`) that exceed `ELASTICSEARCH_SEARCH_GRAPH_TIMEOUT_SECONDS` now fail with a distinct timeout instead of a generic 500. GraphQL returns error extensions `code: 504` / `type: DEADLINE_EXCEEDED` (HTTP status stays 200 per GraphQL convention); Rest.li returns **HTTP 504**. In strict mode (`ELASTICSEARCH_SEARCH_GRAPH_IMPACT_PARTIAL_RESULTS=false`) a hop that runs out of time is now a timeout rather than truncated lineage returned as complete; in partial mode the response is marked `partial`. Timeouts are counted on the existing `datahub.lineage.graph_walk.errors` meter with `error_type="timeout"`. The lineage-visualization path (`getLineage`) is unchanged. **Action:** clients or alerts keyed on `SERVER_ERROR` / 500 for lineage failures should treat 504 / `DEADLINE_EXCEEDED` as a timeout and narrow the query (fewer hops, tighter filters) or raise the timeout.
- #19751 **(GMS / Column lineage APIs)** Querying **downstream lineage for a column** (`schemaField`) now also returns **metrics, charts, and dashboards** that consume that column, not only other columns. This applies to GraphQL `searchAcrossLineage` and other lineage APIs that walk via `LineageGraphFilters.forEntityType` (including `GraphService.getLineage`). The reverse is **not** true: lineage for a metric, chart, or dashboard still does **not** return columns. The UI lineage graph builds column-level edges on its own and is unchanged by this PR. `POST /openapi/v3/lineage/scroll` on a column also still omits metrics, charts, and dashboards — that endpoint uses lineage-annotated registry edges, not the schema-field walk used by `searchAcrossLineage`. **Action:** If an API client assumes every column-lineage result is a `schemaField`, update it to accept `metric`, `chart`, and `dashboard` (or filter those types out). No action if you only use the UI or OpenAPI lineage scroll.

- #18982 **(GMS / Groups)** Fixed two bugs in the inline migration of a group from legacy to native membership. The migration only read the first 500 members, so members past that were left on the legacy aspect while the group was marked native — which permanently disabled the retry. It also stamped the group native before granting native membership, so an interrupted run could leave members in no group at all and unable to re-migrate. Member reads and writes on the removal path are now batched as well. **Action:** none. A group left half-migrated by an earlier run is migrated on the next `addGroupMembers` or `removeGroupMembers` call only if its origin was never stamped; otherwise remove and re-add the affected members.
...
- **(Document embeddings)** The `datahub-documents` source no longer shifts embeddings onto the wrong chunk when a blank chunk sits between non-blank chunks (`characterOffset` still counts every chunk of the original text). Documents whose earlier `semanticContent` contained such a blank chunk stay misaligned until re-embedded, and the source skips documents whose source text hash is unchanged in incremental mode, so operators should force a re-embed for one run by setting `force_reprocess: true` (or disabling `incremental_mode`) in the recipe.
- **(Document embeddings)** The `datahub-documents` source no longer shifts embeddings onto the wrong chunk when a blank chunk sits between non-blank chunks (`characterOffset` still counts every chunk of the original text). Documents whose earlier `semanticContent` contained such a blank chunk stay misaligned until re-embedded, and the source skips documents whose source text hash is unchanged, so operators should force a re-embed for one run by setting `incremental.force_reprocess: true` (or `incremental.enabled: false`) in the `datahub-documents` recipe. Sources that embed documents inline as they ingest them (Notion, Confluence) re-embed a document when their own change detection sees it as new or changed.
- **(Document embeddings)** The `datahub-documents` source no longer shifts embeddings onto the wrong chunk when a blank chunk sits between non-blank chunks (`characterOffset` still counts every chunk of the original text). Documents whose earlier `semanticContent` contained such a blank chunk stay misaligned until re-embedded, and the source skips documents whose source text hash is unchanged, so operators should force a re-embed for one run by setting `incremental.force_reprocess: true` (or `incremental.enabled: false`) in the `datahub-documents` recipe. Sources that embed documents inline as they ingest them (Notion, Confluence) re-embed a document when their own change detection sees it as new or changed; to re-embed their already-ingested documents without waiting for a change, run `datahub-documents` once with `incremental.force_reprocess: true`, which also covers external documents (restrict it with `platform_filter: [notion, confluence]` if needed).
- **(Document embeddings)** The new `classical` embedding provider is a deterministic lexical hashing provider for CI, smoke tests and quickstarts, not semantic search. GMS refuses `EMBEDDING_PROVIDER_TYPE=classical` unless `CLASSICAL_EMBEDDING_ACKNOWLEDGE_LEXICAL_ONLY=true` is also set, and logs a warning while the provider is active. **Action:** none unless you opt in.
- **(Document embeddings)** The `datahub-documents` source no longer applies `embedding.rate_limit` / `documents_per_minute` to in-process embedding providers (`onnx`, and the new `classical`); they have no external API to protect, and a configured limit is now logged and ignored for them. **Action:** none.
- **(Document embeddings)** A server configured with the new `EMBEDDING_PROVIDER_TYPE=classical` needs an ingestion CLI from the same release or later. An older CLI pointed at such a server cannot map the provider, logs a warning, and ingests documents without embeddings. **Action:** upgrade the CLI (including any executor) before switching a server to `classical`.
...
- **(GMS / Split search clusters)** You can optionally send Search V2, Search V3, semantic, graph, timeseries, system-metadata, and usage traffic to named Elasticsearch/OpenSearch clusters. Routing defaults to `primary`; unknown cluster names fail at startup. Each cluster's shim auto-detect / `engineType` is independent of primary. Existing ES health routes (`/check/ready`, `/check/elastic`, `/debug/elastic`, `/health/detailed`) now require **all unique** configured clusters to be non-RED; probes are still cached (`HEALTH_CHECK_CACHE_DURATION_SECONDS`, default 5s). `/health` is unchanged (bootstrap only). Index names default to `INDEX_PREFIX` on every cluster; a non-blank `ELASTICSEARCH_CLUSTERS_*_INDEX_PREFIX` is used for **reads and writes** of families on that cluster. Blank overlay still inherits `INDEX_PREFIX` (or a custom `IndexPrefixResolver`). Single-cluster deploys need no new config. See the cluster and `ELASTICSEARCH_COMPONENT_CLUSTER_*` tables in [environment variables](../deploy/environment-vars.md).
...
- **(Ingestion / Dependencies)** `acryl-datahub` no longer caps `setuptools`, so it installs alongside `setuptools>=83` (which fixes CVE-2026-59890). `setuptools>=82` dropped `pkg_resources`, which the `redshift`/`cockroachdb` SQLAlchemy dialects still import at load, so ingestion installs a small `pkg_resources` compatibility shim at runtime before those load (temporary; removed once `sqlalchemy>=2` is adopted). `stopit`'s timeout helper is now vendored into `datahub.utilities` (MIT; behavior unchanged, still CPython-only), so the `powerbi` extra no longer depends on `stopit` and is fully decoupled from `setuptools`. **Action:** none. Installs under Airflow constraint files that pin an older `setuptools` are unaffected; the `>=83` floor is applied at lock time and in the `datahub-actions` image; the `datahub-ingestion` image resolves setuptools transitively (currently >=83, though not pinned for that image).
...
- **(GMS / Search backend)** **OpenSearch 3.x is now a supported search backend** alongside OpenSearch 2.x and Elasticsearch 8.x. The engine version is auto-detected; no configuration change is needed to move a deployment from OpenSearch 2.x to 3.x. Internally, OpenSearch I/O no longer uses the deprecated `RestHighLevelClient` transport (retired by the OpenSearch project as of 3.0) — both 2.x and 3.x are served by a unified client layer with unchanged wire behavior. Two OpenSearch 3.x caveats: (1) 3.x rejects **new** indexes using the `nmslib` kNN engine — deployments configuring `semanticSearch.models.*.knnEngine: nmslib` must switch to `faiss` (the default) or `lucene`; GMS fails startup with a clear error otherwise. (2) 3.x enforces the 512-byte document-id limit on bulk writes; very long schema-field URNs that indexed on 2.x are rejected on 3.x (logged as bulk failures). For new 3.x deployments, set `ELASTICSEARCH_INDEX_DOC_IDS_SCHEMA_FIELD_HASH_ID_ENABLED=true`; GMS logs a startup warning when running against 3.x with hashing disabled. **Action:** none for existing 2.x deployments.
...
- **(GMS rate limiting)** Rate-limit policy is loaded once by `RateLimitEffectiveConfig`: Spring binds `application.yaml` toggles from the environment; bundled classpath `rate-limit-config.yaml` is the default file; `RATE_LIMITS_CONFIG_FILE` **replaces** that document (not merged with it); `RATE_LIMITS_CONFIG_JSON` Jackson-overlays the result. Hazelcast bootstrap and the rate-limit engine share that instance so file/JSON `endpoint.enabled` / `scoped.enabled` still provision map config. A missing mounted file or invalid JSON fails **GMS** startup; MAE/MCE/upgrade evaluate Hazelcast conditions without the engine and do not fail on a missing overlay. Documents may use `datahub.gms.rateLimits:`, a `rateLimits:` wrapper, or a bare fragment. `file:/etc/datahub/rate-limits.yaml` and a bare path both work. `RATE_LIMITS_CONFIG_FILE_ENABLED` remains ignored. **Action:** none if Helm already sets `RATE_LIMITS_CONFIG_FILE=file:/etc/datahub/rate-limits.yaml`. After upgrade, confirm `GET /openapi/v1/rate-limits/config` shows the mounted `endpoint.rules` and GMS logs `Registered endpoint rate limit rule`.
...
- **(Security / Dependencies)** Apache Parquet stack bumped from 1.18.0 to **1.18.1** for CVE-2026-73334 (`org.apache.parquet.crypto.keytools`: file-controlled KMS URL forwarded to a pluggable `KmsClient` without host validation). 1.18.1 disables file-controlled KMS URLs by default. **Action:** none for operators; rebuild/redeploy picks up the new JARs. DataHub does not use Parquet envelope encryption / keytools.

- **(Security / Dependencies)** Apache Thrift `libthrift` bumped from 0.23.0 to **0.24.0** for CVE-2026-48586 (`TZlibTransport` decompression does not consistently count inflated bytes against the max message size, allowing a zip-bomb DoS). The artifact is forced transitively via Jena ARQ. **Action:** none for operators; rebuild/redeploy picks up the new JARs.

- **(Ingestion / Snowflake)** Semantic views now keep their logical tables' stored casing. Logical table names were previously uppercased before use, which mangled the name shown on each logical dataset (a table declared `Orders` appeared as `ORDERS`) and silently merged logical tables that differ only by case — legal in Snowflake over two different base tables — so one table's dataset, columns and metrics were dropped, and in the non-semantic-model path its columns were given column-level lineage pointing at the wrong physical table. Primary-key and join-key column names follow the same rule and now honour `preserve_column_case`. **Action:** none for most deployments — with the default `convert_urns_to_lowercase: true` the dataset URNs are unchanged, because the name was lowercased after being uppercased and still is. If you run `semantic_views.enabled: true` **and** `convert_urns_to_lowercase: false` **and** a logical table whose name is not already uppercase, that table's dataset URN changes (for example `….ORDERS` becomes `….Orders`) and the previously ingested entity is orphaned; soft-delete the old one or re-ingest with stateful ingestion so it is cleaned up.
- **(Ingestion / Snowflake)** New `preserve_column_case` option keeps Snowflake's column casing in field paths and column-level lineage. Snowflake's quoted identifiers let columns that differ only by case (for example `"col"` and `"COL"`) coexist in one table; until now both were lowercased to the same field path, and the duplicate was then dropped before the schema was written, so one of the columns disappeared from DataHub entirely along with its lineage, tags and profile. The connector now warns whenever columns collapse onto one field path, so affected tables can be found without changing any configuration. **Action:** none by default — `preserve_column_case` is `false`, and output is unchanged. If the new warning appears, set `preserve_column_case: true` to keep the columns distinct. It is also worth enabling without any collision if you use quoted mixed-case columns at all: a column stored as `"MixedCol"` can only be queried by that spelling, so the lowercased field path does not name anything you can select. Note that the option applies to every column, not only quoted or mixed-case ones — with it on, an unquoted column stored as `CUSTOMER_ID` keeps that casing in its field path instead of being lowercased to `customer_id`. Treat this as a one-way door: the option is part of every column's `schemaField` URN, so enabling it after data has been ingested re-keys every column and orphans column-level tags, glossary terms and documentation attached in the UI. Set it once, before the first run, and set it the same way on any `snowflake-queries` recipe pointed at the same account, or their field paths will not line up. Check other sources that resolve Snowflake columns before enabling it. Tableau hard-codes a lowercasing of Snowflake upstream column names, so its column-level lineage stops matching once this option is on. A separate fix makes Tableau resolve those names against the ingested Snowflake schema; **it is not part of this release**, so check whether your version carries it before enabling the option alongside Tableau. Once it does ship, ingestion order starts to matter: ingest Snowflake before Tableau, because Tableau matches against the Snowflake schema already in DataHub, and if Tableau runs first it falls back to lowercasing and its column-level lineage stays missing until the next Tableau run. Looker, Mode and Hex already adopt the ingested casing and need no change.

- **(Ingestion / SQL Profiling)** The SQLAlchemy profiler no longer loses columns that differ only by case. Dialects that normalize identifiers (Snowflake, Oracle) fold reflected column names, so a table with quoted identifiers such as `"col"` and `"COL"` had one of them silently dropped during reflection and left unprofiled. The profiler now re-reads the column list and addresses each column by its as-stored, quoted name. Tables without such a collision, and dialects that do not normalize identifiers, are unaffected. Recovering the column does not always mean you will see two sets of statistics: the profile has to attach to a field the source declared, so on Oracle — whose source folds column names when it builds the schema — a case-only pair stays a single field and only one column's statistics can attach. The profiler now warns in that case, naming the table and the folded path, instead of dropping the second profile silently. Relatedly, Snowflake profile field paths now line up with schema field paths: a column whose stored name was mixed-case and quoted (for example `"MixedCol"`) previously produced a profile that never attached to its schema field, because the driver preserved the casing while the schema lowercased it. **Action:** none. Profiles that were silently unattached now attach, so previously blank column statistics start appearing after the next profiling run. If the new warning about dropped column statistics appears, the two columns share one field path and only one column's statistics are kept — which one depends on column order, so treat the surviving values as belonging to either column. The warning names the table and the folded path so the affected tables can be found.

- #19005 **(Spark lineage / acryl-spark-lineage)** Fixed three defects that silently dropped lineage from the shaded Spark agent, all introduced when the agent began relocating its bundled dependencies. (1) Kafka structured-streaming reads produced **no input datasets** at all, because relocating `org.apache.kafka` rewrote the class-name constant OpenLineage compares against and Spark supplies the canonical `TopicPartition`. (2) Redshift relation lineage was never emitted, because `spark-redshift-community` is provided at runtime but was being relocated. (3) All of the agent's vendored OpenLineage customizations were **inert in the shipped jar**: our copy and the upstream copy were both packaged at the same path and the JVM resolved the upstream one. Separately, the JNI-backed compression libraries (snappy-java, zstd-jni, lz4-java) and JNA are no longer bundled — relocating them broke native symbol resolution, so enabling `compression.type` on the Kafka emitter failed with `UnsatisfiedLinkError`; Spark supplies all of them. **Action:** none required, and Kafka/Redshift lineage starts appearing after upgrade. Note that fix (3) means the agent's dataset path handling (PathSpec / `file_partition_regexp` / trimmer configuration) now genuinely takes effect in the shaded jar, so dataset names for file-based sources may change to the intended, trimmed form — review `spark.datahub.platform.<platform>.path_spec_list` if you tuned it against the previous behavior.

- #19103 **(Ingestion / Document sources)** The `min_text_length` filter now defaults to `0` (was `50`) for the `datahub_documents`, `notion`, and `confluence` sources, so short (non-empty) documents are embedded for semantic search by default instead of being silently skipped. Empty documents are still skipped (`skip_empty_text` / `skip_empty_documents`) — except Confluence hierarchy parent pages, which stay exempt from the empty/short filters so they can anchor the tree. **Action:** none to start embedding short documents; set `min_text_length` (`filtering.min_text_length` for Notion/Confluence) above `0` to restore length-based filtering. Re-running ingestion after upgrade embeds previously-skipped short documents, which increases embedding calls and index size accordingly. Relatedly, `datahub_documents` now falls back to a single text element when the markdown partitioner produces none (e.g. a one-character document), so any non-empty document stays embeddable instead of being silently dropped.
...
- #18690 **(Ingestion / SQL Profiling)** New `profiling.profiling_isolation_level` option for the SQLAlchemy profiler. By default the profiling connection is unchanged: one transaction spans the whole table profile, exactly as before. On MySQL this pins an InnoDB read view and grows the undo log for the duration of each table's profile; on Postgres it holds the connection idle-in-transaction and blocks `VACUUM`. If you see either symptom, set `profiling_isolation_level: AUTOCOMMIT` so each profiling `SELECT` runs on its own. Note the trade-off: under `AUTOCOMMIT` (or `READ_COMMITTED`) individual metrics come from different snapshots, so a profile of a concurrently-written table can be internally inconsistent (for example `uniqueCount` can exceed `rowCount`; this is not clamped). On Postgres this costs nothing, since `READ COMMITTED` already took a fresh snapshot per statement. On MySQL InnoDB it gives up the single consistent snapshot that `REPEATABLE READ` provided — set `profiling_isolation_level: REPEATABLE_READ` if you need both a consistent snapshot and an explicit level.
...
- #16319 **(Auth / Query entities)** Adds two distinct query-visibility privileges. `VIEW_ENTITY_QUERIES` (Dataset, Chart, Data Job) gates queries, view definitions, and `topSqlQueries` in dataset usage statistics (numeric usage data stays gated by `View Dataset Usage` alone); a user needs it on at least one subject dataset to read a query (`QUERY_ENTITY_AUTHORIZATION_REQUIRE_ALL_SUBJECTS=true` requires all of them; `QUERY_ENTITY_AUTHORIZATION_ENABLED=false` disables read enforcement). `VIEW_ALL_QUERIES` is platform-level and unconditional — it short-circuits the per-dataset check entirely, is never consulted by write checks, and is the only way to read a query with no recorded subject datasets, or to satisfy `REQUIRE_ALL_SUBJECTS` for `topSqlQueries` (whose bare SQL strings carry no per-statement dataset list to check). On upgrade, one-time system-update steps backfill both onto every policy that already grants `View Entity Page` (`VIEW_ENTITY_QUERIES` also backfills onto `View Dataset Usage`-only policies, so a usage-only policy doesn't silently lose its pre-upgrade `topSqlQueries` access); fresh installs get both via the default policies regardless of `VIEW_AUTHORIZATION_ENABLED`. **Action:** neither backfill is surgical, and nothing prunes `VIEW_ALL_QUERIES` automatically — if you want the new per-dataset restriction to actually take effect, revoke `VIEW_ALL_QUERIES` from the relevant policies yourself. Under `VIEW_AUTHORIZATION_ENABLED=true`, the upgrade backfill narrows `VIEW_ALL_QUERIES` to built-in (`editable=false`) policies only, but this requires `VIEW_AUTHORIZATION_ENABLED` to reach the **system-update job's own environment** (the stock compose file already passes it through); a custom Helm chart or compose stack must do the same, or the backfill silently treats view authorization as off and widens every matching policy instead — `BOOTSTRAP_SYSTEM_UPDATE_VIEW_ALL_QUERIES_PRIVILEGE_ENABLED=false` skips the backfill entirely (including the intended Root/built-in-role grant) rather than narrowing it, so pass the environment variable through to the system-update job first. Separately, `QUERY_ENTITY_AUTHORIZATION_REQUIRE_ALL_SUBJECTS` now defaults to `COMPAT` (was `false`): a no-op unless `VIEW_AUTHORIZATION_ENABLED` is on, in which case every Query-entity read (direct reads, `listQueries`, REST/OpenAPI, search/related-entity masking) requires `VIEW_ENTITY_QUERIES` on **all** of a query's subject datasets — the same require-all rule view authorization already applied to Query entities via `View Entity Page`, so default users see no change. It matters once you deliberately restrict: if you enable view authorization and revoke `VIEW_ALL_QUERIES` from a policy, users on that policy who hold `VIEW_ENTITY_QUERIES` on only some of a multi-dataset query's subjects stop seeing that query everywhere, including the dataset's Queries tab. `topSqlQueries` is the exception and stays any-subject. Set the var to `false` explicitly to keep any-subject reads under view authorization. Also note that a policy created **after** upgrading that grants only `View Entity Page` no longer reveals query SQL; grant `View Entity Queries` explicitly (the backfill covers policies that existed at upgrade time). See [Policies](../authorization/policies.md#query-entities).
...
- `SKIP_LEGACY_USAGE_EVENT_INDEX_MIGRATION` (default `false`) — Skip moving a usage event index that predates its index template onto the template layout during system update. See Other Notable Changes above and [Environment Variables](../deploy/environment-vars.md).
...
- **(GMS / Structured Properties)** Creating a structured property is now rejected when its `qualifiedName` would collide with another property's Elasticsearch field name. Field names are derived by replacing `.` with `_`, so `certification.status` and `certification_status` both map to the field `certification_status`. Previously both could be created and would silently share (and clobber) the same search field; now `PropertyDefinitionValidator` fails the write both within a single batch and when the normalized field already exists in active entity-index mappings. The check looks at index mappings (not the primary-store definition catalog), so a definition that exists but is not yet mapped — for example during UpdateIndices/MAE lag, or when `ENABLE_STRUCTURED_PROPERTIES_HOOK=false` — may not be detected until the mapping appears; soft-deleted properties whose fields remain mapped still block colliding creates. Existing colliding properties are not retroactively rejected — at index-mapping time, colliding fields with identical mappings keep the property with the lexicographically smallest URN (logged as a warning), while colliding fields with different mappings are omitted entirely (logged as an error) to avoid an incorrect index type. `qualifiedName` remains immutable, so upserts to an existing property skip this check. **Action:** Choose `qualifiedName` values that remain distinct after `.`→`_` normalization. If you already have colliding properties, rename or remove all but one so the intended property is indexed.
- **(GMS / Structured Properties)** String-backed structured property values (`string`, `rich_text`, `date`, `urn`) are now rejected when their UTF-8 encoding exceeds the configured keyword max length (default **32,766 bytes**, Lucene's keyword term limit). Previously oversized values could be written to the primary store and then fail Elasticsearch / OpenSearch indexing with `max_bytes_length_exceeded_exception`. `StructuredPropertiesValidator` rejects those writes for GraphQL, OpenAPI, and MCP. STRING (and V3 RICH_TEXT) keyword mappings also set a byte-safe character `ignore_above` derived from that limit (`keywordMaxLength / 4`, default 8191) so already-stored oversized values skip inverted-index terms on reindex instead of failing the document. Configure via `STRUCTURED_PROPERTIES_KEYWORD_MAX_LENGTH` / `structuredProperties.keywordMaxLength`. **Action:** Shorten Text / Rich Text property values under the limit, or store large content as asset documentation. See the [feature guide limitations](../features/feature-guides/properties/overview.md#value-size-for-text-and-rich-text-properties).
- **(GMS / Structured Properties)** Optional `STRUCTURED_PROPERTIES_DROP_OVERSIZED_KEYWORD_VALUES_FROM_INDEX` / `structuredProperties.dropOversizedKeywordValuesFromIndex` (default **false**) lets oversized string-backed structured property values be written to primary storage while omitting them from Elasticsearch / OpenSearch documents. With the default, writes still 422 as above. GraphQL and OpenAPI reads still return the stored value; search and filters will not. Set this on GMS, MAE, and any standalone MCE consumer (proposed validation can run there when `ALTERNATE_MCP_VALIDATION` is enabled), then restart. **Action:** Enable only if you must ingest values larger than the Lucene keyword limit (for example internal payloads that are not meant to be searched). Prefer moving large content off structured properties.
- **(GMS / View Authorization)** When `VIEW_AUTHORIZATION_ENABLED=true`, entity types are now **restricted by default**. The previous `VIEW_RESTRICTED_ENTITY_TYPES` allowlist is replaced by a registry baseline (`viewUnrestricted: true` on entities in `entity-registry.yml`) plus optional `VIEW_UNRESTRICTED_ENTITY_TYPES` / `_ADD` / `_REMOVE` overlays. Empty `VIEW_UNRESTRICTED_ENTITY_TYPES` keeps the registry baseline; a non-empty value replaces it. Stock `_ADD` covers the previous unrestricted set **minus** types already flagged in the registry (so the effective list stays the same; types already on the baseline are not re-listed). **`document`, `schemaField`, and `container` are not on stock unrestricted overlays** — they are view-restricted by default when view authorization is enabled (`container` is no longer `viewUnrestricted` in the registry). GraphQL container loads use field-strip redaction (`AuthorizationUtils.restrictEntity`) when the actor lacks View Entity Page; the hierarchy cache continues to store real URNs only. Shared authorization utilities enforce these rules across GraphQL, OpenAPI, and RestLi. The former restricted catalog types (`dataset`, `dashboard`, `chart`, ML entities, pipelines, glossary, `domain`, `dataProduct`, `notebook`, `dataProcessInstance`, `dataHubView`, `query`, `tag`) remain restricted. **Action:** Ensure domain-scoped (or other) View Entity Page policies cover containers you still want users to open. Do not re-add `document` / `schemaField` / `container` via `VIEW_UNRESTRICTED_ENTITY_TYPES_ADD` unless you intentionally want them to bypass view checks. On self-hosted OSS this gates entity pages / post-search masking only — OSS does **not** get Cloud query-time search pushdown.
- **(GMS / Documents)** Document view authorization and DocumentService privilege checks now apply across GraphQL, OpenAPI, and RestLi. Document privileges follow standard entity CRUD with `MANAGE_DOCUMENTS` as an additional allowance: CREATE accepts `CREATE_ENTITY` / `EDIT_ENTITY` / `MANAGE_DOCUMENTS`; UPDATE accepts `EDIT_ENTITY` / `MANAGE_DOCUMENTS`; DELETE accepts `DELETE_ENTITY` / `MANAGE_DOCUMENTS`. UPSERT/UPDATE-like writes against a nonexistent document URN are authorized as CREATE; writes against an existing document URN are authorized as UPDATE. Bridge documents additionally inherit VIEW (READ only) from the source entity when they have the `Bridge Document` subtype, a `bridge-*` ID, and resolvable source metadata; an unresolvable source fails closed unless direct document access already allowed the request. UPDATE and DELETE never inherit from the bridge source. **Action:** Grant document CREATE/EDIT/DELETE entity privileges or `MANAGE_DOCUMENTS` as needed; ensure readers have View Entity / Get Entity (or `MANAGE_DOCUMENTS`); do not rely on document unrestricted fail-open.
...
- **(Usage metrics)** The report-driven MCP usage metric was renamed from `mcp_queries` to `mcp_query` to match the `mcp_query` usage operation. Aggregated usage rows and the Micrometer counter (`datahub.usage.mcp_query`) use the new name. **Action:** update any dashboards, alerts, or consumers that filter on `metric_name=mcp_queries` or scrape `datahub.usage.mcp_queries`.

- #18693 **(GraphQL / Forms)** The `batchAssignForm` and `batchRemoveForm` mutations now read and write in batches rather than once per entity, which changes how they behave when part of a batch fails. Previously, if an entity's form state could not be read, that entity was silently skipped, the rest of the batch was applied, and the mutation still returned `true` — callers had no way to tell anything had been dropped. Now such a failure is reported as an error and no entity in the batch is modified. Likewise, a failed write no longer leaves a partially-applied batch: proposals are ingested together, so a failure leaves them untouched instead of updating some entities and not others. When one of the supplied entity urns does not exist, the error now lists every missing urn instead of only the first one found. Successful calls are unchanged, and both mutations remain idempotent. **Action:** none if you already treat a GraphQL error as "retry the whole call". If you relied on a `true` response meaning every entity was updated, note that this was never guaranteed — the mutation still returns `true` for entities that were no-ops (already assigned, or form not present on remove) — but errors are now raised in cases that previously passed silently, so a call that appeared to succeed may now surface a real failure that needs fixing before retrying.

- #14911 **(Spark lineage)** The `acryl-spark-lineage` agent now bundles **OpenLineage 1.50**, which enables OpenLineage's built-in **dataset-name trimmers** by default. These automatically strip common partition directories (`key=value`, date dirs like `year=2024/`, `yyyy/MM/dd`, `yyyyMM`) from filesystem/object-store dataset names. The plugin **disables them automatically when you already configure DataHub path trimming** (`path_spec_list` or `file_partition_regexp`), so those users are unaffected. But recipes with **neither** configured now pick up the upstream auto-stripping, so **dataset URNs for partitioned file/object-store paths change** (the partition segment is dropped) and lineage edges to the old, un-stripped URNs can orphan. **Action:** if you rely on the previous partition-inclusive URNs, either configure `path_spec_list` / `file_partition_regexp` for the affected platforms, or set `spark.datahub.metadata.dataset.openLineageTrimmersEnabled=false` to restore the pre-1.50 naming.

- **(ingest/doris)** A `database` written as `catalog.database` (for example `iceberg_catalog.db_ods`) is now read as a catalog plus a database rather than as one database name. Previously the whole string was passed through as the database, so datasets were emitted as `iceberg_catalog.db_ods.<table>`; they are now emitted as `db_ods.<table>`, and `database_pattern` matches the short database name (`db_ods`) instead of the qualified one. **Action:** Only recipes that set `database` (or `catalog`) to a qualified `catalog.database` value are affected — plain database names are unchanged. On the first run after upgrading, those datasets move to new URNs: with stateful ingestion enabled the old `catalog.database.<table>` entities are soft-deleted and re-created under the short name; without it they remain as orphans and need removing manually (for example `datahub delete --platform doris --env PROD` scoped to the affected datasets, or the DataHub UI). If you ingest the same database names from more than one catalog, set `platform_instance` to the catalog name so the short URNs do not collide.

- **(ingest/teradata)** When `convert_urns_to_lowercase` is enabled, the Teradata connector now also lower-cases **database container** URNs, not just dataset URNs. Previously datasets were lower-cased while their parent database containers kept the source casing, producing datasets parented under a container URN that did not match their own name. **Action:** This only affects deployments that explicitly set `convert_urns_to_lowercase: true` (the default is `false`). On the first run after upgrading, the database container URNs change from source-case to lower-case; with stateful ingestion enabled the old source-cased containers are soft-deleted and re-created lower-cased. No action is required for the default (`false`) configuration.
- #18181 **(Ingestion / Power BI, Mode)** Column-level lineage now preserves the **original casing** of upstream column names instead of lowercasing them. Previously the Power BI and Mode connectors emitted the upstream `schemaField` field path in lowercase (e.g. `orderid`) while the warehouse's schema kept its original casing (e.g. `OrderId`); because DataHub matches column-level edges case-sensitively, no column-level lineage rendered for any warehouse that stores columns in non-lowercase casing. The `schemaField` URNs emitted for Power BI and Mode column-level lineage will therefore change on the next ingestion. **Action:** none for most users — re-run ingestion and the previously-missing column-level edges will render. If you set `convert_column_urns_to_lowercase: true` on an upstream SQL source (e.g. MSSQL) as a workaround for the missing lineage, **remove it** so both sides preserve casing; leaving it enabled re-introduces the mismatch in the opposite direction. Note that `convert_lineage_urns_to_lowercase` no longer lowercases the column portion of the upstream URN (it still governs the dataset portion).

- #18367 **(Ingestion / BigQuery)** BigQuery now sources row count, size, and last-altered time from the supported `INFORMATION_SCHEMA.PARTITIONS` view instead of the undocumented `__TABLES__` Legacy SQL construct, which is unsupported by Google and may change or stop working without notice. `PARTITIONS` is dataset-scoped and free but covers base tables only, so the default stats change per entity type:
  ...
  Note that views and snapshots previously received these stats whenever profiling was enabled; that no longer happens by default, even with profiling on.

  **Action:** to restore the previous behavior for all entity types, set `use_legacy_table_stats: true` on the BigQuery source config. For base and external tables, this only takes effect when table stats are already being fetched (profiling enabled or `use_tables_list_query_v2: true`); for views and snapshots it applies regardless.

- **(GMS / Micrometer)** When `USAGE_AGGREGATION_ENABLED=true`, `datahub_request_count` is emitted from the in-memory usage aggregation flush (default every 60s), not per-request. The actor tag is `actor_class` (`regular`, `system`, `support`) instead of the former per-request `user_category` tag. `agent_class` and `request_api` label formats are unchanged. New tags: `usage_operation` on request and byte counters. New metrics: `datahub.usage.input_bytes`, `datahub.usage.output_bytes`, `datahub.usage.active_identities` (gauge: unique identities per `actor_class` in the flush window). Legacy JMX `requestContext_*` metrics are unaffected. Aggregation is **opt-in** (default off).

- **(Ingestion)** The built-in **`datahub` column-level classifier** (`DataHubClassifier`) has been removed from `acryl-datahub`, along with its `acryl-datahub-classify` dependency, which pinned `numpy<2` and an outdated spaCy stack and blocked dependency upgrades. The classification **framework** (the `Classifier` interface, the classifier registry, and the per-source orchestration) is retained, but no classifier is registered by default. Recipes that set `classification.enabled: true` (on Snowflake, BigQuery, Redshift, dynamodb, or any SQL source) without registering a replacement classifier now fail fast at startup with a message pointing here. Recipes that leave classification disabled (the default) are unaffected. **Action:** Remove the `classification` block from your recipe, or [register a custom classifier](../../metadata-ingestion/docs/dev_guides/classification.md#bring-your-own-classifier). If you still need the built-in classifier, pin the last release that ships it: `pip install 'acryl-datahub==1.6.0.5'`.

- #18014 **(Airflow / Dagster / Prefect / Great Expectations plugins)** The default emit mode changed from synchronous to **ASYNC** to reduce GMS load at high volume. With async, `emit()` no longer raises on a rejected write and there's no read-after-write guarantee. The Python SDK, CLI, and DataHub Actions defaults are unchanged. **Action:** to restore the previous behavior (`SYNC_PRIMARY`), set `[datahub] emit_mode = SYNC_PRIMARY` in `airflow.cfg` (Airflow), `emit_mode: SYNC_PRIMARY` on the Dagster source config, `emit_mode=EmitMode.SYNC_PRIMARY` on the Prefect `DatahubEmitter` block, or `emit_mode: SYNC_PRIMARY` on the Great Expectations `DataHubValidationAction` config.
...
- #17733 **(Ingestion / Omni)** The `max_requests_per_minute` config field has been removed. Rate limiting is now handled server-side via 429 responses with automatic exponential-backoff retry. Concurrency is controlled by the new `max_workers` field (default: 4). **Action:** Remove `max_requests_per_minute` from your Omni recipe if present; adjust `max_workers` to control parallelism.
...
- **(GMS rate limiting)** The rate-limit policy file is now loaded by Spring as a property source (`@PropertySource`) rather than a custom loader, so its `${ENV:default}` placeholders resolve and it binds under `datahub.gms.rateLimits.*`. **Breaking:** `RATE_LIMITS_CONFIG_FILE_ENABLED` and `RATE_LIMITS_CONFIG_JSON` are removed; `RATE_LIMITS_CONFIG_FILE` is now a Spring resource URI and must include a scheme — use `file:/etc/datahub/rate-limits.yaml`, **not** a bare path, or the override is silently ignored and the bundled defaults load; and an external policy file must use the full `datahub.gms.rateLimits:` key path, not a bare `rateLimits:` fragment. **Action:** if you set any of these, migrate before upgrading — drop `RATE_LIMITS_CONFIG_JSON` and `RATE_LIMITS_CONFIG_FILE_ENABLED` (no longer read), add the `file:` prefix to `RATE_LIMITS_CONFIG_FILE`, and re-root any mounted policy file under `datahub.gms.rateLimits:`. A `RATE_LIMITS_CONFIG_FILE` value without a scheme now logs a WARN at startup.
...
- **Docker image tags (`:head` removed, `:quickstart` and `:sha-*` added):** The floating `:head` tag is no longer published to Docker Hub because images were tagged per-service at different times, so `:head` often referred to a mixed set of commits. **Action for Docker Compose / local quickstart:** use `DATAHUB_VERSION=quickstart` (or `datahub docker quickstart --version head`, which resolves to the `quickstart` tag via the version manifest). Immutable `sha-<short_sha>` tags are published during the coordinated depot bake on `master`; the `:quickstart` tag is updated only after the full image matrix passes smoke tests, so all services move together. **Action for Kubernetes / Helm / production:** pin an immutable tag — a release (`v*`) or a commit tag `sha-<7-char short SHA>` (e.g. `sha-38960ae`). Do not use `quickstart` in cluster deployments; it can change on the next green `master` build. Bare short SHA tags (`:abc1234`) are now prefixed: use `:sha-abc1234`. Old CLIs using `--version head` continue to work when they load an updated [`quickstart_version_mapping.yaml`](https://github.com/datahub-project/datahub/blob/master/docker/quickstart/quickstart_version_mapping.yaml) with `docker_tag: quickstart` under the `head` key.
...
- **(GMS / schemaField domain & ownership mirroring)** Opt-in MCP side effects can copy a dataset's `domains` / `ownership` onto each of its `schemaField` entities (`MCP_SIDE_EFFECTS_SCHEMA_FIELD_ENABLED` plus `MCP_SIDE_EFFECTS_SCHEMA_FIELD_DOMAIN_ENABLED` / `MCP_SIDE_EFFECTS_SCHEMA_FIELD_OWNERSHIP_ENABLED`). While a sub-flag is on, dataset domain/ownership deletes cascade to the corresponding field aspects. **Changing a sub-flag (or the master flag) does not immediately rewrite historical fields** — ongoing writes use the new flag state, but existing inventory is reconciled by the `schemaFieldsFromSchemaMetadata` system-update step when a mirror flag is **enabled** (backfill). Turning a flag **off** stops further mirroring and does **not** delete leftover field aspects. **Note:** toggling flags back to a previously SUCCEEDED fingerprint does not re-run the job unless you use manual reprocess (`SYSTEM_UPDATE_SCHEMA_FIELDS_FROM_SCHEMA_METADATA_REPROCESS=true`) or clear/modify that fingerprint's upgrade result — either is valid.
...
- **(Ingestion / Executor)** The `acryl-executor` package is retired. The execution engine it provided now ships inside `acryl-datahub` as `datahub.executor`, so UI-triggered ingestion and connection testing no longer need a separate package. Existing published `acryl-executor` versions remain on PyPI but will not receive further updates. If you set custom task types in the `executor` action's `task_configs`, the old `acryl.executor.execution.*` dotted paths still work but log a warning on every run and will stop working in a future release. **Action:** for most deployments, none — `acryl-datahub-actions[executor]` continues to install what it needs. If your `executor.yaml` names task types explicitly, change the `acryl.executor.` prefix to `datahub.executor.` (for example `acryl.executor.execution.sub_process_ingestion_task.SubProcessIngestionTask` becomes `datahub.executor.execution.sub_process_ingestion_task.SubProcessIngestionTask`).
...
- **(GMS / schemaField MCP side effects + system update)** Optional nested flags under `metadataChangeProposal.sideEffects.schemaField` can mirror dataset `domains` and `ownership` onto schemaField entities (`MCP_SIDE_EFFECTS_SCHEMA_FIELD_DOMAIN_ENABLED` / `MCP_SIDE_EFFECTS_SCHEMA_FIELD_OWNERSHIP_ENABLED`, both requiring `MCP_SIDE_EFFECTS_SCHEMA_FIELD_ENABLED`). Defaults remain off. Enabling increases write fan-out (one MCP per column per mirrored aspect change) and can change DOMAIN / OWNER policy matches for schemaField URNs once those aspects are present.

  - **Backfill:** The non-blocking SystemUpdate step `GenerateSchemaFieldsFromSchemaMetadata` (`SYSTEM_UPDATE_SCHEMA_FIELDS_FROM_SCHEMA_METADATA_ENABLED`) scans dataset `schemaMetadata`/`status`, builds RESTATE MCLItems stamped with `APP_SOURCE=SYSTEM_UPDATE`, and invokes post-MCP schemaField side effects (then async `ingestProposal`) — it does **not** rely on a no-op parent upsert. Enabling a mirror flag backfills mirrored aspects; disabling stops further mirroring and does **not** delete leftover field `domains`/`ownership`. Upgrade id is fingerprinted by the effective domain/ownership mirror flags (`schema-field-from-schema-metadata-v2-d{0|1}-o{0|1}`), so a **first-time** change to a new fingerprint schedules a fresh run on the next SystemUpdateNonBlocking. Keep the MCP flags consistent on GMS, MCE, and the upgrade job.
  - **Cycles / re-runs:** Toggling flags back to a combination that already SUCCEEDED (e.g. off → on → off, or on → off → on) does **not** re-run that fingerprint. To re-run the same fingerprint, either use **manual reprocess** (`SYSTEM_UPDATE_SCHEMA_FIELDS_FROM_SCHEMA_METADATA_REPROCESS=true`) or clear/modify the stored `dataHubUpgradeResult` on the corresponding `urn:li:dataHubUpgrade:schema-field-from-schema-metadata-v2-…` entity so the step is no longer treated as SUCCEEDED/ABORTED. Both approaches are valid. See [Environment Variables](../deploy/environment-vars.md#schema-fields-configuration).
...
- **(GMS / PostgreSQL write locking)** Aspect writes and hard-deletes on PostgreSQL now acquire row locks in a consistent primary-key order — `ORDER BY urn, aspect, version` on the `FOR UPDATE` reads, plus an up-front ordered lock in `deleteUrn`. This prevents lock-order deadlocks between a multi-row `FOR UPDATE` write (e.g. logical-model linking) and a concurrent hard-delete on overlapping rows, which previously surfaced as `RetryLimitReached: Failed to add after N retries`. PostgreSQL-only: on PostgreSQL the default plan can acquire `FOR UPDATE` locks in physical scan order, whereas MySQL/InnoDB already locks the primary-key `IN`-list in index order, so MySQL behavior is unchanged. A new opt-in `EBEAN_ENTITY_WRITE_ADVISORY_LOCK_ENABLED` (PostgreSQL-only, default `false`) additionally serializes concurrent writers to the same entity via a transaction-scoped `pg_advisory_xact_lock` (auto-released on commit). This is deadlock-ordering only, and it **blocks until the lock is released** against a concurrent hard-delete of the same entity — there is no Java-side timeout (`ENTITY_WRITE_LOCK_ACQUIRE_TIMEOUT_SECONDS` bounds only the Hazelcast gate, not this lock); set a Postgres `lock_timeout` if you need to bound it. It is not a herd/contention guard — use `ENTITY_WRITE_LOCK_BACKEND=hazelcast` for bounded, off-connection waits under a write thundering-herd. **Action:** none; the ordering fix is automatic on PostgreSQL and the advisory lock is opt-in.
- **(GMS / PostgreSQL write locking)** Aspect writes and hard-deletes on PostgreSQL now acquire row locks in a consistent primary-key order — `ORDER BY urn, aspect, version` on the `FOR UPDATE` reads, plus an up-front ordered lock in `deleteUrn`. This prevents lock-order deadlocks between a multi-row `FOR UPDATE` write (e.g. logical-model linking) and a concurrent hard-delete on overlapping rows, which previously surfaced as `RetryLimitReached: Failed to add after N retries`. PostgreSQL-only: on PostgreSQL the default plan can acquire `FOR UPDATE` locks in physical scan order, whereas MySQL/InnoDB already locks the primary-key `IN`-list in index order, so MySQL behavior is unchanged. A new opt-in `EBEAN_ENTITY_WRITE_ADVISORY_LOCK_ENABLED` (PostgreSQL only, default `false`) additionally serializes concurrent writers on the same `(urn, aspect)` pair via a transaction-scoped `pg_advisory_xact_lock` (auto-released on commit). Keying on `(urn, aspect)` — the actual CAS / `FOR UPDATE` conflict unit — means cross-aspect writers on the same URN do not serialize (no over-serialization of the common case); whole-entity ops like `deleteUrn` lock the entity's full aspect key-set so delete↔upsert safety is key-set overlap. `pg_advisory_xact_lock` **blocks until the holder's transaction commits/rolls back** — there is no Java-side acquire timeout for this lock (`ENTITY_WRITE_LOCK_ACQUIRE_TIMEOUT_SECONDS` bounds only the Hazelcast write gate, not this advisory lock). To bound the wait, set a PostgreSQL `lock_timeout` (e.g. `SET lock_timeout = '10s'` at the session/role level); on timeout Postgres aborts the waiting statement with `ERROR: canceling statement due to lock timeout` (SQLSTATE `55P03`). Whether that surfaces as a retryable conflict depends on the Ebean retry conflict classification (`EBEAN_RETRY_BACKOFF_SQL_STATES`) — the default classification covers deadlock states (`40001`/`40P01`) but not `55P03`, so a `lock_timeout` abort may end as `RetryLimitReached` unless `55P03` is added to `EBEAN_RETRY_BACKOFF_SQL_STATES`. **Action:** none; the ordering fix is automatic on PostgreSQL and the advisory lock is opt-in.

- **(GMS / MCE / System Update) Optional optimistic locking for aspect writes.** Set `OPTIMISTIC_LOCKING_ENABLED=true` independently on each process to replace `SELECT FOR UPDATE` with compare-and-set updates on `SystemMetadata.version` for Ebean storage. Legacy rows without a version use plain updates until a write stamps one; Cassandra ignores the flag. Optimistic conflicts use the existing configurable Ebean retry/backoff policy (`EBEAN_RETRY_*`, including `Retry-After`). **Action:** none; the flag defaults to `false`.
- **(GMS / MCE / System Update) Optional scoped retry + Hazelcast write gate for write thundering-herd mitigation.** Both require optimistic locking. `SCOPED_RETRY_ENABLED=true` retries only the conflicted URN's branch within the transaction on an optimistic-lock conflict, instead of re-running the whole batch (up to 3 in-transaction attempts per conflict, bounded to avoid compounding with the outer transaction retry; not separately configurable). `ENTITY_WRITE_LOCK_BACKEND=hazelcast` (`none` | `hazelcast`, default `none`) serializes concurrent writers of the same `(urn, aspect)` — the actual CAS conflict unit, so cross-aspect writers on the same URN don't serialize — _off the DB connection pool_ before they contend — the intended fix when DB connections are the bottleneck. It queues waiters in Hazelcast **before** they grab a DB connection (waiters hold zero pooled connections), whereas an in-transaction DB lock (the advisory, or MySQL `GET_LOCK`) pins a connection while blocked and so can't relieve pool exhaustion under a herd. It reuses the existing Hazelcast instance and activates whenever optimistic locking is on (independent of scoped retry; MySQL uses the gate only). On acquire timeout or a Hazelcast outage it degrades to lockless (CAS still guards). `ENTITY_WRITE_LOCK_ACQUIRE_TIMEOUT_SECONDS` (default `10`) bounds the acquire wait; `ENTITY_WRITE_LOCK_LEASE_SECONDS` (default `300`) auto-releases a lock held by a dead/hung holder. The Postgres `pg_advisory_xact_lock` above (`EBEAN_ENTITY_WRITE_ADVISORY_LOCK_ENABLED`) is a separate, independent option; when the Hazelcast gate is engaged the advisory lock is skipped to avoid double-locking (the gate degrades to lockless CAS on timeout/outage, not to the advisory — so the advisory is not a gate fallback). **Action:** none; all default off/`none`.

- **(GMS / MCE / System Update) Ebean transaction retry state clearing.** After a later retry attempt successfully commits or intentionally rolls back, transaction retry state is now cleared (`transactionContext.success()`), so a recovered conflict is not reported as retry exhaustion (`RetryLimitReached`). This is independent of optimistic locking. **Action:** none.
...
- **(GMS / View Authorization)** `schemaField` and `container` are view-restricted by default (not
  ...
  **OpenAPI and RestLi READ-by-URN** use the same parent-inheritance rules when REST API
  authorization is enabled, so GraphQL entity reads and OpenAPI/RestLi GET agree for the same actor
  and schemaField URN. GraphQL container entity loads and ParentContainers breadcrumbs use
  field-strip redaction (real `urn`/`name` may still appear). The entity-graph hierarchy cache
  remains unauthenticated (real URNs only). OSS has no Cloud search pushdown. Resource-scoped
...
- #18727 **(Ingestion / Airbyte)** Stream namespaces are now recovered for connections read through Airbyte's Public API, which omits them from the connection payload. Previously every stream in such a connection fell back to the source connector's default schema, so datasets for streams in other schemas pointed at the wrong table. DataHub now reads the namespace from Airbyte's `/streams` endpoint (which reports namespaces from Airbyte 1.7.0 onwards) and only uses it when it is unambiguous; where several schemas expose a same-named stream and Airbyte does not say which is which, the namespace is left alone and a warning names the candidates. **Action:** none required, but affected datasets change URN — they move from `<database>.<default_schema>.<table>` to `<database>.<actual_schema>.<table>`. Expect new dataset entities on the next run, and soft-delete the stale ones (or enable stateful ingestion so they are marked removed automatically). On Airbyte versions with no `/streams` endpoint behaviour is unchanged, and the run now warns that stream metadata is unavailable.

- #18856 **(Ingestion / Airbyte)** A schema declared per table in the source connector's configuration (`tables[].schema`, used by SQL Server among others) now takes precedence over the connector's single schema-shaped configuration key. That key sometimes holds something that is not a schema at all — a database name, for instance — and it was overriding the per-table value, which then dropped the schema tier from the dataset URN entirely when it matched the database name. A new per-source `default_schema` under `sources_to_platform_instance` supplies a schema for deployments where neither Airbyte nor the connector configuration reveals one; it is only consulted as a last resort, so it can be left in place after an Airbyte upgrade. Runs also warn (`Stream Namespaces Not Reported`) when Airbyte describes a source's streams but reports no namespace for any of them, which is what pre-1.7.0 Airbyte does. **Action:** none required, but datasets that were missing a schema tier change URN — they move from `<database>.<table>` to `<database>.<schema>.<table>`. Expect new dataset entities on the next run, and soft-delete the stale ones (or enable stateful ingestion so they are marked removed automatically).

- **(GMS / search)** **Data Products are now a first-class search filter and facet on their member assets.** Data Product membership remains authoritative on the Data Product side (`dataProductProperties.assets`), but member assets previously carried no data-product field: a `dataProduct` search filter matched nothing (even combined with other filters), and the Data Product facet never appeared in search. A new asset-side `dataProducts` aspect (`array[DataProductAssociation]`, searchable fields `dataProduct` and `isDataProductOutputPort`, `addToFilters`) is kept in sync by `DataProductAssetsSideEffect` whenever a Data Product's asset list changes, so filtering by Data Product composes with any other filter and the facet is returned automatically. Like `domains`, the Data Product URN is included in default free-text search (`queryByDefault`) on member assets. Existing memberships are populated by the ZDU / `MigrateAspects` sweep when `dataProductProperties` schemaVersion advances (v1→v2 via `DataProductPropertiesMigrationMutator`); set `REPROCESS_DATA_PRODUCT_ASSETS=true` to force a re-upsert resync if drift is observed (heals missing ADDs and stale REMOVEs via search). Hard-delete of a Data Product (or DELETE of `dataProductProperties`) scrubs the asset-side aspect. `MCP_SIDE_EFFECTS_DATA_PRODUCT_ASSETS_MAX_FANOUT` (default `500`) controls the batch size for reading existing asset-side `dataProducts` during sync and for search scroll pages during reprocess; all unsynced ADD patches for a commit are emitted in one side-effect pass (apply is Kafka-batched at 500). Very large memberships (tens of thousands of assets) mean a larger one-shot memory/Kafka wave on migrate or reprocess. REMOVE patches from delete are not capped. **Action:** for pre-existing memberships, run `system-update` with MigrateAspects enabled (`SYSTEM_UPDATE_MIGRATE_ASPECTS_ENABLED=true` or `ZDU_STAGE_20=true`, as in quickstart); newly created or edited memberships sync live with no action. New env vars: `MCP_SIDE_EFFECTS_DATA_PRODUCT_ASSETS_ENABLED` (default `true`), `MCP_SIDE_EFFECTS_DATA_PRODUCT_ASSETS_MAX_FANOUT` (default `500`), `REPROCESS_DATA_PRODUCT_ASSETS` (default `false`), and `BOOTSTRAP_SYSTEM_UPDATE_DATA_PRODUCT_ASSETS_BATCH_SIZE` (default `1000`).
- **(GMS / search indexing)** Analyzed-text search fields (`@Searchable(fieldType=TEXT / TEXT_PARTIAL / WORD_GRAM)`) now guard their keyword surfaces with a **byte-safe `ignore_above` (8191 characters, i.e. 32766 ÷ 4)** instead of `32766`. The Elasticsearch/OpenSearch `ignore_above` limit is counted in characters while Lucene's hard 32,766-byte per-term limit is counted in bytes, so a multi-byte value could previously slip past `ignore_above` and still be rejected — dropping the **entire document** from the search index (its tokenized `.delimited` sub-field, which has no per-term limit and is what full-text queries target, indexes the value in full regardless). This is applied as an in-place mapping update, so **no reindex is required**; documents previously dropped by this limit are re-indexed on their next write. The only behavioral change is that exact-match/sort/aggregation on the keyword parent of a TEXT field is now skipped above 8191 characters rather than 32766 — irrelevant for the long-form content these fields hold.
...
- #18102 **(Ingestion framework)** Oversized `queryProperties` aspects no longer fail ingestion runs. The `EnsureAspectSizeProcessor` size guard for `queryProperties` was fixed to measure serialized (JSON) size correctly instead of comparing a raw character count against a byte-based target — the previous unit mismatch caused the guard to give up and emit oversized aspects, which GMS then rejected with a `400 Cannot parse request entity`, failing the run after enough rejections. This most often affected Snowflake sources with `include_queries` enabled against accounts with heavy temp-table query flows (e.g. Hightouch), but applies to any SQL connector that emits query entities. **No action required;** oversized statements are now truncated (with a warning) rather than failing the run.

- #17963 **(Ingestion / Glue)** Lake Formation resource links (which are schemaless pointers in the catalog) now have their schema populated from the owning table so their columns are visible on the ingested dataset. This is controlled by a new `resolve_resource_link_schema` flag, which **defaults to `true`**. The owning table's schema is read from DataHub first (no AWS call when that account is already ingested) and otherwise via a cross-account `glue:GetTable` on the target. **If you ingest a catalog that contains resource links**, after upgrade the connector may issue new cross-account `glue:GetTable` calls and, for any shared table you lack `glue:GetTable` on, emit a warning (the link still ingests, schemaless, and remains reachable through its upstream lineage edge — the run does not fail). **Action:** grant `glue:GetTable` on the shared tables to populate their schemas, or set `resolve_resource_link_schema: false` to keep resource links schemaless as before.

- #17962 **(Spark lineage)** `spark.datahub.metadata.dataset.file_partition_regexp` now also applies to bare-namespace filesystem datasets (`file`, `dbfs`) that previously bypassed partition stripping. This makes behavior consistent with scheme-based FS paths (`s3://`, `hdfs://`, …) and is arguably a bugfix, but **if you have set `file_partition_regexp`**, the emitted dataset URNs for `file`/`dbfs` sources will change (the partition segment is now stripped), so lineage edges to the old, un-stripped URNs can orphan. **Action:** only affects opted-in users — if you rely on the previous `file`/`dbfs` URNs, unset `file_partition_regexp` or re-point downstream lineage to the new (partition-stripped) URNs. Recipes that leave `file_partition_regexp` unset are unaffected (byte-identical output).

- #17617 **(Ingestion / Tableau)** `lineage_overrides.platform_override_map` is now applied to the body of the generated URN, not just to its platform stamp. Previously, an override that crossed naming tiers — most commonly `presto` → `athena`, since Tableau models Athena connections as presto with a hive catalog — produced a URN stamped with the target platform but shaped like the source platform (`catalog.schema.table` rather than `schema.table`). Those URNs never matched what the target platform's own ingestion source emits, so the lineage edges resolved to nothing and no error was raised. **If you use `lineage_overrides` to remap a three-tier platform to a two-tier one**, the emitted upstream URNs change after upgrade and lineage starts resolving. (Overrides in the other direction are unchanged: a two-tier connection has no catalog to fill a three-tier target's first segment, so those URNs stay two-part and the catalog has to come from `platform_instance_map`.) The old URNs were already dangling, but DataHub will have auto-created stub entities for them and lineage now points away from those stubs. **Action:** none for lineage itself. If anyone hand-applied tags, documentation, or ownership to one of those stub datasets, that metadata stays behind on the now-orphaned stub — move it to the real dataset and soft-delete the stub. Recipes without `lineage_overrides`, and overrides between platforms of the same tier, are unaffected.
...
- #18285 **(GMS / UI)** Multi-language support (`I18N_ENABLED`) now defaults to **on** in OSS (previously off). Combined with the UI defaulting to the user's browser locale, users whose browser is set to a non-English language will now see DataHub in that language by default where a translation exists. Several languages are still Beta and may contain untranslated strings. Users can always pick their language under **Settings → Preferences**. **Action:** to restore English-only behavior, set `I18N_ENABLED=false` on GMS.
...
- `SCOPED_RETRY_ENABLED` (default `false`) — Requires optimistic locking. Retry only the conflicted URN's branch on conflict instead of the whole batch.
- `ENTITY_WRITE_LOCK_BACKEND` (default `none`) — `none` | `hazelcast`: per-`(urn, aspect)` write gate that serializes concurrent writers of the same aspect off the DB connection pool (the actual CAS conflict unit — cross-aspect writers on the same URN don't serialize; whole-entity ops like hard-delete lock the entity's full aspect set). Requires optimistic locking; independent of scoped retry.
- `ENTITY_WRITE_LOCK_ACQUIRE_TIMEOUT_SECONDS` (default `10`) — Bounds the write-lock acquire wait before proceeding lockless (CAS still guards).
...
- #18220 **(Ingestion / Unity Catalog)** ML model ingestion controls were fixed. Two things changed:
  ...
  Separately, `MLModel` entities were being emitted with a hardcoded `PROD` environment regardless of the recipe's `env`, while their `MLModelGroup` used the configured `env`. `MLModel` URNs now honor `env`, so groups and models land in the same environment. Models ingested before the fix keep their old `PROD` URNs until re-ingested.

- #17894 **(GMS / metadata service)** Raised Jackson's JSON property-name length limit (`maxNameLength`) on the metadata deserialization paths. Previously only the string-value length limit was raised, leaving `maxNameLength` at Jackson's default of 50,000 characters. Deeply-nested struct field paths (e.g. dbt column-level lineage) can be carried as JSON property names in `upstreamLineage` patches and exceed that, causing GMS to reject the change with `StreamConstraintsException: Name length (N) exceeds the maximum allowed (50000)` when applying the patch. The limit now defaults to 16 MB (matching the existing value-length limit) and is tunable via the new `INGESTION_MAX_SERIALIZED_NAME_LENGTH` environment variable. GMS additionally logs a `WARN` identifying the entity/aspect when a patch carries an over-long property name or string value (the exception itself omits the offending content); these advisory thresholds are tunable via `INGESTION_OVERSIZED_NAME_WARN_THRESHOLD` (default 50,000) and `INGESTION_OVERSIZED_VALUE_WARN_THRESHOLD` (default 10,000,000). Note the string-**value** limit is unchanged (still 16 MB) — only the name limit was raised.
...
- #17860 **(CLI / Python SDK)** Mutual TLS (mTLS) client authentication is now supported for every outbound HTTPS path in the CLI / SDK and any caller that builds a session via `RequestsSessionConfig`. Set `DATAHUB_CLIENT_CERT_PATH` (and optionally `DATAHUB_CLIENT_KEY_PATH` for the kubernetes `tls`-secret layout where cert and key live in separate files) in the process environment and the SDK presents the client certificate during the TLS handshake. No client certificate is sent when neither is configured — fully backwards compatible.
...
- #17927 **(Ingestion / Matillion)** Several improvements to the `matillion-dpc` connector:
  ...
  - **Fixed external links:** pipeline and run links now point at the Data Productivity Cloud (Maia) console (`app.matillion.com`) with a valid query, instead of an invalid `timeFrame` filter. Pipeline links search the pipeline file name only (the console search does not match the full folder path). Streaming pipelines, which have no dedicated console deep-link, no longer emit a link built from the API host that did not resolve.
  ...
  - **`include_external_urls` now defaults to `false`:** Matillion console links (project, run, and pipeline) are no longer emitted unless you opt in with `include_external_urls: true`. On re-ingest, console links emitted by earlier versions are removed. The default was flipped because the pipeline link searches the observability dashboard by file name and only resolves for pipelines that ran recently and whose editor name matches their file name.
  - **Environment and folder containers are always emitted:** `extract_projects_to_containers` now only controls whether the environment and folder containers nest under a top-level project container. Setting it to `false` no longer suppresses those containers — they hang at the root instead.
  - **Multi-environment lineage:** a pipeline path that is published or executed under more than one environment now has its lineage flow and component nested under each of those environments, rather than being attributed to whichever environment was discovered last.
  - **Note on toggling `include_unpublished_pipelines`:** published-only ingestion keys DataFlows on the full pipeline path, while unpublished discovery keys on the base pipeline name, so flipping this flag can change a pipeline's DataFlow/DataProcessInstance URNs and orphan entities emitted under the previous setting. Pick a value per source and keep it stable.
...
- #16912: **(Build / java 25)** DataHub now compiles with **java 25 JDK** and produces **Java 25 bytecode** for the main artifacts; the spark-lineage agent is the exception, targeting Java 17 bytecode for broader Spark runtime compatibility. Self-hosted builds require java 25+.
  ...
  - **For self-hosted deployments:** Build/compile requires java 25 JDK. Runtime environments can use Java 8+ (DataHub produces Java 8 bytecode for compatibility). Spark lineage users must upgrade to Spark 3.5.0+ if using DataHub's Spark integration.
  - **java 25 runtime configuration**: When running DataHub services with java 25, add `--add-opens java.base/java.lang=ALL-UNNAMED --add-opens java.base/java.lang.reflect=ALL-UNNAMED --add-opens java.base/java.nio=ALL-UNNAMED` to `JAVA_OPTS` or your JVM startup command to allow Spark lineage listener reflection-based operations. DataHub Docker images include this automatically.
- #17340: **(Docker Runtime / Java 25 LTS)** Docker images (`datahub-gms`, `datahub-mce-consumer`, `datahub-mae-consumer`, `datahub-upgrade`, `datahub-frontend-react`, `datahub-actions`, and ingestion images) now use **Java 25 LTS runtime** while maintaining **java 25 bytecode compilation** for maximum compatibility. This enables JEP 491 (virtual thread pinning fix in synchronized blocks) and other Java 25 runtime benefits without breaking build toolchain compatibility. Self-hosted users: no action required if using official Docker images. Build system remains on java 25 (Gradle 9.5.0 compatibility); custom deployments compiling from source should follow AGENTS.md for Java version strategy.
- **(Frontend / Play 3 — `DATAHUB_SECRET`)** With the Play 3 upgrade (#17214), `DATAHUB_SECRET` is wired to `play.http.secret.key`. Play 3 rejects application secrets that are unset, left as the default `changeme`, or **shorter than the minimum required for session/flash JWT signing** (default algorithm HS256 needs at least **32 bytes** of key material; see [Play Application secret](https://www.playframework.com/documentation/3.0.x/ApplicationSecret)). If `DATAHUB_SECRET` is too short, **`datahub-frontend` will fail on startup**. Before upgrading, set `DATAHUB_SECRET` to a random value of at least 32 characters (for example `openssl rand -base64 32` or `head -c 32 /dev/urandom | base64`). Helm and other production layouts typically already generate a long secret; this mainly affects ad hoc Docker Compose, copied env files, or hand-crafted short values.
...
- #17466: **(Ingestion / two-tier SQL sources — stored procedure ingestion overhaul.)** Two changes to two-tier SQL connectors (MySQL, MariaDB, Hive, Clickhouse, Teradata, and any other source that extends `TwoTierSQLAlchemySource`):

  1. **URN shape changed (breaking).** Stored-procedure `DataFlow` and `DataJob` URNs no longer duplicate the database name. The flow name now resolves to `{database}.stored_procedures` instead of `{database}.{database}.stored_procedures`. For example, a MariaDB procedure in `test_db` previously emitted `urn:li:dataFlow:(mariadb,test_db.test_db.stored_procedures,PROD)` and now emits `urn:li:dataFlow:(mariadb,test_db.stored_procedures,PROD)`; the corresponding `dataJob` URNs shift accordingly.
  2. **Stored-procedure lineage now actually runs for MySQL/MariaDB (additive).** Native SQL procedures in MySQL/MariaDB store `NULL` in `information_schema.ROUTINES.EXTERNAL_LANGUAGE`, which silently skipped lineage extraction. With this release, `language` defaults to `SQL` when unset and the parser runs over the procedure body. Existing MySQL/MariaDB procedures will start emitting `dataJobInputOutput` aspects with `inputDatasets`, `outputDatasets`, and `fineGrainedLineages` on the next ingestion run. No config change required.

  **What you need to do:** after upgrading, re-run ingestion. Existing `DataFlow` / `DataJob` entities under the old (duplicated) URNs will be orphaned; if you have stateful ingestion with stale-entity removal enabled, the next run will soft-delete them. Otherwise, clean them up with the GraphQL `removeEntities` mutation or the CLI. Any dashboards, saved searches, alerts, or downstream tooling that reference the old `*.{database}.{database}.stored_procedures.*` URNs need to be updated to the new format. Three-tier sources (Postgres, Oracle, MSSQL, DB2, Snowflake, …) are unaffected by the URN change; they already gated procedure language correctly so the lineage change is also a no-op for them.
...
- **(Build / Gradle)** Legacy Gradle properties for overriding artifact download URLs are deprecated in favour of the `datahub.dependencies.*` namespace. Old properties are still honoured but will be removed in a future release. See [Custom Repositories](../developers.md#custom-repositories).
...
- #16342 (Timeline API) The backend `ChangeCategory` enum value `OWNER` has been renamed to `OWNERSHIP` to match the GraphQL `ChangeCategoryType` enum (which has always used `OWNERSHIP`). Clients calling the REST API (`/timeline`) with `OWNER` will still work (backward-compatible alias). The GraphQL schema is unchanged and only recognizes `OWNERSHIP`. **Entity Change Events** (Actions `EntityChangeEvent_v1`, Cloud Entity Events API, EventBridge) serialize this enum into the JSON `category` field, so new events emit `"category": "OWNERSHIP"`. Filters and automations that still match `OWNER` will miss those events and should be updated.
