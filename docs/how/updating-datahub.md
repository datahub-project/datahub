...
- #18670 **(Ingestion / Power BI)** ODBC hierarchical navigation no longer emits truncated two-part upstream URNs for three-tier platforms. When Power BI ODBC navigation (plus optional `dsn_to_database_schema` backfill) yields a database and table but no schema, the connector previously emitted `database.table` for every platform. That matched two-part connectors (MySQL, Athena, Teradata, Oracle's default `schema.table`, …) but produced dangling URNs for platforms whose standalone connectors use `database.schema.table` (or BigQuery's `project.dataset.table`). After upgrade, the following platforms **warn and skip** the upstream instead of emitting the truncated URN: **BigQuery, Snowflake, Postgres, MSSQL, Redshift, Databricks, and Db2**. Other ODBC platforms keep the previous `database.table` fallback, except **Athena**: its navigation now emits `database.table` (stripping the leading `Kind=Database` catalog such as `awsdatacatalog` to match the standalone Athena connector) rather than the previous `catalog.database.table`, and warns+skips a catalog-and-table navigation with no resolvable database instead of emitting a truncated `catalog.table`. The two-tier platforms **Oracle and Hive** also emit `schema.table` (dropping any leading `Kind=Database` pseudo-catalog) and warn+skip when navigation yields a database and table but no resolvable schema. Oracle additionally emits `database.schema.table` when `default_database` is set for that server under `server_to_platform_instance`, so affected users should ensure that value matches an Oracle ingestion configured with `add_database_name_to_urn: true`. The mapping key is the server DataHub extracts from the connection, which for a DSN-only ODBC source is the DSN name (not a TNS alias) — e.g. `server_to_platform_instance: { my_oracle_dsn: { default_database: MYDB } }`. This opt-in is only honored when the recipe uses `server_to_platform_instance`; recipes still on the deprecated `dataset_type_mapping` never consult `default_database`. Existing truncated edges for the affected platforms will not be re-emitted; with stateful ingestion enabled they may be soft-deleted as stale. **Action:** for affected ODBC sources, ensure navigation or `dsn_to_database_schema` supplies the missing tier — the schema (or project/dataset for BigQuery; the database for Athena, whose real database sits below the strippable `Kind=Database` catalog) — so a full name can be built, for example `dsn_to_database_schema: { my_dsn: analytics.public }`, a one-part mapping plus Schema+Table navigation, or a one-part `dsn_to_database_schema: { my_athena_dsn: my_database }` for Athena. Check ingestion warnings titled "Can not determine qualified table name" (or "Cannot build two-tier ODBC table name" for Oracle/Hive) for which DSNs need a mapping. **Athena is affected by these changes and may need a recipe update** (a catalog-and-table navigation with no resolvable database now skips instead of emitting `catalog.table`); no recipe change is required for MySQL, Teradata, or other two-part platforms whose URN shape is unchanged.
