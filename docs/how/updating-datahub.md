...
- #19715 **(Ingestion / LookML remote dependencies)** The LookML source now reads `remote_dependency` and `local_dependency` entries declared in `manifest.lkml`; a key-name bug previously left both lists empty. `remote_dependency` entries are cloned from a URL any LookML committer can change, so the executor now makes outbound `git clone` calls it did not make before. `local_dependency` entries are local project references and are not cloned, but those local projects are now traversed, so LookML `constant:` values defined in their manifests now resolve where they did not before. If your views reference such a constant in `sql_table_name`, the upstream dataset URN for those views changes on this upgrade (previously the constant stayed unresolved and logged a "LookML constant not found" warning), which moves the affected lineage. Each URL is validated first: only `https` and SSH Git URLs are allowed, and loopback, link-local, and cloud-metadata hosts are always rejected; anything else is skipped with a warning. The optional `remote_dependency_domain_pattern` (an `AllowDenyPattern`, default allow-all) restricts which hosts may be cloned. **Action:** if your `manifest.lkml` declares `remote_dependency` URLs, set `remote_dependency_domain_pattern` to the hosts you expect. The default denylist covers only loopback, link-local, and cloud-metadata addresses, so internal or private-network hosts stay reachable; and because git re-resolves the host at clone time, restricting executor network egress to your expected hosts (not the allowlist alone) is what closes DNS rebinding.
