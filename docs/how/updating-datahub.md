...
- (Ingestion / Databricks Unity Catalog) New opt-in flag `include_metric_views` (default `false`) for Unity Catalog Metric Views. When enabled, metric views are emitted with subtype `Metric View` (instead of `Table`), their YAML body is preserved as a `ViewProperties` aspect with `viewLanguage: YAML`, and upstream lineage is derived from the YAML `source` and `joins[].source` fields when those use fully qualified `catalog.schema.table` identifiers; SQL subqueries, 2-part names, and backtick-quoted identifiers are skipped in v1 and the existing table-lineage REST API is used as fallback. Column-level lineage is **not** emitted for metric views in v1 (`include_column_lineage` has no effect on metric views even when enabled). The default flag value is `false` so the upgrade is byte-for-byte identical to the previous release; recipes that want the proper metric-view treatment must opt in explicitly. Requires a `databricks-sdk` recent enough to expose `TableType.METRIC_VIEW` (older SDKs short-circuit silently and the flag has no effect; a startup warning is emitted in that case).
