...
- **(GMS / MAE / MCE / System Update)** DataHub no longer produces or consumes `DataHubUpgradeHistory_v1` (or the pgQueue equivalent). GMS, MAE, and MCE startup no longer wait for that marker. New code can serve mixed `schemaVersion` SQL via ZDU read (and write) mutators; `MigrateAspects` already runs as a non-blocking sweep after pods are live. Compose still starts those services after blocking system-update (`depends_on` / `service_completed_successfully`). Helm/Argo pre-hooks remain the place to finish blocking index work before a rolling update. **`helm rollback` is not a reason to keep this topic:** a rolled-back image reads its own indices, the `{gitVersion}-{revision}` payload often does not change unless Helm bumps `DATAHUB_REVISION`, and dedicated ZDU rollback work owns a real downgrade path. Existing clusters do **not** need to delete the Kafka topic; leftover messages with internal schema id 8 will not deserialize and can be ignored. Unused env vars `DATAHUB_UPGRADE_HISTORY_*` and `BOOTSTRAP_SYSTEM_UPDATE_WAIT_FOR_SYSTEM_UPDATE` are no-ops and can be dropped from values. Internal schema id **8** is reserved (`RESERVED_8`) and must not be reused.
