...
- #19677 **(Documents / Semantic search)** Editing a document's text in the UI now also updates its semantic-search text. A document can carry a `semanticText` aspect, which holds a body that semantic search uses instead of the document's own text. Until now each caller had to update both, and the UI did not, so semantic search kept answering from the old text. A body edit through the UI or the GraphQL API now sets the semantic-search text to the new body. **Action for API clients:** a client that wants its own semantic-search text must pass `semanticText` on every call that changes the body; a call that changes the body without passing one replaces the stored value with the body. A title-only edit passes no body and changes nothing. Writes that go straight to the `documentInfo` aspect through the SDK, OpenAPI or Rest.li are not covered and still update the body on their own. Separately, a blank (whitespace-only) `semanticText` used to take priority and be embedded as whitespace, so the document could not be found even though its body was fine. A blank value now counts as missing everywhere the text to embed is chosen.
