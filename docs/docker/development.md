...
during development.  This file, alone with several others are generated by a python script `generate_docker_quickstart.py`, 
which is in turn, called by the shell script `generate_docker_quickstart.sh` in the folder `docker/quickstart` folder.
