...
| AUTH_OIDC_PRIVATE_KEY_JWT_KID       | Explicit value for the JWT `kid` header. Set it to the key ID assigned by your identity provider when that differs from the certificate thumbprint. If unset, DataHub uses the base64url SHA-256 certificate thumbprint.                          | null    |
