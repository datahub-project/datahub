...
uv pip compile --universal --python-version 3.11 -o requirements.txt requirements-local.in $@
uv pip compile --universal --python-version 3.11 -o requirements-dev.txt --extra dev pyproject.toml requirements-local.in requirements.txt $@
