...
class TestMakeTuningDecision:
    ...
    def test_make_tuning_decision_score_dropped_anomaly(self) -> None:
        ...
        # Use a score above threshold (0.3) but dropped significantly
        decision = make_tuning_decision(
            current_anomaly_score=0.4,
            previous_anomaly_score=0.8,  # Significant drop (0.4 drop > 0.25 threshold)
            has_previous_config=True,
            anomaly_score_threshold=0.3,  # Above current score
            drop_threshold=0.25,
        )
