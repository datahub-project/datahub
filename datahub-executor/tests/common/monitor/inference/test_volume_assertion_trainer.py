...
def mock_anomalies_data() -> List[Anomaly]:
    ...
    for i in range(10):
        ...
        anomaly.metric = Metric(
            value=1000 + value_factor * 100, timestamp_ms=timestamp_ms
        )
...
def test_get_metric_data_with_prefetched_metrics(
    ...
    assert result == coalesce_metrics(mock_metrics_data, mock_prefetched_metrics_data)
...
def test_train_and_update_assertion(
    ...
    mock_dependencies[
        ...
        mock_metrics_data, timedelta(hours=1), 48, sensitivity, floor_value=0.0
    )
