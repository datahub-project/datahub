...
public class EntityClient {
  ...
  public Entity get(@Nonnull final Urn urn) throws RemoteInvocationException {
    final GetRequest<Entity> getRequest = ENTITIES_REQUEST_BUILDERS.get().id(urn.toString()).build();
  ...
  public Map<Urn, Entity> batchGet(@Nonnull final Set<Urn> urns) throws RemoteInvocationException {
    ...
    for (List<Urn> urnsInBatch : entityUrnBatches) {
      BatchGetEntityRequest<String, Entity> batchGetRequest = ENTITIES_REQUEST_BUILDERS.batchGet()
          .ids(urnsInBatch.stream().map(urn -> urn.toString()).collect(Collectors.toSet()))
          .build();
      final Map<Urn, Entity> batchResponse = sendClientRequest(batchGetRequest).getEntity()
          .getResults()
          .entrySet()
          .stream()
  ...
  @Nonnull
  public AutoCompleteResult autoComplete(@Nonnull String entityType, @Nonnull String query,
      @Nonnull Map<String, String> requestFilters, @Nonnull int limit) throws RemoteInvocationException {
    EntitiesDoAutocompleteRequestBuilder requestBuilder = ENTITIES_REQUEST_BUILDERS.actionAutocomplete()
        .entityParam(entityType)
        .queryParam(query)
        .filterParam(newFilter(requestFilters))
  ...
  public BrowseResult browse(@Nonnull String entityType, @Nonnull String path,
      @Nullable Map<String, String> requestFilters, int start, int limit) throws RemoteInvocationException {
  ...
  public void update(@Nonnull final Entity entity) throws RemoteInvocationException {
    ...
    final FieldDef<?> entityFieldDef =
        new FieldDef<>(PARAM_ENTITY, Entity.class, DataTemplateUtil.getSchema(String.class));
  ...
  public SearchResult search(@Nonnull String entity, @Nonnull String input, @Nullable StringArray aspectNames,
      @Nullable Map<String, String> requestFilters, int start, int count) throws RemoteInvocationException {
