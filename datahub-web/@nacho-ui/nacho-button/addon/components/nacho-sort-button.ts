...
import template from '../templates/components/nacho-sort-button';
...
@layout(template)
@tagName('button')
@classNames('nacho-button', buttonClass)
export default class NachoSortButton extends Component {
  ...
  constructor() {
    ...
    typeof this.baseClass === 'string' || (this.baseClass = '');
    typeof this.onChange === 'function' || (this.onChange = noop);
    typeof this.isSorting === 'boolean' || (this.isSorting = false);
    typeof this.sortDirection === 'string' || (this.sortDirection = SortDirection.ASCEND);
  }
