...
import EntityListContainer from '@datahub/lists/components/entity-list-container';
...
module('Integration | Component | entity-list-container', function(hooks): void {
    ...
    stubService(
      ...
      (() => {
        ...
        return {
          ...
          removeFromList(instance: IStoredEntityAttrs) {
  ...
  test('container component attributes', async function(assert): Promise<void> {
    const component = await getRenderedComponent({
      ...
      testContext: this,
      template: hbs`<EntityListContainer @entityType={{this.entityType}} />`
    });
  ...
  test('component yield values', async function(assert): Promise<void> {
    const component = await getRenderedComponent({
      ...
      testContext: this,
      template: hbs`
      <EntityListContainer @entityType={{this.entityType}}  as |container|>
        <div class='test-div'>
          {{container.name}}
          {{container.count}}
