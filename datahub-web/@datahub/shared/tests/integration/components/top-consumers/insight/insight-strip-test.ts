...
import { baseClass, PREVIEW_LIMIT } from '@datahub/shared/components/top-consumers/insight/insight-strip';
...
module('Integration | Component | top-consumers/insight/insight-strip', function(hooks): void {
  ...
  test('it renders and behaves as expected', async function(assert): Promise<void> {
    ...
    await render(hbs`
      <TopConsumers::Insight::InsightStrip
        @title={{this.title}}
