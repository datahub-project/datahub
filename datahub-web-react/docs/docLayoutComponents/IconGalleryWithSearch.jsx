...
import { IconGrid, IconGridItem, IconDisplayBlock } from "./DocsComponents";

export const IconGalleryWithSearch = ({ icons }) => {
	...
	return (
		<>
			...
			<div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px' }}>
				...
				<div style={{ display: 'flex', gap: '8px' }}>
					<Button
						onClick={() => setVariant(variant === 'outline' ? 'filled' : 'outline')}
						{...smButtonProps}
					>
				...
				{filteredIcons.map((icon) => (
					<IconGridItem>
						<IconDisplayBlock key={icon} title={icon}>
							<Icon icon={icon} variant={variant} size="2xl" />
						</IconDisplayBlock>
						<span>{icon}</span>
					</IconGridItem>
				))}
