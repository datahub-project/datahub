...
describe('Phase 1: Bundle Optimization', () => {
    ...
    const phosphorIconsPath = path.join(
        projectRoot,
        'src/alchemy-components/components/Icon/phosphor-icons.ts',
    );
    ...
    describe('Icon Import Validation', () => {
        test('no files should import directly from @phosphor-icons/react', () => {
            ...
            files.forEach((file) => {
                ...
                const directImportMatch = content.match(
                    /import\s+(?!type\s).*from ['"]@phosphor-icons\/react['"]/,
                );
        ...
        test('all imported icons should be exported in custom bundle', () => {
            ...
            if (missingIcons.size > 0) {
                const missingList = Array.from(missingIcons)
                    ...
                    .map(
                        (icon) =>
                            `export { ${icon} } from '@phosphor-icons/react/dist/csr/${icon}';`,
                    )
                    .join('\n');

                throw new Error(
                    `Found ${missingIcons.size} missing icon exports:\n\n${missingList}`,
                );
            }
        ...
        test('all dynamic icon references should be in custom bundle', () => {
            ...
            if (dynamicMissingIcons.size > 0) {
                ...
                throw new Error(
                    `Found ${dynamicMissingIcons.size} missing dynamic icon references:\n\n${missingList}`,
                );
            }
    ...
    describe('Custom Icon Bundle', () => {
        ...
        test('custom bundle should use CSR imports for tree-shaking', () => {
            ...
            const importMatches = content.matchAll(
                /from ['"]@phosphor-icons\/react\/dist\/csr\/\w+['"]/g,
            );
    ...
    describe('Bundle Size Validation (requires build)', () => {
        ...
        test.skipIf(!fs.existsSync(distPath))(
            'total bundle should be under 15 MB minified',
            () => {
    ...
    describe('Chunk Generation (requires build)', () => {
        ...
        test.skipIf(!fs.existsSync(distPath))(
            'should generate expected vendor chunks',
            () => {
                const files = fs.readdirSync(distPath);
                const jsFiles = files.filter((f) => f.endsWith('.js'));
        ...
        test.skipIf(!fs.existsSync(distPath))(
            'framework chunk should be smaller than vendor chunk',
            () => {
                const files = fs.readdirSync(distPath);

                const frameworkFile = files.find((f) => f.includes('framework'));
                const vendorFile = files.find((f) => f.includes('vendor') && !f.includes('-vendor'));

                expect(frameworkFile).toBeDefined();
                expect(vendorFile).toBeDefined();

                const frameworkSize = fs.statSync(
                    path.join(distPath, frameworkFile!),
                ).size;
