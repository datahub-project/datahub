...
export const useComprehensiveImport = ({ onProgress }: UseComprehensiveImportProps): UseComprehensiveImportReturn => {
    ...
    const { executePatchEntitiesMutation, executeGetOwnershipTypesQuery, executeAddRelatedTermsMutation } =
    ...
    const updateProgress = useCallback(
        (updates: Partial<ComprehensiveImportProgress>) => {
            setProgress((prev) => {
    ...
    const executeComprehensiveImport = useCallback(
        async (
            allEntities: Entity[],
    ...
    const startImport = useCallback(
        async (entities: Entity[], existingEntities: Entity[]): Promise<void> => {
            if (isProcessing) {
                return;
            }
