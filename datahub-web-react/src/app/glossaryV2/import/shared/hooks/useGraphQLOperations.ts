                ...
                entity {
                    ...
                    ... on GlossaryTerm {
                        ...
                        contains: relationships(
                            input: { types: ["HasA"], direction: OUTGOING, start: 0, count: 1000 }
                        ) {
                            relationships {
                                entity {
                                    ... on GlossaryTerm {
                        ...
                        relatedTerms: relationships(
                            input: { types: ["RelatedTo"], direction: OUTGOING, start: 0, count: 1000 }
                        ) {
                            relationships {
                                entity {
                                    ... on GlossaryTerm {
...
export function useGraphQLOperations(): UseGraphQLOperationsReturn {
    ...
    const executeUnifiedGlossaryQuery = useCallback(
        async (variables: {
            input: {
                types: string[];
                query: string;
            ...
            try {
                ...
                while (hasMore) {
                    ...
                    const results =
                        data?.scrollAcrossEntities?.searchResults?.map((result: any) => result.entity) || [];
                ...
                if (error.networkError) {
                    throw new Error(
                        `Network error: ${error.networkError.message || 'Failed to connect to server'}. The query may be too complex or the server may be unresponsive.`,
                    );
                }

                if (error.message?.includes('JSON')) {
                    throw new Error(
                        'Server returned invalid response. This may be due to a timeout or server error. Try reducing the amount of data being fetched.',
                    );
                }
    ...
    const executePatchEntitiesMutation = useCallback(
        async (input: EntityPatchInput[]): Promise<any> => {
            try {
    ...
    const executeAddRelatedTermsMutation = useCallback(
    ...
    const executeSetDomainMutation = useCallback(
        async (entityUrn: string, domainUrn: string): Promise<any> => {
            try {
                const { data } = await apolloClient.mutate({
    ...
    const executeBatchSetDomainMutation = useCallback(
        async (domainUrn: string, entityUrns: string[]): Promise<any> => {
            try {
                const { data } = await apolloClient.mutate({
    ...
    const executeGetOwnershipTypesQuery = useCallback(
        async (variables: {
            input: {
                start: number;
                count: number;
...
export { UNIFIED_GLOSSARY_QUERY, PATCH_ENTITIES_MUTATION, GET_OWNERSHIP_TYPES };
