...
describe('ownershipParsingUtils', () => {
    ...
    describe('createOwnershipPatchOperations', () => {
        ...
        it('should create patch operations for multiple ownerships', () => {
            ...
            expect(result[0].path).toBe('/owners');
            const ownersArray = JSON.parse(result[0].value!);
            expect(ownersArray).toHaveLength(2);
            expect(ownersArray[0].owner).toBe('urn:li:corpuser:admin');
            expect(ownersArray[1].owner).toBe('urn:li:corpGroup:bfoo');
        });
    ...
    describe('integration tests', () => {
        it('should handle the complete workflow from CSV string to patch operations', () => {
            ...
            const ownershipTypeMap = new Map([['technical owner', 'urn:li:ownershipType:technical-urn']]);
            ...
            expect(patches[0].path).toBe('/owners');
            const ownersArray = JSON.parse(patches[0].value!);
            expect(ownersArray).toHaveLength(2);
            expect(ownersArray[0].owner).toBe('urn:li:corpuser:bfoo');
            expect(ownersArray[1].owner).toBe('urn:li:corpuser:datahub');
        });
