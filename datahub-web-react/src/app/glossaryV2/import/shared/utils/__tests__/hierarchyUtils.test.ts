...
describe('HierarchyNameResolver', () => {
    describe('parseHierarchicalName', () => {
        it('should return the last part of a hierarchical name', () => {
            expect(HierarchyNameResolver.parseHierarchicalName('Business Terms.Business Terms Nested')).toBe(
                'Business Terms Nested',
            );
        });

        it('should return the original name if no dots', () => {
            expect(HierarchyNameResolver.parseHierarchicalName('Simple Name')).toBe('Simple Name');
        });

        it('should handle multiple levels', () => {
            expect(HierarchyNameResolver.parseHierarchicalName('Level1.Level2.Level3')).toBe('Level3');
        });

        it('should handle empty or whitespace names', () => {
            expect(HierarchyNameResolver.parseHierarchicalName('')).toBe('');
            expect(HierarchyNameResolver.parseHierarchicalName('   ')).toBe('');
        });

        it('should trim whitespace from parts', () => {
            expect(HierarchyNameResolver.parseHierarchicalName(' Parent . Child ')).toBe('Child');
        });
    ...
    describe('findParentEntity', () => {
        it('should find parent by hierarchical name', () => {
            const result = HierarchyNameResolver.findParentEntity('Business Terms.Business Terms Nested', mockEntities);
            expect(result).toBeDefined();
            expect(result?.name).toBe('Business Terms Nested');
        });

        it('should find parent by simple name', () => {
            const result = HierarchyNameResolver.findParentEntity('Business Terms', mockEntities);
            expect(result).toBeDefined();
            expect(result?.name).toBe('Business Terms');
        });

        it('should return undefined if parent not found', () => {
            const result = HierarchyNameResolver.findParentEntity('Non Existent Parent', mockEntities);
    ...
    describe('findParentEntityCaseInsensitive', () => {
        it('should find parent with case-insensitive search', () => {
            const result = HierarchyNameResolver.findParentEntityCaseInsensitive('business terms', mockEntities);
            expect(result).toBeDefined();
            expect(result?.name).toBe('Business Terms');
        });
    ...
    describe('resolveParentUrns', () => {
        ...
        it('should handle missing parent entities', () => {
            ...
            const result = HierarchyNameResolver.resolveParentUrns(entityWithMissingParent, mockEntities, new Map());
    ...
    describe('getPathParts', () => {
        it('should split hierarchical name into parts', () => {
            expect(HierarchyNameResolver.getPathParts('Parent.Child')).toEqual(['Parent', 'Child']);
        });

        it('should handle multiple levels', () => {
            expect(HierarchyNameResolver.getPathParts('A.B.C.D')).toEqual(['A', 'B', 'C', 'D']);
        });
    ...
    describe('buildHierarchicalName', () => {
        it('should build hierarchical name from parts', () => {
            expect(HierarchyNameResolver.buildHierarchicalName(['Parent', 'Child'])).toBe('Parent.Child');
        });
