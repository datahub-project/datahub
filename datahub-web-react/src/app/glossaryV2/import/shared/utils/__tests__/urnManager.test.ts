...
import { AUTO_GENERATE_ALLOWED_ENTITY_TYPES, UrnManager } from '@app/glossaryV2/import/shared/utils/urnManager';
...
describe('UrnManager', () => {
    ...
    describe('generateEntityUrn', () => {
        ...
        it('should throw error for unsupported entity type', () => {
            expect(() => UrnManager.generateEntityUrn('dataset')).toThrow(
    ...
    describe('generateOwnershipTypeUrn', () => {
        it('should generate URN with sanitized name', () => {
            expect(UrnManager.generateOwnershipTypeUrn('Technical Owner')).toBe('urn:li:ownershipType:technical-owner');
        });

        it('should handle special characters', () => {
            expect(UrnManager.generateOwnershipTypeUrn('Data Owner (Primary)')).toBe(
                'urn:li:ownershipType:data-owner--primary-',
            );
        });

        it('should convert to lowercase', () => {
            expect(UrnManager.generateOwnershipTypeUrn('DEVELOPER')).toBe('urn:li:ownershipType:developer');
        });
    });
    ...
    describe('isValidUrn', () => {
        ...
        it('should handle URNs with multiple colons in ID', () => {
            expect(UrnManager.isValidUrn('urn:li:dataset:(urn:li:dataPlatform:kafka,topic.name,PROD)')).toBe(true);
        });
    });

    describe('extractEntityTypeFromUrn', () => {
        it('should extract entity type from valid URN', () => {
            expect(UrnManager.extractEntityTypeFromUrn('urn:li:glossaryTerm:abc-123')).toBe('glossaryTerm');
            expect(UrnManager.extractEntityTypeFromUrn('urn:li:glossaryNode:node-1')).toBe('glossaryNode');
        });
    ...
    describe('extractEntityIdFromUrn', () => {
        it('should extract ID from valid URN', () => {
            expect(UrnManager.extractEntityIdFromUrn('urn:li:glossaryTerm:abc-123')).toBe('abc-123');
            expect(UrnManager.extractEntityIdFromUrn('urn:li:glossaryNode:node-1')).toBe('node-1');
        });

        it('should handle complex IDs with colons', () => {
            expect(
                UrnManager.extractEntityIdFromUrn('urn:li:dataset:(urn:li:dataPlatform:kafka,topic.name,PROD)'),
    ...
    describe('resolveEntityUrn', () => {
        ...
        it('should throw error if no URN available', () => {
            ...
            expect(() => UrnManager.resolveEntityUrn(entity, urnMap)).toThrow(
                'No URN available for entity Entity 1 (entity-1)',
            );
        });
    });
