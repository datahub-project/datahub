...
const TIERED_SCOPE_TYPES_MAP = {
    ...
    ACTIONS_AND_DESCRIPTION: [AssertionEditabilityScopeType.ACTIONS_AND_DESCRIPTION, AssertionEditabilityScopeType.FULL],
    ACTIONS_ONLY: [AssertionEditabilityScopeType.ACTIONS_ONLY, AssertionEditabilityScopeType.ACTIONS_AND_DESCRIPTION, AssertionEditabilityScopeType.FULL],
}
...
export const getAssertionEditabilityType = (assertion: Assertion): AssertionEditabilityScopeType => {
    ...
    // 1. Full editing
    const isSourceTypeValidForFullEditing = TIERED_SCOPE_TYPES_MAP.FULL.includes(SOURCE_TYPE_TO_EDITING_SCOPE[assertionSourceType])
    const isAssertionTypeValidForFullEditing = TIERED_SCOPE_TYPES_MAP.FULL.includes(ASSERTION_TYPE_TO_EDITING_SCOPE[assertionType])
    ...
    const isSourceTypeValidForActionAndDescriptionEditing = TIERED_SCOPE_TYPES_MAP.ACTIONS_AND_DESCRIPTION.includes(SOURCE_TYPE_TO_EDITING_SCOPE[assertionSourceType])
    const isAssertionTypeValidForActionAndDescriptionEditing = TIERED_SCOPE_TYPES_MAP.ACTIONS_AND_DESCRIPTION.includes(ASSERTION_TYPE_TO_EDITING_SCOPE[assertionType])
    ...
    const isSourceTypeValidForActionOnlyEditing = TIERED_SCOPE_TYPES_MAP.ACTIONS_ONLY.includes(SOURCE_TYPE_TO_EDITING_SCOPE[assertionSourceType]);
    const isAssertionTypeValidForActionOnlyEditing = TIERED_SCOPE_TYPES_MAP.ACTIONS_ONLY.includes(ASSERTION_TYPE_TO_EDITING_SCOPE[assertionType])
