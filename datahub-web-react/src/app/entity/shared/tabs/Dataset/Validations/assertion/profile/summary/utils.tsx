    ...
    if (assertionInfo?.description) {
        ...
        switch (assertionInfo?.type) {
            ...
            case AssertionType.Field:
                primaryLabel = <FieldAssertionDescription assertionInfo={assertionInfo.fieldAssertion as FieldAssertionInfo} />
                break;
            case AssertionType.DataSchema:
                primaryLabel = <SchemaAssertionDescription assertionInfo={assertionInfo.schemaAssertion as SchemaAssertionInfo} />
                break;
            default:
                break;
        }
...
const useBuildSecondaryLabel = (assertionInfo?: Maybe<AssertionInfo>): JSX.Element | null => {
    ...
    if (createdActor?.corpUser && entityRegistry.getDisplayName(EntityType.CorpUser, createdActor.corpUser)) {
        secondaryLabelMessage = `Created by ${entityRegistry.getDisplayName(EntityType.CorpUser, createdActor.corpUser)}`;

        tooltipCreatedByMessage = `Created by: ${entityRegistry.getDisplayName(EntityType.CorpUser, createdActor.corpUser)}${createdActor.corpUser?.info?.email ? ` (${createdActor.corpUser.info.email})` : ''
            } on ${assertionInfo?.source?.created?.time
