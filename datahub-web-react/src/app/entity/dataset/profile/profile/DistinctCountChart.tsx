...
export default function DistinctCountChart({ profiles, fieldPath, interval, dateRange }: Props) {
    const data = profiles
        .map((profile) => {
            ...
            const fieldProfiles = profile.fieldProfiles
                ...
                .filter((field) => field.uniqueCount !== null && field.uniqueCount !== undefined);

            if (fieldProfiles?.length === 1) {
                const fieldProfile = fieldProfiles[0];
                return {
                    x: dateStr,
                    y: fieldProfile.uniqueCount as number,
                };
            }
            return null;
        })
