...
import SelectRoleV2 from '@app/identityV2/user/SelectRoleV2';
...
export default function UserListItemV2({ user, canManageUserCredentials, selectRoleOptions, onDelete, refetch }: Props) {
    ...
    const items = [
        {
            key: 'reset',
            label: (
                <MenuItemStyle disabled={!shouldShowPasswordReset} onClick={() => setIsViewingResetToken(true)} data-testid="reset-menu-item">
        ...
        <List.Item>
            <UserItemContainer>
                <Link to={entityRegistry.getEntityUrl(EntityType.CorpUser, user.urn)}>
                    <UserHeaderContainer>
                        <CustomAvatar size={32} name={displayName} photoUrl={user.editableProperties?.pictureLink || undefined} />
            ...
            <ButtonGroup id={USERS_ASSIGN_ROLE_ID}>
                <SelectRoleV2 user={user} userRoleUrn={userRoleUrn || ''} selectRoleOptions={selectRoleOptions} refetch={refetch} />
                <Dropdown trigger={["click"]} menu={{ items }}>
                    <MenuIcon fontSize={20} data-testid={`userItem-${shouldShowPasswordReset ? 'native' : 'non-native'}`} />
                </Dropdown>
            </ButtonGroup>
            <ViewResetTokenModal open={isViewingResetToken} userUrn={user.urn} username={user.username} onClose={() => setIsViewingResetToken(false)} />
        </List.Item>
    );
}
