...
import { USERS_ASSIGN_ROLE_ID, USERS_INTRO_ID, USERS_INVITE_LINK_ID, USERS_SSO_ID } from '@app/onboarding/config/UsersOnboardingConfig';
...
import UserListItemV2 from '@app/identityV2/user/UserListItemV2';
...
export const UserListV2 = () => {
    ...
    const [isViewingInviteUsers, setIsViewingInviteUsers] = useState(false);
    ...
    const { loading: usersLoading, error: usersError, data: usersData, client, refetch: usersRefetch } = useListUsersQuery({
        variables: {
            input: {
                start,
                count: pageSize,
    ...
    const { loading: rolesLoading, error: rolesError, data: rolesData } = useListRolesQuery({
    ...
    return (
        <>
            ...
            <UserContainer>
                <TabToolbar>
                    <div>
                        <Button id={USERS_INVITE_LINK_ID} disabled={!canManagePolicies} type="text" onClick={() => setIsViewingInviteUsers(true)}>
                            <UserPlus size={16} /> Invite Users
                        </Button>
                    </div>
                    <SearchBar
                        initialQuery={query || ''}
                ...
                {canManagePolicies && <InviteUsersModal open={isViewingInviteUsers} onClose={() => setIsViewingInviteUsers(false)} />}
            </UserContainer>
        </>
    );
};
