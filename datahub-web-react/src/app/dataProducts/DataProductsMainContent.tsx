...
const DataProductsModuleHeader = styled(ModuleHeader)`
    &:hover {
...
/**
 * DataProductsMainContent - Landing page at /dataProducts.
 *
 * Summary stats plus a single recent-data-products module (Metrics-style list rows).
 */
export default function DataProductsMainContent() {
    ...
    if (loading) {
        ...
        body = (
            <EmptyListHint>
                <EmptyState
                    icon={Storefront}
                    title={t('dataProducts.homeLoadErrorTitle')}
                    description={t('dataProducts.homeLoadErrorDescription')}
                    size="lg"
                />
            </EmptyListHint>
        );
        ...
        body = (
            <EmptyListHint>
                <EmptyState
                    icon={Storefront}
                    title={t('dataProducts.homeEmptyTitle')}
                    description={t('dataProducts.homeEmptyDescription')}
                    size="lg"
                    action={{
                        label: t('dataProducts.homeEmptyAction'),
                        onClick: openCreateModal,
                        dataTestId: 'data-products-create-data-product-cta',
                    }}
                />
            </EmptyListHint>
        );
        ...
        body = (
            <>
                <SummaryCards>
                    <Card
                        dataTestId="data-products-count-products"
                        icon={<SummaryStatIconBadge icon={Storefront} tone="brand" />}
                        style={cardStyle}
                        title={String(displayedTotalProducts)}
                        subTitle={t('dataProducts.totalDataProducts')}
                    />
                    <Card
                        dataTestId="data-products-count-applications"
                        icon={<SummaryStatIconBadge icon={AppWindow} tone="info" />}
                        style={cardStyle}
                        title={String(applicationCount)}
                        subTitle={t('dataProducts.sourceApplications')}
                    />
                    <Card
                        dataTestId="data-products-latest-update"
                        icon={<SummaryStatIconBadge icon={Clock} tone="neutral" />}
                        style={cardStyle}
                        title={latestCreatedLabel ?? '—'}
                        subTitle={t('dataProducts.lastCreated')}
                    />
                </SummaryCards>

                <RecentProductsModule $height="316px" data-testid="data-products-recent-products">
                    <DataProductsModuleHeader>
                        <ModuleName text={t('dataProducts.recentDataProducts')} />
                    </DataProductsModuleHeader>
                    <ModuleContent $hasFooter={recentProducts.length > MAX_RECENT}>
                        {recentProducts.length === 0 ? (
                            <EmptyListHint>
                                <EmptyState icon={Storefront} title={t('dataProducts.emptyTreeTitle')} size="sm" />
                            </EmptyListHint>
                        ) : (
                            visibleProducts.map((product) => (
                                <AutoCompleteEntityItem
                                    ...
                                    hideMatches
                                    dataTestId={`data-products-recent-product-${product.urn}`}
                                />
                            ))
                        )}
                    </ModuleContent>
    ...
    return (
        <ContentCard data-testid="data-products-main-content">
            <PageHeader>
                <PageTitle
                    title={t('dataProducts.homeTitle')}
                    subTitle={t('dataProducts.homeBlurb')}
                    actionButton={{
