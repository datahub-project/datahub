...
import { buildDataProductsSidebarFilters } from '@app/dataProducts/utils/dataProductsSidebarFilters';
import { dataProductsSidebarSearchQuery } from '@app/dataProducts/utils/dataProductsSidebarMode';
...
const DATA_PRODUCTS_SIDEBAR_SEARCH_COUNT = 50;
...
/**
 * Flat data products sidebar search via scrollAcrossEntities.
 * Spans all data products at every depth — used when query and/or filters are active.
 */
export default function useDataProductsSidebarSearch({
    searchQuery,
    domainUrns = [],
    tagUrns = [],
    termUrns = [],
    ...
    const query = dataProductsSidebarSearchQuery(searchQuery);
    ...
    const {
        ...
        variables: {
            input: {
                ...
                count: DATA_PRODUCTS_SIDEBAR_SEARCH_COUNT,
