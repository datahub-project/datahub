...
export const AddOrganizationMembersModal = ({ urn, visible, onCloseModal, onSubmit }: Props) => {
    const entityRegistry = useEntityRegistry();
    const [selectedMembers, setSelectedMembers] = useState<any[]>([]);
    const [inputValue, setInputValue] = useState('');
    const [addUserToOrganizationsMutation] = useAddUserToOrganizationsMutation();
    ...
    const onAdd = async () => {
        ...
        try {
            // We need to call the mutation for each user, as the mutation is addUserToOrganizations (singular user, plural orgs)
            ...
            if (e instanceof Error) {
                message.error({ content: `Failed to add members: \n ${e.message || ''}`, duration: 3 });
            }
        } finally {
            onSubmit();
            onModalClose();
