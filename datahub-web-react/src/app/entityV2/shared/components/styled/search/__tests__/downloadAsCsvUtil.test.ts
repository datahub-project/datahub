...
describe('downloadAsCsvUtil', () => {
    ...
    describe('transformGenericEntityPropertiesToCsvRow', () => {
        ...
        it('should include degree when present in result', () => {
            ...
            expect(result).toHaveLength(20); // 19 base fields + 1 degree field
            expect(result[19]).toBe('3'); // degree field
        });
        ...
        it('should not include degree when not present in result', () => {
            ...
            expect(result).toHaveLength(19); // 19 base fields only
        });

        it('should handle stat summary fields for datasets', () => {
            const propertiesWithDatasetStats = {
                ...mockProperties,
                statsSummary: {
                    rowCount: 1000,
                    sizeInBytes: 50000,
                    queryCountLast30Days: 25,
                    uniqueUserCountLast30Days: 5,
                },
            };
            ...
            const result = transformGenericEntityPropertiesToCsvRow(
                mockEntityRegistry,
                propertiesWithDatasetStats as any,
                entityUrl,
                mockResult,
            );

            expect(result[14]).toBe(''); // view count (empty for datasets)
            expect(result[15]).toBe('5'); // unique users
            expect(result[16]).toBe('1000'); // row count
            expect(result[17]).toBe('50000'); // size in bytes
            expect(result[18]).toBe('25'); // queries last month
        });

        it('should handle stat summary fields for dashboards', () => {
            ...
            const propertiesWithDashboardStats = {
                ...mockProperties,
                statsSummary: {
                    viewCount: 150,
                    uniqueUserCountLast30Days: 10,
                },
            };

            const entityUrl = '/entity/dashboard/test';
            const result = transformGenericEntityPropertiesToCsvRow(
                mockEntityRegistry,
                propertiesWithDashboardStats as any,
                entityUrl,
                dashboardResult,
            );

            expect(result[14]).toBe('150'); // view count
            expect(result[15]).toBe('10'); // unique users
            expect(result[16]).toBe(''); // row count (empty for dashboards)
            expect(result[17]).toBe(''); // size in bytes (empty for dashboards)
            expect(result[18]).toBe(''); // queries last month (empty for dashboards)
        });
    ...
    describe('transformResultsToCsvRow', () => {
        ...
        it('should handle results with degree field', () => {
            ...
            expect(result[0]).toHaveLength(20); // Should include degree field
            expect(result[0][19]).toBe('1'); // degree value
        });
    });
});
