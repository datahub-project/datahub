...
export default function useExtractFieldTagsInfo(
    ...
    return (record: SchemaField, defaultUneditableTags: GlobalTags | null = null) => {
        ...
        const flatProposedTags = proposedTags?.flatMap((request) =>
            request.params?.tagProposal?.tag
                ? [request.params?.tagProposal?.tag]
                : request.params?.tagProposal?.tags || [],
        );
        ...
        return {
            ...
            proposedTags,
            numberOfTags:
                schemaFieldTags.length + editableTags.length + uneditableTags.length + flatProposedTags.length,
        };
    };
}
