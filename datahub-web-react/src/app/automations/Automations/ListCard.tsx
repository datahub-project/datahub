...
const Loading = ({ ...props }: any) => (
    <LoadingOutlined style={{ fontSize: 8, marginRight: '4px', ...props.style }} spin {...props} />
);
...
const PropagationDetails = ({ status, statusLoading }: any) => {
    ...
    if (!status || status === null || !status.main) {
        ...
        return (
            <ResultContainer>
                <div>Assets updated: N/A</div>
                <div style={{ textAlign: 'right' }}>Not running</div>
            </ResultContainer>
        );
    }
    ...
    const {
        main: { started_at, success_count },
    } = status;

    const startedAt = started_at !== undefined ? toRelativeTimeString(started_at || 0) : 'unknown';

    const successCount = success_count !== undefined ? formatNumberWithoutAbbreviation(success_count) : 0;

    return (
        <ResultContainer>
            <div>
                <span>Assets updated: {successCount}</span>
            </div>
            <div style={{ textAlign: 'right' }}>Started {startedAt}</div>
        </ResultContainer>
    );
};
...
export const AutomationsListCard = ({ automation }: any) => {
    ...
    useEffect(() => {
        if (data) {
            if (!status || status === null || !status.main) setCurrentStatus(AutomationStatus.STOPPED);
            else setCurrentStatus(AutomationStatus.RUNNING);
        }
    }, [data, status]);
    ...
    return (
        <>
                    ...
                    <div>
                        ...
                        <Details>
                            {type === AutomationTypes.ACTION && (
                                <PropagationDetails status={status} statusLoading={loading} />
                            )}
            ...
            <AutomationModal isOpen={isOpen} setIsOpen={setIsOpen} type="EDIT" data={automation} />
        </>
    );
};
