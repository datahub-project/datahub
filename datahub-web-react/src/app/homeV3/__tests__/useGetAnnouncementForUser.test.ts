...
describe('useGetAnnouncementsForUser', () => {
    ...
    it('should return filtered announcements and loading state', () => {
        (useUserContext as unknown as ReturnType<typeof vi.fn>).mockReturnValue({ user: mockUser });
    ...
    it('should call updateUserHomePageSettings and refetch on dismiss', async () => {
        ...
        const refetchArg = refetch.mock.calls[0][0];
        expect(refetchArg.input.orFilters[0].and.find((f) => f.field === 'urn').values).toContain('urn:2');
    });
