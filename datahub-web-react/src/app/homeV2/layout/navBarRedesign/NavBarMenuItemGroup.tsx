...
const GroupTitle = styled.div<{ $isCollapsed?: boolean }>`
...
export default function NavBarMenuItemGroup({ title, $isCollapsed, children }: React.PropsWithChildren<Props>) {
    return (
        <GroupContainer role="group">
            {/* Kept mounted while collapsed so the group keeps its vertical rhythm; CSS hides it. */}
            <GroupTitle $isCollapsed={$isCollapsed}>{title}</GroupTitle>
            <div>{children}</div>
        </GroupContainer>
    );
}
