...
import { MFESchema, isNavPageMfe } from '@app/mfeframework/mfeConfigLoader';

/**
 * Returns menu items for MFEs when subNavigationMode is FALSE.
 * Only nav.page MFEs with showInNav=true are included.
 */
export function getMfeMenuItems(mfeConfig: MFESchema): NavBarMenuLinkItem[] {
    ...
    return mfeConfig.microFrontends
        .filter((mfe) => isNavPageMfe(mfe) && mfe.flags?.showInNav)
        .map((mfe) => ({
