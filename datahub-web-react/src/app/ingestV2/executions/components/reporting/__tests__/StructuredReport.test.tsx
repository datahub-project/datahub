...
function renderWithTheme(ui: React.ReactElement) {
    return render(<ThemeProvider theme={themeV2}>{ui}</ThemeProvider>);
}
...
describe('StructuredReport Component', () => {
    ...
    it('should use correct colors for each item type', () => {
        ...
        const errorList = screen.getByTestId('item-list-WarningDiamond');
        expect(errorList).toHaveAttribute('data-color', themeV2.colors.bgSurfaceError);
        expect(errorList).toHaveAttribute('data-text-color', themeV2.colors.textOnSurfaceError);

        const warningList = screen.getByTestId('item-list-WarningCircle');
        expect(warningList).toHaveAttribute('data-color', themeV2.colors.bgSurfaceWarning);
        expect(warningList).toHaveAttribute('data-text-color', themeV2.colors.textOnSurfaceWarning);
    });
