 ...
 *
 * Member nodes start at FetchStatus.UNNEEDED for both directions: the Domain lineage view is
 * about Domain -> Domain edges (declared via `domainUpstreams`), not per-member asset lineage,
 * so we don't trigger per-member `searchAcrossLineage` walks here. Member-level lineage is
 * surfaced on the dedicated DataProduct / dataset lineage tabs.
 */
