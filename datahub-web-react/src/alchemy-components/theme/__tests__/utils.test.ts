...
import { getThemedIconColor, getThemedTextColor } from '@components/theme/utils';
...
describe.each(themes)('themed color resolution for $id', (theme) => {
    it.each([
        ...
        expect(getThemedTextColor(color, 500, theme)).toBe(theme.colors[token]);
    });
    ...
    it.each([
        ...
        expect(getThemedIconColor(color, 500, theme)).toBe(theme.colors[token]);
    });
