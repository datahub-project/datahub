...
public class ElasticSearchClustersTest {
  ...
  @Test
  public void testClusterPrefixOverlayWinsOverSharedIndexPrefix() {
    IndexConfiguration defaults = IndexConfiguration.builder().prefix("prod").build();
    SearchClusterSettings cluster =
        SearchClusterSettings.builder()
            .uri("http://search2:9200")
            .index(SearchClusterIndexSettings.builder().prefix("tenant").build())
            .build();

    assertEquals(cluster.effectiveIndex(defaults).getPrefix(), "tenant");
    assertEquals(cluster.effectiveIndex(defaults).getFinalPrefix(), "tenant_");
  }

  @Test
  public void testBlankClusterPrefixInheritsSharedIndexPrefix() {
    IndexConfiguration defaults = IndexConfiguration.builder().prefix("prod").build();
    SearchClusterSettings cluster =
        SearchClusterSettings.builder()
            .uri("http://search2:9200")
  ...
  @Test
  public void testBlankAnalyzerAndMappingOverlayIsUnset() {
    SearchClusterIndexSettings overlay =
        SearchClusterIndexSettings.builder().analyzerConfig("").mappingConfig("   ").build();

    assertEquals(overlay.getAnalyzerConfig(), null);
    assertEquals(overlay.getMappingConfig(), null);
  }
