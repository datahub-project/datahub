 ...
 *
 * <p>This is the seam that keeps deployment-specific identity out of {@link FeatureFlagProvider}. OSS
 * targets nothing: one deployment, one set of values. An extension module contributes a {@code @Primary}
 * bean that targets on whatever identity it isolates by, without the facade or any caller learning such an
 * identity exists.
 *
 * <p>It is a separate interface rather than inline in the facade because a {@link
 * dev.openfeature.sdk.FeatureProvider} is handed an {@link EvaluationContext} and never the operation — and
 * an {@code EvaluationContext} can only carry primitive-ish values, so the operation cannot be smuggled
 * through it. The translation therefore has to happen before the provider is called, in something a
 * deployment can swap.
 *
