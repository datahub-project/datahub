...
public class ConfigProvider {
  ...
  /**
   * Yaml file path of plugin configuration file. Content of this file should match with {@link
   * Config}
   */
  private final Path configFilePath;

  /**
   * Directory where all plugins are mounted in DataHub GMS. Default pluginBaseDir is
  ...
  public Optional<Config> load() {
    ...
    if (!this.configFilePath.toFile().exists()) {
      log.warn(
          "Configuration {} file not found at location {}", CONFIG_FILE_NAME, this.pluginBaseDir);
