...
/**
 * Responsible for creating {@link AuthenticatorPluginConfig} instance.
 * This provider is register in {@link com.datahub.plugins.factory.PluginConfigFactory} as provider of Authenticator configuration
 */
public class AuthenticatorPluginConfigProvider extends AuthPluginConfigProvider {
  ...
  public AuthPluginConfig createAuthPluginConfig(@Nonnull PluginConfig pluginConfig) {
    ...
    // Make the pluginJar file path either from name of plugin or explicitly from plugins[].params.jarFileName
    ...
    return new AuthenticatorPluginConfig(pluginConfig.getName(), pluginConfig.getEnabled(), authParam.getClassName(),
        pluginConfig.getPluginHomeDirectory(), pluginJar, authParam.getConfigs());
  }
}
