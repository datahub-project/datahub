...
public enum SecurityMode {
   ...
   *
   * <p>Plugins are allowed to connect on below ports only 1) port greater than 1024 2) port 80 3)
   * port 443 All other ports connection are disallowed.
   *
   * <p>Plugins are allowed to read and write files on PLUGIN_HOME directory only and all other
   * read/write access are denied.
   */
  RESTRICTED(SecurityMode::restrictModePermissionSupplier),

  /**
   * Plugins has full access. In this mode plugin can read/write to any directory, can connect to
   * any port and can read environment variables.
   */
  ...
  private static Permissions restrictModePermissionSupplier(Path sourceCodeDirectory) {
    ...
    permissions.add(
        new SocketPermission(
            "*:1024-", "connect,resolve")); // Allow to connect access to all socket above 1024
    permissions.add(
        new SocketPermission("*:80", "connect,resolve")); // Allow to connect access to HTTP port
    permissions.add(
        new SocketPermission("*:443", "connect,resolve")); // Allow to connect access to HTTPS port
