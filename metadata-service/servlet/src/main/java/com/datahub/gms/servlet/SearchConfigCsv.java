...
public class SearchConfigCsv extends HttpServlet {
  ...
  private void writeSearchCsv(WebApplicationContext ctx, PrintWriter pw) {
    ...
    String[] header = {"entity", "query_category", "match_category", "query_type", "field_name",
            "field_weight", "search_analyzer", "case_insensitive", "query_boost", "raw"};
    writer.println(header);

    SEARCHABLE_ENTITY_TYPES.stream()
            .map(entityType -> {
              try {
                EntitySpec entitySpec = entityRegistry.getEntitySpec(EntityTypeMapper.getName(entityType));
            ...
            .forEach(entitySpecOpt -> {
              ...
              BoolQueryBuilder simpleQueryString = (BoolQueryBuilder) relevancyQuery.should().get(0);
              ...
              for (QueryBuilder simpBuilder : simpleQueryString.should()) {
                ...
                for (Map.Entry<String, Float> fieldWeight : sqsb.fields().entrySet()) {
                  String[] row = {entitySpec.getName(), "relevancy", "fulltext", sqsb.getClass().getSimpleName(),
                          fieldWeight.getKey(),
                          fieldWeight.getValue().toString(), sqsb.analyzer(), "true", String.valueOf(sqsb.boost()),
              ...
              for (QueryBuilder builder : exactPrefixMatch.should()) {
                if (builder instanceof TermQueryBuilder) {
                  ...
                  String[] row = {entitySpec.getName(), "relevancy", "exact_match", tqb.getClass().getSimpleName(),
                          tqb.fieldName(),
                          String.valueOf(tqb.boost()), KEYWORD_ANALYZER, String.valueOf(tqb.caseInsensitive()), "",
                  ...
                  String[] row = {entitySpec.getName(), "relevancy", "prefix_match", mppqb.getClass().getSimpleName(),
                          mppqb.fieldName(),
                          String.valueOf(mppqb.boost()), "", "true", "", mppqb.toString().replaceAll("\n", "")};
                  writer.println(row);
                } else {
                  throw new IllegalStateException("Unhandled exact prefix builder: " + builder.getClass().getName());
                }
              }

              for (FunctionScoreQueryBuilder.FilterFunctionBuilder ffb : rankingQuery.filterFunctionBuilders()) {
                if (ffb.getFilter() instanceof MatchAllQueryBuilder) {
                  ...
                  if (ffb.getScoreFunction() instanceof WeightBuilder) {
                    ...
                    String[] row = {entitySpec.getName(), "score", filter.getClass().getSimpleName(),
                            scoreFunction.getClass().getSimpleName(), "*",
                            String.valueOf(scoreFunction.getWeight()), "", "true", String.valueOf(filter.boost()),
                            String.format("{\"filter\":%s,\"scoreFunction\":%s", filter,
                                    CSVWriter.builderToString(scoreFunction)).replaceAll("\n", "")};
                    writer.println(row);
                  } else if (ffb.getScoreFunction() instanceof FieldValueFactorFunctionBuilder) {
                    FieldValueFactorFunctionBuilder scoreFunction = (FieldValueFactorFunctionBuilder) ffb.getScoreFunction();
                    String[] row = {entitySpec.getName(), "score", filter.getClass().getSimpleName(),
                            scoreFunction.getClass().getSimpleName(), scoreFunction.fieldName(),
                            String.valueOf(scoreFunction.factor()), "", "true", String.valueOf(filter.boost()),
                            String.format("{\"filter\":%s,\"scoreFunction\":%s", filter, CSVWriter.builderToString(scoreFunction)).replaceAll("\n", "")};
                    writer.println(row);
                  } else {
                    throw new IllegalStateException("Unhandled score function: " + ffb.getScoreFunction());
                  }
                  ...
                  if (ffb.getScoreFunction() instanceof WeightBuilder) {
                    WeightBuilder scoreFunction = (WeightBuilder) ffb.getScoreFunction();
                    String[] row = {entitySpec.getName(), "score", filter.getClass().getSimpleName(),
                            scoreFunction.getClass().getSimpleName(), filter.fieldName() + "=" + filter.value().toString(),
                            String.valueOf(scoreFunction.getWeight()), KEYWORD_ANALYZER, String.valueOf(filter.caseInsensitive()),
                            String.valueOf(filter.boost()), String.format("{\"filter\":%s,\"scoreFunction\":%s", filter,
                            CSVWriter.builderToString(scoreFunction)).replaceAll("\n", "")};
                    writer.println(row);
                  } else {
                    throw new IllegalStateException("Unhandled score function: " + ffb.getScoreFunction());
                  ...
                  throw new IllegalStateException("Unhandled function score filter: " + ffb.getFilter());
                }
              }
            });
  }
