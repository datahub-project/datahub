...
public class StatefulTokenService extends StatelessTokenService {
  ...
  public String generateAccessToken(@Nonnull TokenType type, @Nonnull Actor actor, long expiresInMs, long createdAtInMs,
    ...
    AuditStamp auditStamp = AuditStampUtils.createDefaultAuditStamp().setActor(UrnUtils.getUrn(actorUrn));
  ...
  public void revokeAccessToken(@Nonnull String hashedToken) throws TokenException {
    ...
    if (_entityService.exists(tokenUrn)) {
      ...
      _revokedTokenCache.put(salt(hashedToken), true);
      return;
    }
  ...
  private String salt(String input) {
    byte[] saltingKeyBytes = this.saltingKey.getBytes();
    final byte[] inputBytes = input.getBytes();
    byte[] concatBytes = ArrayUtils.addAll(saltingKeyBytes, inputBytes);
    return Base64.getEncoder().encodeToString(concatBytes);
  }
}
