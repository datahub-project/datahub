...
/**
 * Authenticator that verifies legacy API-gateway compliant JSON web tokens for Acryl SaaS
 * (generated inside Acryl Admin Server).
 *
 * <p>This authenticator requires the following configurations:
 *
 * <p>- signingKey: a key used to sign all JWT tokens using the provided signingAlgorithm. This
 * should be THE SAME ONE used by Acryl Admin Server to mint new auth tokens.
 */
@Slf4j
public class LegacyTokenAuthenticator implements Authenticator {

  private static final String ACRYL_ISSUER =
      "admin.acryl.io"; // The issuer used to generate the Acryl Admin Server Token
  ...
  private boolean isValidToken(final String token) throws TokenException {
    ...
    try {
      ...
      final Claims claims =
          (Claims) Jwts.parserBuilder().setSigningKey(base64Key).build().parse(token).getBody();
      final String issuer = claims.get("iss", String.class);
      if (!issuer.equals(ACRYL_ISSUER)) {
        throw new TokenException(
            String.format("Failed to validate DataHub Token. Not issued by %s", ACRYL_ISSUER));
      }
      return true;
    } catch (Exception e) {
