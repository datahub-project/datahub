...
public class SearchClusterConnectionIdentityTest {
  ...
  @Test
  public void testBlankClusterMappingFilesInheritSharedEntityIndexFiles() {
    EntityIndexConfiguration sharedEntityIndex =
        EntityIndexConfiguration.builder()
            .v3(
                EntityIndexVersionConfiguration.builder()
    ...
    assertEquals(
        secondary.getEntityIndex().getV3().getAnalyzerConfig(),
        "search_entity_analyzer_config.yaml");
    assertEquals(
        secondary.getEntityIndex().getV3().getMappingConfig(), "search_entity_mapping_config.yaml");
  }
