...
public class UpgradeDefaultBrowsePathsStepTest {
  ...
  public void testExecuteFirstTime() throws Exception {
    ...
    final EntityRegistry registry = new TestEntityRegistry();
    Mockito.when(mockService.getEntityRegistry()).thenReturn(registry);
    Mockito.when(mockService.buildDefaultBrowsePath(Mockito.eq(testUrn1))).thenReturn(
        new BrowsePaths().setPaths(new StringArray(ImmutableList.of("/prod/kafka"))));
    Mockito.when(mockService.buildDefaultBrowsePath(Mockito.eq(testUrn2))).thenReturn(
        new BrowsePaths().setPaths(new StringArray(ImmutableList.of("/prod/kafka"))));
    ...
    final List<RecordTemplate> browsePaths1 = ImmutableList.of(
        new BrowsePaths().setPaths(new StringArray(ImmutableList.of(BrowsePathUtils.getLegacyDefaultBrowsePath(testUrn1, registry)))),
        new BrowsePaths().setPaths(new StringArray(ImmutableList.of(BrowsePathUtils.getLegacyDefaultBrowsePath(testUrn2, registry))))
    );

    final List<ExtraInfo> extraInfos1 = ImmutableList.of(
      new ExtraInfo()
          .setUrn(testUrn1)
          .setVersion(0L)
  ...
  public void testDoesNotRunWhenBrowsePathIsNotQualified() throws Exception {
    ...
    Urn testUrn3 = UrnUtils.getUrn("urn:li:dataset:(urn:li:dataPlatform:kafka,SampleKafkaDataset3,PROD)"); // Do not migrate
    Urn testUrn4 = UrnUtils.getUrn("urn:li:dataset:(urn:li:dataPlatform:kafka,SampleKafkaDataset4,PROD)"); // Do not migrate
  ...
  private void initMockServiceOtherEntities(EntityService mockService) {
    List<String> skippedEntityTypes = ImmutableList.of(
