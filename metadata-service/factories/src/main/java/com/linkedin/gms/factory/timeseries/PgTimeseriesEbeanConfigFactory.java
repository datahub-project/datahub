...
/**
 * Dedicated Ebean {@link Database} for SqlSetup {@code postgres.pgTimeseries} aspect-row traffic
 * (dual-write sink and PostgreSQL {@code TimeseriesAspectService}). Uses {@code
 * postgres.pgTimeseries.pool.*} (same pattern as {@code postgres.pgQueue}) so timeseries JDBC is
 * sized separately from the main entity-store {@code ebeanServer}.
 *
 * <p>Defaults fall through to {@code ebean.url} / {@code ebean.username} / {@code ebean.password}.
 * SqlSetup DDL ({@code PgTimeseriesSchemaStep}) still runs on the main upgrade/Ebean datasource
 * where applicable; runtime reads/writes bind here.
 */
@Slf4j
@Configuration
