...
public class IngestDefaultPersonasAndViews implements BootstrapStep {
  ...
  public void execute(@Nonnull OperationContext systemOperationContext) throws Exception {
    ...
    ingestPersonaViews(systemOperationContext);

    // Ingest default personas
    ingestPersonas(systemOperationContext);
  }

  private void ingestPersonaViews(@Nonnull final OperationContext systemOperationContext)
      throws Exception {
    ...
    final JsonNode viewsObj =
        systemOperationContext
            .getObjectMapper()
            .readTree(new ClassPathResource("./boot/persona_views.json").getInputStream());
  ...
  private void ingestPersonas(@Nonnull final OperationContext systemOperationContext)
      throws Exception {
    // 0. Execute preflight check to see whether we need to ingest personas
    ...
    final JsonNode personasObj =
        systemOperationContext
            .getObjectMapper()
            .readTree(new ClassPathResource("./boot/personas.json").getInputStream());
