...
public class UpgradeDefaultBrowsePathsStep extends UpgradeStep {
  ...
  private int getAndMigrateBrowsePaths(String entityType, int start, AuditStamp auditStamp)
    ...
    if (latestAspects.getTotalCount() == 0
      ...
      log.debug(
          String.format(
              "Found 0 browse paths for entity with type %s. Skipping migration!", entityType));
      return 0;
    }
    ...
    if (latestAspects.getValues().size() != latestAspects.getMetadata().getExtraInfos().size()) {
      ...
      log.warn(
          "Failed to match browse path aspects with corresponding urns. Found mismatched length between aspects ({})"
