...
public class DocumentImportServiceTest {
  ...
  @Test
  public void testImportDocuments_continuesAfterSingleCandidateFailure() throws Exception {
    SystemEntityClient mockClient = mock(SystemEntityClient.class);
    when(mockClient.exists(any(OperationContext.class), any(Urn.class))).thenReturn(false);
    AtomicInteger ingestCalls = new AtomicInteger(0);
    doAnswer(
            invocation -> {
              if (ingestCalls.incrementAndGet() == 2) {
                throw new RuntimeException("ingest failed");
              }
              return null;
            })
        .when(mockClient)
        .batchIngestProposals(any(OperationContext.class), any(), anyBoolean());
    ...
    assertEquals(result.getCreatedCount(), 1);
    assertEquals(result.getFailedCount(), 1);
    assertEquals(result.getUpdatedCount(), 0);
    assertEquals(result.getErrors().size(), 1);
    assertTrue(result.getErrors().get(0).startsWith("upload.docB:"));
  }
