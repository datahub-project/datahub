...
public class ServiceTestUtils {
  ...
  private final static Map<String, Class<? extends RecordTemplate>> aspectSpecMap = Map.of(
  ...
  public static OpenApiClient createMockDomainsClient(@Nullable Domains domains)
      throws Exception {
  ...
  private static OpenApiClient createMockClient(
      @Nullable RecordTemplate aspect, String aspectName) throws Exception {
    OpenApiClient mockClient = Mockito.mock(OpenApiClient.class, Mockito.withSettings().verboseLogging());
    BatchGetUrnRequest batchGetUrnRequest = new BatchGetUrnRequest(List.of(TEST_ENTITY_URN_1.toString(),
        TEST_ENTITY_URN_2.toString()), Collections.singletonList(aspectName), true);
    ...
    if (aspect != null) {
      ...
      GenericEntity testEntity1 =
          GenericEntity.builder().urn(TEST_ENTITY_URN_1.toString()).build(new ObjectMapper(), aspectMap1);
      ...
      GenericEntity testEntity2 =
          GenericEntity.builder().urn(TEST_ENTITY_URN_2.toString()).build(new ObjectMapper(), aspectMap2);
    ...
    Mockito.when(
            mockClient.getBatchUrns(Mockito.eq(Constants.DATASET_ENTITY_NAME), Mockito.eq(batchGetUrnRequest), Mockito.nullable(
                String.class)))
    ...
    Mockito.when(mockRegistry.getEntitySpec(Mockito.eq(Constants.DATASET_ENTITY_NAME))).thenReturn(mockEntitySpec);
    ...
    Mockito.when(mockAspectSpec.getDataTemplateClass()).thenReturn((Class<RecordTemplate>) aspectSpecMap.get(aspectName));
    return mockClient;
  }
