...
public interface GraphService {
   ...
   *
   * Calling this method with {@link RelationshipDirection} `UNDIRECTED` in `relationshipFilter`
  ...
  default EntityLineageResult getLineage(@Nonnull Urn entityUrn, @Nonnull LineageDirection direction,
    ...
    if (!CollectionUtils.isEmpty(edgesByDirection.get(true))) {
      ...
      RelatedEntitiesResult outgoingEdges =
          findRelatedEntities(null, QueryUtils.newFilter("urn", entityUrn.toString()), graphFilters.getAllowedEntityTypes(),
              QueryUtils.EMPTY_FILTER,
              relationshipTypes, QueryUtils.newRelationshipFilter(QueryUtils.EMPTY_FILTER, RelationshipDirection.OUTGOING), offset,
              count);
    ...
    if (!CollectionUtils.isEmpty(edgesByDirection.get(false))) {
      ...
      RelatedEntitiesResult incomingEdges =
          findRelatedEntities(null, QueryUtils.newFilter("urn", entityUrn.toString()), graphFilters.getAllowedEntityTypes(),
              QueryUtils.EMPTY_FILTER,
              relationshipTypes, QueryUtils.newRelationshipFilter(QueryUtils.EMPTY_FILTER, RelationshipDirection.INCOMING), offset,
              count);
   ...
   *
   * Calling this method with a {@link RelationshipDirection} `UNDIRECTED` in `relationshipFilter`
   * is equivalent to the union of `OUTGOING` and `INCOMING` (without duplicates).
   */
