...
public class EntityResource extends CollectionResourceTaskTemplate<String, Entity> {
  ...
  public Task<AnyRecord> get(
    ...
    return RestliUtil.toTask(
        () -> {
          ...
          final Entity entity = entityService.getEntity(urn, projectedAspects);
          if (entity == null) {
        ...
        () -> {
          final Set<String> projectedAspects =
              aspectNames == null
                  ? Collections.emptySet()
                  : new HashSet<>(Arrays.asList(aspectNames));
          return entityService.getEntities(urns, projectedAspects).entrySet().stream()
              .collect(
                  Collectors.toMap(
  ...
  public Task<Void> ingest(
    ...
    return RestliUtil.toTask(
        () -> {
          entityService.ingestEntity(entity, auditStamp, finalSystemMetadata);
          return null;
        },
  ...
  public Task<SearchResult> search(
    ...
    OperationContext opContext = OperationContext.asSession(systemOperationContext.withSearchFlags(flags -> searchFlags != null ? searchFlags : new SearchFlags().setFulltext(Boolean.TRUE.equals(fulltext))),
    ...
    return RestliUtil.toTask(
        () -> {
          ...
          return validateSearchResult(result, entityService);
        },
  ...
  public Task<ScrollResult> scrollAcrossEntities(
    ...
    return RestliUtil.toTask(
        () -> {
          ...
          return validateScrollResult(result, entityService);
        },
        ...
        () -> {
          ...
          return restrictedService.encryptRestricted(resultUrn -> {
            if (opContext.getOperationContextConfig().getSearchAuthorizationConfiguration().isEnabled()
              ...
              return canViewEntity(auth, authorizer, resultUrn);
            }
            return true;
          }, result);
          },
  ...
  public Task<StringArray> getBrowsePaths(
    ...
    return RestliUtil.toTask(
        () -> new StringArray(entitySearchService.getBrowsePaths(urnToEntityName(urn), urn)),
  ...
  */
    ...
    return RestliUtil.toTask(
        () -> {
          ...
          if ((dryRun == null) || (!dryRun)) {
            ...
            entityService.rollbackWithConditions(aspectRowsToDelete, conditions, false);
          }
          return response;
        },
        ...
        () -> {
          ...
          List<String> timeseriesAspectNames =
              EntitySpecUtils.getEntityTimeseriesAspectNames(
                  entityService.getEntityRegistry(), urn.getEntityType());
          ...
          if (aspectName == null) {
            RollbackRunResult result = entityService.deleteUrn(urn);
  ...
  private Long deleteTimeseriesAspects(
    ...
    if (!isAPIAuthorized(
            AuthenticationContext.getAuthentication(),
            authorizer,
            PoliciesConfig.lookupAPIPrivilege(TIMESERIES, DELETE),
            new EntitySpec(urn.getEntityType(), urn.toString()))) {
      throw new RestLiServiceException(
    ...
    for (final String aspect : aspectsToDelete) {
      DeleteAspectValuesResult result =
          timeseriesAspectService.deleteAspectValues(entityType, aspect, filter);
  ...
  public Task<DeleteReferencesResponse> deleteReferencesTo(
    ...
    return RestliUtil.toTask(
        () -> deleteEntityService.deleteReferencesTo(urn, dryRun),
  ...
  public Task<Void> setWriteable(
    ...
    if (!isAPIAuthorized(
            AuthenticationContext.getAuthentication(),
            authorizer,
            Disjunctive.disjoint(PoliciesConfig.SET_WRITEABLE_PRIVILEGE))) {
      throw new RestLiServiceException(
  ...
  public Task<ListUrnsResult> listUrns(
    ...
    return RestliUtil.toTask(() -> {
      ListUrnsResult result = entityService.listUrns(entityName, start, count);
      if (!isAPIAuthorized(
              auth,
              authorizer,
              PoliciesConfig.lookupAPIPrivilege(ENTITY, READ))) {
  ...
  public Task<String> applyRetention(
    ...
    if (!isAPIAuthorized(
            AuthenticationContext.getAuthentication(),
            authorizer,
            Disjunctive.disjoint(PoliciesConfig.APPLY_RETENTION_PRIVILEGE),
            resourceSpec)) {
      throw new RestLiServiceException(
          HttpStatus.S_403_FORBIDDEN, "User is unauthorized to apply retention.");
    }
    return RestliUtil.toTask(
        () -> entityService.batchApplyRetention(start, count, attemptWithVersion, aspectName, urn),
