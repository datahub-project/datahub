...
public class AuthServiceControllerTest extends AbstractTestNGSpringContextTests {
  ...
  public void stubSessionEligibility() {
    ...
    reset(mockNativeUserService);
    reset(mockInviteTokenService);
    ...
    Authentication defaultAuth =
        new Authentication(
            new Actor(ActorType.USER, SYSTEM_CLIENT_ID),
            String.format("Basic %s:%s", SYSTEM_CLIENT_ID, "systemSecret"));
  ...
  @Test
  public void testGetSsoSettingsMissingAuthReturnsUnauthorized() {
    AuthenticationContext.remove();
