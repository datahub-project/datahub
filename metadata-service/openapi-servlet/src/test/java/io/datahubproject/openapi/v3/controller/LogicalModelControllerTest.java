...
public class LogicalModelControllerTest extends AbstractTestNGSpringContextTests {
  ...
  private static HttpServletRequest mockOpenapiRequest() {
  ...
  @Test
  public void testSetLogicalParentsDeniedWhenUnauthorized() throws Exception {
    ...
    String jsonBody = objectMapper.writeValueAsString(Map.of("parent_field1", "child_field1"));
    HttpServletRequest request = mockOpenapiRequest();

    try (MockedStatic<EntityAspectAuthorizationUtils> authUtils =
      ...
      Assert.assertThrows(
          ...
          () ->
              LogicalModelController.setLogicalParentsHelper(
                  request,
                  childDatasetUrnStr,
                  parentDatasetUrnStr,
                  jsonBody,
  ...
  public void testSetLogicalParentsHappyPath() throws Exception {
    ...
    // Create field path mapping (LinkedHashMap keeps JSON key order stable for serialization)
    Map<String, String> fieldPathMap = new LinkedHashMap<>();
    fieldPathMap.put("parent_field1", "child_field1");
    fieldPathMap.put("parent_field2", "child_field2");
    ...
    HttpServletRequest request = mockOpenapiRequest();
    ResponseEntity<List<GenericEntityV3>> response =
        LogicalModelController.setLogicalParentsHelper(
            request,
            childDatasetUrnStr,
            parentDatasetUrnStr,
    ...
    Map<Urn, Urn> parentByChild = extractLogicalParentLinks(capturedBatch);
    assertEquals(parentDatasetUrn, parentByChild.get(childDatasetUrn));
    assertEquals(
        SchemaFieldUtils.generateSchemaFieldUrn(parentDatasetUrn, "parent_field1"),
        parentByChild.get(childField1Urn));
    assertEquals(
        SchemaFieldUtils.generateSchemaFieldUrn(parentDatasetUrn, "parent_field2"),
        parentByChild.get(childField2Urn));

    for (MCPItem item : capturedBatch.getMCPItems()) {
      MetadataChangeProposal mcp = item.getMetadataChangeProposal();
      LogicalParent aspect =
          GenericRecordUtils.deserializeAspect(
              mcp.getAspect().getValue(), JSON, LogicalParent.class);
      assertEquals(LOGICAL_PARENT_ASPECT_NAME, mcp.getAspectName());
      assertEquals(
          opContext.getActorContext().getActorUrn().toString(),
          aspect.getParent().getLastModified().getActor().toString());
    }
  }
  ...
  public void testSetLogicalParentsWithoutSchemaMetadata() throws Exception {
    ...
    List<IngestResult> mockResults =
        List.of(
            createMockIngestResultWithLogicalParent(childDatasetUrn, parentDatasetUrn),
            createMockIngestResultWithLogicalParent(
                SchemaFieldUtils.generateSchemaFieldUrn(childDatasetUrn, "child_field1"),
                SchemaFieldUtils.generateSchemaFieldUrn(parentDatasetUrn, "parent_field1")));
    when(mockEntityService.ingestProposal(any(), any(), eq(false))).thenReturn(mockResults);
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post(
                    setLogicalParentsPath(childDatasetUrnStr, parentDatasetUrnStr))
  ...
  private static Map<Urn, Urn> extractLogicalParentLinks(AspectsBatch batch) throws Exception {
    Map<Urn, Urn> parentByChild = new HashMap<>();
    for (MCPItem item : batch.getMCPItems()) {
      MetadataChangeProposal mcp = item.getMetadataChangeProposal();
      LogicalParent aspect =
          GenericRecordUtils.deserializeAspect(
              mcp.getAspect().getValue(), JSON, LogicalParent.class);
      parentByChild.put(mcp.getEntityUrn(), aspect.getParent().getDestinationUrn());
    }
  ...
  private static String setLogicalParentsPath(
      String childDatasetUrnStr, String parentDatasetUrnStr) {
    return "/openapi/v3/logical/"
        + encodePath(childDatasetUrnStr, UTF_8)
        + "/relationship/physicalInstanceOf/"
        + encodePath(parentDatasetUrnStr, UTF_8);
  }
}
