...
public class OpenApiClient {
  ...
  public OpenApiClient(String gmsHost, int gmsPort, boolean useSsl, OperationContext systemOperationContext) {
  ...
  public BatchGetUrnResponse getBatchUrnsSystemAuth(String entityName, BatchGetUrnRequest request) {
    return getBatchUrns(entityName, request, systemOperationContext.getSystemAuthentication().get().getCredentials());
  }

  public BatchGetUrnResponse getBatchUrns(String entityName, BatchGetUrnRequest request, String authCredentials) {
    String url =
        (useSsl ? "https://" : "http://") + gmsHost + ":" + gmsPort + OPENAPI_PATH + entityName + BATCH_ENTITY_GET;
  ...
  private static BatchGetUrnResponse mapResponse(ClassicHttpResponse response) {
    ...
    try {
      ...
      serializedResponse = OBJECT_MAPPER.readValue(result.toString(StandardCharsets.UTF_8), BatchGetUrnResponse.class);
    } catch (IOException e) {
