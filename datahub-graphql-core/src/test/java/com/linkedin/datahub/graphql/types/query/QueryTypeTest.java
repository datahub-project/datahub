...
public class QueryTypeTest {
  ...
  private static final Urn TEST_DATASET_2_URN = UrnUtils.getUrn("urn:li:dataset:(urn:li:dataPlatform:hive,test-2,PROD)");
  private static final Urn TEST_USER_URN = UrnUtils.getUrn("urn:li:corpuser:test");

  private static final QueryProperties TEST_QUERY_PROPERTIES_1 = new QueryProperties()
      .setName("Query Name")
  ...
  private static final QuerySubjects TEST_QUERY_SUBJECTS_1 = new QuerySubjects()
      .setSubjects(new QuerySubjectArray(
          ImmutableList.of(
              new QuerySubject()
                .setEntity(TEST_DATASET_URN)
          )
      ));
  private static final QueryProperties TEST_QUERY_PROPERTIES_2 = new QueryProperties()
      ...
      .setCreated(new AuditStamp().setActor(TEST_USER_URN).setTime(0L))
      .setLastModified(new AuditStamp().setActor(TEST_USER_URN).setTime(1L));
  private static final QuerySubjects TEST_QUERY_SUBJECTS_2 = new QuerySubjects()
      .setSubjects(new QuerySubjectArray(
          ImmutableList.of(
              new QuerySubject()
                  .setEntity(TEST_DATASET_2_URN)
          )
      ));
  ...
  public void testBatchLoad() throws Exception {
    ...
    Mockito.when(client.batchGetV2(Mockito.eq(Constants.QUERY_ENTITY_NAME),
        ...
        .thenReturn(ImmutableMap.of(queryUrn1, new EntityResponse().setEntityName(Constants.QUERY_ENTITY_NAME)
                .setUrn(queryUrn1)
                .setAspects(new EnvelopedAspectMap(query1Aspects)), queryUrn2,
            new EntityResponse().setEntityName(Constants.QUERY_ENTITY_NAME)
                .setUrn(queryUrn2)
    ...
    List<DataFetcherResult<QueryEntity>> result =
        type.batchLoad(ImmutableList.of(TEST_QUERY_URN.toString(), TEST_QUERY_2_URN.toString()), mockContext);

    // Verify response
    Mockito.verify(client, Mockito.times(1))
        .batchGetV2(Mockito.eq(Constants.QUERY_ENTITY_NAME), Mockito.eq(ImmutableSet.of(queryUrn1, queryUrn2)), Mockito.eq(QueryType.ASPECTS_TO_FETCH),
            Mockito.any(Authentication.class));
  ...
  public void testBatchLoadNullEntity() throws Exception {
    ...
    QueryContext mockContext = Mockito.mock(QueryContext.class);
    Mockito.when(mockContext.getAuthentication()).thenReturn(Mockito.mock(Authentication.class));
    List<DataFetcherResult<QueryEntity>> result = type.batchLoad(ImmutableList.of(
        TEST_QUERY_URN.toString(),
        TEST_QUERY_2_URN.toString()),
      mockContext);
  ...
  public void testBatchLoadClientException() throws Exception {
    ...
    assertThrows(RuntimeException.class, () -> type.batchLoad(ImmutableList.of(TEST_QUERY_URN.toString(), TEST_QUERY_2_URN.toString()),
        context));
  }
  ...
  private void verifyQuery1(QueryEntity query) {
    ...
    assertEquals(query.getProperties().getSource().toString(), TEST_QUERY_PROPERTIES_1.getSource().toString());
    assertEquals(query.getProperties().getStatement().getLanguage().toString(), TEST_QUERY_PROPERTIES_1.getStatement().getLanguage().toString());
    assertEquals(query.getProperties().getStatement().getValue(), TEST_QUERY_PROPERTIES_1.getStatement().getValue());
    assertEquals(query.getProperties().getCreated().getActor(), TEST_QUERY_PROPERTIES_1.getCreated().getActor().toString());
    assertEquals(query.getProperties().getCreated().getTime(), TEST_QUERY_PROPERTIES_1.getCreated().getTime());
    assertEquals(query.getProperties().getLastModified().getActor(), TEST_QUERY_PROPERTIES_1.getLastModified().getActor().toString());
    assertEquals(query.getProperties().getLastModified().getTime(), TEST_QUERY_PROPERTIES_1.getLastModified().getTime());
    assertEquals(query.getSubjects().get(0).getDataset().getUrn(), TEST_QUERY_SUBJECTS_1.getSubjects().get(0).getEntity().toString());
  }
  ...
  private void verifyQuery2(QueryEntity query) {
    ...
    assertEquals(query.getProperties().getSource().toString(), TEST_QUERY_PROPERTIES_2.getSource().toString());
    assertEquals(query.getProperties().getStatement().getLanguage().toString(), TEST_QUERY_PROPERTIES_2.getStatement().getLanguage().toString());
    assertEquals(query.getProperties().getStatement().getValue(), TEST_QUERY_PROPERTIES_2.getStatement().getValue());
    assertEquals(query.getProperties().getCreated().getActor(), TEST_QUERY_PROPERTIES_2.getCreated().getActor().toString());
    assertEquals(query.getProperties().getCreated().getTime(), TEST_QUERY_PROPERTIES_2.getCreated().getTime());
    assertEquals(query.getProperties().getLastModified().getActor(), TEST_QUERY_PROPERTIES_2.getLastModified().getActor().toString());
    assertEquals(query.getProperties().getLastModified().getTime(), TEST_QUERY_PROPERTIES_2.getLastModified().getTime());
    assertEquals(query.getSubjects().get(0).getDataset().getUrn(), TEST_QUERY_SUBJECTS_2.getSubjects().get(0).getEntity().toString());
  }
}