...
public class BatchUpdateDeprecationResolverTest {
  ...
  public void testGetSuccessNoExistingDeprecation() throws Exception {
    ...
    Mockito.when(
            mockService.getLatestAspects(
                ...
                Mockito.eq(Set.of(Constants.DEPRECATION_ASPECT_NAME)),
                Mockito.eq(false)))
        .thenReturn(null);
  ...
  public void testGetSuccessExistingDeprecation() throws Exception {
    ...
    Map<Urn, List<RecordTemplate>> result =
        Map.of(
            UrnUtils.getUrn(TEST_ENTITY_URN_1), List.of(originalDeprecation),
            UrnUtils.getUrn(TEST_ENTITY_URN_2), List.of(originalDeprecation));
    Mockito.when(
            mockService.getLatestAspects(
                any(),
                Mockito.eq(
