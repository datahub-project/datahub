...
public class ApplicationMapperTest {
  private static final Urn TEST_APPLICATION_URN =
      Urn.createFromTuple(Constants.APPLICATION_ENTITY_NAME, "application1");
  ...
  public void testMapDomainsAssociations() throws URISyntaxException {
    ...
    final EntityResponse response =
        new EntityResponse()
            .setEntityName(Constants.APPLICATION_ENTITY_NAME)
            .setUrn(TEST_APPLICATION_URN)
            .setAspects(new EnvelopedAspectMap(aspects));

    final Application actual = ApplicationMapper.map(null, response);
