...
public class PatchEntitiesResolverTest {
  ...
  @Test
  public void testPatchEntitiesWithAutoGeneratedUrns() throws Exception {
    // Arrange - Test batch patch with auto-generated URNs
    PatchEntityInput input1 = createGlossaryTermInput(null, "Auto Generated Term 1");
    PatchEntityInput input2 = createGlossaryTermInput(null, "Auto Generated Term 2");
    PatchEntityInput[] inputArray = {input1, input2};
    ...
    // Check first result - should have auto-generated URN
    PatchEntityResult result1 = results.get(0);
    assertNotNull(result1);
    assertTrue(result1.getUrn().startsWith("urn:li:glossaryTerm:"));
    assertEquals(result1.getName(), "\"Auto Generated Term 1\"");
    assertTrue(result1.getSuccess());
    assertNull(result1.getError());

    // Check second result - should have auto-generated URN
    PatchEntityResult result2 = results.get(1);
    assertNotNull(result2);
    assertTrue(result2.getUrn().startsWith("urn:li:glossaryTerm:"));
    assertEquals(result2.getName(), "\"Auto Generated Term 2\"");
    assertTrue(result2.getSuccess());
    assertNull(result2.getError());
  }
  ...
  @Test
  public void testPatchOwnershipTypeWithAutoGeneratedUrn() throws Exception {
    // Arrange - Test patch ownership type with auto-generated URN
    PatchEntityInput input = createOwnershipTypeInput(null, "Test Ownership Type");
    PatchEntityInput[] inputArray = {input};

    when(_environment.getArgument("input")).thenReturn(inputArray);

    // Mock entity registry for ownership type
    com.linkedin.metadata.models.EntitySpec ownershipTypeEntitySpec =
        mock(com.linkedin.metadata.models.EntitySpec.class);
    AspectSpec ownershipTypeInfoAspectSpec = mock(AspectSpec.class);

    when(_entityRegistry.getEntitySpec("ownershipType")).thenReturn(ownershipTypeEntitySpec);
    when(ownershipTypeEntitySpec.getAspectSpec("ownershipTypeInfo"))
        .thenReturn(ownershipTypeInfoAspectSpec);
    ...
    PatchEntityResult result = results.get(0);
    assertNotNull(result);
    // Should have auto-generated URN for ownership type
    assertTrue(result.getUrn().startsWith("urn:li:ownershipType:"));
    assertEquals(result.getName(), "\"Test Ownership Type\"");
    assertTrue(result.getSuccess());
    assertNull(result.getError());
  }
  ...
  private PatchEntityInput createOwnershipTypeInput(String urn, String name) {
    PatchEntityInput input = new PatchEntityInput();
    input.setUrn(urn);
    input.setEntityType("ownershipType");
    input.setAspectName("ownershipTypeInfo");
    input.setPatch(
        Arrays.asList(
            createPatchOperation(PatchOperationType.ADD, "/name", "\"" + name + "\""),
            createPatchOperation(
                PatchOperationType.ADD, "/description", "\"Test ownership type description\"")));
    return input;
  }
