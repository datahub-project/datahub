...
public class RemoveEntityFromOrganizationsResolverTest {
  ...
  private RemoveEntityFromOrganizationsResolver _resolver;
  ...
  public void setupTest() {
    ...
    _resolver = new RemoveEntityFromOrganizationsResolver(_entityClient);
  }

  @Test
  public void testRemoveEntityFromOrganizationsSuccess() throws Exception {
    ...
    Organizations updatedOrgs =
        GenericRecordUtils.deserializeAspect(
            ...
            Organizations.class);
    assertEquals(updatedOrgs.getOrganizations().size(), 1);
    assertEquals(updatedOrgs.getOrganizations().get(0).toString(), ORG_URN_2);
  }

  @Test
  public void testRemoveEntityUnauthorized() {
  ...
  @Test
  public void testRemoveEntityWithNoExistingOrganizations() throws Exception {
    QueryContext mockContext = getMockAllowContext();
    Mockito.when(_dataFetchingEnvironment.getArgument(eq("entityUrn"))).thenReturn(ENTITY_URN);
    Mockito.when(_dataFetchingEnvironment.getArgument(eq("organizationUrns")))
        .thenReturn(Arrays.asList(ORG_URN_1));
    ...
    Organizations updatedOrgs =
        GenericRecordUtils.deserializeAspect(
            ...
            proposalCaptor.getValue().getAspect().getContentType(),
            Organizations.class);
    assertEquals(updatedOrgs.getOrganizations().size(), 0);
  }

  @Test
  public void testRemoveEntityWithInvalidUrn() {
    QueryContext mockContext = getMockAllowContext();
    Mockito.when(_dataFetchingEnvironment.getArgument(eq("entityUrn"))).thenReturn(ENTITY_URN);
    Mockito.when(_dataFetchingEnvironment.getArgument(eq("organizationUrns")))
