...
public class DeleteBusinessAttributeResolverTest {
    ...
    public void testSuccess() throws Exception {
        ...
        Mockito.when(mockClient.exists(Urn.createFromString(TEST_BUSINESS_ATTRIBUTE_URN), mockAuthentication)).thenReturn(true);
    ...
    public void testUnauthorized() throws Exception {
        ...
        DeleteBusinessAttributeResolver resolver = new DeleteBusinessAttributeResolver(mockClient);
        AuthorizationException actualException = expectThrows(AuthorizationException.class, () -> resolver.get(mockEnv).get());
        assertTrue(actualException.getMessage().equals("Unauthorized to perform this action. Please contact your DataHub administrator."));
    ...
    public void testEntityNotExists() throws Exception {
        ...
        setupAllowContext();
        Mockito.when(mockEnv.getArgument("urn")).thenReturn(TEST_BUSINESS_ATTRIBUTE_URN);
        Mockito.when(mockClient.exists(Urn.createFromString(TEST_BUSINESS_ATTRIBUTE_URN), mockAuthentication)).thenReturn(false);

        DeleteBusinessAttributeResolver resolver = new DeleteBusinessAttributeResolver(mockClient);
        RuntimeException actualException = expectThrows(RuntimeException.class, () -> resolver.get(mockEnv).get());
