...
public class FreshnessAssertionMapperTest {
    ...
    public void testMapCronFreshnessAssertionInfo() throws Exception {
        FreshnessAssertionInfo freshnessAssertionInfo = new FreshnessAssertionInfo()
                ...
                .setFilter(new DatasetFilter().setType(DatasetFilterType.SQL).setSql("WHERE value > 5;"))
                .setSchedule(new FreshnessAssertionSchedule().setType(FreshnessAssertionScheduleType.CRON)
                        .setCron(new FreshnessCronSchedule().setCron("0 0 0 * * ? *")
                                .setTimezone("America/Los_Angeles").setWindowStartOffsetMs(10L)));

        com.linkedin.datahub.graphql.generated.FreshnessAssertionInfo result = FreshnessAssertionMapper
                .mapFreshnessAssertionInfo(freshnessAssertionInfo);
        ...
        Assert.assertEquals(result.getFilter().getType(), com.linkedin.datahub.graphql.generated.DatasetFilterType.SQL);
        Assert.assertEquals(result.getFilter().getSql(), "WHERE value > 5;");
        Assert.assertEquals(result.getSchedule().getType(),
    ...
    public void testMapFixedIntervalFreshnessAssertionInfo() throws Exception {
        FreshnessAssertionInfo freshnessAssertionInfo = new FreshnessAssertionInfo()
                ...
                .setType(FreshnessAssertionType.DATASET_CHANGE)
                .setFilter(new DatasetFilter().setType(DatasetFilterType.SQL).setSql("WHERE value > 5;"))
                .setSchedule(new FreshnessAssertionSchedule().setType(FreshnessAssertionScheduleType.FIXED_INTERVAL)
                        .setFixedInterval(new FixedIntervalSchedule().setUnit(CalendarInterval.DAY).setMultiple(10)));
        ...
        Assert.assertEquals(result.getSchedule().getFixedInterval().getUnit(),
