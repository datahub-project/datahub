...
public class RemoveBusinessAttributeResolverTest {
  ...
  private static final String RESOURCE_URN =
      "urn:li:dataset:(urn:li:dataPlatform:postgres,postgres.auth,PROD)";
  ...
  public void testSuccess() throws Exception {
    ...
    Mockito.when(
            mockClient.exists(Urn.createFromString((BUSINESS_ATTRIBUTE_URN)), mockAuthentication))
        .thenReturn(true);
    Mockito.when(mockService.exists(Urn.createFromString(RESOURCE_URN), true)).thenReturn(true);
    Mockito.when(
            mockService.getAspect(
                Urn.createFromString(RESOURCE_URN), Constants.SCHEMA_METADATA_ASPECT_NAME, 0))
        .thenReturn(schemaMetadata());
    Mockito.when(
            EntityUtils.getAspectFromEntity(
                RESOURCE_URN, Constants.EDITABLE_SCHEMA_METADATA_ASPECT_NAME, mockService, null))
        .thenReturn(editableSchemaMetadata());

    RemoveBusinessAttributeResolver resolver =
        new RemoveBusinessAttributeResolver(mockClient, mockService);
    resolver.get(mockEnv).get();

    Mockito.verify(mockClient, Mockito.times(1))
        .ingestProposal(Mockito.any(MetadataChangeProposal.class), Mockito.eq(mockAuthentication));
  }
  ...
  public void testResourceNotExists() throws Exception {
    ...
    Mockito.when(
            mockClient.exists(Urn.createFromString((BUSINESS_ATTRIBUTE_URN)), mockAuthentication))
        .thenReturn(true);
    Mockito.when(mockService.exists(Urn.createFromString(RESOURCE_URN), true)).thenReturn(false);
    Mockito.when(
            mockService.getAspect(
                Urn.createFromString(RESOURCE_URN), Constants.SCHEMA_METADATA_ASPECT_NAME, 0))
        .thenReturn(schemaMetadata());
