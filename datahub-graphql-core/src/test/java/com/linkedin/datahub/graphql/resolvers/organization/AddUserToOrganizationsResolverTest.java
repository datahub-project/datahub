...
public class AddUserToOrganizationsResolverTest {
  ...
  private AddUserToOrganizationsResolver _resolver;
  ...
  public void setupTest() {
    ...
    _resolver = new AddUserToOrganizationsResolver(_entityClient);
  }

  @Test
  public void testAddUserToOrganizationsSuccess() throws Exception {
  ...
  @Test
  public void testAddUserWithExistingOrganizations() throws Exception {
    QueryContext mockContext = getMockAllowContext();
    Mockito.when(_dataFetchingEnvironment.getArgument(eq("userUrn"))).thenReturn(USER_URN);
    Mockito.when(_dataFetchingEnvironment.getArgument(eq("organizationUrns")))
    ...
    assertEquals(updatedUserOrgs.getOrganizations().size(), 2);
  }
  ...
  @Test
  public void testAddUserWithInvalidUrn() {
    QueryContext mockContext = getMockAllowContext();
    Mockito.when(_dataFetchingEnvironment.getArgument(eq("userUrn"))).thenReturn(USER_URN);
    Mockito.when(_dataFetchingEnvironment.getArgument(eq("organizationUrns")))
