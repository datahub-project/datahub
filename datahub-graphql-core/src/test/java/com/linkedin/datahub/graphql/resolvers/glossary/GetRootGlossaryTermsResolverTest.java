...
public class GetRootGlossaryTermsResolverTest {
  ...
  public void testGetSuccess() throws Exception {
            ...
            new SearchResult()
                .setEntities(
                    new SearchEntityArray(
                        ImmutableSet.of(
                            new SearchEntity().setEntity(Urn.createFromString(glossaryTermUrn1)),
                            new SearchEntity().setEntity(Urn.createFromString(glossaryTermUrn2)))))
                .setFrom(0)
                .setNumEntities(2));
    ...
    assertEquals(
        result.getTerms().get(0).getUrn(), Urn.createFromString(glossaryTermUrn1).toString());
    assertEquals(
        result.getTerms().get(1).getUrn(), Urn.createFromString(glossaryTermUrn2).toString());
  }
