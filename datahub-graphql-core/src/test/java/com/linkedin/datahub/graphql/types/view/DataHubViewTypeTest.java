...
public class DataHubViewTypeTest {
   ...
   *
   * View Definitions currently support a limited Filter structure, which includes a single Logical filter set.
   * Either a set of OR criteria with 1 value in each nested "and", or a single OR criteria with a set of nested ANDs.
   *
   * This enables us to easily support merging more complex View predicates in the future without a data migration,
   * should the need arise.
   */
  private static final DataHubViewInfo TEST_VALID_VIEW_INFO = new DataHubViewInfo()
  ...
  /**
   * An Invalid View is on which has been ingested manually, which should not occur under normal operation of DataHub.
   *
   * This would be a complex view with multiple OR and nested AND predicates.
   */
  private static final DataHubViewInfo TEST_INVALID_VIEW_INFO = new DataHubViewInfo()
      .setType(DataHubViewType.PERSONAL)
      .setName("test")
      .setDescription("test description")
      .setCreated(new AuditStamp().setTime(0L).setActor(TEST_USER_URN))
  ...
  public void testBatchLoadValidView() throws Exception {
    ...
    com.linkedin.datahub.graphql.types.view.DataHubViewType type = new com.linkedin.datahub.graphql.types.view.DataHubViewType(client);

    QueryContext mockContext = Mockito.mock(QueryContext.class);
    Mockito.when(mockContext.getAuthentication()).thenReturn(Mockito.mock(Authentication.class));
    List<DataFetcherResult<DataHubView>> result = type.batchLoad(ImmutableList.of(TEST_VIEW_URN, TEST_VIEW_URN_2), mockContext);
    ...
    assertEquals(view.getDefinition().getFilter().getFilters().get(0).getCondition(), FilterOperator.EQUAL);
    assertEquals(view.getDefinition().getFilter().getFilters().get(0).getField(), "test");
    assertEquals(view.getDefinition().getFilter().getFilters().get(0).getValues(), ImmutableList.of("value1", "value2"));
  ...
  public void testBatchLoadInvalidView() throws Exception {
    // If an Invalid View Definition is found in MySQL, we will return an Empty no-op View. (and log a warning).
    EntityClient client = Mockito.mock(EntityClient.class);
    ...
    QueryContext mockContext = Mockito.mock(QueryContext.class);
    Mockito.when(mockContext.getAuthentication()).thenReturn(Mockito.mock(Authentication.class));
    List<DataFetcherResult<DataHubView>> result = type.batchLoad(ImmutableList.of(TEST_VIEW_URN), mockContext);
  ...
  public void testBatchLoadClientException() throws Exception {
    ...
    Mockito.doThrow(RemoteInvocationException.class).when(mockClient).batchGetV2(
        ...
        Mockito.anySet(),
        Mockito.any(Authentication.class));
    com.linkedin.datahub.graphql.types.view.DataHubViewType type = new com.linkedin.datahub.graphql.types.view.DataHubViewType(mockClient);
    ...
    assertThrows(RuntimeException.class, () -> type.batchLoad(ImmutableList.of(TEST_VIEW_URN, TEST_VIEW_URN_2),
        context));
  }
}