...
public class GetRootDataProductsResolverTest {

  private static final String DATA_PRODUCT_URN_1 = "urn:li:dataProduct:root-1";
  private static final String DATA_PRODUCT_URN_2 = "urn:li:dataProduct:root-2";
  ...
  public void testGetSuccess() throws Exception {
    ...
    Mockito.when(
            mockClient.search(
                ...
                eq("*"),
                eq(buildRootDataProductsFilter()),
                any(),
                eq(0),
                eq(10)))
        .thenReturn(
    ...
    final GetRootDataProductsResult result =
        new GetRootDataProductsResolver(mockClient).get(mockEnv).get();

    assertEquals(result.getStart(), 0);
    assertEquals(result.getCount(), 2);
    assertEquals(result.getTotal(), 2);
    assertEquals(result.getDataProducts().get(0).getUrn(), DATA_PRODUCT_URN_1);
    assertEquals(result.getDataProducts().get(1).getUrn(), DATA_PRODUCT_URN_2);
  }

  @Test
  public void testHasParentDataProductFilterIsApplied() throws Exception {
    final EntityClient mockClient = Mockito.mock(EntityClient.class);
    final QueryContext mockContext = Mockito.mock(QueryContext.class);
    final OperationContext mockOpContext = Mockito.mock(OperationContext.class);
    Mockito.when(mockContext.getOperationContext()).thenReturn(mockOpContext);
  ...
  private Filter buildRootDataProductsFilter() {
    final CriterionArray array =
        new CriterionArray(
            ImmutableList.of(
                buildCriterion(
                    GetRootDataProductsResolver.HAS_PARENT_DATA_PRODUCT_FIELD_NAME,
                    Condition.EQUAL,
                    "false")));
