...
public class UpdateBusinessAttributeResolverTest {
    ...
    private static final String TEST_BUSINESS_ATTRIBUTE_DESCRIPTION_UPDATED = "test-description-updated";
    ...
    public void testSuccess() throws Exception {
        init();
        setupAllowContext();
        final UpdateBusinessAttributeInput testInput = new UpdateBusinessAttributeInput(
        ...
        Mockito.when(mockClient.exists(TEST_BUSINESS_ATTRIBUTE_URN_OBJ, mockAuthentication)).thenReturn(true);
        Mockito.when(businessAttributeService.getBusinessAttributeEntityResponse(TEST_BUSINESS_ATTRIBUTE_URN_OBJ, mockAuthentication))
        ...
        Mockito.when(mockClient.ingestProposal(Mockito.any(MetadataChangeProposal.class), Mockito.eq(mockAuthentication)))
                .thenReturn(TEST_BUSINESS_ATTRIBUTE_URN);

        UpdateBusinessAttributeResolver resolver = new UpdateBusinessAttributeResolver(mockClient, businessAttributeService);
        resolver.get(mockEnv).get();

        //verify
        Mockito.verify(mockClient, Mockito.times(1)).ingestProposal(
                Mockito.argThat(new CreateBusinessAttributeProposalMatcher(updatedMetadataChangeProposal())),
    ...
    public void testNotExists() throws Exception {
        ...
        Mockito.when(mockEnv.getArgument(Mockito.eq("input"))).thenReturn(testInput);
        Mockito.when(mockEnv.getArgument("urn")).thenReturn(TEST_BUSINESS_ATTRIBUTE_URN);
        Mockito.when(mockClient.exists(TEST_BUSINESS_ATTRIBUTE_URN_OBJ, mockAuthentication)).thenReturn(false);

        UpdateBusinessAttributeResolver resolver = new UpdateBusinessAttributeResolver(mockClient, businessAttributeService);
        RuntimeException expectedException = expectThrows(RuntimeException.class, () -> resolver.get(mockEnv));
        assertTrue(expectedException.getMessage().equals(String.format("This urn does not exist: %s", TEST_BUSINESS_ATTRIBUTE_URN)));
    ...
    private MetadataChangeProposal updatedMetadataChangeProposal() {
        ...
        return AspectUtils.buildMetadataChangeProposal(TEST_BUSINESS_ATTRIBUTE_URN_OBJ,
