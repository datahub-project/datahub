...
public class UpdateNameResolverTest {
    ...
    private static final String TEST_BUSINESS_ATTRIBUTE_NAME_UPDATED = "test-business-attribute-updated";
    private static final String TEST_BUSINESS_ATTRIBUTE_DESCRIPTION = "test-description";
    private static final String TEST_BUSINESS_ATTRIBUTE_URN = "urn:li:businessAttribute:7d0c4283-de02-4043-aaf2-698b04274658";
    private static final Urn TEST_BUSINESS_ATTRIBUTE_URN_OBJ = UrnUtils.getUrn(TEST_BUSINESS_ATTRIBUTE_URN);
    ...
    public void testSuccess() throws Exception {
        init();
        setupAllowContext();
        UpdateNameInput testInput = new UpdateNameInput(TEST_BUSINESS_ATTRIBUTE_NAME_UPDATED, TEST_BUSINESS_ATTRIBUTE_URN);
        ...
        Mockito.when(mockService.exists(TEST_BUSINESS_ATTRIBUTE_URN_OBJ)).thenReturn(true);
        ...
        MetadataChangeProposal proposal = MutationUtils.buildMetadataChangeProposalWithUrn(
    ...
    public void testNameConflict() throws Exception {
        init();
        setupAllowContext();
        UpdateNameInput testInput = new UpdateNameInput(TEST_BUSINESS_ATTRIBUTE_NAME, TEST_BUSINESS_ATTRIBUTE_URN);
        Mockito.when(mockEnv.getArgument(Mockito.eq("input"))).thenReturn(testInput);
        Mockito.when(mockEnv.getArgument("urn")).thenReturn(TEST_BUSINESS_ATTRIBUTE_URN);
        Mockito.when(mockService.exists(TEST_BUSINESS_ATTRIBUTE_URN_OBJ)).thenReturn(true);
        Mockito.when(EntityUtils.getAspectFromEntity(
