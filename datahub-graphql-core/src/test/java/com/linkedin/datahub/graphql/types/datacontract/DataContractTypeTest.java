...
public class DataContractTypeTest {
  ...
  private static final DataContractKey TEST_DATA_CONTRACT_1_KEY = new DataContractKey()
      .setId("id-1");
  private static final DataContractProperties TEST_DATA_CONTRACT_1_PROPERTIES = new DataContractProperties()
      .setEntity(TEST_DATASET_URN)
      .setDataQuality(new DataQualityContractArray(
          ImmutableList.of(
              new DataQualityContract()
                .setAssertion(DATA_QUALITY_ASSERTION_URN)
          )
      ))
      .setFreshness(new FreshnessContractArray(
          ImmutableList.of(
              new FreshnessContract()
                  .setAssertion(FRESHNESS_ASSERTION_URN)
          )
      ))
  ...
  private static final DataContractStatus TEST_DATA_CONTRACT_1_STATUS = new DataContractStatus()
        ...
        .thenReturn(ImmutableMap.of(
            ...
            new EntityResponse()
                ...
                .setAspects(new EnvelopedAspectMap(ImmutableMap.of(
                    ...
                    new EnvelopedAspect().setValue(new Aspect(TEST_DATA_CONTRACT_1_KEY.data())),
                    Constants.DATA_CONTRACT_PROPERTIES_ASPECT_NAME,
                    new EnvelopedAspect().setValue(new Aspect(TEST_DATA_CONTRACT_1_PROPERTIES.data())),
                    Constants.DATA_CONTRACT_STATUS_ASPECT_NAME,
                    new EnvelopedAspect().setValue(new Aspect(TEST_DATA_CONTRACT_1_STATUS.data()))
                )))));

    DataContractType type = new DataContractType(client);

    QueryContext mockContext = getMockAllowContext();
    List<DataFetcherResult<DataContract>> result = type.batchLoad(ImmutableList.of(TEST_DATA_CONTRACT_1_URN, TEST_DATA_CONTRACT_2_URN), mockContext);
  ...
  public void testBatchLoadClientException() throws Exception {
    ...
    assertThrows(RuntimeException.class, () -> type.batchLoad(ImmutableList.of(TEST_DATA_CONTRACT_1_URN, TEST_DATA_CONTRACT_2_URN),
        context));
  }
}
