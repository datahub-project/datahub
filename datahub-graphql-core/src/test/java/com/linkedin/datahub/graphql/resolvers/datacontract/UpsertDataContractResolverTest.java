...
public class UpsertDataContractResolverTest {
  ...
  private static final Urn TEST_DATASET_URN = UrnUtils.getUrn("urn:li:dataset:(urn:li:dataPlatform:snowflake,test,PROD)");
  private static final Urn TEST_FRESHNESS_ASSERTION_URN = UrnUtils.getUrn("urn:li:assertion:freshness");
  ...
  private static final UpsertDataContractInput TEST_CREATE_INPUT = new UpsertDataContractInput(
      TEST_DATASET_URN.toString(),
      ImmutableList.of(
          new FreshnessContractInput(TEST_FRESHNESS_ASSERTION_URN.toString())
      ),
      ImmutableList.of(
          new SchemaContractInput(TEST_SCHEMA_ASSERTION_URN.toString())
      ),
      ImmutableList.of(
          new DataQualityContractInput(TEST_QUALITY_ASSERTION_URN.toString())
      ),
      DataContractState.PENDING,
      "test-id"
  );

  private static final UpsertDataContractInput TEST_VALID_UPDATE_INPUT = new UpsertDataContractInput(
      TEST_DATASET_URN.toString(),
      ImmutableList.of(
          new FreshnessContractInput(TEST_FRESHNESS_ASSERTION_URN.toString())
      ),
  ...
  public void testGetSuccessCreate() throws Exception {
    ...
    key.setId("test-id");
    final Urn dataContractUrn = EntityKeyUtils.convertEntityKeyToUrn(key, Constants.DATA_CONTRACT_ENTITY_NAME);
    ...
    props.setDataQuality(new DataQualityContractArray(
        ImmutableList.of(
            new DataQualityContract()
                .setAssertion(TEST_QUALITY_ASSERTION_URN)
        )
    ));
    props.setFreshness(new FreshnessContractArray(
        ImmutableList.of(
            new FreshnessContract()
                .setAssertion(TEST_FRESHNESS_ASSERTION_URN)
        )
    ));
    props.setSchema(new SchemaContractArray(
        ImmutableList.of(
            new SchemaContract()
                .setAssertion(TEST_SCHEMA_ASSERTION_URN)
        )
    ));
    ...
    UpsertDataContractResolver resolver = new UpsertDataContractResolver(mockClient, mockGraphClient);
    ...
    propertiesProposal.setSystemMetadata(new SystemMetadata().setProperties(new StringMap(ImmutableMap.of("appSource", "ui"))));
    ...
    statusProposal.setSystemMetadata(new SystemMetadata().setProperties(new StringMap(ImmutableMap.of("appSource", "ui"))));
  ...
  public void testGetFailureEntityDoesNotExist() throws Exception {
    ...
    GraphClient mockGraphClient = Mockito.mock(GraphClient.class);
    initMockGraphClient(mockGraphClient, TEST_CONTRACT_URN);
    Mockito.when(mockClient.exists(Mockito.eq(TEST_DATASET_URN), Mockito.any(Authentication.class))).thenReturn(false);
    UpsertDataContractResolver resolver = new UpsertDataContractResolver(mockClient, mockGraphClient);
  ...
  public void testGetFailureAssertionDoesNotExist() throws Exception {
    ...
    GraphClient mockGraphClient = Mockito.mock(GraphClient.class);
    initMockGraphClient(mockGraphClient, TEST_CONTRACT_URN);
    Mockito.when(mockClient.exists(Mockito.eq(TEST_DATASET_URN), Mockito.any(Authentication.class))).thenReturn(true);
    Mockito.when(mockClient.exists(Mockito.eq(TEST_FRESHNESS_ASSERTION_URN), Mockito.any(Authentication.class))).thenReturn(false);
    Mockito.when(mockClient.exists(Mockito.eq(TEST_QUALITY_ASSERTION_URN), Mockito.any(Authentication.class))).thenReturn(false);
    Mockito.when(mockClient.exists(Mockito.eq(TEST_SCHEMA_ASSERTION_URN), Mockito.any(Authentication.class))).thenReturn(false);
    UpsertDataContractResolver resolver = new UpsertDataContractResolver(mockClient, mockGraphClient);
  ...
  private void initMockGraphClient(GraphClient client, Urn existingContractUrn) {
    if (existingContractUrn != null) {
      Mockito.when(client.getRelatedEntities(
          ...
          Mockito.eq(1),
          Mockito.anyString()
      )).thenReturn(new EntityRelationships().setTotal(1).setCount(1).setStart(0).setRelationships(new EntityRelationshipArray(
          ImmutableList.of(
              new EntityRelationship()
                .setEntity(existingContractUrn)
                .setType(CONTRACT_FOR_RELATIONSHIP)
                .setCreated(new AuditStamp().setActor(TEST_ACTOR_URN).setTime(0L))
          )
      )));
    } else {
      Mockito.when(client.getRelatedEntities(
          ...
          Mockito.eq(1),
          Mockito.anyString()
      )).thenReturn(new EntityRelationships().setTotal(0).setCount(0).setStart(0).setRelationships(new EntityRelationshipArray(
          Collections.emptyList()
      )));
    }
  }

  private void initMockEntityClient(EntityClient client, Urn existingContractUrn, DataContractProperties newContractProperties) throws Exception {
    if (existingContractUrn != null) {
      Mockito.when(client.exists(Mockito.eq(existingContractUrn), Mockito.any(Authentication.class))).thenReturn(true);
    }
    Mockito.when(client.exists(Mockito.eq(TEST_DATASET_URN), Mockito.any(Authentication.class))).thenReturn(true);
    Mockito.when(client.exists(Mockito.eq(TEST_QUALITY_ASSERTION_URN), Mockito.any(Authentication.class))).thenReturn(true);
    Mockito.when(client.exists(Mockito.eq(TEST_FRESHNESS_ASSERTION_URN), Mockito.any(Authentication.class))).thenReturn(true);
    Mockito.when(client.exists(Mockito.eq(TEST_SCHEMA_ASSERTION_URN), Mockito.any(Authentication.class))).thenReturn(true);
