...
public class SqlAssertionMapperTest {
        ...
        public void testMapMetricSqlAssertionInfo() throws Exception {
                SqlAssertionInfo sqlAssertionInfo = new SqlAssertionInfo()
                                ...
                                .setParameters(new AssertionStdParameters().setValue(new AssertionStdParameter()
                                                .setType(AssertionStdParameterType.NUMBER).setValue(("5"))));

                com.linkedin.datahub.graphql.generated.SqlAssertionInfo result = SqlAssertionMapper
                                .mapSqlAssertionInfo(sqlAssertionInfo);
                Assert.assertEquals(result.getEntityUrn(),
                                "urn:li:dataset:(urn:li:dataPlatform:foo,bar,baz)");
                Assert.assertEquals(result.getType(),
                                com.linkedin.datahub.graphql.generated.SqlAssertionType.METRIC);
        ...
        public void testMapMetricChangeSqlAssertionInfo() throws Exception {
                ...
                Assert.assertEquals(result.getEntityUrn(),
                                "urn:li:dataset:(urn:li:dataPlatform:foo,bar,baz)");
                Assert.assertEquals(result.getType(),
                                com.linkedin.datahub.graphql.generated.SqlAssertionType.METRIC_CHANGE);
                Assert.assertEquals(result.getStatement(), "SELECT COUNT(*) FROM foo.bar.baz");
                Assert.assertEquals(result.getOperator(),
                                com.linkedin.datahub.graphql.generated.AssertionStdOperator.GREATER_THAN);
                Assert.assertEquals(result.getParameters().getValue().getType(),
                                com.linkedin.datahub.graphql.generated.AssertionStdParameterType.NUMBER);
                Assert.assertEquals(result.getParameters().getValue().getValue(), "5");
                Assert.assertEquals(result.getChangeType(),
