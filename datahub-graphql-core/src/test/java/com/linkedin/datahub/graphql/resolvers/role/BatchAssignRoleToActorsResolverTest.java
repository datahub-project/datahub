...
public class BatchAssignRoleToActorsResolverTest {
  ...
  private BatchAssignRoleToActorsResolver _resolver;
  ...
  public void setupTest() {
    ...
    _resolver = new BatchAssignRoleToActorsResolver(_entityClient);
  }
  ...
  public void testRoleDoesNotExistFails() throws Exception {
    ...
    BatchAssignRoleToActorsInput input = new BatchAssignRoleToActorsInput();
    ...
    when(_entityClient.exists(eq(Urn.createFromString(ROLE_URN_STRING)), any())).thenReturn(false);
  ...
  public void testSomeActorsExist() throws Exception {
    ...
    when(_entityClient.exists(eq(Urn.createFromString(ROLE_URN_STRING)), any())).thenReturn(true);
    when(_entityClient.exists(eq(Urn.createFromString(FIRST_ACTOR_URN_STRING)), any())).thenReturn(true);
    when(_entityClient.exists(eq(Urn.createFromString(SECOND_ACTOR_URN_STRING)), any())).thenReturn(false);

    assertTrue(_resolver.get(_dataFetchingEnvironment).join());
    // Only the first actor should be assigned to the role since the second actor does not exist
    verify(_entityClient, times(1)).ingestProposal(any(), any());
  }
  ...
  public void testAllActorsExist() throws Exception {
    ...
    when(_entityClient.exists(eq(Urn.createFromString(ROLE_URN_STRING)), any())).thenReturn(true);
    when(_entityClient.exists(eq(Urn.createFromString(FIRST_ACTOR_URN_STRING)), any())).thenReturn(true);
    when(_entityClient.exists(eq(Urn.createFromString(SECOND_ACTOR_URN_STRING)), any())).thenReturn(true);
