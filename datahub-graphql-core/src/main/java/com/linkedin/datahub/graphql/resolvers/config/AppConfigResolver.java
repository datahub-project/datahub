...
public class AppConfigResolver implements DataFetcher<CompletableFuture<AppConfig>> {
  ...
  private EntityType mapResourceTypeToEntityType(final String resourceType) {
    ...
    } else if (com.linkedin.metadata.authorization.PoliciesConfig.DATA_HUB_VIEW_PRIVILEGES
        .getResourceType()
        .equals(resourceType)) {
      return EntityType.DATAHUB_VIEW;
    } else if (com.linkedin.metadata.authorization.PoliciesConfig.POST_PRIVILEGES
        .getResourceType()
        .equals(resourceType)) {
