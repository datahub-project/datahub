...
public class DataContractType implements com.linkedin.datahub.graphql.types.EntityType<DataContract, String> {
  ...
  @Override
  public List<DataFetcherResult<DataContract>> batchLoad(@Nonnull List<String> urns, @Nonnull QueryContext context) throws Exception {
    final List<Urn> dataContractUrns = urns.stream()
        .map(UrnUtils::getUrn)
    ...
    try {
      ...
      return gmsResults.stream()
          .map(gmsResult ->
              gmsResult == null ? null : DataFetcherResult.<DataContract>newResult()
