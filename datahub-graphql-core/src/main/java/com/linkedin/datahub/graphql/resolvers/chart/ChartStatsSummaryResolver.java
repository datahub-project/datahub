...
public class ChartStatsSummaryResolver
    ...
    return GraphQLConcurrencyUtils.supplyAsync(
        () -> {
          try {
            ...
            log.error(
                String.format(
                    "Failed to load chart usage summary for resource %s", resourceUrn.toString()),
                e);
    ...
    try {
      ...
      if (response != null
        ...
        return new UsageFeatures(
            response.getAspects().get(Constants.USAGE_FEATURES_ASPECT_NAME).getValue().data());
      } else {
   ...
   */
  private ChartStatsSummary getSummaryFromUsageFeatures(
