 ...
 */
public class CreateActionExecutionRequestResolver implements DataFetcher<CompletableFuture<Boolean>> {
  ...
  public CreateActionExecutionRequestResolver(final EntityClient entityClient,
      final IntegrationsService integrationsService, final ActionPipelineConfiguration actionConfiguration) {
    ...
    return CompletableFuture.supplyAsync(() -> {
      ...
      if (IngestionAuthUtils.canManageIngestion(context)) {
        ...
        final CreateActionExecutionRequestInput input =
            bindArgument(environment.getArgument("input"), CreateActionExecutionRequestInput.class);
        ...
        try {
          ...
          final Map<Urn, EntityResponse> response =
              _entityClient.batchGetV2(ACTION_PIPELINE_ENTITY_NAME, ImmutableSet.of(actionPipelineUrn),
                  ImmutableSet.of(ACTION_INFO_ASPECT_NAME), context.getAuthentication());

          if (!response.containsKey(actionPipelineUrn)) {
            throw new DataHubGraphQLException(
                String.format("Failed to find action pipeline with urn %s", actionPipelineUrn.toString()),
                DataHubGraphQLErrorCode.BAD_REQUEST);
          }

          final EnvelopedAspect envelopedInfo = response.get(actionPipelineUrn).getAspects().get(ACTION_INFO_ASPECT_NAME);
          final DataHubActionInfo actionPipelineInfo = new DataHubActionInfo(envelopedInfo.getValue().data());

          if (!actionPipelineInfo.getConfig().hasRecipe()) {
            throw new DataHubGraphQLException(
                String.format("Failed to find valid action pipeline with urn %s. Missing recipe", actionPipelineUrn.toString()),
                DataHubGraphQLErrorCode.BAD_REQUEST);
          }
          ...
          execInput.setSource(
              new ExecutionRequestSource().setType(MANUAL_EXECUTION_SOURCE_NAME).setIngestionSource(actionPipelineUrn));
          //execInput.setExecutorId(actionPipelineInfo.getConfig().getExecutorId(), SetMode.IGNORE_NULL);
...
//          arguments.put(VERSION_ARG_NAME, actionPipelineInfo.getConfig().hasVersion()
//              ? actionPipelineInfo.getConfig().getVersion()
//              : "0.0.0dev0"
//          );
//          if (actionPipelineInfo.getConfig().hasVersion()) {
//            arguments.put(VERSION_ARG_NAME, actionPipelineInfo.getConfig().getVersion());
//          }
          String debugMode = "false";
//          if (actionPipelineInfo.getConfig().hasDebugMode()) {
//            debugMode = actionPipelineInfo.getConfig().isDebugMode() ? "true" : "false";
//          }
