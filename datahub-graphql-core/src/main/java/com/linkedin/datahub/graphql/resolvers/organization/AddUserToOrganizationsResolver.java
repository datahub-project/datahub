...
/**
 * Resolver for adding users to organizations directly. This is a convenience wrapper that always
 * uses userOrganizations aspect.
 */
@Slf4j
@RequiredArgsConstructor
public class AddUserToOrganizationsResolver implements DataFetcher<CompletableFuture<Boolean>> {
    ...
    return GraphQLConcurrencyUtils.supplyAsync(
        () -> {
          try {
            ...
            for (Urn orgUrn : organizationUrns) {
              if (!OrganizationAuthUtils.canManageOrganizationMembers(context, orgUrn)) {
                throw new AuthorizationException(
                    "Unauthorized to add user to organization " + orgUrn);
              }
            }
            ...
            log.error("Failed to add user to organizations", e);
            throw new RuntimeException("Failed to add user to organizations: " + e.getMessage(), e);
          }
        },
