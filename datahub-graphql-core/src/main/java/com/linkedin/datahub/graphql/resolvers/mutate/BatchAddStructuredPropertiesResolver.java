...
public class BatchAddStructuredPropertiesResolver
  ...
  public CompletableFuture<Boolean> get(DataFetchingEnvironment environment) throws Exception {
    final BatchAddStructuredPropertiesInput input =
        bindArgument(environment.getArgument("input"), BatchAddStructuredPropertiesInput.class);
    final List<StructuredPropertyInputParams> structuredProperties =
        input.getStructuredProperties();
    final List<ResourceRefInput> resources = input.getResources();
    final QueryContext context = environment.getContext();

    return GraphQLConcurrencyUtils.supplyAsync(
        () -> {
          validateStructuredProperties(context.getOperationContext(), structuredProperties);
          validateInputResources(context.getOperationContext(), resources, context);

          try {
            batchAddStructuredProperties(
                context.getOperationContext(), structuredProperties, resources, context);
            return true;
          } catch (Exception e) {
            log.error(
  ...
  private void validateStructuredProperties(
      ...
      List<StructuredPropertyInputParams> structuredProperties) {
    for (StructuredPropertyInputParams property : structuredProperties) {
      final Urn propertyUrn = UrnUtils.getUrn(property.getStructuredPropertyUrn());

      try {
        if (!_entityClient.exists(opContext, propertyUrn)) {
          throw new IllegalArgumentException(
              String.format("Structured property with urn %s does not exist.", propertyUrn));
        }
      } catch (RemoteInvocationException e) {
        throw new RuntimeException(
            String.format(
                "Failed to validate structured property existence for urn %s", propertyUrn),
            e);
      }
    }
  }
  ...
  private void batchAddStructuredProperties(
      @Nonnull OperationContext opContext,
      List<StructuredPropertyInputParams> structuredProperties,
      List<ResourceRefInput> resources,
      QueryContext context) {
    log.debug(
        "Batch adding structured properties. properties: {}, resources: {}",
    ...
    try {
      for (ResourceRefInput resource : resources) {
        ...
        for (StructuredPropertyInputParams property : structuredProperties) {
          ...
          StructuredPropertyValueAssignment assignment = propertyMap.get(propertyUrn.toString());
          if (assignment == null) {
            assignment = new StructuredPropertyValueAssignment();
            assignment.setPropertyUrn(propertyUrn);
            assignment.setCreated(auditStamp);
          }
          assignment.setValues(values);
          assignment.setLastModified(auditStamp);

          propertyMap.put(propertyUrn.toString(), assignment);
        }
      ...
      throw new RuntimeException(
          String.format(
              "Failed to batch add structured properties %s to resources with urns %s!",
              structuredProperties,
              resources.stream()
