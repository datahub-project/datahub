...
public class AsyncBatchSubmitFormPromptResolver
  ...
  private String buildJsonTestDefinition(
    ...
    Filter filter = getFilterFromInput(context, input);
    ...
    getEntityNames(input.getTypes()).forEach(typesArray::add);
  ...
  /*
   * Creates the final filter to use to apply this action to entities
   */
  private Filter getFilterFromInput(QueryContext context, AsyncBatchSubmitFormPromptInput input) {
