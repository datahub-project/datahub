...
public class UpdateGlobalViewsSettingsResolver implements DataFetcher<CompletableFuture<Boolean>> {
  ...
  public CompletableFuture<Boolean> get(final DataFetchingEnvironment environment) throws Exception {
    ...
    final UpdateGlobalViewsSettingsInput input = bindArgument(environment.getArgument("input"), UpdateGlobalViewsSettingsInput.class);

    return CompletableFuture.supplyAsync(() -> {
      if (AuthorizationUtils.canManageGlobalViews(context)) {
        try {
          // First, fetch the existing global settings. This does a R-M-F.
          final GlobalSettingsInfo maybeGlobalSettings = _settingsService.getGlobalSettings(context.getAuthentication());

          final GlobalSettingsInfo newGlobalSettings = maybeGlobalSettings != null
              ? maybeGlobalSettings
              : new GlobalSettingsInfo();

          final GlobalViewsSettings newGlobalViewsSettings = newGlobalSettings.hasViews()
          ...
          throw new RuntimeException(String.format("Failed to update global view settings! %s", input), e);
        }
      }
