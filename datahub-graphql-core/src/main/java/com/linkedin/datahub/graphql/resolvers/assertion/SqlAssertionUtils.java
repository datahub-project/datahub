...
public class SqlAssertionUtils {
    @Nonnull
    public static com.linkedin.assertion.SqlAssertionInfo createSqlAssertionInfo(@Nonnull final CreateSqlAssertionInput input) {
        final com.linkedin.assertion.SqlAssertionInfo result = new com.linkedin.assertion.SqlAssertionInfo();
        ...
        if (input.getType() == com.linkedin.datahub.graphql.generated.SqlAssertionType.METRIC_CHANGE) {
            if (input.getChangeType() != null) {
                ...
                throw new IllegalArgumentException("Change type must be specified for METRIC_CHANGE assertion");
            }
        }
