...
public class GlossaryTermsMapper {
  ...
  private com.linkedin.datahub.graphql.generated.GlossaryTermAssociation mapGlossaryTermAssociation(
      @Nonnull final QueryContext context,
      @Nonnull final GlossaryTermAssociation glossaryTermAssociation,
      @Nonnull final Urn entityUrn) {
    ...
    resultGlossaryTerm.setUrn(glossaryTermAssociation.getUrn().toString());
    resultGlossaryTerm.setName(
        GlossaryTermUtils.getGlossaryTermName(glossaryTermAssociation.getUrn().getNameEntity()));
    result.setTerm(resultGlossaryTerm);

    if (glossaryTermAssociation.hasActor()) {
      CorpUser actor = new CorpUser();
      actor.setUrn(glossaryTermAssociation.getActor().toString());
    ...
    if (glossaryTermAssociation.hasAttribution()
        && glossaryTermAssociation.getAttribution() != null) {
      result.setAttribution(
          MetadataAttributionMapper.map(context, glossaryTermAssociation.getAttribution()));
    }
