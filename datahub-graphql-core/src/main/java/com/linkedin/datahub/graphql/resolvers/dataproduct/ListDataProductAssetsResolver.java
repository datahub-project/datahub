...
public class ListDataProductAssetsResolver implements DataFetcher<CompletableFuture<SearchResults>> {
  ...
  public CompletableFuture<SearchResults> get(DataFetchingEnvironment environment) {
    ...
    try {
      ...
      if (entityResponse != null && entityResponse.getAspects().containsKey(Constants.DATA_PRODUCT_PROPERTIES_ASPECT_NAME)) {
        final DataMap data = entityResponse.getAspects().get(Constants.DATA_PRODUCT_PROPERTIES_ASPECT_NAME).getValue().data();
        final DataProductProperties dataProductProperties = new DataProductProperties(data);
        if (dataProductProperties.hasAssets()) {
          assetUrns.addAll(dataProductProperties.getAssets().stream().map(DataProductAssociation::getDestinationUrn).collect(Collectors.toList()));
        }
      }
    } catch (Exception e) {
      log.error(String.format("Failed to list data product assets with urn %s", dataProductUrn), e);
      throw new RuntimeException(String.format("Failed to list data product assets with urn %s", dataProductUrn), e);
    }
    ...
    return CompletableFuture.supplyAsync(() -> {
      // if no assets in data product properties, exit early before search and return empty results
      if (assetUrns.size() == 0) {
