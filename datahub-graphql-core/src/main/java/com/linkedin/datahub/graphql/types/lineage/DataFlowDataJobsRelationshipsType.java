...
public class DataFlowDataJobsRelationshipsType implements LoadableType<DataFlowDataJobsRelationships> {
    ...
    @Override
    public List<DataFlowDataJobsRelationships> batchLoad(final List<String> keys, final QueryContext context) {
        ...
        try {
            return keys.stream().map(urn -> {
                try {
                    ...
                    return DataFlowDataJobsRelationshipsMapper.map(relationships);
