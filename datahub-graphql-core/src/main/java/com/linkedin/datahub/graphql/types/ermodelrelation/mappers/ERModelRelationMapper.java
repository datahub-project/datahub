...
public class ERModelRelationMapper implements ModelMapper<EntityResponse, ERModelRelation> {
  ...
  public ERModelRelation apply(final EntityResponse entityResponse) {
    ...
    if (aspectMap != null && aspectMap.containsKey(EDITABLE_ERMODELRELATION_PROPERTIES_ASPECT_NAME)) {
      mappingHelper.mapToResult(EDITABLE_ERMODELRELATION_PROPERTIES_ASPECT_NAME, this::mapEditableProperties);
    }
    if (aspectMap != null && aspectMap.containsKey(INSTITUTIONAL_MEMORY_ASPECT_NAME)) {
      mappingHelper.mapToResult(INSTITUTIONAL_MEMORY_ASPECT_NAME, (ermodelrelation, dataMap) ->
              ermodelrelation.setInstitutionalMemory(InstitutionalMemoryMapper.map(new InstitutionalMemory(dataMap), entityUrn)));
    }
    if (aspectMap != null && aspectMap.containsKey(OWNERSHIP_ASPECT_NAME)) {
      mappingHelper.mapToResult(OWNERSHIP_ASPECT_NAME, (ermodelrelation, dataMap) ->
    ...
    if (aspectMap != null && aspectMap.containsKey(STATUS_ASPECT_NAME)) {
      mappingHelper.mapToResult(STATUS_ASPECT_NAME, (ermodelrelation, dataMap) ->
    ...
    if (aspectMap != null && aspectMap.containsKey(GLOBAL_TAGS_ASPECT_NAME)) {
      mappingHelper.mapToResult(GLOBAL_TAGS_ASPECT_NAME, (ermodelrelation, dataMap) -> this.mapGlobalTags(ermodelrelation, dataMap, entityUrn));
    }
    if (aspectMap != null && aspectMap.containsKey(GLOSSARY_TERMS_ASPECT_NAME)) {
      mappingHelper.mapToResult(GLOSSARY_TERMS_ASPECT_NAME, (ermodelrelation, dataMap) ->
              ermodelrelation.setGlossaryTerms(GlossaryTermsMapper.map(new GlossaryTerms(dataMap), entityUrn)));
    }
    return mappingHelper.getResult();
  }

  private void mapEditableProperties(@Nonnull ERModelRelation ermodelrelation, @Nonnull DataMap dataMap) {
    final EditableERModelRelationProperties editableERModelRelationProperties = new EditableERModelRelationProperties(dataMap);
  ...
  private void mapERModelRelationKey(@Nonnull ERModelRelation ermodelrelation, @Nonnull DataMap datamap) {
  ...
  private void mapProperties(@Nonnull ERModelRelation ermodelrelation, @Nonnull DataMap dataMap) {
    final ERModelRelationProperties ermodelrelationProperties = new ERModelRelationProperties(dataMap);
    ermodelrelation.setProperties(com.linkedin.datahub.graphql.generated.ERModelRelationProperties.builder()
        ...
        .setErmodelrelationFieldMapping(mapERModelRelationFieldMappings(ermodelrelationProperties))
        .setCreatedTime(ermodelrelationProperties.hasCreated() && ermodelrelationProperties.getCreated().getTime() > 0
                ? ermodelrelationProperties.getCreated().getTime() : 0)
        .build());
    if (ermodelrelationProperties.hasCreated() && ermodelrelationProperties.getCreated().hasActor()) {
      ermodelrelation.getProperties().setCreatedActor(UrnToEntityMapper.map(ermodelrelationProperties.getCreated().getActor()));
    }
  }
  ...
  private com.linkedin.datahub.graphql.generated.ERModelRelationFieldMapping mapERModelRelationFieldMappings(
    ...
    return com.linkedin.datahub.graphql.generated.ERModelRelationFieldMapping.builder()
        .setFieldMappings(ermodelrelationProperties.getErmodelrelationFieldMapping()
            .getFieldMappings()
            .stream()
  ...
  private void mapGlobalTags(@Nonnull ERModelRelation ermodelrelation, @Nonnull DataMap dataMap, @Nonnull final Urn entityUrn) {
