    ...
    return CompletableFuture.supplyAsync(
        () -> {
          try {
            ...
            log.error(
                String.format(
                    "Failed to check whether ingestion source for entity %s exists",
                    entityUrn.toString()),
                e);
            return null;
          }
  ...
  private IngestionSource getIngestionSourceForEntity(
    ...
    for (RunInfo run : runs) {
      IngestionSource ingestionSource =
          tryGetIngestionSourceForRunId(run.getId(), entityUrn, context);
