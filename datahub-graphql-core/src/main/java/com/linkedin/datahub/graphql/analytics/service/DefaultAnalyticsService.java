...
/**
 * Search-cluster ({@link SearchClientShim}) analytics for usage and entity indices. Used when usage
 * events are not served from {@link PostgresAnalyticsService}.
 */
@Slf4j
public class DefaultAnalyticsService implements AnalyticsService {
  ...
  @Nullable private final SearchClientShim<?> _elasticClient;

  public DefaultAnalyticsService(
      @Nullable SearchClientShim<?> elasticClient, @Nonnull IndexConvention indexConvention) {
  ...
  public String getUsageIndexName() {
    return _indexConvention.getIndexName(AnalyticsService.DATAHUB_USAGE_EVENT_INDEX);
  }
  ...
  public List<NamedBar> getBarChart(
    ...
    if (showMissing) {
      termAgg.missing(AnalyticsService.NA);
    }
    ...
    if (dimensions.size() == 2) {
      ...
      if (showMissing) {
        secondTermAgg.missing(AnalyticsService.NA);
      }
  ...
  private Filter executeAndExtract(
    ...
    if (_elasticClient == null) {
      throw new IllegalStateException(
          "Analytics requires search indexing for this index (configure platform search / "
              + "elasticsearch.enabled), or use Postgres-backed usage analytics.");
    }
    try {
