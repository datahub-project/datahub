...
public class DataContractMapper {
  ...
  public static DataContract mapContract(@Nonnull final EntityResponse entityResponse) {
    ...
    final EnvelopedAspect dataContractProperties = aspects.get(Constants.DATA_CONTRACT_PROPERTIES_ASPECT_NAME);
    if (dataContractProperties != null) {
      result.setProperties(
          mapProperties(new com.linkedin.datacontract.DataContractProperties(dataContractProperties.getValue().data())));
    } else {
      throw new RuntimeException(String.format("Data Contract does not exist!. urn: %s", entityResponse.getUrn()));
    }

    final EnvelopedAspect dataContractStatus = aspects.get(Constants.DATA_CONTRACT_STATUS_ASPECT_NAME);
    if (dataContractStatus != null) {
      result.setStatus(mapStatus(new com.linkedin.datacontract.DataContractStatus(dataContractStatus.getValue().data())));
    }

    return result;
  }

  private static DataContractProperties mapProperties(final com.linkedin.datacontract.DataContractProperties properties) {
    ...
    if (properties.hasSchema()) {
      result.setSchema(properties.getSchema().stream()
          .map(DataContractMapper::mapSchemaContract)
          .collect(Collectors.toList()));
    }
    if (properties.hasFreshness()) {
      result.setFreshness(properties.getFreshness().stream()
          .map(DataContractMapper::mapFreshnessContract)
          .collect(Collectors.toList()));
    }
    if (properties.hasDataQuality()) {
      result.setDataQuality(properties.getDataQuality().stream()
  ...
  private static DataContractStatus mapStatus(final com.linkedin.datacontract.DataContractStatus status) {
