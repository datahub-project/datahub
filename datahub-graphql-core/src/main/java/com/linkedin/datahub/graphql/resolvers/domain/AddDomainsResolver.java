...
/**
 * Resolver used for adding multiple Domains to a Metadata Asset. Preserves existing domains.
 * Requires the EDIT_DOMAINS privilege for a particular asset.
 */
@Slf4j
@RequiredArgsConstructor
public class AddDomainsResolver implements DataFetcher<CompletableFuture<Boolean>> {
  ...
  public CompletableFuture<Boolean> get(DataFetchingEnvironment environment) throws Exception {
    ...
    return GraphQLConcurrencyUtils.supplyAsync(
        () -> {
          ...
          validateAddDomainsInput(context, entityUrn, domainUrns);
          ...
          try {
            ...
            log.error(
                "Failed to add Domains to resource with entity urn {}, domain urns {}: {}",
                entityUrn,
                domainUrns,
                e.getMessage());
            throw new RuntimeException(
                String.format(
                    "Failed to add Domains to resource with entity urn %s, domain urns %s",
  ...
  private void validateAddDomainsInput(QueryContext context, Urn entityUrn, List<Urn> domainUrns) {
    for (Urn domainUrn : domainUrns) {
      DomainUtils.validateDomain(context.getOperationContext(), domainUrn, _entityService);
    }

    if (!_entityService.exists(context.getOperationContext(), entityUrn, true)) {
      throw new IllegalArgumentException(
          String.format("Failed to add Domains to Entity %s. Entity does not exist.", entityUrn));
    }
  }
}
