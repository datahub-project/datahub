...
public class SchemaAssertionUtils {
  ...
  @Nonnull
  public static AssertionEvaluationParameters mapEvaluationParameters(
      @Nullable final DatasetSchemaAssertionParametersInput input) {
    final DatasetSchemaAssertionParameters result = new DatasetSchemaAssertionParameters();
    if (input == null) {
      result.setSourceType(DatasetSchemaSourceType.DATAHUB_SCHEMA);
    } else {
      result.setSourceType(DatasetSchemaSourceType.valueOf(input.getSourceType().toString()));
    }
    return new AssertionEvaluationParameters()
        .setType(AssertionEvaluationParametersType.DATASET_SCHEMA)
        .setDatasetSchemaParameters(result);
  }
