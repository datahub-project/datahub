    ...
    return CompletableFuture.supplyAsync(() -> {
      ...
      if (IngestionAuthUtils.canManageIngestion(context)) {
        ...
        try {
          ...
          actionPipeline.put("action", getActionBlock(input.getType(), input.getConfig().getRecipe()));

          // Fetch the original ingestion source
          final DataHubActionKey key = new DataHubActionKey();
          key.setId(input.getName());
          final Urn actionPipelineUrn = EntityKeyUtils.convertEntityKeyToUrn(key, "dataHubAction");
          ...
          actionInfo.setConfig(new DataHubActionConfig()
              .setRecipe(actionPipeline.toString()));
          log.info("Action Info aspect = {}", actionInfo);

          final MetadataChangeProposal proposal = buildMetadataChangeProposalWithUrn(actionPipelineUrn,
              "dataHubActionInfo", actionInfo);
          log.info("Entity client = {}", _entityClient);
          log.info("proposal = {}", proposal);
          String result =  _entityClient.ingestProposal(proposal, context.getAuthentication(), false);
          log.info("result = {}", result);
          return result;
        } catch (Exception e) {
          log.error("Failed to ingest action pipeline", e);
          throw new RuntimeException(String.format("Failed to create new action pipeline %s", input), e);
        }
      }
