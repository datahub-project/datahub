...
public class DataHubViewMapper implements ModelMapper<EntityResponse, DataHubView> {
  ...
  @Nonnull
  private DataHubViewDefinition mapViewDefinition(@Nonnull final com.linkedin.view.DataHubViewDefinition definition) {
    final DataHubViewDefinition result = new DataHubViewDefinition();
    result.setFilter(mapFilter(definition.getFilter()));
    result.setEntityTypes(definition.getEntityTypes().stream().map(EntityTypeMapper::getType).collect(
        Collectors.toList()));
    return result;
  }

  @Nullable
  private DataHubViewFilter mapFilter(@Nonnull final com.linkedin.metadata.query.filter.Filter filter) {
    // This assumes that people DO NOT emit Views on their own, since we expect that the Filter structure is within
    // a finite set of possibilities.
    //
    // If we find a View that was ingested manually and malformed, then we log that and return a default.
  ...
  /**
   * This converts a list of Conjunctive Criterion into a flattened list
   * of FacetFilters. This method makes the assumption that WE (our GraphQL API)
   * has minted the View and that each or criterion contains at maximum one nested condition.
   */
  @Nonnull
  private List<FacetFilter> mapOrFilters(@Nullable final List<ConjunctiveCriterion> ors) {
    ...
    if (ors.stream().anyMatch(or -> or.hasAnd() && or.getAnd().size() > 1)) {
      log.warn(String.format(
          "Detected a View with a malformed filter clause. OR view has children conjunctions with more than one Criterion. Returning empty filters. %s", ors));
      return Collections.emptyList();
    }
