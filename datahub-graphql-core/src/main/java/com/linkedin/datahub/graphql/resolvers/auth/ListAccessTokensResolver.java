...
public class ListAccessTokensResolver implements DataFetcher<CompletableFuture<ListAccessTokenResult>> {
    ...
    private static final String EXPIRES_AT_FIELD_NAME = "expiredAt";
    private static final Set<String> FACET_FIELDS = ImmutableSet.of("ownerUrn", "actorUrn", "name", "createdAt",
            "expiredAt", "description");
    ...
    public CompletableFuture<ListAccessTokenResult> get(DataFetchingEnvironment environment) throws Exception {
        return CompletableFuture.supplyAsync(() -> {
            ...
            final List<FacetFilterInput> filters = input.getFilters() == null ? ImmutableList.of() : input.getFilters();

            log.info("User {} listing access tokens with filters {}", context.getActorUrn(), filters.toString());

            if (AuthorizationUtils.canManageAllPersonalTokens(context) || isListingSelfTokens(filters, context)) {
                try {
