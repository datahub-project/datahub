  ...
  public CompletableFuture<AssertionResult> get(final DataFetchingEnvironment environment) throws Exception {
    ...
    return CompletableFuture.supplyAsync(() -> {
      if (AssertionUtils.isAuthorizedToEditAssertionFromAssertee(context, asserteeUrn)) {
        if (input.getType() == AssertionType.SQL) {
          // Format the assertion info
          final com.linkedin.assertion.AssertionResult assertionResult = _monitorService.testSqlAssertion(
              asserteeUrn,
              connectionUrn,
              SqlAssertionUtils.createSqlAssertionInfo(input.getSqlTestInput())
          );

          // Then, return the assertion test result
          return AssertionRunEventMapper.mapResult(assertionResult);
        }
