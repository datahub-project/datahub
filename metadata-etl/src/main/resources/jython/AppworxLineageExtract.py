...
class AppworxLineageExtract:
    ...
    for row in rows:
      try:
        ...
        try:
          ...
          self.logger.warn(
            "Fails to find script file: %s/%s. Skipping the file" % (results['script_path'], results['script_name']))
          continue
            ...
            self.aw_cursor.execute(update_staging_lineage_query %
                                   (int(row['app_id']), int(row['flow_exec_id']), int(row['job_exec_id']),
                                    ...
                                    ('/' + schema_name + '/' + results['table'][0]['abstracted_path']) \
                                      if schema_name else results['table'][0]['abstracted_path'],
                                    results['table'][0]['full_path'],
      ...
      try:
        ...
        for k, tab in enumerate(results['table']):
          self.aw_cursor.execute(update_staging_lineage_query %
                                 (int(row['app_id']), int(row['flow_exec_id']), int(row['job_exec_id']),
                                  row['flow_path'], row['job_name'], int(row['start_time']), int(row['end_time']),
                                  db_id if tab['table_type'] == 'source' else 0, tab['abstracted_path'],
                                  tab['full_path'],
        ...
        if m is not None:
          ...
          if pig_script is not None:
            ...
            if any(src_tgt_map) == 0:
              self.logger.warn(
                'Pig log parser fails to retrieve relevant information from file: %s. Most probably Pig script did not complete successfully' %
                row['gzipped_file_name'])
              continue
            ...
            for k, v in src_tgt_map.items():
              ...
              self.aw_cursor.execute(update_staging_lineage_query %
                                     (int(row['app_id']), int(row['flow_exec_id']), int(row['job_exec_id']),
                                      ...
                                      src_tgt_map[k]['max_end_partition'] if src_tgt_map[k][
                                                                               'table_type'] != 'snapshot' else None,
                                      src_tgt_map[k]['max_end_partition'] if src_tgt_map[k][
                                                                               'table_type'] != 'snapshot' else None,
  ...
  def process_kafka_sonora_hadoop_get(self):
    ...
    for row in rows:
      try:
        self.logger.info('Parsing log file: %s' % row['gzipped_file_name'])
        results = AppworxLogParser(log_file_name=row['gzipped_file_name']).parse_log({},
                                                                                     command_type='KFK_SONORA_HADOOP_GET')
        if any(results) == 0:
          self.logger.info('Skipped parsing %s' % row['gzipped_file_name'])
          continue
          ...
          for index, tab in enumerate(results['table']):
            self.aw_cursor.execute(update_staging_lineage_query %
                                   (int(row['app_id']), int(row['flow_exec_id']), int(row['job_exec_id']),
                                    ...
                                    tab['frequency'], tab['frequency'],
                                    tab['storage_type'],
                                    'source' if index == 0 else 'target', index + 1, 0,
                                    'Read' if index == 0 else 'Write',
    ...
    for row in rows:
      try:
        ...
        update_source_code_query = \
          ...
          """ % (int(row['app_id']), int(row['job_id']), int(row['attempt_id']), results['script_name'],
                 results['script_path'])
