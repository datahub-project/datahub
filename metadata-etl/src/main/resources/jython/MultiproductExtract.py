...
class MultiproductLoad:
  ...
  def get_multiproducts(self):
    ...
    '''
    resp = requests.get(args[Constant.MP_PRODUCT_URL], verify=False)
  ...
  def get_project_repo(self):
    ...
    csv_columns = ["project_name", "scm_type", "owner_type", "owner_name", "create_time", "num_of_repos", "repos",
                   "license", "description"]
    gitli_project_csv = args[Constant.MP_GIT_PROJECT_OUTPUT_KEY]
  ...
  def get_acl_owners(self):
    ...
    for key, repo in self.git_repo.iteritems():
      repo_url = '{}/{}'.format(args[Constant.MP_GIT_REPO_URL], repo['scm_repo_fullname'])
      try:
        resp = requests.get('{}/source/acl'.format(repo_url), verify=False)
      except Exception as ex:
        self.logger.info("Error getting acl {}/source/acl - {}".format(repo_url, ex.message))
        continue
      if resp.status_code != 200:
      ...
      if resp.headers['content-type'].split(';')[0] == 'text/html':
        for acl_name in re_acl_names.findall(resp.content):
          try:
            ...
            continue
          if resp.status_code != 200:
            self.logger.debug('Request Error: GET repo {} acl {} - {}'.format(repo, acl_name, resp.status_code))
            continue
