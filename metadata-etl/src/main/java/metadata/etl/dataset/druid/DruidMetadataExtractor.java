...
public class DruidMetadataExtractor {
	...
	private final String SEGMENT_QUERY = "{\"queryType\":\"segmentMetadata\",\"dataSource\":\"$DATASOURCE\", \"intervals\":[\"$INTERVAL\"]}"; 
	private final String INTERVAL_QUERY = "{\"queryType\":\"timeBoundary\",\"dataSource\":\"$DATASOURCE\"}";
	private final org.joda.time.format.DateTimeFormatter DATETIME_FORMAT = DateTimeFormat.forPattern("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'");
	
	
	public DruidMetadataExtractor () throws Exception{
		druid_host = Constant.DRUID_HOST_URL;
	...
	public void format_field_metadata(String datasource, JSONArray seg_metadata) throws Exception{
		...
		for (int i=0; i<seg_metadata.length(); i++){
			...
			while (itr.hasNext()){
				...
				String out =    dict_field.get("datasource")
						+ "\t" + dict_field.get("intervals")
						+ "\t" + dict_field.get("name") 
						+ "\t" + dict_field.get("type")
						+ "\t" + dict_field.get("size")
						+ "\t" + dict_field.get("cardinality")
						+ "\t" + dict_field.get("errorMessage")
						+ "\n";
	...
	//		dict_dataset.put("partition_layout_pattern_id", "");		//TODO: Check the partition layout pattern
			...
			String out = dict_dataset.get("name") + "\t"
					   ...
					   + dict_dataset.get("schema_type") + "\t"
					   + dict_dataset.get("properties") + "\t"
					   + dict_dataset.get("fields") + "\t"
					   + dict_dataset.get("urn") + "\t"
