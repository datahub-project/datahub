...
def test_consumers_mode_persisted_in_env_file(tmp_path, monkeypatch):
    env_file = tmp_path / "test.env"
    monkeypatch.setattr(datahub_dev, "DEV_ENV_FILE", env_file)
    assert datahub_dev._is_consumers_mode() is False
    datahub_dev._set_consumers_mode(True)
    assert datahub_dev._is_consumers_mode() is True
    assert "DATAHUB_DEV_CONSUMERS=true" in env_file.read_text()
    datahub_dev._set_consumers_mode(False)
    assert datahub_dev._is_consumers_mode() is False
    assert "DATAHUB_DEV_CONSUMERS" not in env_file.read_text()


def test_start_parser_consumers_flags():
    parser = datahub_dev.build_parser()
    args = parser.parse_args(["start", "--consumers"])
    assert args.consumers is True
    assert args.no_consumers is False
    args = parser.parse_args(["start", "--no-consumers"])
    assert args.consumers is False
    assert args.no_consumers is True


def test_docker_service_ready_matches_substr():
    services = {
    ...
    assert datahub_dev._docker_service_ready(services, "mae-consumer") is True
    assert datahub_dev._docker_service_ready(services, "mce-consumer") is False
