...
- In connector code and comments, never reference other connectors by name — even when mirroring another connector's approach.
- No "AI slop" comments: strip filler/obvious comments; keep only comments that explain *why*.
- In connectors, surface skips, dropped/unmatched items, and failures via `self.report.warning(...)`/`report.failure(...)` (with title/message/context/exc) and report counters — never a silent broad-except debug-only log.

## Learned Workspace Facts

- The user contributes to the public `datahub-project/datahub` repo and can create and push branches directly on `datahub-public-repo` (PRs against origin, based on `master`).
