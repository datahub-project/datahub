...
- Snowflake semantic views: opt-in `semantic_views.emit_semantic_model_entities` switches dataset (subtype Semantic View) → semanticModel + metric entities. Governance migration is `datahub migrate snowflake-semantic-views` (`snowflake_semantic_view_migration.py`); design doc at `docs/developers/snowflake-semantic-view-migration.md`. Soft-delete of old URNs is stateful ingestion, not the CLI. Column fan-out: DataHub-only field tags/terms go to metric entities (METRIC) or schemaField URNs under the semanticModel (dims/facts); synthetic DIMENSION/FACT/METRIC tags are stripped. `browsePathsV2` is connector-owned — do not migrate it.
