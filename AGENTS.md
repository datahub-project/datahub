...
- Do not add docstrings at the top of files, functions, methods, or Pydantic models unless genuinely necessary; proactively strip "AI slop" comments and boilerplate docstrings (this is requested repeatedly).
- Keep ingestion source config field descriptions short and concise — long descriptions are repeatedly flagged.
- Never reference specific customers, customer names/data, or internal ticket numbers in code comments, docs, or PR titles/descriptions.
- Prefer Pydantic models (in a `models.py`) over tuples or `from typing import Any` for return types and internal data; tuples are hard to track and `Any` should be replaced with exact annotations.

## Learned Workspace Facts

- New ingestion connectors should mirror existing connector structure: a `constants.py`, a `models.py` of Pydantic models, plus unit and integration tests. Core integration tests should run against a `docker-compose` fixture like other connectors; mocks are acceptable only for cloud-only/SaaS features that have no local instance.
