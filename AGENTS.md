...
- In `metadata-ingestion` connector code, avoid double-quoted string literals: hoist magic strings (including SQL snippets) into module-level constants — use a dedicated `constants.py`/`queries.py` for SQL — keep regex pre-compiled, and prefer `.format()` templates over scattered inline f-strings.
...
- Prefer `self.report.warning(...)` and report counters over bare `logger` for skips and edge cases — the report also writes to the log and surfaces to operators. On decode/parse/discovery failure, return `None` and report rather than emitting synthetic placeholder fields (e.g. `base64`/`binary_data`/decode-error dicts) that silently pollute the profile; narrow broad `except`s, match on exception type not `str(e)`, and never silently fall back to a full scan.
- For lineage, use the central `SqlParsingAggregator` (`create_lineage_from_sql_statements`) with a platform map instead of setting sqlglot dialects per-connector, and don't leave column-level edges coarse: resolve upstream/downstream schemas from the DataHub graph when available, load known URNs from the platform/platform_instance/env mapping, match columns case-insensitively, and mirror existing connectors for cross-platform mapping, two- vs three-part names, and temp-table handling. Best-effort is fine, but try everything.
- In connector code, use explicit type annotations rather than `from typing import Any` (Unions are fine when a value genuinely has multiple types), prefer `Dict`/`List` from `typing` over the builtin `dict`/`list`, and give config fields constrained pydantic types (`PositiveInt`/`NonNegativeInt`, bounded values) instead of bare `int`.
- When given a code review with 🔴 blockers / 🟡 warnings / ℹ️ suggestions, fix all of them — including the informational "FYI"/suggestion items — not just the blockers.
