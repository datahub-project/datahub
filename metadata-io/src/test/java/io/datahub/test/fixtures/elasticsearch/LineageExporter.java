...
public class LineageExporter<O> {
    ...
    @Builder.Default
    private int queryStatementSize = 32000;
    @NonNull
    ...
    public void exportGraphIndex(Set<String> urns, Set<String> visitedUrns, Set<String> visitedIds, int hops) {
        ...
        if (!urns.isEmpty()) {
            ...
            Lists.partition(Arrays.asList(urns.toArray(String[]::new)), queryStatementSize).forEach(batch -> {
                boolQueryBuilder.should(QueryBuilders.termsQuery("source.urn", batch.toArray(String[]::new)));
                boolQueryBuilder.should(QueryBuilders.termsQuery("destination.urn", batch.toArray(String[]::new)));
            });
            boolQueryBuilder.minimumShouldMatch(1);

            // Exclude visited
            Lists.partition(Arrays.asList(visitedIds.toArray(String[]::new)), queryStatementSize).forEach(batch ->
                boolQueryBuilder.mustNot(QueryBuilders.idsQuery().addIds(batch.toArray(String[]::new)))
            );
            ...
            Set<String> discoveredUrns = docs.stream().flatMap(d -> Stream.of(d.destination.urn, d.source.urn))
    ...
    public void exportEntityIndex(Set<String> ids, Set<String> visitedIds, int hops) {
        ...
        if (!ids.isEmpty()) {
            ...
            Lists.partition(Arrays.asList(urns.toArray(String[]::new)), queryStatementSize).forEach(batch ->
                    boolQueryBuilder.should(QueryBuilders.termsQuery("upstreams.keyword", batch.toArray(String[]::new)))
            );
            Lists.partition(Arrays.asList(ids.toArray(String[]::new)), queryStatementSize).forEach(batch ->
                    boolQueryBuilder.should(QueryBuilders.idsQuery().addIds(batch.toArray(String[]::new)))
            );
            boolQueryBuilder.minimumShouldMatch(1);
            ...
            nextIds.addAll(docIds.stream()
                    .filter(Objects::nonNull)
                    .filter(docId -> !visitedIds.contains(docId))
                    .collect(Collectors.toSet()));
            nextIds.addAll(docs.stream()
