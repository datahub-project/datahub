...
public class OwnerServiceTest {
  ...
  private void testAddOwnersExistingOwner() throws Exception {
    ...
    existingOwnership.setOwners(new OwnerArray(ImmutableList.of(
    ...
    List<MetadataChangeProposal> events = service.buildAddOwnersProposals(
    ...
    OwnerArray expected = new OwnerArray(
        ImmutableList.of(
            new Owner().setOwner(TEST_OWNER_URN_1).setType(OwnershipType.NONE),
    ...
    Ownership ownerAspect1 = GenericRecordUtils.deserializeAspect(
    ...
    Ownership ownerAspect2 = GenericRecordUtils.deserializeAspect(
  ...
  private void testRemoveOwnerExistingOwners() throws Exception {
    ...
    List<MetadataChangeProposal> events = service.buildRemoveOwnersProposals(
    ...
    Ownership expected = new Ownership().setOwners(new OwnerArray(
        ImmutableList.of(new Owner().setOwner(TEST_OWNER_URN_2).setType(OwnershipType.DATA_STEWARD))));

    MetadataChangeProposal event1 = events.get(0);
    Assert.assertEquals(event1.getAspectName(), Constants.OWNERSHIP_ASPECT_NAME);
    Assert.assertEquals(event1.getEntityType(), Constants.DATASET_ENTITY_NAME);
    RecordTemplate ownersAspect1 = GenericRecordUtils.deserializeAspect(
        event1.getAspect().getValue(),
        event1.getAspect().getContentType(),
        Ownership.class);
    ...
    Assert.assertEquals(event2.getAspectName(), Constants.OWNERSHIP_ASPECT_NAME);
    Assert.assertEquals(event2.getEntityType(), Constants.DATASET_ENTITY_NAME);
    RecordTemplate ownersAspect2 = GenericRecordUtils.deserializeAspect(
        event2.getAspect().getValue(),
        event2.getAspect().getContentType(),
        Ownership.class);
  ...
  private void testRemoveOwnerNoExistingOwners() throws Exception {
    ...
    Assert.assertEquals(event1.getAspectName(), Constants.OWNERSHIP_ASPECT_NAME);
    Assert.assertEquals(event1.getEntityType(), Constants.DATASET_ENTITY_NAME);
    Ownership ownersAspect1 = GenericRecordUtils.deserializeAspect(
        event1.getAspect().getValue(),
        event1.getAspect().getContentType(),
        Ownership.class);
    ...
    Assert.assertEquals(event2.getAspectName(), Constants.OWNERSHIP_ASPECT_NAME);
    Assert.assertEquals(event2.getEntityType(), Constants.DATASET_ENTITY_NAME);
    Ownership ownersAspect2 = GenericRecordUtils.deserializeAspect(
        event2.getAspect().getValue(),
        event2.getAspect().getContentType(),
        Ownership.class);
  ...
  private static EntityClient createMockOwnersClient(@Nullable Ownership existingOwnership) throws Exception {
