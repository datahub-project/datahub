...
public class AnomalyServiceTest {
  ...
  private static final Urn TEST_NON_EXISTENT_ANOMALY_URN =
      UrnUtils.getUrn("urn:li:anomaly:test-non-existent");
  ...
  private void testUpdateAnomaliesSummary() throws Exception {
    ...
    Mockito.verify(mockClient, Mockito.times(1))
        .ingestProposal(
            any(OperationContext.class), Mockito.eq(mockAnomalySummaryMcp()), Mockito.eq(false), true);
  }
  ...
  private void testRaiseAnomalyRequiredFields() throws Exception {
    ...
    final AnomalyInfo expectedInfo =
        new AnomalyInfo()
