...
public class LineageSearchServiceTest {
  ...
  public void init() {
    ...
    ImpactConfiguration impactConfig = ImpactConfiguration.builder().build();

    // Create cache configuration
    CacheConfiguration cacheConfig = CacheConfiguration.builder().build();
    SearchCacheConfiguration searchCacheConfig = SearchCacheConfiguration.builder().build();
    SearchLineageCacheConfiguration searchLineageCacheConfig =
        SearchLineageCacheConfiguration.builder().build();

    // Create GraphService configuration
    GraphServiceConfiguration graphServiceConfig = GraphServiceConfiguration.builder().build();
    LimitConfig limitConfig = LimitConfig.builder().build();
    ResultsLimitConfig resultsConfig = ResultsLimitConfig.builder().build();
    ...
    cacheConfig.setSearch(searchCacheConfig);
    searchCacheConfig.setLineage(searchLineageCacheConfig);
    searchLineageCacheConfig.setTtlSeconds(600L); // 10 minutes
    searchLineageCacheConfig.setLightningThreshold(1000L);

    // Create MetadataChangeProposalConfig to avoid NPE
    MetadataChangeProposalConfig metadataChangeProposalConfig = new MetadataChangeProposalConfig();
    MetadataChangeProposalConfig.SideEffectsConfig sideEffectsConfig =
        new MetadataChangeProposalConfig.SideEffectsConfig();
    MetadataChangeProposalConfig.SideEffectConfig schemaFieldConfig =
        new MetadataChangeProposalConfig.SideEffectConfig();
    _appConfig.setMetadataChangeProposal(metadataChangeProposalConfig);
    metadataChangeProposalConfig.setSideEffects(sideEffectsConfig);
    sideEffectsConfig.setSchemaField(schemaFieldConfig);
    schemaFieldConfig.setEnabled(false);

    // Set up GraphService configuration
    when(_graphService.getGraphServiceConfig()).thenReturn(graphServiceConfig);
    graphServiceConfig.setLimit(limitConfig);
    limitConfig.setResults(resultsConfig);
    resultsConfig.setApiDefault(100);
