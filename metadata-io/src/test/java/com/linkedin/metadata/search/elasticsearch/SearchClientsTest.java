...
public class SearchClientsTest {
  ...
  public void testComponentForManagedIndexClassifiesGraphAndSystemMetadata() {
    ...
    assertSame(
        SearchClients.componentForManagedIndex(
            opContext, convention.getTimeseriesAspectIndexName(opContext, "dataset", "operation")),
  ...
  /**
   * ESWriteDAO is shared by graph, timeseries and system-metadata, so routing a resolved name must
   * not assume it is an entity family.
   */
  @Test
  public void testForIndexRoutesNonEntityIndicesToTheirOwnComponent() {
    SearchClientShim<?> v2 = mock(SearchClientShim.class);
    SearchClientShim<?> graph = mock(SearchClientShim.class);
    ...
    assertSame(
        SearchClients.forIndex(opContext, convention.getIndexName(opContext, "graph_service_v1")),
        graph);
    assertSame(
        SearchClients.forIndex(
            opContext, convention.getTimeseriesAspectIndexName(opContext, "dataset", "operation")),
        timeseries);
    assertSame(SearchClients.forIndex(opContext, "datasetindex_v2"), v2);
  }
}
