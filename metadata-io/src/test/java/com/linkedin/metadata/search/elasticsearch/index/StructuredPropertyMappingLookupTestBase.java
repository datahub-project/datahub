...
public abstract class StructuredPropertyMappingLookupTestBase
  ...
  public void testFindsFlatStructuredPropertyFieldFromV2Mapping() throws IOException {
    assertTrue(lookup.fieldExists("certification_status"));
    assertFalse(lookup.fieldExists("other_status"));
  }

  @Test
  public void testFindsNestedVersionedFieldFromV3Mapping() throws IOException {
    assertTrue(lookup.fieldExists("_versioned.certification_status.00000000000001.string"));
    assertFalse(lookup.fieldExists("_versioned.certification_status.00000000000001.number"));
  }

  @Test
  public void testIgnoresMappingsOutsideEntityIndexPatterns() throws IOException {
    assertFalse(lookup.fieldExists("should_not_match"));
  }

  @Test
  public void testSeesNewlyPutStructuredPropertyMapping() throws IOException {
    assertFalse(lookup.fieldExists("newly_added_status"));
    ...
    getSearchClient().putIndexMapping(putMappingRequest, RequestOptions.DEFAULT);

    assertTrue(lookup.fieldExists("newly_added_status"));
  }
  ...
  private void createIndex(@Nonnull String indexName, @Nonnull Map<String, Object> mapping)
    ...
    getSearchClient().createIndex(createRequest, RequestOptions.DEFAULT);
  }
