...
public class IgnoreUnknownMutatorTest {
  ...
  @Test
  public void testDropsUnknownAspectWithoutFailingBatch() {
    IgnoreUnknownMutator test = new IgnoreUnknownMutator();
    test.setConfig(TEST_PLUGIN_CONFIG);
    ...
    List<MCPItem> testItems =
        List.of(
            ProposedItem.builder()
                .buildAllowingUnknownAspect(
                    new MetadataChangeProposal()
                        .setEntityUrn(TEST_DATASET_URN)
                        .setAspectName("notARegisteredAspect")
                        .setEntityType(DATASET_ENTITY_NAME)
                        .setChangeType(ChangeType.UPSERT)
                        .setAspect(
                            new GenericAspect()
                                .setContentType("application/json")
                                .setValue(ByteString.copyString("{}", StandardCharsets.UTF_8)))
                        .setSystemMetadata(new SystemMetadata()),
                    AuditStampUtils.createDefaultAuditStamp(),
                    TEST_REGISTRY),
            ProposedItem.builder()
    ...
    assertEquals(result.get(0).getAspectName(), DATASET_PROPERTIES_ASPECT_NAME);
    assertEquals(
        result.get(0).getAspect(DatasetProperties.class),
        new DatasetProperties().setCustomProperties(new StringMap(Map.of("prop1", "ok"))));
  }
}
