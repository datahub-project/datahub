...
public class PrivilegeGrantAuthorizationValidatorTest {
  ...
  private void existingAspect(Urn urn, String aspectName, RecordTemplate aspect) {
    when(aspectRetriever.getLatestAspectObjects(anySet(), anySet()))
        .thenAnswer(
            args -> {
              final Set<Urn> urns = args.getArgument(0);
              final Set<String> aspectNames = args.getArgument(1);
  ...
  private List<AspectValidationException> validate(BatchItem... items) {
    ...
    return validator.validateItems(batch, batch, retrieverContext, session);
  }
  ...
  public void testCurrentStateReadIsBatchedPerEntityType() {
    grantPrivileges(MANAGE_POLICIES);
    when(aspectRetriever.getLatestAspectObjects(anySet(), anySet())).thenReturn(Map.of());
    ...
    verify(aspectRetriever, times(2)).getLatestAspectObjects(anySet(), anySet());
    verify(aspectRetriever, never()).getLatestAspectObject(any(Urn.class), anyString());
  }
  ...
  public void testUnguardedAspectsTriggerNoRead() {
    ...
    verify(aspectRetriever, never()).getLatestAspectObjects(anySet(), anySet());
  }
