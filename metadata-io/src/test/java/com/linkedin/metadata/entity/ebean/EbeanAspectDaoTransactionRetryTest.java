...
public class EbeanAspectDaoTransactionRetryTest {
  ...
  public void setUp() {
    ...
    dao = new TestableEbeanAspectDao(mockServer, config, metricUtils);
  ...
  public void testDeadlockExhaustion_attemptsFourTimes_sleepsThreeTimes_throwsConflict() {
    ...
    DatabaseTransactionConflictException thrown =
        expectThrows(
            ...
            () -> dao.runInTransactionWithRetryUnlocked(block, null, MAX_RETRIES));
  ...
  public void testDeadlockExhaustion_propagatesConfiguredRetryAfterSeconds() {
        ...
        EbeanConfiguration.builder()
            .transactionRetry(
                TransactionRetryConfiguration.builder()
                    ...
                    .build())
            .build();
    TestableEbeanAspectDao customDao = new TestableEbeanAspectDao(mockServer, config, metricUtils);
    ...
    DatabaseTransactionConflictException thrown =
        expectThrows(
            DatabaseTransactionConflictException.class,
            () -> customDao.runInTransactionWithRetryUnlocked(block, null, MAX_RETRIES));
  ...
  public void testDeadlockExhaustion_withBatch_recordsIngestPathMetrics() {
    ...
    expectThrows(
        DatabaseTransactionConflictException.class,
        () -> dao.runInTransactionWithRetryUnlocked(block, batch, MAX_RETRIES));
  ...
  private static final class TestableEbeanAspectDao extends EbeanAspectDao {
    ...
    TestableEbeanAspectDao(Database server, EbeanConfiguration config, MetricUtils metricUtils) {
      super(server, config, metricUtils, List.of(), null);
    }
