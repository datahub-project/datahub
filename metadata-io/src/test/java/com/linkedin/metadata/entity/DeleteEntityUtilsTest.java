...
public class DeleteEntityUtilsTest extends TestCase {
  ...
  public void testEmptyStructRemoval() {
    ...
    final Aspect updatedAspect =
        DeleteEntityUtils.getAspectWithReferenceRemoved(
            "hello", aspect, schema, new PathSpec("key_a"));
  ...
  /**
   * Tests that Aspect Processor deletes a required value from a record referenced by another
   * record.
   */
  @Test
  public void testNestedFieldRemoval() {
    ...
    final Aspect updatedAspect =
        DeleteEntityUtils.getAspectWithReferenceRemoved(
            "hello", aspect, schema, new PathSpec("key_c", "key_a"));

    assertFalse(updatedAspect.data().containsKey("key_c"));
  }

  /**
   * Tests that Aspect Processor is able to delete an optional sub-field while preserving nested
   * structs.
   */
  @Test
  ...
  /**
   * Tests that the Aspect Processor will delete an entire struct if after removal of a field, it
   * becomes empty & is optional at some higher level.
   */
  @Test
  public void testRemovalOptionalFieldWithNonOptionalSubfield() {
    ...
    final Aspect updatedAspect =
        DeleteEntityUtils.getAspectWithReferenceRemoved(
            "world", aspect, schema, new PathSpec("key_c", "key_b"));
  ...
  /**
   * Tests that Aspect Processor is able to remove sub-field from array field while preserving
   * nested structs.
   */
  @Test
  public void testRemovalNestedFieldFromArray() {
    final String value =
        "{\"key_c\": [{\"key_a\": \"hello\", \"key_b\": \"world\"}, {\"key_b\": \"extra info\"}]}";
    final Aspect aspect = RecordUtils.toRecordTemplate(Aspect.class, value);
    ...
    assertFalse(
        ((DataMap) ((DataList) updatedAspect.data().get("key_c")).get(0)).containsKey("key_a"));
    assertTrue(
        ((DataMap) ((DataList) updatedAspect.data().get("key_c")).get(0)).containsKey("key_b"));
    assertTrue(
        ((DataMap) ((DataList) updatedAspect.data().get("key_c")).get(1)).containsKey("key_b"));
    assertEquals(
        "world", ((DataMap) ((DataList) updatedAspect.data().get("key_c")).get(0)).get("key_b"));
    assertEquals(
        "extra info",
        ((DataMap) ((DataList) updatedAspect.data().get("key_c")).get(1)).get("key_b"));
  }
  ...
  /**
   * Tests that Aspect Processor removes nested structs more than 1 level deep from an optional
   * field.
   */
  @Test
  public void testNestedNonOptionalSubFieldsOnOptionalField() {
    ...
    final Aspect updatedAspect =
        DeleteEntityUtils.getAspectWithReferenceRemoved(
            "hello", aspect, schema, new PathSpec("key_c", "key_b", "key_a"));

    assertFalse(updatedAspect.data().containsKey("key_c"));
  }

  /**
   * Tests that the aspect processor is able to remove fields that are deeply nested where the
   * top-level field is optional. This example is based on the SchemaMetadata object.
   */
  @Test
  public void testSchemaMetadataDelete() {
    final String value =
        "{\"fields\": [{\"globalTags\": {\"tags\": [{\"tag\": \"urn:li:tag:Dimension\"}]}}]}";
    final Aspect aspect = RecordUtils.toRecordTemplate(Aspect.class, value);
    ...
    assertEquals(
        ...
        ((DataList)
                ((DataMap)
                        ((DataMap) ((DataList) updatedAspect.data().get("fields")).get(0))
                            .get("globalTags"))
                    .get("tags"))
            .size());
  }
