...
public class DomainPropertiesChangeEventGeneratorTest {

  private static final String ENTITY_URN = "urn:li:domain:engineering";
  ...
  private static EntityAspect makePropertiesAspect(String name, String description, long version) {
    ...
    aspect.setAspect("domainProperties");
  ...
  /** Creates an EntityAspect with null metadata — simulates the initial empty state. */
  ...
  public void testNameAdded() {
    DomainPropertiesChangeEventGenerator generator = new DomainPropertiesChangeEventGenerator();

    EntityAspect previous = makeEmptyAspect(0);
    EntityAspect current = makePropertiesAspect("My Domain", null, 1);
    ...
    assertTrue(event.getDescription().contains("My Domain"));
  }
