...
public class ESGraphQueryDAOTest {
  ...
  private static final String TEST_QUERY_FILE_LIMITED = "elasticsearch/sample_filters/lineage_query_filters_limited.json";
  private static final String TEST_QUERY_FILE_FULL = "elasticsearch/sample_filters/lineage_query_filters_full.json";
  ...
  private static void testGetQueryForLineageFullArguments() throws Exception {
    ...
    String expectedQueryFullEmptyFilters = Resources.toString(urlFullEmptyFilters, StandardCharsets.UTF_8);
    URL urlFullMultipleFilters = Resources.getResource(TEST_QUERY_FILE_FULL_MULTIPLE_FILTERS);
    String expectedQueryFullMultipleFilters = Resources.toString(urlFullMultipleFilters, StandardCharsets.UTF_8);

    List<Urn> urns = List.of(Urn.createFromString("urn:li:dataset:test-urn"));
    List<Urn> urnsMultiple1 = ImmutableList.of(UrnUtils.getUrn("urn:li:dataset:test-urn"),
        UrnUtils.getUrn("urn:li:dataset:test-urn2"),
        UrnUtils.getUrn("urn:li:dataset:test-urn3"));
    List<Urn> urnsMultiple2 = ImmutableList.of(UrnUtils.getUrn("urn:li:chart:test-urn"),
    ...
    List<LineageRegistry.EdgeInfo> edgeInfosMultiple1 = ImmutableList.of(
        new LineageRegistry.EdgeInfo("DownstreamOf", RelationshipDirection.OUTGOING, Constants.DATASET_ENTITY_NAME),
        new LineageRegistry.EdgeInfo("Consumes", RelationshipDirection.OUTGOING, Constants.DATASET_ENTITY_NAME));
    List<LineageRegistry.EdgeInfo> edgeInfosMultiple2 = ImmutableList.of(
        new LineageRegistry.EdgeInfo("DownstreamOf", RelationshipDirection.OUTGOING, Constants.DATA_JOB_ENTITY_NAME),
        new LineageRegistry.EdgeInfo("Consumes", RelationshipDirection.OUTGOING, Constants.DATA_JOB_ENTITY_NAME));
    String entityType = "testEntityType";
    Map<String, List<Urn>> urnsPerEntityType = Map.of(entityType, urns);
    Map<String, List<Urn>> urnsPerEntityTypeMultiple = Map.of(Constants.DATASET_ENTITY_NAME, urnsMultiple1,
        Constants.CHART_ENTITY_NAME, urnsMultiple2);
    Map<String, List<LineageRegistry.EdgeInfo>> edgesPerEntityType = Map.of(entityType, edgeInfos);
    Map<String, List<LineageRegistry.EdgeInfo>> edgesPerEntityTypeMultiple = Map.of(
    ...
    GraphFilters graphFiltersMultiple = new GraphFilters(ImmutableList.of(Constants.DATASET_ENTITY_NAME,
        Constants.DASHBOARD_ENTITY_NAME, Constants.DATA_JOB_ENTITY_NAME));
    Long startTime = 0L;
    Long endTime = 1L;

    QueryBuilder limitedBuilder = ESGraphQueryDAO.getLineageQueryForEntityType(
        urns,
        edgeInfos,
        graphFilters
    );

    QueryBuilder fullBuilder = ESGraphQueryDAO.getLineageQuery(urnsPerEntityType,
        edgesPerEntityType, graphFilters, startTime, endTime);

    QueryBuilder fullBuilderEmptyFilters = ESGraphQueryDAO.getLineageQuery(urnsPerEntityType, edgesPerEntityType,
        GraphFilters.emptyGraphFilters, null, null);

    QueryBuilder fullBuilderMultipleFilters = ESGraphQueryDAO.getLineageQuery(urnsPerEntityTypeMultiple, edgesPerEntityTypeMultiple,
        graphFiltersMultiple, startTime, endTime);
