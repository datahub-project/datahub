...
public class PostgresGraphFilterSqlTest {
  ...
  @Test
  public void relationshipFilterSourceAndDestinationUrns() {
    List<Object> params = new ArrayList<>();
    Criterion source =
        new Criterion()
            .setField("source.urn")
    ...
    Criterion dest =
        new Criterion()
            .setField("destination.urn")
            .setCondition(Condition.EQUAL)
            .setValues(new StringArray(ImmutableList.of("urn:li:dataset:(d,e,f)")));
    RelationshipFilter rf =
        new RelationshipFilter()
            .setOr(
                new ConjunctiveCriterionArray(
                    new ConjunctiveCriterion()
                        .setAnd(new CriterionArray(ImmutableList.of(source, dest)))));
    String sql =
        PostgresGraphFilterSql.relationshipFilterSql(
            rf, "vs.urn", "vt.urn", "et.type_name", params);
    Assert.assertTrue(sql.contains("vs.urn = ?"));
    Assert.assertTrue(sql.contains("vt.urn = ?"));
