...
public class PgTimeseriesSqlMigrationSqlTest {
  ...
  public void schemaSqlSubstitutesPrefixToken() throws Exception {
    ...
    assertTrue(substituted.contains("metadata_timeseries_aspect"));
  }

  @Test
  public void truncateIndexSqlSubstitutesPrefixToken() throws Exception {
    String raw =
        PostgresSqlUtils.loadClasspathSql(
            getClass().getClassLoader(),
            "sqlsetup/pgtimeseries/migrations/V002__truncate_index.sql");
    ...
    assertFalse(substituted.contains("__PGTIMESERIES_PREFIX__"));
    assertTrue(substituted.contains("idx_metadata_timeseries_aspect_truncate"));
    assertTrue(substituted.contains("(entity_name, aspect_name, event_time)"));
  }
