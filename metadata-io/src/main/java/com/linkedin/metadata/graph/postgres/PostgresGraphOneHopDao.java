...
public class PostgresGraphOneHopDao {
  ...
  private RelatedEntitiesResult findRelatedUndirected(
    ...
    GraphFilters out = withDirection(graphFilters, RelationshipDirection.OUTGOING);
    GraphFilters in = withDirection(graphFilters, RelationshipDirection.INCOMING);
  ...
  private String buildInnerSelectCore(GraphFilters gf, AnchorAlias anchor, List<Object> params) {
    ...
    appendRelationshipEndpointFilter(gf, sb, params);
    appendAllowedEdgeTriplets(gf, sb, params);
  ...
  public RelatedEntitiesScrollResult scrollRelatedEntities(
    ...
    RelationshipDirection dir = graphFilters.getRelationshipDirection();
    if (dir == RelationshipDirection.UNDIRECTED) {
      return scrollRelatedUndirected(graphFilters, offset, limit);
    }
    AnchorAlias anchor = anchorAliases(dir);
    List<Object> params = new ArrayList<>();
  ...
  private void appendJoinBody(
    ...
    appendRelationshipEndpointFilter(graphFilters, sb, params);
    appendAllowedEdgeTriplets(graphFilters, sb, params);
  ...
  private static void appendRelationshipEndpointFilter(
      GraphFilters graphFilters, StringBuilder sb, List<Object> params) {
    String relFilter =
        PostgresGraphFilterSql.relationshipFilterSql(
            graphFilters.getRelationshipFilter(), "vs.urn", "vt.urn", "et.type_name", params);
    sb.append(" AND ").append(relFilter);
  }

  private static void appendAllowedEdgeTriplets(
      GraphFilters graphFilters, StringBuilder sb, List<Object> params) {
    java.util.Set<Pair<String, LineageRegistry.EdgeInfo>> triplets =
        graphFilters.getAllowedEdgeTriplets();
    ...
    sb.append(" AND (");
    boolean first = true;
    for (Pair<String, LineageRegistry.EdgeInfo> pair : triplets) {
      if (!first) {
        sb.append(" OR ");
      }
      first = false;
      String sourceType;
      String destType;
      if (dir == RelationshipDirection.INCOMING) {
        sourceType = pair.getValue().getOpposingEntityType();
        destType = pair.getKey();
      } else {
        sourceType = pair.getKey();
        destType = pair.getValue().getOpposingEntityType();
      }
      sb.append("(")
          .append(PostgresGraphFilterSql.entityTypeExpr("vs.urn"))
          .append(" = ? AND ")
          .append(PostgresGraphFilterSql.entityTypeExpr("vt.urn"))
          .append(" = ? AND et.type_name = ?)");
      params.add(sourceType.toLowerCase());
      params.add(destType.toLowerCase());
      params.add(pair.getValue().getType());
    }
    sb.append(")");
  }

  private static GraphFilters withDirection(GraphFilters graphFilters, RelationshipDirection dir) {
    RelationshipFilter rf = new RelationshipFilter().setDirection(dir);
    if (graphFilters.getRelationshipFilter() != null
        && graphFilters.getRelationshipFilter().getOr() != null) {
      rf.setOr(graphFilters.getRelationshipFilter().getOr());
    }
    GraphFilters copy =
        new GraphFilters(
            ...
            rf);
    copy.setAllowedEdgeTriplets(graphFilters.getAllowedEdgeTriplets());
    return copy;
  }

  private RelatedEntitiesScrollResult scrollRelatedUndirected(
      GraphFilters graphFilters, int offset, int limit) {
    GraphFilters out = withDirection(graphFilters, RelationshipDirection.OUTGOING);
    GraphFilters in = withDirection(graphFilters, RelationshipDirection.INCOMING);
    List<Object> params = new ArrayList<>();
    String outBranch =
        undirectedScrollBranch(out, anchorAliases(RelationshipDirection.OUTGOING), params);
    String inBranch =
        undirectedScrollBranch(in, anchorAliases(RelationshipDirection.INCOMING), params);
    String sql =
        "SELECT rel_type, related_urn, anchor_urn, via_col FROM ("
            + outBranch
            + " UNION "
            + inBranch
            + ") u ORDER BY anchor_urn, related_urn, rel_type LIMIT ? OFFSET ?";
    params.add(limit);
    params.add(offset);
    List<Object> countParams = new ArrayList<>();
    String outCount =
        undirectedScrollBranch(out, anchorAliases(RelationshipDirection.OUTGOING), countParams);
    String inCount =
        undirectedScrollBranch(in, anchorAliases(RelationshipDirection.INCOMING), countParams);
    String countSql = "SELECT COUNT(*) FROM (" + outCount + " UNION " + inCount + ") s";
    try (Connection conn = database.dataSource().getConnection()) {
      int total = queryInt(conn, countSql, countParams);
      List<RelatedEntities> entities = new ArrayList<>();
      try (PreparedStatement ps = conn.prepareStatement(sql)) {
        ...
        try (ResultSet rs = ps.executeQuery()) {
          while (rs.next()) {
            ...
            entities.add(
                new RelatedEntities(
                    rel, relatedUrn, anchorUrn, RelationshipDirection.UNDIRECTED, viaStr));
          }
        }
      }
      String nextScroll = null;
      ...
      throw new IllegalStateException("Postgres graph undirected scrollRelatedEntities failed", e);
    }
  }

  private String undirectedScrollBranch(GraphFilters gf, AnchorAlias anchor, List<Object> params) {
    StringBuilder from = new StringBuilder();
    appendJoinBody(gf, anchor, from, params);
    return "SELECT DISTINCT "
        + "et.type_name AS rel_type, "
        + anchor.oppositeUrnFilterAlias
        + ".urn AS related_urn, "
        + anchor.anchorUrnFilterAlias
        + ".urn AS anchor_urn, "
        + "NULLIF(TRIM(e.properties->>'via'), '') AS via_col FROM "
        + from;
  }
