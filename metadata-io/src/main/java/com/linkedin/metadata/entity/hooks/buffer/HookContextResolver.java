 ...
 * OperationContext} (with a {@link com.linkedin.metadata.aspect.RetrieverContext}) for a
 * drained group, so the background {@link PostCommitHookDrainer} can replay each committed MCL
 * through its hook under the correct context — routing to the same underlying database that
 * produced it.
 *
 ...
 * enqueue (when the request {@link OperationContext} is available) to reconstruct a correct
 * context at drain.
 *
 ...
 *
 * <p><b>Failure contract.</b> {@link #groupKey} and {@link #resolveOpContext} distinguish
 * permanent from transient failures: throw {@link UnresolvableHookKeyException} for a key that
 * will <em>never</em> resolve (e.g. a subtype the resolver does not produce — a wiring bug or a
 * stale rolling-deploy entry). The drainer drops such keys from the buffer so they don't re-throw
 * every tick. Any other {@link RuntimeException} is treated as transient: the drainer logs it and
 * leaves the key queued for retry on the next tick, so a transient blip cannot silently skip a
 * hook replay.
 */
public interface HookContextResolver extends OffloadContextResolver<HookKey> {
   ...
   * entries sharing a routing context are replayed together under one {@link
   * #resolveOpContext}. Implementations should return a value that is stable across calls for the
   * same routing context (e.g. a tenant id extracted from the key).
   */
  @Nonnull
  ...
  HookContextResolver NO_OP =
      new HookContextResolver() {
        ...
        @Override
        public OperationContext resolveOpContext(HookKey key, OperationContext systemOperationContext) {
