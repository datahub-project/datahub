...
public class EbeanAspectDao implements AspectDao, AspectMigrationsDao {
  ...
  public EbeanAspectDao(@Nonnull final Database server, @Nonnull final Database primary) {
    _server = server;
    _primary = primary;
  }
  ...
  private boolean validateConnection() {
    ...
    if (!AspectStorageValidationUtil.checkV2TableExists(_primary)) {
  ...
  private void saveEbeanAspect(@Nonnull final EbeanAspectV2 ebeanAspect, final boolean insert) {
    validateConnection();
    if (insert) {
      _primary.insert(ebeanAspect);
    } else {
      _primary.update(ebeanAspect);
    }
  }
  ...
  public void deleteAspect(@Nonnull final EntityAspect aspect) {
    validateConnection();
    EbeanAspectV2 ebeanAspect = EbeanAspectV2.fromEntityAspect(aspect);
    _primary.delete(ebeanAspect);
  }

  @Override
  public int deleteUrn(@Nonnull final String urn) {
    validateConnection();
    return _primary.createQuery(EbeanAspectV2.class).where().eq(EbeanAspectV2.URN_COLUMN, urn).delete();
  }
  ...
  public <T> T runInTransactionWithRetry(@Nonnull final Function<Transaction, T> block, final int maxTransactionRetry) {
    ...
    T result = null;
    do {
      try (Transaction transaction = _primary.beginTransaction(TxScope.requiresNew().setIsolation(TxIsolation.REPEATABLE_READ))) {
