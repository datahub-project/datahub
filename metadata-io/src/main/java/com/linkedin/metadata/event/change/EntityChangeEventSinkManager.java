          ...
          CompletableFuture.runAsync(
              () -> {
                try {
                  ...
                  // destination.
                  // TODO: Better handling via failed events log. For now, simply print entire event
                  // so we know which have failed.
                  log.error(
                      String.format(
                          "Caught exception while attempting to sink change event to sink %s. event: %s",
                          sink.getClass(), RecordUtils.toJsonString(event)),
                      e);
                }
              }));
    }
    return CompletableFuture.allOf(
        sinkFutures.toArray(new CompletableFuture[this.sinkRegistry.size()]));
  }
}
