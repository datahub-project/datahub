...
public class IndexBuilder {
    ...
    public void buildIndex() throws IOException {
        ...
        if (originalCount != reindexedCount) {
            log.info("Post-reindex document count is different, source_doc_count: {} reindex_doc_count: {}",
        ...
        if (aliasesResponse.getAliases().isEmpty()) {
            ...
            searchClient.indices().delete(new DeleteIndexRequest().indices(
    ...
    private long getCount(String indexName) throws IOException {
        return searchClient
                .count(new CountRequest(indexName).query(QueryBuilders.matchAllQuery()), RequestOptions.DEFAULT)
                .getCount();
    }
    ...
    private Optional<Integer> getMaxNgramDiff() {
        Set<IndexType> allIndexTypes = entitySpec.getSearchableFieldSpecs().stream()
                .map(SearchableFieldSpec::getIndexSettings).flatMap(List::stream)
                .map(IndexSetting::getIndexType).collect(Collectors.toSet());
