...
public class BrowsePathV2Utils {
  ...
  final private static String DEFAULT_FOLDER_NAME = "Default";

  /**
   * Generates a default browsePathsV2 aspect for a given urn.
   *
   * If the entity has containers, get its whole container path and set those urns in the path of browsePathsV2.
   * If it's a dataset, generate the path from the dataset name like we do for default browsePaths V1.
   * If it's a data job, set its parent data flow in the path.
   * For everything else, place it in a "Default" folder so we can still navigate to it through browse in the UI.
   * This default method should be unneeded once ingestion produces higher quality browsePathsV2 aspects.
   */
  public static BrowsePathsV2 getDefaultBrowsePathV2(
    ...
    switch (urn.getEntityType()) {
      case Constants.DATASET_ENTITY_NAME:
        ...
        if (useContainerPaths && datasetContainerPathEntries.size() > 0) {
          ...
          if (defaultDatasetPathEntries.size() > 0) {
            browsePathEntries.addAll(getDefaultDatasetPathEntries(dsKey.getName().toLowerCase(), dataPlatformDelimiter));
          } else {
      ...
      case Constants.DATA_JOB_ENTITY_NAME:
        ...
        browsePathEntries.add(createBrowsePathEntry(dataJobKey.getFlow().toString(), dataJobKey.getFlow()));
        break;
      default:
        browsePathEntries.add(createBrowsePathEntry(DEFAULT_FOLDER_NAME, null));
        break;
  ...
  private static void aggregateParentContainers(List<Urn> containerUrns, Urn entityUrn, EntityService entityService) {
    try {
      ...
      log.error(String.format("Error getting containers for entity with urn %s while adding default browsePathV2", entityUrn), e);
    }
  }

  /**
   * Gets the path of containers for a given entity to create a browsePathV2 off of.
   * Recursively call aggregateParentContainers to get the full container path to be included in this path.
   */
  private static BrowsePathEntryArray getContainerPathEntries(@Nonnull final Urn entityUrn, @Nonnull final EntityService entityService) {
    BrowsePathEntryArray browsePathEntries = new BrowsePathEntryArray();
