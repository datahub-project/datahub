...
/**
 * Expands parentDocument filter URN terms across the document hierarchy using the IsChildOf
 * relationship (child → parent). DESCENDANTS_INCL walks INCOMING edges so a parent document filter
 * matches nested child documents.
 */
@Builder
public class DocumentExpansionRewriter extends BaseQueryFilterRewriter {
  ...
  public Set<String> getRewriterFieldNames() {
    return Set.of("parentDocument.keyword");
  }
      ...
      switch (rewriterContext.getCondition() == null
        ...
        case DESCENDANTS_INCL:
          return expandUrnsByGraph(
              opContext,
              filterQuery,
              Set.of(IS_CHILD_OF_RELATIONSHIP_NAME),
