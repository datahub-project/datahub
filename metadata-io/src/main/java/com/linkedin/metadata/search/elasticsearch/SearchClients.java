...
public final class SearchClients {
  ...
  /**
   * Component that owns a DataHub-managed index: entity families, usage, graph, timeseries, or
   * system-metadata (including zero-downtime backing names that share the concrete prefix).
   */
  @Nonnull
  ...
  public static SearchComponent componentForManagedIndex(
    ...
    if (IndexConvention.matchesTimeseriesAspectIndexFamily(indexOrPattern)) {
      return SearchComponent.TIMESERIES;
    }
    throw new IllegalArgumentException(
        "Index '"
            + indexOrPattern
            + "' is not a recognized search, graph, timeseries, system-metadata, or usage index."
            + " Unrecognized names are a setup error; refusing to use the primary cluster.");
  }
  ...
  /**
   * Route one resolved index name to whichever component owns it — entity family, usage, graph,
   * timeseries, or system-metadata. Empty means "no target index", in which case {@code config}
   * picks the keyword family.
   */
  @Nonnull
  ...
  public static SearchClientShim<?> forIndex(
      ...
      @Nullable String indexName) {
    if (indexName == null || indexName.isEmpty()) {
      return forComponent(opContext, keywordComponent(config));
    }
    return forComponent(opContext, componentForManagedIndex(opContext, indexName));
  }
