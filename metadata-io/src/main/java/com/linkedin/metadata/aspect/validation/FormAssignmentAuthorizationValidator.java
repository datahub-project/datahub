...
public class FormAssignmentAuthorizationValidator extends AbstractAspectAuthorizationValidator {
  ...
  /**
   * One read per entity type: the retriever scopes a lookup to the first URN's entity type, so a
   * mixed set would silently return nothing for the other types and make every unchanged assignment
   * look like a new one.
   */
  @Nonnull
  private static Map<Urn, Map<String, Aspect>> loadCurrentForms(
      @Nonnull AspectRetriever aspectRetriever, @Nonnull Set<Urn> urns) {
    ...
    urns.stream()
        ...
        .forEach(
            typedUrns ->
                current.putAll(
                    aspectRetriever.getLatestAspectObjects(typedUrns, Set.of(FORMS_ASPECT_NAME))));
    return current;
  }
