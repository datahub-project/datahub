...
/** This mutator will convert specified fields within aspects that are unrecognized to structured properties */
@Slf4j
@Setter
@Getter
...
public class ExtendedModelStructuredPropertyMutator extends MutationHook {
  ...
  @Nonnull private Map<AspectPluginConfig.EntityAspectName, Map<String, StructuredPropertyDefinition>>
      aspectFieldMappings;
  ...
  public ExtendedModelStructuredPropertyMutator(ExtendedModelValidationConfiguration config, boolean extensionsEnabled) {
    List<AspectPluginConfig.EntityAspectName> entityAspectNames = new ArrayList<>();
    Map<String, StructuredPropertyDefinition> structuredPropertyMappings = createStructuredPropertyMappings(config);
    Map<AspectPluginConfig.EntityAspectName, Map<String, StructuredPropertyDefinition>> aspectFieldMappings =
    ...
    setConfig(
        AspectPluginConfig.builder()
            ...
            .supportedEntityAspectNames(entityAspectNames)
            .build())
        .setAspectFieldMappings(aspectFieldMappings);
  }

  private Map<String, StructuredPropertyDefinition> createStructuredPropertyMappings(ExtendedModelValidationConfiguration config) {
    Map<String, StructuredPropertyDefinition> structuredPropertyMappings = new HashMap<>();

    for (StructuredPropertyConfiguration structuredPropertyConfiguration : config.getStructuredProperties()) {
      ...
      if (StringUtils.isBlank(structuredPropertyConfiguration.getQualifiedName())) {
        log.error("Unable to parse configured structured property extension due to invalid qualified name: {}",
            structuredPropertyConfiguration);
        continue;
      }
      structuredPropertyDefinition.setQualifiedName(structuredPropertyConfiguration.getQualifiedName());

      if (StringUtils.isBlank(structuredPropertyConfiguration.getType())) {
        log.error("Unable to parse configured structured property extension due to invalid type: {}",
            structuredPropertyConfiguration);
        continue;
      }
      ...
      structuredPropertyDefinition.setCardinality(structuredPropertyConfiguration.getCardinality() != null ?
          mapCardinality(structuredPropertyConfiguration.getCardinality()) : PropertyCardinality.SINGLE);
      if (structuredPropertyConfiguration.getDescription() != null) {
        structuredPropertyDefinition.setDescription(structuredPropertyConfiguration.getDescription());
      }
      if (structuredPropertyConfiguration.getDisplayName() != null) {
        structuredPropertyDefinition.setDisplayName(structuredPropertyConfiguration.getDisplayName());
      }
      if (structuredPropertyConfiguration.getAllowedValues() != null) {
        structuredPropertyDefinition.setAllowedValues(mapAllowedValues(structuredPropertyConfiguration.getAllowedValues()));
      }

      // Required to set entity types after this step
      structuredPropertyMappings.put(structuredPropertyConfiguration.getQualifiedName(), structuredPropertyDefinition);
    }

    return structuredPropertyMappings;
  }

  private Map<AspectPluginConfig.EntityAspectName, Map<String, StructuredPropertyDefinition>> createAspectFieldMappings
      (Map<String, StructuredPropertyDefinition> structuredPropertyMappings, ExtendedModelValidationConfiguration config,
    ...
    for (EntityConfiguration entityConfiguration : config.getEntities()) {
      AspectPluginConfig.EntityAspectName.EntityAspectNameBuilder entityAspectNameBuilder =
          AspectPluginConfig.EntityAspectName.builder()
              .entityName(entityConfiguration.getEntity());
      for (AspectConfiguration aspectConfiguration : entityConfiguration.getAspects()) {
        AspectPluginConfig.EntityAspectName entityAspectName = entityAspectNameBuilder
            .aspectName(aspectConfiguration.getAspect())
            .build();
        entityAspectNames.add(entityAspectName);
        for (FieldConfiguration fieldConfiguration : aspectConfiguration.getFields()) {
          if (!structuredPropertyMappings.containsKey(fieldConfiguration.getConfig().get(STRUCTURED_PROPERTY_FIELD))) {
            log.error("Unable to find valid structured property definition for field: {}", fieldConfiguration);
            continue;
          }
          // Add in supported entity type
          StructuredPropertyDefinition propertyDefinition = structuredPropertyMappings.get(
              fieldConfiguration.getConfig().get(STRUCTURED_PROPERTY_FIELD));
          UrnArray entityTypesSupported = Optional.ofNullable(propertyDefinition.getEntityTypes(GetMode.NULL))
              .orElse(new UrnArray());
          Set<Urn> entityTypesSupportedSet = new HashSet<>(entityTypesSupported);
          // TODO: Handle schema field case
          entityTypesSupportedSet.add(UrnUtils.getUrn(ENTITY_TYPE_URN_PREFIX + entityConfiguration.getEntity()));
          propertyDefinition.setEntityTypes(new UrnArray(entityTypesSupportedSet));


          aspectFieldMappings.put(entityAspectName, ImmutableMap.of(fieldConfiguration.getFieldName(),
              propertyDefinition));
        }
      }
    }
  ...
  private static PropertyCardinality mapCardinality(@Nonnull StructuredPropertyConfiguration.Cardinality cardinality) {
    switch (cardinality) {
      case MULTIPLE:
  ...
  private static PropertyValueArray mapAllowedValues(List<StructuredPropertyConfiguration.PropertyValue> propertyValues) {
    List<PropertyValue> props = propertyValues.stream()
        .map(propertyValue -> new PropertyValue().setValue(PrimitivePropertyValue.create(propertyValue.getValue())))
  ...
  protected Stream<MCPItem> proposalMutation(
    ...
    List<MCPItem> filteredOriginalItems = mcpItems.stream()
        .peek(
            item -> {
              try {
                DataMap dataMap = RecordUtils.toDataMap(item.getMetadataChangeProposal().getAspect().getValue()
                ...
                Map<String, StructuredPropertyDefinition> fieldMap = this.aspectFieldMappings.get(entityAspectName);
                if (fieldMap == null) {
                  // No mapping, ignore
                  return;
                }
                for (String fieldKey : fieldMap.keySet()) {
                  // Nested keys are represented by dots, assumes no field itself has a dot in it which should be safe
                  ...
                  for (String fieldPart : fieldParts) {
                    ...
                    if (value instanceof DataMap) {
                      value = ((DataMap) value).get(fieldPart);
                    }
                    if (value == null) {
                  ...
                  if (value != null) {
                    ...
                    // TODO: Handle schema field case
                    Urn patchItemUrn = resolveUrn(item, dataMap, fieldParts, value);
                    propertiesBuilder.urn(item.getUrn());
                    if (structuredPropertyDefinition.getValueType().equals(STRING_DATA_TYPE_URN)) {
                      if (PropertyCardinality.SINGLE.equals(structuredPropertyDefinition.getCardinality())) {
                        ...
                        if (value instanceof List) {
                          valueStrings = ((List<?>) value).stream()
                      ...
                      log.error("Unsupported value type: {} unable to create structured property.",
                          structuredPropertyDefinition.getValueType());
                      continue;
                    }
                    MetadataChangeProposal proposal = propertiesBuilder.build();
                    MCPItem patchItem = PatchItemImpl.builder()
                        .urn(item.getUrn())
                        .entitySpec(item.getEntitySpec())
                        .aspectName(STRUCTURED_PROPERTIES_ASPECT_NAME)
                        .aspectSpec(
                            item.getEntitySpec().getAspectSpec(STRUCTURED_PROPERTIES_ASPECT_NAME))
    ...
    // Recollect into intermediate list instead of direct use of stream to avoid concurrent modification
  ...
  private static Urn resolveUrn(MCPItem item, DataMap dataMap, String[] fieldParts, Object value) {
    // Special case for schema fields, apply to appropriate schema field based on aspect
    if ((EDITABLE_SCHEMA_METADATA_ASPECT_NAME.equals(item.getAspectName()) ||
      ...
      return getSchemaFieldUrn(item, dataMap, fieldParts, value);
    }
  ...
  private static Urn getSchemaFieldUrn(MCPItem item, DataMap dataMap, String[] fieldParts, Object value) {
    return null;
  }
}
