...
public class DomainExtractionUtils {
   ...
   */
  @Nonnull
  public static Set<Urn> extractDomainsFromMCP(
      @Nonnull MetadataChangeProposal mcp, @Nonnull ObjectMapper objectMapper) {
    ...
    try {
      ...
      if (mcp.getChangeType() == com.linkedin.events.metadata.ChangeType.PATCH) {
        return extractDomainsFromPatchDocument(aspectValue, objectMapper);
      }
   ...
   */
  @Nonnull
  private static Set<Urn> extractDomainsFromPatchDocument(
      @Nonnull String patchDocument, @Nonnull ObjectMapper objectMapper) {
    Set<Urn> domainUrns = new HashSet<>();

    try {
      // Deserialize to GenericJsonPatch for structured access
      GenericJsonPatch patch = objectMapper.readValue(patchDocument, GenericJsonPatch.class);
      ...
      for (GenericJsonPatch.PatchOp op : patch.getPatch()) {
        ...
        if (path != null
            && (path.startsWith("/" + DomainsTemplate.DOMAINS_FIELD_NAME)
                || path.equals("/" + DomainsTemplate.DOMAINS_FIELD_NAME))) {
          if (value != null) {
  ...
  public static Map<Urn, Set<Urn>> extractNewDomainsFromMCPs(
      @Nonnull Collection<MetadataChangeProposal> mcps, @Nonnull ObjectMapper objectMapper) {
    ...
    for (MetadataChangeProposal mcp : mcps) {
      ...
      Set<Urn> newDomains = extractDomainsFromMCP(mcp, objectMapper);
