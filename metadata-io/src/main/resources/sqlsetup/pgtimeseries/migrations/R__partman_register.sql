-- Register pgTimeseries aspect parent with pg_partman. Session: SET search_path.
-- Java replaces __PARTMAN_PARENT_QUALIFIED__ with schema-qualified relation text.
-- Tokens: __PARTMAN_INTERVAL__, __PARTMAN_PREMAKE__, __PARTMAN_FORCE_OVERWRITE__
--
-- pg_partman objects live in the extension install schema (pg_extension.extnamespace), not always "partman".
-- By default, interval/premake are sticky after first create_parent. Set force overwrite to update them.
