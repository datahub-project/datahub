...
public class DatasetFieldUrnTest {
  ...
  public void testSerialization() throws URISyntaxException {
    ...
    final DatasetUrn datasetUrn = datasetFieldUrn.getDataset();

    Assert.assertEquals(datasetFieldUrn.getField(), FIELD_NAME);
    Assert.assertEquals(datasetUrn.getDatasetNameEntity(), DATASET_NAME);
    Assert.assertEquals(datasetUrn.getPlatformEntity().getPlatformNameEntity(), PLATFORM);
    Assert.assertEquals(datasetUrn.getOriginEntity(), FabricType.PROD);
    Assert.assertEquals(datasetFieldUrn.toString(), datasetFieldString,
  ...
  public void testUrnConstructors() {
    ...
    final DatasetUrn datasetUrn = datasetFieldUrn1.getDataset();
    final DatasetFieldUrn datasetFieldUrn2 = new DatasetFieldUrn(datasetUrn, FIELD_NAME);

    Assert.assertEquals(datasetFieldUrn1, datasetFieldUrn2);
  }
}