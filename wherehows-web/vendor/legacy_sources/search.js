function legacySearch() {
  ...
  if (searchForm) {
    searchInput = searchForm.querySelector('#global-search-input');
    ...
    searchInput.addEventListener('keypress', e => e.keyCode === 13 && handleSearchInput(e));
  }
  ...
  const handleSearchInput = function () {
    if (searchInput) {
      ...
      const searchRouteHash = `#/search?keywords=${btoa(keyword)}&category=${window.g_currentCategory}&source=default&page=1`;
      keyword && (document.location.hash = searchRouteHash);

      // Piwik track search keyword
      _paq.push(['trackSiteSearch', keyword, window.g_currentCategory, false]);
    }
  };
  ...
  const searchBtn = document.querySelector('#global-search-button');
  if (searchBtn) {
