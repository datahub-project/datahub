...
class TestGeneratedSqlValidity:
    """Tests that validate the SQL generated by the CLI is well-formed."""

    # Standard args matching the README example (--enable-mutations is the default)
    _BASE_ARGS = [
        "--sf-account",
        "YOUR_ACCOUNT",
        "--sf-user",
        ...
        "YOUR_SCHEMA",
        "--datahub-url",
        "https://your-datahub.acryl.io",
        "--datahub-token",
        "YOUR_TOKEN",
        "--enable-mutations",
    ]

    def _invoke(self, tmp_path: Path) -> tuple:
        from click.testing import CliRunner

        runner = CliRunner()
        result = runner.invoke(
            create_snowflake_agent,
            self._BASE_ARGS + ["--output-dir", str(tmp_path)],
        )
        return result, tmp_path

    def test_cli_exits_successfully_with_enable_mutations(self, tmp_path: Path) -> None:
        result, _ = self._invoke(tmp_path)
        assert result.exit_code == 0, f"CLI failed:\n{result.output}"

    def test_all_five_sql_files_are_generated(self, tmp_path: Path) -> None:
        _, out = self._invoke(tmp_path)
        for name in [
            "00_configuration.sql",
            "01_network_rules.sql",
            "02_datahub_udfs.sql",
    ...
    def test_all_sql_files_are_non_empty(self, tmp_path: Path) -> None:
        _, out = self._invoke(tmp_path)
        for name in [
            "00_configuration.sql",
            "01_network_rules.sql",
            ...
            content = (out / name).read_text().strip()
            assert content, f"{name} is empty"

    def test_sql_statements_end_with_semicolons(self, tmp_path: Path) -> None:
        """Every non-comment, non-blank line that closes a statement must end with ';'."""
        import re
        ...
        for name in [
            ...
            # Strip block comments ($$...$$) and line comments, then find bare semicolons
            stripped = re.sub(r"\$\$.*?\$\$", "$$...$$", content, flags=re.DOTALL)
            stripped = re.sub(r"--[^\n]*", "", stripped)
            statements = [s.strip() for s in stripped.split(";") if s.strip()]
            assert statements, f"{name} contains no SQL statements"

    def test_udf_sql_contains_create_function_statements(self, tmp_path: Path) -> None:
        _, out = self._invoke(tmp_path)
        content = (out / "02_datahub_udfs.sql").read_text()
        assert "CREATE OR REPLACE FUNCTION" in content

    def test_udf_sql_includes_mutation_udfs_when_enabled(self, tmp_path: Path) -> None:
        _, out = self._invoke(tmp_path)
        content = (out / "02_datahub_udfs.sql").read_text()
        for mutation_udf in ["ADD_TAGS", "REMOVE_TAGS", "UPDATE_DESCRIPTION"]:
            assert mutation_udf in content, (
                f"Mutation UDF {mutation_udf} missing from 02_datahub_udfs.sql "
                "even though --enable-mutations was passed"
            )

    def test_udf_sql_python_bodies_are_syntactically_valid(
        self, tmp_path: Path
    ) -> None:
        """Every Python body embedded between $$ ... $$ in the UDF SQL must compile."""
        import ast
        import re

        _, out = self._invoke(tmp_path)
        content = (out / "02_datahub_udfs.sql").read_text()
        bodies = re.findall(r"\$\$(.*?)\$\$", content, re.DOTALL)
        assert bodies, "No $$ ... $$ Python bodies found in 02_datahub_udfs.sql"
        for i, body in enumerate(bodies):
            try:
                ast.parse(body)
            except SyntaxError as exc:
                raise AssertionError(
                    f"Python body #{i + 1} in 02_datahub_udfs.sql has a syntax error: {exc}"
                ) from exc

    def test_network_rules_sql_contains_datahub_domain(self, tmp_path: Path) -> None:
        _, out = self._invoke(tmp_path)
        content = (out / "01_network_rules.sql").read_text()
        assert "your-datahub.acryl.io" in content

    def test_cortex_agent_sql_contains_agent_definition(self, tmp_path: Path) -> None:
        _, out = self._invoke(tmp_path)
        content = (out / "04_cortex_agent.sql").read_text()
        assert "CREATE OR REPLACE" in content
