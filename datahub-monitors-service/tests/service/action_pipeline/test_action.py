...
with mock.patch(
    "datahub_monitors.workers.helpers.update_celery_credentials"
) as mock_func:
    from datahub_monitors.service.action_pipeline.action import (
        ...
        MonitorServiceAction,
    )


class TestMonitorServiceAction:
    def setup_method(self) -> None:
        self.action = MonitorServiceAction(False, True, "default")
    ...
    @mock.patch(
        "datahub_monitors.service.action_pipeline.action.update_celery_credentials"
    )
    @mock.patch(
        "datahub_monitors.workers.tasks.evaluate_execution_request_input.apply_async"
    )
    def test_execution_request_input(
    ...
    def test_execution_request_input_no_aspect(
        self, apply_async_mock: Mock, update_creds_mock: Mock
    ) -> None:
    ...
    @mock.patch(
        "datahub_monitors.service.action_pipeline.action.setup_ingestion_executor"
    )
    ...
    def test_execution_request_input_local(
        ...
        self.action = MonitorServiceAction(True, True, "default")
        self.action.act(self.event)
