...
class MonitorServiceAction(Action):
            ...
            if (
                ...
                if (
                    ...
                    if self.embedded_worker_enabled:
                        ...
                        self._handle_embedded_worker_enabled_execution_request_signal(
                            orig_event
                        )
    ...
    def _handle_execution_request_input(
        ...
        if (
            ...
            thread = threading.Thread(
                target=self._evaluate_execution_request_input,
                args=(orig_event,),
            )
            thread.start()
            logger.info(
    ...
    def _handle_embedded_worker_enabled_execution_request_signal(
        self, orig_event: MetadataChangeLogClass
    ) -> None:
        if not orig_event.aspect:
            logger.error(
                f"Unable to parse Execution Request Signal, no aspect {orig_event.entityUrn}.."
            )
            return None
        ...
        if aspect_dict["signal"] == "KILL":
            thread = threading.Thread(
                target=self._evaluate_execution_request_signal,
                args=(orig_event,),
            )
            thread.start()
    ...
    def _evaluate_execution_request_signal(
        self, orig_event: MetadataChangeLogClass
    ) -> None:
        signal, exec_id = extract_execution_request_signal(orig_event)
        if signal and exec_id:
            if exec_id in self.ingestion_executor.task_futures:
