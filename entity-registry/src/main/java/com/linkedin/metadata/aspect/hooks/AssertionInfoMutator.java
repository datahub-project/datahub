...
public class AssertionInfoMutator extends MutationHook {
  ...
  private static boolean processAssertionInfoAspect(ChangeMCP item) {
    ...
    if (next != null && next.hasFieldAssertion()) {
      final FieldAssertionInfo fieldInfo = next.getFieldAssertion();
      if (!fieldInfo.hasFieldPath()) {
        final String path = AssertionUtils.getFieldPathFromFieldAssertion(fieldInfo);
        if (path != null) {
