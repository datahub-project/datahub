...
public class SystemMetadataUtils {
  ...
  /**
   * Sets the aspect modified audit information for non-system users. When a change is generated by
   * the system user this is still captured in the row, but this avoids the system user overwriting
   * audit information in the cases where the system user is acting on behalf of an authenticated
   * user in cases like async ingestion paths and some internal service flows.
   *
