...
public class FormInfoHook implements MetadataChangeLogHook {
  ...
  private void handleSendFormNotifications(@Nonnull final MetadataChangeLog event) {
    ...
    // 1. Check conditions on previous info compared to new info
    ...
    boolean actorsChanged = haveActorsChanged(formInfo, prevFormInfo);
  ...
  /*
   * Determines if anything has changed from the previous aspect in the actors field of formInfo
   */
  boolean haveActorsChanged(
      @Nonnull final FormInfo formInfo, @Nullable final FormInfo prevFormInfo) {
    Boolean prevOwners = null;
    UrnArray prevUsers = new UrnArray();
    UrnArray prevGroups = new UrnArray();
    if (prevFormInfo != null) {
      prevOwners = prevFormInfo.getActors().isOwners();
      prevUsers =
          prevFormInfo.getActors().getUsers() != null
              ? prevFormInfo.getActors().getUsers()
              : new UrnArray();
      prevGroups =
          prevFormInfo.getActors().getGroups() != null
              ? prevFormInfo.getActors().getGroups()
              : new UrnArray();
    }

    Boolean owners = formInfo.getActors().isOwners();
    UrnArray users =
        formInfo.getActors().getUsers() != null ? formInfo.getActors().getUsers() : new UrnArray();
    UrnArray groups =
        formInfo.getActors().getGroups() != null
            ? formInfo.getActors().getGroups()
            : new UrnArray();

    // if owners have changed, we added new users, or added new groups, return true
    if (!owners.equals(prevOwners)
        || !prevUsers.containsAll(users)
        || !prevGroups.containsAll(groups)) {
      return true;
    }
