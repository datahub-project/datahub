 ...
 * skip the assertion.
 *
 * <p>b) Anomalies: In contract, when an Inferred (system) Assertion passes, any active anomalies
 * that were generated based on a previous Assertion failure will be resolved. Note that resolution
 * is different from REVIEW. Anomalies will still be subject to review, even if they are marked in
 * the resolved state.
 *
 ...
 *
 * <p>a) Anomalies: When an Inferred (system) Assertion fails, this hook will generate an Anomaly
 * which reflects what has been detected. In the future, we intend to add ability to disable anomaly
 * generation for particular assets, or globally. If there is an active anomaly which has already
 * been generated based on the results of the assertion, it will simply be updated to reflect the
 * current state. We will NOT generate duplicate anomalies based on the same inferred assertion. In
 * the future, we will likely need to prevent new anomalies from being generated if existing
 * anomalies were manually reviewed / resolved recently. (To prevent a thrashing between Active and
 * Resolved anomalies until the models are able to update).
 *
 ...
 *
 * <p>b) Anomalies: When an Inferred Assertion is deleted, this hook will search for any active
 * anomalies that were generated by the Assertion. It will then cascade the delete to the anomalies,
 * deleting it in it's entirety.
 *
...
public class AssertionActionsHook implements MetadataChangeLogHook {
  ...
  private void handleAssertionDeleted(@Nonnull final Urn assertionUrn) {
    ...
    // Case 2: Remove any active anomalies that were generated as a direct result of the assertion
    // being deleted.
    ...
    if (isInferredAssertion(info)) {
      ...
      try {
        ...
        if (searchResult.hasEntities() && searchResult.getEntities().size() > 0) {
          searchResult
              .getEntities()
              .forEach(entity -> resolveAnomaly(entity.getEntity(), assertionUrn, runEvent));
        }
        ...
        throw new RuntimeException(
            String.format(
                "Caught exception while attempting to resolve active assertion related to entity urn %s and assertion urn %s. This means"
                    + " that active anomalies may need to be resolved manually!",
                entityUrn, assertionUrn),
            e);
      }
    }
          ...
          new AnomalySource()
              ...
              .setProperties(
                  new AnomalySourceProperties()
                      .setAssertionRunEventTime(runEvent.getTimestampMillis()))
  ...
  private boolean hasActiveAssertionAnomaly(
      @Nonnull final Urn entityUrn, @Nonnull final Urn assertionUrn) {
    try {
      final SearchResult searchResult =
          entityClient.search(
      ...
      throw new RuntimeException(
          String.format(
              "Caught exception while attempting to retrieve active assertion related to entity urn %s and assertion urn %s",
              entityUrn, assertionUrn),
          e);
    }
  }
  ...
  private void tryDeleteActiveAnomalies(@Nonnull final Urn assertionUrn) {
    try {
      ...
      log.error(
          String.format(
              "Caught exception while attempting to delete active anomalies associated with removed assertion urn %s! "
                  + "This means that a stale anomaly may remain active.",
              assertionUrn),
          e);
    }
  }
  ...
  private void deleteActiveAnomalies(@Nonnull final Urn assertionUrn) throws Exception {
    ...
    if (searchResult.hasEntities() && searchResult.getEntities().size() > 0) {
      ...
      searchResult
          ...
          .forEach(
              res -> {
                // We have found an active anomaly. Let's remove it (along with any references to
                // it).
                ...
                try {
                  ...
                  log.error(
                      String.format(
                              "Caught exception while attempting to delete anomaly with urn %s. This may mean that the anomaly",
                              anomalyUrn)
                          + " or references to it could not be removed, resulting in stale information!");
                  // Do not throw so that processing can continue.
                }
  ...
  private Filter buildActiveEntityAnomaliesFilter(
      @Nonnull final Urn entityUrn, @Nonnull final Urn assertionUrn) {
    final Map<String, String> criterionMap = new HashMap<>();
  ...
  private AnomalyType getAnomalyTypeFromAssertionInfo(@Nonnull final AssertionInfo info) {
    switch (info.getType()) {
      ...
      default:
        throw new IllegalArgumentException(
            "Failed to map to an anomaly type! Unsupported Assertion type provided.");
    }
  }
}
