 ...
 *
 * This generator supports generating notifications for the following aspects:
 *
 *    - owners changes
...
public class EntityChangeNotificationGenerator extends BaseMclNotificationGenerator {
  ...
  private  <T extends RecordTemplate> List<ChangeEvent> generateChangeEvents(
    ...
    final List<EntityChangeEventGenerator<T>> aspectDiffers =
        _entityChangeEventGeneratorRegistry.getEntityChangeEventGenerators(aspectName)
            .stream()
            .map(
                generator -> (EntityChangeEventGenerator<T>) generator) // Note: Assumes that correct types have been registered for the aspect.
            .collect(Collectors.toList());
