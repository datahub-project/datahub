...
test.describe('lineage_graph', () => {
  ...
  test('can see full history', async ({ page, logger, logDir }) => {
    const lineagePage = new DatasetPage(page, logger, logDir);
  ...
  test('can edit upstream lineage', async ({ page, logger, logDir }) => {
    ...
    const lineagePage = new DatasetPage(page, logger, logDir);
    await lineagePage.goToLineageGraph(DATASET_ENTITY_TYPE, DATASET_URN);
    await lineagePage.lineage.openManageLineageMenu(DATASET_URN);
    await lineagePage.lineage.clickEditUpstreamLineage();
    ...
    await lineagePage.lineage.searchInLineageEditModal('playwright_health_test');
  ...
  test('displays complete lineage graph with all node types', async ({ page, logger, logDir }) => {
    ...
    // Check initial state
    await lp.lineage.checkNodeExists(NODE1_DATASET_URN);
    await lp.lineage.checkNodeExists(NODE2_DATASET_URN);
    await lp.lineage.checkNodeExists(NODE5_DATASET_MANUAL_URN);
    await lp.lineage.checkEdgeExists(NODE1_DATASET_URN, NODE2_DATASET_URN);
    await lp.lineage.checkEdgeExists(NODE1_DATASET_URN, NODE5_DATASET_MANUAL_URN);

    // Fields lineage — hover/unhover cycle for column edges
    await lp.lineage.expandContractColumns(NODE2_DATASET_URN);
    await lp.lineage.hoverColumn(NODE2_DATASET_URN, 'record_id');
    await lp.lineage.checkEdgeBetweenColumnsExists(NODE1_DATASET_URN, 'record_id', NODE2_DATASET_URN, 'record_id');
    await lp.lineage.unhoverColumn(NODE2_DATASET_URN, 'record_id');
    await lp.lineage.checkEdgeBetweenColumnsNotExists(NODE1_DATASET_URN, 'record_id', NODE2_DATASET_URN, 'record_id');

    await lp.lineage.hoverColumn(NODE2_DATASET_URN, 'next_record_id');
    await lp.lineage.checkEdgeBetweenColumnsExists(NODE1_DATASET_URN, 'next_record_id', NODE2_DATASET_URN, 'next_record_id');
    await lp.lineage.unhoverColumn(NODE2_DATASET_URN, 'next_record_id');
    await lp.lineage.checkEdgeBetweenColumnsNotExists(NODE1_DATASET_URN, 'next_record_id', NODE2_DATASET_URN, 'next_record_id');

    // Click (select) a column — edges should persist
    await lp.lineage.selectColumn(NODE2_DATASET_URN, 'record_id');
    await lp.lineage.checkEdgeBetweenColumnsExists(NODE1_DATASET_URN, 'record_id', NODE2_DATASET_URN, 'record_id');

    await lp.lineage.selectColumn(NODE2_DATASET_URN, 'next_record_id');
    await lp.lineage.checkEdgeBetweenColumnsExists(NODE1_DATASET_URN, 'next_record_id', NODE2_DATASET_URN, 'next_record_id');
    // After selecting next_record_id, record_id edge should be deselected
    await lp.lineage.checkEdgeBetweenColumnsNotExists(NODE1_DATASET_URN, 'record_id', NODE2_DATASET_URN, 'record_id');

    // Expand one node at a time
    await lp.lineage.expandOne(NODE2_DATASET_URN);
    await lp.lineage.checkNodeExists(NODE3_DATASET_URN);
    await lp.lineage.checkEdgeExists(NODE2_DATASET_URN, NODE3_DATASET_URN);

    await lp.lineage.expandOne(NODE3_DATASET_URN);
    await lp.lineage.checkNodeExists(NODE4_DATASET_URN);
    await lp.lineage.checkEdgeExists(NODE3_DATASET_URN, NODE4_DATASET_URN);

    // Expand all from a node — should reveal the full sub-graph
    await lp.lineage.expandAll(NODE5_DATASET_MANUAL_URN);
    await lp.lineage.checkNodeExists(NODE6_DATASET_URN);
    await lp.lineage.checkNodeExists(NODE4_DATASET_URN);
    await lp.lineage.checkEdgeExists(NODE5_DATASET_MANUAL_URN, NODE6_DATASET_URN);
    await lp.lineage.checkEdgeExists(NODE6_DATASET_URN, NODE4_DATASET_URN);
    await lp.lineage.checkEdgeExists(NODE3_DATASET_URN, NODE4_DATASET_URN);

    // DataJob chain
    await lp.lineage.checkNodeExists(NODE7_DATAJOB_URN);
    await lp.lineage.checkNodeExists(NODE8_DATASET_URN);
    await lp.lineage.checkEdgeExists(NODE4_DATASET_URN, NODE7_DATAJOB_URN);
    await lp.lineage.checkEdgeExists(NODE7_DATAJOB_URN, NODE8_DATASET_URN);

    // Dataset → Chart → Dashboard
    await lp.lineage.checkNodeExists(NODE9_CHART_URN);
    await lp.lineage.checkNodeExists(NODE10_DASHBOARD_URN);
    await lp.lineage.checkEdgeExists(NODE8_DATASET_URN, NODE9_CHART_URN);
    await lp.lineage.checkEdgeExists(NODE9_CHART_URN, NODE10_DASHBOARD_URN);

    // DBT chain
    await lp.lineage.checkNodeExists(NODE11_DBT_URN);
    await lp.lineage.checkNodeExists(NODE12_DATASET_URN);
    await lp.lineage.checkEdgeExists(NODE8_DATASET_URN, NODE11_DBT_URN);
    await lp.lineage.checkEdgeExists(NODE11_DBT_URN, NODE12_DATASET_URN);

    // Contract single node
    await lp.lineage.contract(NODE9_CHART_URN);
    await lp.lineage.checkNodeNotExists(NODE10_DASHBOARD_URN);

    // Contract a node with multiple downstream edges — all dependents removed
    await lp.lineage.contract(NODE4_DATASET_URN);
    await lp.lineage.checkNodeNotExists(NODE7_DATAJOB_URN);
    await lp.lineage.checkNodeNotExists(NODE8_DATASET_URN);
    await lp.lineage.checkNodeNotExists(NODE9_CHART_URN);
    await lp.lineage.checkNodeNotExists(NODE11_DBT_URN);
    await lp.lineage.checkNodeNotExists(NODE12_DATASET_URN);

    // Contract a node that shares an edge — sibling node should survive
    await lp.lineage.contract(NODE5_DATASET_MANUAL_URN);
    await lp.lineage.checkNodeNotExists(NODE6_DATASET_URN);
    await lp.lineage.checkNodeExists(NODE4_DATASET_URN); // shared downstream should still exist

    // Downstream direction — expand one starting from a chart
    await lp.goToLineageGraph(CHART_ENTITY_TYPE, NODE9_CHART_URN);
    await lp.lineage.checkNodeExists(NODE9_CHART_URN);
    await lp.lineage.checkNodeExists(NODE8_DATASET_URN);
    await lp.lineage.expandOne(NODE8_DATASET_URN);
    await lp.lineage.checkNodeExists(NODE7_DATAJOB_URN);
    await lp.lineage.checkNodeExists(NODE4_DATASET_URN);
    await lp.lineage.checkEdgeExists(NODE7_DATAJOB_URN, NODE8_DATASET_URN);
    await lp.lineage.checkEdgeExists(NODE4_DATASET_URN, NODE7_DATAJOB_URN);
    ...
    await lp.lineage.checkNodeExists(NODE9_CHART_URN);
    await lp.lineage.checkNodeExists(NODE8_DATASET_URN);
    await lp.lineage.expandAll(NODE8_DATASET_URN);
    await lp.lineage.checkNodeExists(NODE7_DATAJOB_URN);
    await lp.lineage.checkNodeExists(NODE4_DATASET_URN);
    await lp.lineage.checkNodeExists(NODE6_DATASET_URN);
    await lp.lineage.checkNodeExists(NODE5_DATASET_MANUAL_URN);
  ...
  test('should allow to expand and filter children', async ({ page, logger, logDir }) => {
    ...
    await lp.lineage.checkNodeExists(FILTERING_NODE7_URN);

    // ── Upstream filtering node ────────────────────────────────────────────

    // Initial state — 4 of 6 shown by default
    await lp.lineage.checkFilterNodeExists(FILTERING_NODE7_URN, 'up');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'up', '4 of 6 shown');
    await lp.lineage.checkFilterCounter(FILTERING_NODE7_URN, 'up', 'platform', 'PostgreSQL', '3');
    await lp.lineage.checkFilterCounter(FILTERING_NODE7_URN, 'up', 'platform', 'Snowflake', '3');
    await lp.lineage.checkFilterCounter(FILTERING_NODE7_URN, 'up', 'subtype', 'Datasets', '6');
    await lp.lineage.checkNodeNotExists(FILTERING_NODE1_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE2_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE3_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE4_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE5_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE6_URN);

    // Show more — all 6 upstream nodes visible
    await lp.lineage.showMore(FILTERING_NODE7_URN, 'up');
    await lp.lineage.checkNodeExists(FILTERING_NODE1_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE2_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE3_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE4_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE5_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE6_URN);
    ...
    await lp.lineage.filterNodes(FILTERING_NODE7_URN, 'up', 'xx');
    await lp.lineage.checkNodeExists(FILTERING_NODE1_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE2_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE3_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE4_URN);
    ...
    await lp.lineage.filterNodes(FILTERING_NODE7_URN, 'up', 'node6');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'up', '1 of 6 shown');
    await lp.lineage.checkFilterMatches(FILTERING_NODE7_URN, 'up', '1');
    await lp.lineage.checkNodeNotExists(FILTERING_NODE1_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE2_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE3_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE4_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE5_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE6_URN);

    // Filter matching multiple nodes
    await lp.lineage.filterNodes(FILTERING_NODE7_URN, 'up', 'node');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'up', '6 of 6 shown');
    await lp.lineage.checkFilterMatches(FILTERING_NODE7_URN, 'up', '6');
    await lp.lineage.checkNodeExists(FILTERING_NODE1_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE2_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE3_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE4_URN);
    ...
    await lp.lineage.filterNodes(FILTERING_NODE7_URN, 'up', 'unknown');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'up', '0 of 6 shown');
    await lp.lineage.checkFilterMatches(FILTERING_NODE7_URN, 'up', '0');
    await lp.lineage.checkNodeNotExists(FILTERING_NODE1_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE2_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE3_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE4_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE5_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE6_URN);

    // Platform filter — PostgreSQL
    await lp.lineage.filterNodes(FILTERING_NODE7_URN, 'up', 'postgres');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'up', '3 of 6 shown');
    await lp.lineage.checkFilterMatches(FILTERING_NODE7_URN, 'up', '3');
    await lp.lineage.checkNodeExists(FILTERING_NODE1_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE2_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE3_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE4_URN);
    ...
    await lp.lineage.filterNodes(FILTERING_NODE7_URN, 'up', 'snowflake');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'up', '3 of 6 shown');
    await lp.lineage.checkFilterMatches(FILTERING_NODE7_URN, 'up', '3');
    await lp.lineage.checkNodeNotExists(FILTERING_NODE1_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE2_URN);
    ...
    await lp.lineage.checkFilterNodeExists(FILTERING_NODE7_URN, 'down');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'down', '4 of 13 shown');
    await lp.lineage.checkFilterCounter(FILTERING_NODE7_URN, 'down', 'platform', 'PostgreSQL', '3');
    await lp.lineage.checkFilterCounter(FILTERING_NODE7_URN, 'down', 'platform', 'Snowflake', '10');
    await lp.lineage.checkFilterCounter(FILTERING_NODE7_URN, 'down', 'subtype', 'Datasets', '13');
    await lp.lineage.checkNodeNotExists(FILTERING_NODE8_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE9_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE10_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE11_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE12_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE13_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE14_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE15_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE16_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE17_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE18_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE19_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE20_URN);

    // Show more — additional nodes appear
    await lp.lineage.showMore(FILTERING_NODE7_URN, 'down');
    await lp.lineage.checkNodeNotExists(FILTERING_NODE8_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE9_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE10_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE11_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE12_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE13_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE14_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE15_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE16_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE17_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE18_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE19_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE20_URN);
    ...
    // Show less — back to 4
    await lp.lineage.showLess(FILTERING_NODE7_URN, 'down');
    await lp.lineage.checkNodeNotExists(FILTERING_NODE8_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE9_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE10_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE11_URN);
    ...
    await lp.lineage.showAll(FILTERING_NODE7_URN, 'down');
    await lp.lineage.checkNodeExists(FILTERING_NODE8_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE9_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE10_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE11_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE12_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE13_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE14_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE15_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE16_URN);
    ...
    await lp.lineage.filterNodes(FILTERING_NODE7_URN, 'down', 'node13');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'down', '1 of 13 shown');
    await lp.lineage.checkFilterMatches(FILTERING_NODE7_URN, 'down', '1');
    await lp.lineage.checkNodeNotExists(FILTERING_NODE8_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE9_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE10_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE11_URN);
    ...
    await lp.lineage.checkNodeNotExists(FILTERING_NODE15_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE16_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE17_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE18_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE19_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE20_URN);

    // Downstream filter — all nodes match
    await lp.lineage.filterNodes(FILTERING_NODE7_URN, 'down', 'node');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'down', '13 of 13 shown');
    await lp.lineage.checkNodeExists(FILTERING_NODE8_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE9_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE10_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE11_URN);
    ...
    await lp.lineage.filterNodes(FILTERING_NODE7_URN, 'down', 'unknown');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'down', '0 of 13 shown');
    await lp.lineage.checkFilterMatches(FILTERING_NODE7_URN, 'down', '0');
    await lp.lineage.checkNodeNotExists(FILTERING_NODE8_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE20_URN);

    // Platform filter — PostgreSQL downstream
    await lp.lineage.filterNodes(FILTERING_NODE7_URN, 'down', 'postgres');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'down', '3 of 13 shown');
    await lp.lineage.checkFilterMatches(FILTERING_NODE7_URN, 'down', '3');
    await lp.lineage.checkNodeExists(FILTERING_NODE8_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE9_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE10_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE11_URN);
    ...
    await lp.lineage.filterNodes(FILTERING_NODE7_URN, 'down', 'snowflake');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'down', '10 of 13 shown');
    await lp.lineage.checkFilterMatches(FILTERING_NODE7_URN, 'down', '10');
    await lp.lineage.checkNodeNotExists(FILTERING_NODE8_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE9_URN);
    await lp.lineage.checkNodeNotExists(FILTERING_NODE10_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE11_URN);
    ...
    await lp.lineage.checkNodeExists(FILTERING_NODE1_URN);
    await lp.lineage.checkNodeExists(FILTERING_NODE7_URN);
    await lp.lineage.checkEdgeExists(FILTERING_NODE1_URN, FILTERING_NODE7_URN);

    await lp.lineage.expandOne(FILTERING_NODE7_URN);
    await lp.lineage.checkFilterNodeExists(FILTERING_NODE7_URN, 'down');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'down', '4 of 13 shown');
    await lp.lineage.checkFilterCounter(FILTERING_NODE7_URN, 'down', 'platform', 'PostgreSQL', '3');
    await lp.lineage.checkFilterCounter(FILTERING_NODE7_URN, 'down', 'platform', 'Snowflake', '10');
    ...
    await lp.lineage.clearFilter(FILTERING_NODE7_URN, 'down');
    await lp.lineage.checkFilterNodeExists(FILTERING_NODE7_URN, 'down');
    await lp.lineage.ensureFilterNodeTitleHasText(FILTERING_NODE7_URN, 'down', '4 of 13 shown');
    await lp.lineage.checkFilterCounter(FILTERING_NODE7_URN, 'down', 'platform', 'PostgreSQL', '3');
    await lp.lineage.checkFilterCounter(FILTERING_NODE7_URN, 'down', 'platform', 'Snowflake', '10');
