...
test.describe('summary tab - about section', () => {
  ...
  test('about section', async ({ page }) => {
    ...
    await tab.expectLinkExists('testLink');

    await tab.updateLink('testLink', 'https://test-updated.com', 'testLinkUpdated');
    await tab.expectLinkExists('testLinkUpdated');

    await tab.removeLink('testLinkUpdated');
    await expect(tab.expectLinkNotExists('testLinkUpdated')).not.toBeAttached({
