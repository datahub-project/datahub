...
export class LineageTab implements Tab {
  ...
  async checkEdgeExists(urn1: string, urn2: string): Promise<void> {
    await expect(this.page.locator(`[data-testid="rf__edge-${urn1}-:-${urn2}"]`)).toBeAttached({ timeout: 10000 });
  }

  async checkEdgeBetweenColumnsExists(urn1: string, col1: string, urn2: string, col2: string): Promise<void> {
    await expect(this.page.locator(`[data-testid="rf__edge-${urn1}::${col1}-${urn2}::${col2}"]`)).toBeAttached({
      timeout: 10000,
    });
  }

  async checkEdgeBetweenColumnsNotExists(urn1: string, col1: string, urn2: string, col2: string): Promise<void> {
    await expect(this.page.locator(`[data-testid="rf__edge-${urn1}::${col1}-${urn2}::${col2}"]`)).not.toBeAttached({
      timeout: 5000,
    });
  }
  ...
  async checkFilterCounter(
    ...
    await expect(
      this.getFilterNode(urn, direction).locator(`[data-testid="filter-counter-${counterSection}-${counterType}"]`),
  ...
  async hoverColumn(urn: string, col: string): Promise<void> {
    await this.page.locator(`[data-testid="lineage-node-${urn}"]`).locator(`[data-testid="column-${col}"]`).hover();
  }
