...
export class ContextDocumentsPage extends BasePage {
  ...
  constructor(page: Page, logger?: DataHubLogger, logDir?: string) {
    ...
    this.importModal = page.getByRole('dialog').filter({ has: page.getByText('Import Documents', { exact: true }) });
  }
  ...
  async uploadFiles(files: { name: string; mimeType: string; buffer: Buffer }[]): Promise<void> {
    await this.importModal.getByTestId('import-documents-file-input').setInputFiles(files);
  }
