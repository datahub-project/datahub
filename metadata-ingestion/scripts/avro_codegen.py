...
def python_type(avro_type: str) -> str:
    if avro_type == 'string':
        return 'str'
    elif isinstance(avro_type, dict) and avro_type.get('type') == 'enum' and avro_type.get('name') == 'FabricType':
...
def generate_urn_class(entity_type: str, key_aspect: dict) -> str:
    ...
    if entity_type not in {"dataPlatform", "dataset"}:
        # TODO support all types
        return ''

    fields = key_aspect["fields"]

    init_args = ', '.join(f"{field_name(field)}: {field_type(field)}" for field in fields)
    super_args = ', '.join(field_name(field) for field in fields)
    arg_count = len(fields)
    parse_ids_mapping = ', '.join(f"{field_name(field)}=entity_ids[{i}]" for i, field in enumerate(fields))
    ...
    # TODO add docs
    # TODO add support for urns that contain other urn types (e.g. dataset refs dataPlatformUrn)
    # TODO add mapping to/from autogenerated key aspect classes

    code = f"""

class {class_name}(SpecificUrn, entity_type="{entity_type}"):
    UNDERLYING_KEY_ASPECT = '{key_aspect['name']}'
