...
def calculate_schema_hash(schema_file_path: Path) -> str:
    """
    Calculate a deterministic MD5 hash from the schema.avsc file.
    This ensures consistent hashing regardless of JSON key ordering.
    """
    with open(schema_file_path, "r", encoding="utf-8") as f:
        schema_data = json.load(f)

    # Generate deterministic JSON string with sorted keys
    json_string = json.dumps(
        schema_data, separators=(",", ":"), sort_keys=True, ensure_ascii=True
    )

    # TODO: remove 'doc' fields from the schema before hashing to avoid unnecessary hash changes, anything else?

    # Generate MD5 hash
    return hashlib.md5(json_string.encode("utf-8")).hexdigest()


def create_schema_version_file(outdir: Path, schema_hash: str) -> None:
    """
    Create a Python file with the schema hash constant.
    """
    version_content = f'''# This file is autogenerated by /metadata-ingestion/scripts/avro_codegen.py
# Do not modify manually!

# Hash of the schema.avsc file for version tracking
SCHEMA_CLASSES_HASH_VERSION = "{schema_hash}"
'''

    (outdir / "schema_classes_version.py").write_text(version_content)
...
def generate(
    ...
    # Calculate hash of the generated schema.avsc file and create version file
    schema_file_path = Path(outdir) / "schema.avsc"
    assert schema_file_path.exists(), (
        "schema.avsc should exist after write_schema_files"
    )
    schema_hash = calculate_schema_hash(schema_file_path)
    create_schema_version_file(Path(outdir), schema_hash)
