...
# and replace them with compatible ones. There's a two snags with this approach:
#   - We need to manually remove the Airflox 2.x providers that were split into separate packages
#   - Airflow likes strict dependencies pins and restricts the requests version, which
#     in turn restricts the urllib3 version. By manually forcing the requests version up,
#     we're technically incompatible with Airflow 1.x but in practice seem to work ok.
