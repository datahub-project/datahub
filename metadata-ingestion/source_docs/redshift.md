...
| `include_operational_stats` |          | `true`                                                         | Whether to display operational stats. Currently, only the last updated timestamp is displayed in both local time and UTC. A dataset is considered to have been updated when an INSERT or DELETE operation took place. |
