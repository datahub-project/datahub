...
def stateful_ingest(
    pytestconfig, tmp_path, mock_datahub_graph, mocked_functions_reference
):
    test_resources_dir: pathlib.Path = (
        pytestconfig.rootpath / "tests/integration/azure_ad"
    )
    ...
    with patch(
        ...
        pipeline = Pipeline.create(
            {
                "pipeline_name": "stateful_execution",
                "run_id": "test-azure-ad",
                "source": {
                    "type": "azure-ad",
                    "config": {
                ...
                "sink": {
                    "type": "file",
                    "config": {
                        "filename": f"{tmp_path}/azure_ad_mces_stateful_config.json",
                    },
                },
            }
        )
...
def test_azure_ad_stateful_ingestion(
    ...
    pipeline1 = stateful_ingest(
        pytestconfig, tmp_path, mock_datahub_graph, mocked_functions
    )
    checkpoint1 = get_current_checkpoint_from_pipeline(pipeline1)
    assert checkpoint1
    assert checkpoint1.state

    pipeline2 = stateful_ingest(
        pytestconfig, tmp_path, mock_datahub_graph, stateful_mocked_functions
    )
