...
def test_event_schema_urn_caching(pytestconfig):
    ...
    with patch(
        ...
        # Verify only one API call was made (caching worked). This is the
        # deterministic form of "the cached calls were faster": calls 2 and 3
        # never reached the slow API at all, so there is nothing left for a
        # timing ratio to prove. The timings below are diagnostics only.
