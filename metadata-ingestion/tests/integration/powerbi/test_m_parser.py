...
M_QUERIES = [
    ...
    'let\n Source = DatabricksMultiCloud.Catalogs("dbc-4b423f2e-28fc.cloud.databricks.com", "/sql/1.0/warehouses/cf2cfe3fe1d4c7ea", [Catalog=null, Database=null, EnableAutomaticProxyDiscovery=null]),\n ml_prod_Database = Source{[Name="ml_prod",Kind="Database"]}[Data],\n membership_corn_Schema = ml_prod_Database{[Name="membership_corn",Kind="Schema"]}[Data],\n time_exp_data_v3_Table = membership_corn_Schema{[Name="time_exp_data_v3",Kind="Table"]}[Data],\n #"Renamed Columns" = Table.RenameColumns(time_exp_data_v3_Table)\nin\n #"Renamed Columns"',
    'let\n Source = DatabricksMultiCloud.Catalogs("dbc-4b423f2e-28fc.cloud.databricks.com", "/sql/1.0/warehouses/cf2cfe3fe1d4c7ea", [Catalog="data_analysis", Database="ballotsummary", EnableAutomaticProxyDiscovery=null]),\n committee_data_ballotsummary_dev = Source{[Item="committee_data",Schema="ballotsummary",Catalog="data_analysis"]}[Data],\n #"Added Index" = Table.AddIndexColumn(committee_data_ballotsummary_dev, "Index", 1, 1, Int64.Type),\n #"Renamed Columns" = Table.RenameColumns(#"Added Index",)\nin\n #"Renamed Columns"',
]
...
def test_databricks_catalog_pattern():
    table: powerbi_data_classes.Table = powerbi_data_classes.Table(
