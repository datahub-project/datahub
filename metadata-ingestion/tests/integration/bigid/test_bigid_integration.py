...
def test_bigid_source_golden() -> None:
    ...
    assert os.path.exists(golden_path), (
        f"Golden file missing: {golden_path}. Run with UPDATE_GOLDEN=1 to generate it."
    )
...
def test_editable_schema_metadata_emitted() -> None:
    ...
    mcps = _run_source()
    enriched = {
        m["entityUrn"] for m in mcps if m.get("aspectName") == "editableSchemaMetadata"
    }
...
def test_unlinked_classifier_term_content() -> None:
    ...
    assert aspect.get("name") == "Phone", (
        f"Expected name='Phone', got {aspect.get('name')!r}"
    )
...
def test_unstructured_enrichment_emits_dataset_level_glossary_terms() -> None:
    ...
    terms_mcps = [
        m
        for m in mcps
        if m["entityUrn"] == sharepoint_urn and m.get("aspectName") == "glossaryTerms"
    ]
...
def test_unstructured_enrichment_emits_dataset_profile() -> None:
    ...
    profile_mcps = [
        m
        for m in mcps
        if m["entityUrn"] == sharepoint_urn and m.get("aspectName") == "datasetProfile"
    ]
    assert len(profile_mcps) == 1, (
        "Expected exactly 1 datasetProfile MCP on SharePoint dataset"
    )

    aspect = _parse_aspect(profile_mcps[0])
    assert aspect.get("rowCount") == 42, (
        f"Expected rowCount=42 (total_pii_count), got {aspect.get('rowCount')}"
    )
    assert aspect.get("columnCount") == 0, (
        "Unstructured dataset must have columnCount=0"
    )
...
def test_domain_entities_emit_status_aspect() -> None:
    ...
    mcps = _run_source({"domain_mode": "auto_namespaced"})
    domain_urns = {
        m["entityUrn"] for m in mcps if m.get("aspectName") == "domainProperties"
    }
    status_urns = {
        m["entityUrn"]
        for m in mcps
        if m.get("aspectName") == "status"
        and "urn:li:domain:" in m.get("entityUrn", "")
    }
    missing = domain_urns - status_urns
    assert not missing, f"Domain entities missing status aspect: {missing}"
