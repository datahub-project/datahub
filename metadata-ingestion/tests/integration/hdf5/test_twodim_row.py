...
def test_hdf5_compound_dataset(pytestconfig, tmp_path, mock_time):
    ...
    pipeline = Pipeline.create(
        {
            ...
            "sink": {
                "type": "file",
                "config": {
                    "filename": f"{tmp_path}/hdf5_twodim_row_test.json",
                },
            },
        }
    )
    ...
    mce_helpers.check_golden_file(
        pytestconfig,
        output_path=tmp_path / "hdf5_twodim_row_test.json",
        golden_path=test_resources_dir / "hdf5_twodim_row_test_golden.json",
        ignore_paths=[
