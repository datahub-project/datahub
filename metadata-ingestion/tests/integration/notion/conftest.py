...
def notion_test_root_page(
    ...
    This creates a clean organizational structure:
        ...
        └── DataHub Test Root Page (pytest session)
            ├── Test Page - Synced Blocks
            ├── Test Page - Numbered Lists
            └── Test Page - Complex Content
    ...
    print(f"\n✓ Created Test Root Page: {test_root_page_id}")
...
def test_page_synced_blocks(
    ...
    print(f"  ✓ Created test page: Synced Blocks ({page_id})")
    ...
    try:
        ...
        for block in blocks["results"]:  # type: ignore[index]
            if (
                ...
                print(f"    ✓ Found original synced block: {original_synced_block_id}")
                break
        ...
        if original_synced_block_id:
            ...
            print(
                f"    ✓ Created reference synced block pointing to {original_synced_block_id}"
            )
        else:
            print("    ⚠ Could not find original synced block to create reference")

    except Exception as e:
        print(f"    ⚠ Failed to create reference synced block: {e}")
...
def test_page_numbered_lists(
    ...
    page_id = page_response["id"]  # type: ignore[index]
    page_url = f"https://www.notion.so/{page_id.replace('-', '')}"
    print(f"  ✓ Created test page: Simple Content ({page_id})")
    print(f"    View at: {page_url}")
...
def test_page_complex_content(
    ...
    page_id = page_response["id"]  # type: ignore[index]
    page_url = f"https://www.notion.so/{page_id.replace('-', '')}"
    print(f"  ✓ Created test page: Complex Content ({page_id})")
    print(f"    View at: {page_url}")
