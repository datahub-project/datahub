...
LOAD DATA LOCAL INPATH '/opt/hive/examples/files/kv1.txt' INTO TABLE db1.pokes PARTITION (baz='dummy');

CREATE TABLE IF NOT EXISTS db2.pokes (foo INT, bar STRING);
LOAD DATA LOCAL INPATH '/opt/hive/examples/files/kv1.txt' INTO TABLE db2.pokes;
...
(
 ...
 service array<STRUCT<type: STRING,provider: ARRAY<INT>>> COMMENT 'service types and providers'
)  TBLPROPERTIES ('comment' = 'This table has array of structs', 'another.comment' = 'This table has no partitions');

WITH
test_data as (
    SELECT 989 property_id, array(
        NAMED_STRUCT('type','Cleaning','provider', ARRAY(587, 887)),
...
(
 property_id INT,
 service STRUCT<type: STRING,provider: STRUCT<name:VARCHAR(50), id:TINYINT>>
);
