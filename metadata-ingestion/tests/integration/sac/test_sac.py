...
def test_sac(
    ...
    with open(f"{test_resources_dir}/metadata.xml", mode="rb") as f:
        ...
        def match_metadata(request, context):
            _check_authorization(request.headers)

            context.headers["content-type"] = "application/xml"

            return content

        requests_mock.get(f"{MOCK_TENANT_URL}/api/v1/$metadata", content=match_metadata)
...
def _check_authorization(headers: Dict) -> None:
