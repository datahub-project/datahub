...
@freeze_time(FROZEN_TIME)
@pytest.mark.integration
def test_openapi_2_0_ingest(pytestconfig, tmp_path):
    test_resources_dir = pytestconfig.rootpath / "tests/integration/openapi"

    # Run the metadata ingestion pipeline.
    config_file = (test_resources_dir / "openapi_2_0_to_file.yml").resolve()
    run_datahub_cmd(["ingest", "-c", f"{config_file}"], tmp_path=tmp_path)

    # Verify the output.
    mce_helpers.check_golden_file(
        pytestconfig,
        output_path="/tmp/openapi_2_0_mces.json",
        golden_path=test_resources_dir / "openapi_2_0_mces_golden.json",
    )
