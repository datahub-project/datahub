...
def setup_user_and_get_id(setup_token):
    setup_response = requests.post(
        ...
        None,
        {"token": f"{setup_token}",
         "prefs": {"site_name": "Acryl", "site_locale": "en", "allow_tracking": "false"}, "database": None,
         "user": {"first_name": "admin", "last_name": "admin", "email": "admin@metabase.com", "password": "admin12345",
                  "site_name": "Acryl"}},
    )
...
def test_metabase_ingest(docker_compose_runner, pytestconfig, tmp_path, mock_time):
    ...
    with docker_compose_runner(
        ...
        # create dashboard
        create_dashboard_response = session.post(f"{METABASE_URL}/api/dashboard",
                                                 None,
        ...
        create_card_file_path = (test_resources_dir / "setup/create_card.json").resolve()
        create_card_json = mce_helpers.load_json_file(create_card_file_path)
        card_create_response = session.post(f"{METABASE_URL}/api/card", None, create_card_json)
        card_id = card_create_response.json()['id']
        session.post(f"{METABASE_URL}/api/dashboard/{dashboard_id}/cards", None, {"cardId": card_id})
        ...
        with fs_helpers.isolated_filesystem(tmp_path):
            ...
            # Verify the output.
            output = mce_helpers.load_json_file("metabase_mces.json")
            golden = mce_helpers.load_json_file(test_resources_dir / "metabase_mces_golden.json")

            # replace timestamps from output because these will always be different
            output[0]['proposedSnapshot']['com.linkedin.pegasus2avro.metadata.snapshot.DashboardSnapshot'] \
                ['aspects'][0]['com.linkedin.pegasus2avro.dashboard.DashboardInfo']['lastModified'] \
                ['created']['time'] = None
            ...
            output[1]['proposedSnapshot']['com.linkedin.pegasus2avro.metadata.snapshot.ChartSnapshot']['aspects'][0] \
                ['com.linkedin.pegasus2avro.chart.ChartInfo']['lastModified'] \
                ['created']['time'] = None
