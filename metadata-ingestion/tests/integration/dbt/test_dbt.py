...
def test_dbt_ingest(pytestconfig, tmp_path, mock_time, **kwargs):
    ...
    config_variants = [
        ...
        DbtTestConfig(
            "dbt-test-with-schemas-filtered-schema",
            test_resources_dir,
            test_resources_dir,
            tmp_path,
            "dbt_with_schemas_mces.json",
            "dbt_with_schemas_mces_filtered_schema_golden.json",
            source_config_modifiers={
                "load_schemas": True,
                "disable_dbt_node_creation": True,
                "enable_meta_mapping": True,
                "owner_extraction_pattern": r"^@(?P<owner>(.*))",
                "schema_pattern": {"deny": ["dbt_postgres"]},
            },
        ),
        DbtTestConfig(
            "dbt-test-with-schemas-filtered-database",
            test_resources_dir,
            test_resources_dir,
            tmp_path,
            "dbt_with_schemas_mces.json",
            "dbt_with_schemas_mces_schema_filtered_database_golden.json",
            source_config_modifiers={
                "load_schemas": True,
                "disable_dbt_node_creation": True,
                "enable_meta_mapping": True,
                "owner_extraction_pattern": r"^@(?P<owner>(.*))",
                "database_pattern": {"deny": ["pagila"]},
            },
        ),
        DbtTestConfig(
