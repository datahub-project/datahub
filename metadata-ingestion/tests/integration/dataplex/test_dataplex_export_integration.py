"""Integration tests for the Dataplex export extraction method.
...
def canned_export_lines() -> List[str]:
    """The canned export output shared by the submit-mode and read-only tests."""
...
def test_dataplex_readonly_export_integration(
    ...
    only the most recently written one is ingested, and the result matches
    the same golden file as the submit-mode run.
    """
    mock_credentials = Mock()
    mock_google_auth.return_value = (mock_credentials, "test-project")

    current_blob = make_blob(
        "metadata/job=nightly-2024-01-15/entry_group=@bigquery/part-0.jsonl",
        canned_export_lines(),
        time_created=datetime(2024, 1, 15, tzinfo=timezone.utc),
    )
    # An older export's partition under the same path must be ignored.
    older_blob = make_blob(
        "metadata/job=nightly-2024-01-01/entry_group=@bigquery/part-0.jsonl",
        [
            make_export_entry_line(
                entry_id="stale",
                fqn="bigquery:test-project.stale.table",
