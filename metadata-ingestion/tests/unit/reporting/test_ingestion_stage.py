...
def test_ingestion_stage_context_records_duration():
    ...
    with report.new_stage(stage="Test Stage"):
        pass
    assert len(report._ingestion_stage_durations) == 1
    assert "Test Stage" in next(iter(report._ingestion_stage_durations.keys()))
...
def test_ingestion_stage_context_handles_exceptions():
    ...
    try:
        ...
        pass
    assert len(report._ingestion_stage_durations) == 1
    assert "Test Stage" in next(iter(report._ingestion_stage_durations))
...
def test_ingestion_stage_context_report_handles_multiple_stages():
    ...
    with report.new_stage(stage="Test Stage 3"):
        time.sleep(0.1)
    assert len(report._ingestion_stage_durations) == 3
    assert all(
        isinstance(duration, float) and duration > 0.0
        for duration in report._ingestion_stage_durations.values()
    )

    sorted_stages = list(sorted(report._ingestion_stage_durations.keys()))
