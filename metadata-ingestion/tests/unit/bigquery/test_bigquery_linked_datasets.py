...
class TestLinkedDatasetInfo:
    def test_extra_properties_includes_source_when_publisher_resolved(self):
    ...
    def test_extra_properties_falls_back_to_data_exchange(self):
        info = LinkedDatasetInfo(
            consumer_project_id="c-proj",
            consumer_dataset="shared_dataset",
            publisher_project_number="111",
    ...
    def test_extra_properties_omits_unpopulated_keys(self):
        info = LinkedDatasetInfo(
            consumer_project_id="c-proj",
            consumer_dataset="shared_dataset",
            publisher_project_number="111",
    ...
    def test_extra_properties_no_source_when_publisher_unresolved(self):
        info = LinkedDatasetInfo(
            consumer_project_id="c-proj",
            consumer_dataset="shared_dataset",
            publisher_project_number="111",
...
class TestPopulateForProject:
    def test_only_bigquery_dataset_subscriptions_advance(self):
    ...
    def test_dataset_pattern_filter_short_circuits(self):
        config = _make_config(
        ...
        # Only shared_a survives the filter; shared_b is short-circuited before
        # any get_dataset / get_project call is made.
        assert handler.get_info("consumer-project", "shared_a") is not None
        assert handler.get_info("consumer-project", "shared_b") is None
        bq.get_dataset.assert_called_once_with("consumer-project.shared_a")

    def test_locations_lowercased_for_ah_call(self):
        handler = _make_handler()
    ...
    def test_state_counters_incremented(self):
        handler = _make_handler()
        ...
        # All three states still get ingested and emit lineage; only the
        # informational counters differ.
...
class TestLineageEmission:
    def _seed_with_linked_dataset(
        ...
        *,
        config: Optional[BigQueryV2Config] = None,
        publisher_project_id: str = "publisher-project",
        publisher_dataset: str = "publisher_dataset",
    ) -> BigQueryLinkedDatasetsHandler:
        handler = _make_handler(config=config)
        # Bypass detection by populating the lookup directly — we want to
        # exercise the emission shape in isolation.
    ...
    def _column(self, name: str) -> BigqueryColumn:
    ...
    def _aspects(self, wus: Iterable[MetadataWorkUnit]) -> List[Any]:
        out: List[Any] = []
        for wu in wus:
            mcp = wu.metadata
    ...
    def test_emits_siblings_and_upstream_lineage_with_per_column_finegrained(
        ...
        columns = [self._column(c) for c in ("id", "email", "name")]
    ...
    def test_convert_column_urns_to_lowercase_lowercases_both_sides(self):
        config = _make_config(convert_column_urns_to_lowercase=True)
        handler = self._seed_with_linked_dataset(config=config)
        columns = [self._column("Email"), self._column("USER_ID")]
        ...
        upstream = next(a for a in self._aspects(wus) if isinstance(a, UpstreamLineage))
        fine_grained = upstream.fineGrainedLineages
        assert fine_grained is not None
        for fgl in fine_grained:
            ...
            assert fgl.downstreams[0].endswith(",email)") or fgl.downstreams[
                0
            ].endswith(",user_id)")
...
@pytest.mark.parametrize(
    "publisher_project_id, publisher_dataset, expected_has_publisher",
    [
        ("publisher-project", "publisher_dataset", True),
        (None, "publisher_dataset", False),
        ("publisher-project", None, False),
        (None, None, False),
    ],
)
def test_has_publisher(
    ...
    assert info.has_publisher is expected_has_publisher
