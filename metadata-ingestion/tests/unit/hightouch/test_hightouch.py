"""Unit tests for HightouchSource."""
...
class TestHightouchIngestionSource(TestCase):
    """Test cases for HightouchIngestionSource."""

    def setUp(self):
        """Set up test fixtures."""
        self.config = HightouchSourceConfig(
    ...
    def test_init(self, mock_api_client_class):
        """Test HightouchSource initialization."""
        source = HightouchIngestionSource(self.config, self.ctx)

        assert source.config == self.config
        assert isinstance(source.report, type(source.report))
        mock_api_client_class.assert_called_once_with(self.config.api_config)

    @patch("datahub.ingestion.source.hightouch.hightouch.HightouchAPIClient")
    def test_get_platform_for_source_with_mapping(self, mock_api_client_class):
        """Test platform detection with custom mapping."""
        ...
        source = HightouchIngestionSource(config, self.ctx)

        source_entity = HightouchSource(
            id="source1",
            name="Test Source",
            slug="test-source",
            type="snowflake",
    ...
    @patch("datahub.ingestion.source.hightouch.hightouch.HightouchAPIClient")
    def test_get_platform_for_source_known_mapping(self, mock_api_client_class):
        """Test platform detection with known platform mapping."""
        source = HightouchIngestionSource(self.config, self.ctx)
    ...
    @patch("datahub.ingestion.source.hightouch.hightouch.HightouchAPIClient")
    def test_get_platform_for_destination_with_mapping(self, mock_api_client_class):
        """Test destination platform detection with custom mapping."""
        config = HightouchSourceConfig(
            api_config=HightouchAPIConfig(api_key="test"),
    ...
    @patch("datahub.ingestion.source.hightouch.hightouch.HightouchAPIClient")
    def test_generate_model_dataset(self, mock_api_client_class):
        """Test model dataset generation."""
        source_instance = HightouchIngestionSource(self.config, self.ctx)
    ...
    @patch("datahub.ingestion.source.hightouch.hightouch.HightouchAPIClient")
    def test_generate_dataflow_from_sync(self, mock_api_client_class):
        """Test dataflow generation from sync."""
        source_instance = HightouchIngestionSource(self.config, self.ctx)
        ...
        assert dataflow.urn.cluster == "PROD"  # env is stored as cluster in DataFlowUrn

    @patch("datahub.ingestion.source.hightouch.hightouch.HightouchAPIClient")
    def test_get_workunits_internal(self, mock_api_client_class):
        ...
        # Create source and generate workunits
        source_instance = HightouchIngestionSource(self.config, self.ctx)
    ...
    @patch("datahub.ingestion.source.hightouch.hightouch.HightouchAPIClient")
    def test_sync_patterns_filtering(self, mock_api_client_class):
        """Test sync filtering with patterns."""
        ...
        source_instance = HightouchIngestionSource(config, self.ctx)
        workunits = list(source_instance.get_workunits_internal())

        # Only prod-sync should be processed (test-sync is filtered out)
        # The exact count depends on what entities are created, but we should have some workunits
        assert len(workunits) >= 0
