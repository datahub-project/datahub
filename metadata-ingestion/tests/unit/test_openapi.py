...
class TestAPISourceSchemaExtraction(unittest.TestCase):
    ...
    def test_extract_request_schema_malformed_parameters_reaches_report_not_just_log(
        ...
        # A malformed 'parameters' value must surface as a report warning, not
        # just a log line, so operators see it in the ingestion report.
    ...
    def test_merge_allof_same_pattern_collision_preserves_value_schema(self):
        # Colliding same-pattern value schemas are merged (the value schema is
        # preserved), not dropped, when the same pattern appears in two members.
        sw_dict = _EMPTY_OPENAPI_SW
        schema = {
            "allOf": [
                {"patternProperties": {"^x": {"type": "string", "format": "uuid"}}},
                {"patternProperties": {"^x": {"type": "string"}}},
            ]
        }
        ...
        pattern_schema = resolved["patternProperties"]["^x"]
        self.assertEqual(pattern_schema.get("type"), "string")
    ...
    def test_property_names_ref_preserves_sibling_allof(self):
        ...
        # The $ref under propertyNames is resolved (nothing left for jsonref to
        # choke on) and the resolved target's type survives the sibling allOf merge.
    ...
    def test_merge_allof_map_plus_oneof_keeps_map_value_type(self):
        # Regression: allOf combining a map (additionalProperties) with a oneOf
        # member must keep the map. json_schema_util classifies oneOf/anyOf as a
        # union before it checks additionalProperties and does not forward the map
        # value schema through the union path, so preserving the oneOf here would
        # silently drop the map<string, string> value type from extraction.
        schema = {
            "type": "object",
            "allOf": [
                {"additionalProperties": {"type": "string"}},
        ...
        merged = merge_allof_schemas(schema, _EMPTY_OPENAPI_SW, max_depth=10)
        self.assertEqual(merged.get("additionalProperties"), {"type": "string"})
        self.assertNotIn("oneOf", merged)
        self.assertNotIn("anyOf", merged)

    def test_merge_allof_object_with_props_and_additional_keeps_oneof(self):
        # The map guard must NOT fire when named properties are present: such a
        # schema is an object, not a map, so json_schema_util's union path forwards
        # properties/required AND extracts the oneOf branches. Dropping oneOf here
        # (just because additionalProperties is also set) would discard the
        # union-branch fields.
        schema = {
            "type": "object",
            "properties": {"id": {"type": "string"}},
        ...
        merged = merge_allof_schemas(schema, _EMPTY_OPENAPI_SW, max_depth=10)
        self.assertEqual(
            merged.get("oneOf"), [{"properties": {"catDog": {"type": "string"}}}]
        )
        self.assertIn("catDog", json.dumps(merged))
