...
def test_is_special_value():
    ...
    assert is_special_value(-1)  # Common sentinel value
    assert is_special_value(2147483647)  # Integer.MAX_VALUE
    assert is_special_value(9.223372036854776e18)  # Java Long MAX_VALUE
...
def test_process_field_statistics(profiler, sample_data):
    ...
    assert data_type.upper() in ["NUMERIC", "NUMBER", "FLOAT"]
    ...
    data_type = stats.data_type or ""
    assert data_type.upper() in ["STRING", "TEXT", "VARCHAR"]
    assert stats.null_count == 1
    assert stats.null_proportion == 0.2  # 1/5 = 0.2
...
def test_histogram_generation(profiler):
    ...
    with patch.object(profiler, "_create_histogram") as mock_create_histogram:
        ...
        with patch.object(
            profiler, "_process_field_statistics", return_value=test_stats
        ):
...
@patch("datahub.ingestion.source.kafka.kafka_profiler.KafkaProfiler._get_field_path")
@patch(
...
def test_profile_samples(
    ...
    with patch.object(profiler, "_process_field_statistics") as mock_process_stats:
        ...
        typed_sample_data = cast(List[Dict[str, Any]], sample_data)
        result = profiler.profile_samples(typed_sample_data, schema_metadata)

        # Verify result
        assert result is mock_profile
        assert mock_create_profile.call_count == 1

        # Make sure _init_schema_fields was called
        mock_init_schema.assert_called_once_with(schema_metadata)
...
def test_profile_topic_static_method(sample_data, schema_metadata):
    ...
    result = KafkaProfiler.profile_topic("empty_topic", [], schema_metadata, config)
...
def test_flatten_json_max_depth():
    ...
    for key in flattened:
        assert key.count(".") <= 1  # At most 1 dot (2 levels)
...
def test_expensive_profiling_limits():
    ...
    if len(processed_fields) > 2:
        ...
        assert len(later_field.sample_values) == 0
