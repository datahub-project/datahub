...
class TestDomainHierarchyBehavior(unittest.TestCase):
    ...
    def test_domain_created_from_iri_hierarchy(self):
        ...
        # All domains (root + subdomains) should be in result.domains so all get MCPs
        ...
        bank_domain = next(
            d for d in result.domains if tuple(d.path_segments) == ("bank.com",)
        )
        trading_domain = next(
            (
                sd
    ...
    def test_domain_parent_child_relationships(self):
        ...
        root_domains = [d for d in result.domains if d.parent_domain_urn is None]
        subdomains = [d for d in result.domains if d.parent_domain_urn is not None]
        ...
        trading_in_list = next(
            (d for d in result.domains if d.urn == trading_domain.urn), None
        )
        loans_in_list = next(
            (d for d in result.domains if d.urn == loans_domain.urn), None
        )
        self.assertIsNotNone(
