...
def test_structured_properties() -> None:
    ...
    assert c.structured_properties is not None
    assert len(c.structured_properties) == 2

    # Update first property
    c.set_structured_property("urn:li:structuredProperty:sp1", ["updated_value"])

    assert c.structured_properties is not None
    assert len(c.structured_properties) == 2

    # Add new property
    c.set_structured_property("urn:li:structuredProperty:sp3", ["new_value"])

    sp_dict = {prop.propertyUrn: prop.values for prop in c.structured_properties}

    assert sp_dict == {
        "urn:li:structuredProperty:sp1": ["updated_value"],
        "urn:li:structuredProperty:sp2": ["value2"],
        "urn:li:structuredProperty:sp3": ["new_value"],
    }

    assert_entity_golden(c, _GOLDEN_DIR / "test_structured_properties_golden.json")
