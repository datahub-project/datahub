...
def test_odbc_strip_athena_catalog_from_upstreams(odbc_lineage):
    ...
    stripped_lineage = odbc_lineage._strip_athena_catalog_from_lineage(
        original_lineage, platform_pair
    )
...
def test_odbc_strip_glue_catalog_from_lineage(odbc_lineage):
    """Test that catalog stripping also works for glue platform."""
    from datahub.ingestion.source.powerbi.config import DataPlatformPair
    from datahub.ingestion.source.powerbi.m_query.data_classes import (
        DataPlatformTable,
        Lineage,
    ...
    platform_pair = DataPlatformPair(
        datahub_data_platform_name="glue",
        powerbi_data_platform_name="AWS Glue",
    )
    ...
    original_lineage = Lineage(
        upstreams=[
            DataPlatformTable(
                data_platform_pair=platform_pair,
                urn="urn:li:dataset:(urn:li:dataPlatform:glue,awsdatacatalog.mydb.mytable,PROD)",
            )
        ],
        column_lineage=[
            ColumnLineageInfo(
                ...
                upstreams=[
                    ColumnRef(
                        table="urn:li:dataset:(urn:li:dataPlatform:glue,awsdatacatalog.mydb.mytable,PROD)",
                        column="id",
                    )
                ],
            )
    ...
    assert (
        stripped_lineage.upstreams[0].urn
        == "urn:li:dataset:(urn:li:dataPlatform:glue,mydb.mytable,PROD)"
    )

    # Check column lineage URN is stripped
    assert (
        stripped_lineage.column_lineage[0].upstreams[0].table
        == "urn:li:dataset:(urn:li:dataPlatform:glue,mydb.mytable,PROD)"
    )
