...
def _extractor() -> tuple:
    """Create a FlinkCatalogExtractor with a mocked SQL client."""
...
class TestCatalogExtractorErrorRecovery:
    def test_get_catalogs_failure_returns_gracefully(self) -> None:
        """Total failure to list catalogs emits warning and returns empty."""
    ...
    def test_table_failure_continues_to_next_table(self) -> None:
        """Failure to process one table doesn't stop ingestion of others."""
        extractor, sql_client, report = _extractor()
        ...
        # good_table and other_table processed; bad_table skipped with warning
