...
def test_dataproduct_ownership_type_urn_patch_yaml(
    ...
    data_product_file = test_resources_dir / "dataproduct_ownership_type_urn_different_owner.yaml"
    original_data_product: DataProduct = DataProduct.from_yaml(data_product_file, mock_graph)

    data_product: DataProduct = DataProduct.from_datahub(
        mock_graph, id="urn:li:dataProduct:pet_of_the_week"
    )

    dataproduct_output_file = tmp_path / f"patch_dataproduct_ownership_type_urn_different_owner.yaml"
    data_product.patch_yaml(original_data_product, dataproduct_output_file)

    assert not check_yaml_golden_file(str(dataproduct_output_file),
