...
@time_machine.travel("2023-01-02 00:00:00", tick=False)
@patch(
    "datahub.ingestion.workunit_processors.ensure_aspect_size.EnsureAspectSizeProcessor.ensure_semantic_model_info_size"
)
def test_wu_processor_triggered_by_semantic_model_info(
    ensure_semantic_model_info_size_mock, processor
):
    list(
        processor.process(
            [
                MetadataChangeProposalWrapper(
                    entityUrn="urn:li:semanticModel:(urn:li:dataPlatform:snowflake,db.schema,view)",
                    aspect=proper_semantic_model_info(),
                ).as_workunit()
            ]
        )
    )
    ensure_semantic_model_info_size_mock.assert_called_once()
