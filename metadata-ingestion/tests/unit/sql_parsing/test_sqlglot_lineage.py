...
class TestSigmaSqlPreprocessing:
    ...
    def test_case_when_alias_dot(self) -> None:
        ...
        from datahub.sql_parsing.sqlglot_lineage import _preprocess_query_for_sigma

        # case whenq11.col -> case when q11.col
        result = _preprocess_query_for_sigma("SELECT case whenq11.value THEN 1 END")
        assert "case when q11.value" in result

        # case whenq123.col -> case when q123.col
        result = _preprocess_query_for_sigma("SELECT case whenq123.id > 0 THEN 1 END")
    ...
    def test_case_when_identifier_operator(self) -> None:
        ...
        result = _preprocess_query_for_sigma("SELECT case whenarr_down>0 THEN 1 END")
        assert "case when arr_down >" in result

        # case whenvalue= -> case when value =
        result = _preprocess_query_for_sigma("SELECT case whenvalue=1 THEN 'a' END")
    ...
    def test_case_when_identifier_keyword(self) -> None:
        ...
        result = _preprocess_query_for_sigma(
        ...
        result = _preprocess_query_for_sigma(
            "SELECT case whenmy_col is null THEN 0 END"
        )
    ...
    def test_when_identifier_operator_no_space(self) -> None:
        ...
        result = _preprocess_query_for_sigma("CASE whenarr_down>0 THEN 1 END")
    ...
    def test_combined_sigma_patterns(self) -> None:
        ...
        FROM mytable
        """
        result = _preprocess_query_for_sigma(query)
