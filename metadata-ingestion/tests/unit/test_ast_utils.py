...
# M_QUERIES[0] from tests/integration/powerbi/test_m_parser.py — the canonical
# Snowflake three-step let expression used across PowerBI parser tests.
_SNOWFLAKE_M_EXPRESSION = (
    "let\n"
...
def snowflake_node_map() -> NodeIdMap:
    ...
    _clear_bridge()
    bridge = get_bridge()
    result = bridge.parse(_SNOWFLAKE_M_EXPRESSION)
    _clear_bridge()
    return result
...
def test_find_nodes_by_kind_returns_all_matching(snowflake_node_map: NodeIdMap) -> None:
...
def test_find_nodes_by_kind_returns_deep_nodes(snowflake_node_map: NodeIdMap) -> None:
...
def test_get_invoke_callee_name_snowflake(snowflake_node_map: NodeIdMap) -> None:
...
def test_get_invoke_callee_name_returns_none_for_non_invoke() -> None:
    fake_node = {
...
def test_get_literal_value_text() -> None:
    node = {
        "kind": "LiteralExpression",
        "literal": '"my_database"',
        "literalKind": "Text",
...
def test_get_literal_value_non_text_returns_none() -> None:
    node = {"kind": "LiteralExpression", "literal": "42", "literalKind": "Number"}
    assert get_literal_value(node) is None


def test_get_literal_value_null_returns_none() -> None:
    node = {"kind": "LiteralExpression", "literal": "null", "literalKind": "Null"}
    assert get_literal_value(node) is None


def test_get_literal_value_non_literal_returns_none() -> None:
...
def test_resolve_identifier_finds_variable(snowflake_node_map: NodeIdMap) -> None:
...
def test_get_record_field_values_empty_record() -> None:
