...
class TestDataplexEntriesProcessorDesign:
    ...
    def test_entries_report_tracks_counts_and_default_sampling(self) -> None:
        ...
        for index in range(12):
            report.report_entry(
                f"entry-{index}", filtered_fqn=False, filtered_name=True
            )

        for index in range(12):
            report.report_entry(f"fqn-{index}", filtered_fqn=True, filtered_name=False)

        for index in range(12):
            report.report_entry(
                f"processed-{index}", filtered_fqn=False, filtered_name=False
            )
    ...
    def test_collect_entries_streams_before_spanner_search(self) -> None:
        ...
        catalog_client.list_entry_groups.return_value = [entry_group]
