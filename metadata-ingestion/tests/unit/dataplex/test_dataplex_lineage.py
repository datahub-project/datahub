...
def test_error_classification_transient() -> None:
    """Test classification of transient errors."""
    config = DataplexConfig(project_ids=["test-project"])
    report = DataplexReport()
    extractor = DataplexLineageExtractor(config=config, report=report)

    # Test various transient error types
    timeout_error = Exception("Request timeout after 30 seconds")
    assert "TRANSIENT" in extractor._classify_error(timeout_error)

    rate_limit_error = Exception("429 Rate limit exceeded")
    assert "TRANSIENT" in extractor._classify_error(rate_limit_error)

    network_error = Exception("Network connection reset by peer")
    assert "TRANSIENT" in extractor._classify_error(network_error)

    deadline_error = Exception("Deadline exceeded for operation")
    assert "TRANSIENT" in extractor._classify_error(deadline_error)


def test_error_classification_permanent() -> None:
    ...
    assert "PERMANENT" in extractor._classify_error(auth_error)

    permission_error = Exception("403 Permission denied")
    assert "PERMANENT" in extractor._classify_error(permission_error)

    not_found_error = Exception("404 Resource not found")
    assert "PERMANENT" in extractor._classify_error(not_found_error)

    invalid_arg_error = Exception("400 Bad request: Invalid argument")
    assert "PERMANENT" in extractor._classify_error(invalid_arg_error)


def test_error_classification_unknown() -> None:
    ...
    classification = extractor._classify_error(unknown_error)
    assert "UNKNOWN" in classification
    assert "Exception" in classification


def test_fail_fast_mode_enabled() -> None:
    ...
    # Mock lineage client that raises an error
    mock_client = MagicMock()
    mock_client.search_links.side_effect = Exception("API Error")
    ...
    # Should raise RuntimeError with fail_fast enabled
...
def test_fail_fast_mode_disabled() -> None:
    ...
    assert result is None
    assert report.num_lineage_entries_failed == 1
...
def test_circuit_breaker_triggers() -> None:
    ...
    # Mock lineage client that always fails
    mock_client = MagicMock()
    mock_client.search_links.side_effect = Exception("API Error")
    ...
    # First 3 failures should return None
    for i in range(3):
        result = extractor.get_lineage_for_entity("test-project", entity)
        assert result is None
        assert extractor.consecutive_failures == i + 1

    # Circuit breaker should now be triggered
    assert extractor.circuit_breaker_triggered is True
...
def test_circuit_breaker_resets_on_success() -> None:
    ...
    mock_link = MagicMock()
    mock_link.source.fully_qualified_name = (
        "bigquery:test-project.dataset.upstream_table"
    )
    mock_client.search_links.side_effect = None
...
def test_circuit_breaker_disabled() -> None:
    ...
    # Should be able to fail many times without circuit breaker triggering
    for _ in range(20):
        result = extractor.get_lineage_for_entity("test-project", entity)
        assert result is None

    # Circuit breaker should not be triggered
    assert extractor.circuit_breaker_triggered is False


def test_circuit_breaker_with_fail_fast() -> None:
    ...
    # First failure should raise immediately due to fail_fast
    with pytest.raises(RuntimeError) as exc_info:
        extractor.get_lineage_for_entity("test-project", entity)
