...
class TestSnowflakeTasksExtractor:
    ...
    def test_predecessor_filtered_by_pattern_emits_no_edge(self) -> None:
        """A predecessor task_pattern excluded has no DataJob to point at, so the
        edge is dropped and reported rather than left dangling — the same
        guarantee the CALL path makes for filtered-out procedures."""
        config = _make_config()
        config.task_pattern.deny = [".*TASK_A.*"]
        task_a = _make_task(name="task_a")
        task_b = _make_task(name="task_b", predecessors=["task_a"])

        wus, report = _collect_workunits([task_a, task_b], config=config)

        # Only task_b is emitted, and it carries no lineage aspect at all.
        assert report.tasks_scanned == 1
        assert _data_job_input_outputs(wus) == []
    ...
    def test_execute_immediate_yields_no_datajob_edge(self) -> None:
        ...
        The fixture deliberately contains a procedure named IMMEDIATE: without
        the guard the parser reads that keyword as the procedure name and this
        resolves to a real edge, which is the regression being pinned.
        """
        task = _make_task(
            name="dynamic_task",
        ...
        wus, report = _collect_workunits(
            [task], procedures=[_make_procedure("IMMEDIATE", "(stmt VARCHAR)")]
        )
    ...
    def test_arity_mismatch_on_lone_procedure_still_resolves(self) -> None:
        """Arity disambiguates overloads; it can't refute a lone match. Dropping
        the only procedure of that name because the counts disagree would lose
        real lineage every time a signature we can't fully read (defaults,
        variadic-looking type spellings) meets a valid call."""
        config = _make_config()
        procedure = _make_procedure("my_proc", "(arg1 VARCHAR)")
    ...
    def test_omitted_defaulted_arguments_pick_the_right_overload(self) -> None:
        """Snowflake lets a caller omit parameters declared DEFAULT, so a
        signature accepts a range of argument counts, not one exact count."""
        config = _make_config()
        one_arg = _make_procedure("my_procedure", "(arg1 VARCHAR)")
        defaulted = _make_procedure(
            "my_procedure", "(arg1 VARCHAR, arg2 VARCHAR DEFAULT 'x')"
        )
        # 2 args given: only the DEFAULT-bearing overload accepts that count.
        wus, report = _collect_workunits(
            [_make_task(name="proc_task", definition="CALL my_procedure('a', 'b')")],
            config=config,
            procedures=[one_arg, defaulted],
        )
        assert _data_job_input_outputs(wus)[0].inputDatajobs == [
            _procedure_urn(config, report, defaulted)
        ]

        # 1 arg given: both accept it, so it stays ambiguous.
        wus, report = _collect_workunits(
            [_make_task(name="proc_task", definition="CALL my_procedure('a')")],
            config=config,
            procedures=[one_arg, defaulted],
        )
        assert _data_job_input_outputs(wus) == []
        assert any(
            "Ambiguous Procedure Call" in (w.title or "") for w in report.warnings
        )

    def test_comma_inside_a_default_literal_does_not_inflate_arity(self) -> None:
        """A default value may itself contain a comma. Counting it as a separator
        makes the signature look wider than it is and picks the wrong overload."""
        config = _make_config()
        separator_default = _make_procedure(
            "my_procedure", "(arg1 VARCHAR DEFAULT ',', arg2 INT)"
        )
        three_arg = _make_procedure(
            "my_procedure", "(arg1 VARCHAR, arg2 INT, arg3 INT)"
        )
        wus, report = _collect_workunits(
            [_make_task(name="proc_task", definition="CALL my_procedure(1)")],
            config=config,
            procedures=[separator_default, three_arg],
        )

        assert _data_job_input_outputs(wus)[0].inputDatajobs == [
            _procedure_urn(config, report, separator_default)
        ]
        assert not report.warnings

    def test_commas_inside_a_type_do_not_inflate_arity(self) -> None:
        """``NUMBER(10,2)`` is one parameter. Counting it as two picks the wrong
        overload."""
        config = _make_config()
        one_arg = _make_procedure("my_procedure", "(arg1 NUMBER(10,2))")
        two_arg = _make_procedure("my_procedure", "(arg1 NUMBER(10,2), arg2 VARCHAR)")
        wus, report = _collect_workunits(
            [_make_task(name="proc_task", definition="CALL my_procedure(1.5)")],
            config=config,
            procedures=[one_arg, two_arg],
        )

        assert _data_job_input_outputs(wus)[0].inputDatajobs == [
            _procedure_urn(config, report, one_arg)
        ]
        assert not report.warnings

    def test_call_to_filtered_out_procedure_emits_no_edge(self) -> None:
        """procedure_pattern is applied in the schema-generation loop, not in the
        data dictionary, so a procedure Snowflake listed may never have been
        emitted. Pointing at it would dangle."""
        config = _make_config()
        config.procedure_pattern = AllowDenyPattern(deny=[".*my_proc.*"])
        task = _make_task(name="proc_task", definition="CALL my_proc('a')")
        wus, report = _collect_workunits(
            [task],
            config=config,
    ...
    def test_call_into_filtered_out_schema_emits_no_edge(self) -> None:
        """schema_pattern is applied in the schema-generation loop, so a
        procedure in a schema this run skipped was never emitted. Honour
        match_fully_qualified_names the way the rest of the connector does."""
        config = _make_config()
        config.match_fully_qualified_names = True
        config.schema_pattern = AllowDenyPattern(deny=["TEST_DB.PUBLIC"])
        task = _make_task(name="proc_task", definition="CALL my_proc('a')")
        wus, _ = _collect_workunits(
            [task],
            config=config,
    ...
    def test_procedure_lookup_failure_is_reported(self) -> None:
        """A failed procedure listing silently drops every CALL edge in the
        schema; operators need to see why."""
        task = _make_task(name="proc_task", definition="CALL my_proc('a')")
        config = _make_config()
        report = SnowflakeV2Report()
        identifiers = SnowflakeIdentifierBuilder(
        ...
        data_dict = MagicMock()
        data_dict.get_tasks_for_schema.return_value = [task]
        data_dict.get_procedures_for_database.side_effect = RuntimeError("no grants")
        ...
        assert _data_job_input_outputs(wus) == []
        assert any(
            "Procedure Lookup Failed" in (w.title or "") for w in report.warnings
        )

    def test_call_resolves_to_catalog_casing_without_lowercasing(self) -> None:
        """With convert_urns_to_lowercase disabled the urn keeps the source
        casing, so it has to come from the catalog rather than from whatever the
        task author typed in the CALL."""
        config = SnowflakeV2Config(
            account_id="test_account",
            username="user",
            password="pass",  # type: ignore
        ...
        procedure = _make_procedure("MY_PROC", "(arg1 VARCHAR)")
        task = _make_task(
            name="proc_task", definition="CALL test_db.public.my_proc('a')"
        )
        wus, report = _collect_workunits([task], config=config, procedures=[procedure])
