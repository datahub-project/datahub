    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ...
                (
                    "("
                    "ARRAY_SIZE(FILTER(direct_objects_accessed, o -> "
                    "("
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) RLIKE 'PROD_.*' OR "
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) RLIKE 'DEV_.*'"
                    ")"
                    ")) > 0 OR "
                    "ARRAY_SIZE(FILTER(objects_modified, o -> "
                    "("
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) RLIKE 'PROD_.*' OR "
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) RLIKE 'DEV_.*'"
                    ")"
                    ")) > 0 OR "
                    "(("
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) RLIKE 'PROD_.*' OR "
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) RLIKE 'DEV_.*'"
                    "))"
                    ")"
                )
                .replace("    ", "")
            ...
            pytest.param(
                ...
                (
                    ...
                    "ARRAY_SIZE(FILTER(direct_objects_accessed, o -> "
                    "(("
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) NOT RLIKE '.*_TEMP' AND "
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) NOT RLIKE '.*_STAGING'"
                    "))"
                    ")) > 0 OR "
                    "ARRAY_SIZE(FILTER(objects_modified, o -> "
                    "(("
                    ...
                    ")) > 0 OR "
                    "((("
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) NOT RLIKE '.*_TEMP' AND "
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) NOT RLIKE '.*_STAGING'"
                    ")))"
                    ")"
                )
                .replace("    ", "")
            ...
            pytest.param(
                ...
                (
                    ...
                    "ARRAY_SIZE(FILTER(direct_objects_accessed, o -> "
                    "("
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) = 'DB1' OR "
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) = 'DB2'"
                    ")"
                    ")) > 0 OR "
                    "ARRAY_SIZE(FILTER(objects_modified, o -> "
                    "("
                    ...
                    ")) > 0 OR "
                    "(("
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) = 'DB1' OR "
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) = 'DB2'"
                    "))"
                    ")"
                )
                .replace("    ", "")
            ...
            pytest.param(
                ...
                (
                    ...
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) = 'DB2'"
                    ") OR "
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) RLIKE 'PROD_.*'"
                    ")) > 0 OR "
                    "ARRAY_SIZE(FILTER(objects_modified, o -> "
                    "("
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) = 'DB1' OR "
                    ...
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) = 'DB2'"
                    ") OR "
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) RLIKE 'PROD_.*')"
                    ")"
                )
                .replace("    ", "")
                .replace("\n", ""),
            ...
            pytest.param(
                ...
                (
                    "("
                    "ARRAY_SIZE(FILTER(direct_objects_accessed, o -> "
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) = 'SPECIAL_DB' OR "
                    "(("
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) RLIKE 'PROD_.*' OR "
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) RLIKE 'DEV_.*'"
                    ") AND "
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) NOT RLIKE '.*_TEMP')"
                    ")) > 0 OR "
                    "ARRAY_SIZE(FILTER(objects_modified, o -> "
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) = 'SPECIAL_DB' OR "
                    "(("
                    ...
                    ")) > 0 OR "
                    "("
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) = 'SPECIAL_DB' OR "
                    "(("
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) RLIKE 'PROD_.*' OR "
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) RLIKE 'DEV_.*'"
                    ") AND "
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) NOT RLIKE '.*_TEMP')"
                    ")"
                    ")"
                )
                .replace("    ", "")
            ...
            pytest.param(
                ...
                (
                    ...
                    "ARRAY_SIZE(FILTER(direct_objects_accessed, o -> "
                    "("
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) = 'DB''1' OR "
                    "SPLIT_PART(UPPER(o:objectName), '.', 1) = 'DB''2'"
                    ")"
                    ")) > 0 OR "
                    "ARRAY_SIZE(FILTER(objects_modified, o -> "
                    "("
                    ...
                    ")) > 0 OR "
                    "(("
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) = 'DB''1' OR "
                    "SPLIT_PART(UPPER(object_modified_by_ddl:objectName), '.', 1) = 'DB''2'"
                    "))"
                    ")"
                )
                .replace("    ", "")
