...
def test_allowed_allowed_tables(s3_uri: str, allowed_tables: List[str], expected: bool) -> None:
    # arrange
    path_spec = PathSpec(
...
def test_dir_allowed_allowed_tables(s3_uri: str, allowed_tables: List[str], expected: bool) -> None:
    # arrange
    path_spec = PathSpec(
        include="s3://bucket/{table}/{partition0}/*.csv",
        allowed_tables=allowed_tables,
...
@pytest.mark.parametrize(
    "include, s3_uri, expected",
    [
        ("s3://bucket/{table}/{partition0}/*.csv", "s3://bucket/table/p1/test.csv", "table"),
        ("s3://bucket/data1/{partition0}/test.csv", "s3://bucket/data1/p1/test.csv", None),
    ],
)
...
def test_get_table_name_raises_error(s3_uri: str) -> None:
    # arrange
    path_spec = PathSpec(
        include="s3://bucket/dir1/{table}/{partition0}/*.csv"
    )

    # act, assert
    with pytest.raises(ValueError) as e:
        path_spec._get_table_name(s3_uri)
    assert str(e.value) == f"Unable to parse table name from path: {s3_uri}"
...
def test_is_allowed_tables_returns_expected(table_name: str, allowed_tables: str, expected: bool) -> None:
    # arrange
    path_spec = PathSpec(
        include="s3://bucket/{table}/{partition0}/*.csv",
        allowed_tables=allowed_tables,
