...
3. The API client must have read access to `/v2/connections` so the connector can
   ...
   this requirement may need to be regenerated or have their scope expanded; if
   the call fails the ingest continues with an empty registry and a
   `Sigma Connection registry build failed` warning is recorded in the run report.
