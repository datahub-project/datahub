...
- **Cubes and views** as datasets, grouped under a container representing the deployment. Cubes use subtype `Cube`; views use subtype `Semantic Model` (the same subtype as dbt semantic-layer models). The container links back to the deployment UI (derived from `api_url`, or set `deployment_url`).
