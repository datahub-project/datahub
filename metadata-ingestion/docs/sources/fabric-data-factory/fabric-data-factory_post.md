...
- **Column-level lineage scope**: Only Copy activities produce column-level lineage. Default (by-name) mappings require both source and destination schemas to be known, except for auto-created destinations, which require only the source schema (see [Column-Level Lineage](#column-level-lineage)); dynamic (expression-based) mappings are not extracted. Hierarchical (JSON) `path` mappings are emitted as dotted column paths and may not match nested field paths ingested by other connectors.
