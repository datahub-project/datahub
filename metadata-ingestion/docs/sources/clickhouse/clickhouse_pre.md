...
Yes — `GRANT SELECT ON <database>.*` grants read access to the actual table data. Asset and structural-lineage extraction themselves only read ClickHouse metadata (`system.tables` / `system.columns` / view definitions). Profiling is the capability that runs live `SELECT`s against your tables.
