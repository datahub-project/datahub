...
- **Pub/Sub API** (`pubsub.googleapis.com`) — required only for resolving Pub/Sub subscription lineage upstreams to their backing topic (`resolve_pubsub_subscriptions: true`)
- **BigQuery API** (`bigquery.googleapis.com`) — required only for fetching operation SQL from the BigQuery Jobs API (`include_lineage_operation_sql: true`)
...
| Pub/Sub subscription resolution (`resolve_pubsub_subscriptions: true`) | [`roles/pubsub.viewer`](https://cloud.google.com/pubsub/docs/access-control#roles) on each subscription's project — provides `pubsub.subscriptions.get`. Also requires the [Pub/Sub API](https://console.cloud.google.com/apis/library/pubsub.googleapis.com) to be enabled on those projects                                       |
| Operation SQL enrichment (`include_lineage_operation_sql: true`)       | [`roles/bigquery.resourceViewer`](https://cloud.google.com/bigquery/docs/access-control#bigquery.resourceViewer) on the projects where the BigQuery jobs ran — jobs are private to their creator by default                                                                                                                         |
