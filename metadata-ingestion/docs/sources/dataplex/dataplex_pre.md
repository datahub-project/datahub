...
At least one of these must be set. Auto-discovery via `project_labels` or `project_id_pattern` requires the service account to have `roles/resourcemanager.projectViewer` (or equivalent) on the parent organization or folder; when `project_ids` is set explicitly, no Resource Manager permissions are needed.
...
| Project auto-discovery via `project_id_pattern` or `project_labels`  | [`roles/resourcemanager.projectViewer`](https://cloud.google.com/resource-manager/docs/access-control-proj) on the parent organization/folder — to list projects via the API |
