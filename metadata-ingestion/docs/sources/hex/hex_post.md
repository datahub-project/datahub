...
By default the Hex source mirrors the SQL-cell path: case-insensitive platforms (snowflake, postgres, redshift, …) are lowercased automatically and case-sensitive platforms (bigquery, db2) preserve author case. If the upstream warehouse was ingested with a non-default `convert_urns_to_lowercase` setting, set `convert_urns_to_lowercase` on the matching entry in `connection_platform_map` to override the platform default. Note this only rewrites the lineage edge's destination URN; it does not affect the SQL-cell parsing path, and it does not lowercase `platform_instance` — if the warehouse was ingested with a mixed-case `platform_instance` and `convert_urns_to_lowercase: true`, the instance segment must already be lowercase (or unset) for the edge to resolve, since `platform_instance` is preserved as-is by the Hex source.
