...
Casing is chosen by the shared schema resolver. When the upstream table already exists in DataHub, the resolver matches the casing it actually landed with — so ingesting the warehouse before Hex is the most reliable way to get lineage to line up. On a miss it falls back to a per-platform default: lowercase for case-insensitive platforms (snowflake, postgres, redshift, mssql, …) and case-preserving for bigquery (and, in the resolver's classification, db2). Note that only the Snowflake source defaults to `convert_urns_to_lowercase: true`; postgres, redshift and mssql default to preserving case, so if one of those warehouses was ingested with the default and its tables are not yet in DataHub, the fallback can guess wrong. The resolver also folds BigQuery date-shard suffixes (`events_20240101` → `events_yyyymmdd`) so partitioned tables line up.

Platforms without a `sqlglot` dialect — db2, vertica, synapse, greenplum, impala, netezza, cockroachdb — are skipped at parse time in the `queriedTables` path with `unparseable_table_name` (surfaced under `skipped_cells` with the underlying exception in `detail`), so the resolver's casing rule for those platforms is not reached in practice.
