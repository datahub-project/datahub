...
- **Two-tier vs three-tier.** Oracle defaults to a two-tier (`schema.table`) URN. If you ingest Oracle with `add_database_name_to_urn: true` (three-tier `database.schema.table`), set `connector_database` for that catalog in `catalog_to_connector_details` so the generated URN matches. For Oracle, an all-uppercase `connector_database` is lowercased to match the identifier normalization the `oracle` source applies, so you can copy the value straight from Oracle (e.g. `ORCLPDB1`); a mixed-case value is taken as a quoted identifier and left as-is.
