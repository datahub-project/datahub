...
4. **Lineage**: View definitions are passed to the SQL parsing aggregator to derive table- and column-level view → upstream lineage. Unqualified and `schema.table` references resolve to the view's own item; `item.schema.table` references resolve to other items of the same workspace — see [Cross-Item Lineage](#cross-item-lineage).

#### Cross-Item Lineage

Fabric SQL lets a view or query reference tables in other Lakehouses / Warehouses of the **same workspace** by **display name**, using 3-part `item.schema.table` names ([cross-database queries](https://learn.microsoft.com/en-us/fabric/data-warehouse/query-warehouse#write-a-cross-database-query)), while DataHub dataset URNs are keyed by GUIDs (`<workspaceId>.<itemId>.<schema>.<table>`). The connector translates display names to GUIDs so lineage lands on the real datasets, for both view definitions and observed queries from `queryinsights` (e.g. Warehouse `INSERT ... SELECT` or `CREATE TABLE ... AS SELECT` that read a Lakehouse):
...
- Item names are matched case-insensitively, with or without `[...]` / `"..."` quoting. Item GUIDs are accepted in place of names.
- Fabric has no cross-workspace table names: the SQL endpoint rejects a 4-part `workspace.item.schema.table` name (it is interpreted as a linked server). Names with 4 or more parts are therefore always treated as unresolved (see below).
- A workspace's Lakehouses / Warehouses are all listed and indexed before any of its items is processed. Items excluded by `lakehouse_pattern` / `warehouse_pattern` are still indexed, so references to them resolve to their (GUID-based) URNs. Item types turned off with `extract_lakehouses: false` / `extract_warehouses: false` are not listed, so references to them are not resolved.
- Column-level lineage is produced for cross-item references. Column metadata of ingested tables and views is registered with the SQL parser, so `SELECT *` expands and lineage confidence is high when the referenced item's columns were extracted. `queryinsights` queries are parsed after every item's tables and views are registered, so this does not depend on the order in which items are processed.
...
A reference that cannot be resolved — the item is not ingested, does not exist, its name is ambiguous (e.g. two items with the same display name in one workspace), listing the workspace's items failed, or the name has four or more parts — is **skipped** instead of producing a dangling URN: it is removed from lineage, usage, operations, and query subjects, and an `Unresolved Cross-Item SQL Reference` warning with the reference, workspace, and reason is added to the ingestion report. The report also counts `num_cross_item_references_resolved` / `num_cross_item_references_unresolved` (distinct references), `num_lineage_upstreams_dropped_unresolved` (upstream edges removed), and `num_lineage_aspects_dropped_unresolved` (lineage aspects dropped because every upstream was unresolved).
...
| `procedural_statement` | T-SQL control-of-flow and session statements (`SET`, `IF`, `DECLARE`, `WHILE`, `EXEC`, `BEGIN TRANSACTION`, `COMMIT`, ...), such as the bodies of the ODBC driver's `sp_*` catalog procedures (`set @ODBCVer = 3`). They carry no table lineage and cannot be parsed. A row that also contains `INSERT` / `UPDATE` / `DELETE` / `MERGE` / `CREATE TABLE` / `SELECT ... INTO` (e.g. `IF OBJECT_ID(...) IS NOT NULL DROP TABLE ...; CREATE TABLE ... AS SELECT ...`) is parsed instead. |
...
- **Cross-Item References Are Same-Workspace Only**: `item.schema.table` references resolve only to Lakehouses / Warehouses of the referencing item's workspace, which is how Fabric scopes cross-database queries. References to other item types (e.g. SQL databases, mirrored databases) and 4-part names are not resolved and are reported as warnings. A table that is a OneLake shortcut resolves to the shortcut table in the referencing item, not to the shortcut's target. With `platform_instance`, all resolved URNs use the recipe's platform instance.
- **No Spark / Notebook Lineage**: Lineage comes from view definitions and Warehouse queries (`queryinsights`). Tables written by Fabric notebooks or Spark job definitions get no lineage from this source; send the Spark runtime's OpenLineage events to DataHub to get it (table- and column-level, on the same `fabric-onelake` URNs this source emits): see [Microsoft Fabric (OneLake) in the OpenLineage docs](https://docs.datahub.com/docs/lineage/openlineage#microsoft-fabric-onelake).
- **Usage Statistics Latency**: Fabric populates `queryinsights` asynchronously: [completed queries can take up to 15 minutes to appear](https://learn.microsoft.com/en-us/fabric/data-warehouse/query-insights#where-can-you-see-query-insights), longer under heavy concurrent load. A query that completes shortly before a run may not be visible to that run yet. With stateful ingestion, the next run's usage window starts where the previous one ended, so such late-arriving queries can be missed. When validating lineage or usage for a query you just ran, wait several minutes before running ingestion.
