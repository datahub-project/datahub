...
| Oracle     | `schema.table`          | `schema.name` / `db.schema` if set, otherwise `connection.user` (the connecting Oracle user = owner) |
| Snowflake  | `database.schema.table` | From `schema.name` connector config                    |

**Oracle note**: The Oracle source connector supports `add_database_name_to_urn: true`, which changes URNs from `schema.table` to `database.schema.table`. If your Oracle datasets were ingested with this option enabled, the Kafka Connect lineage will not resolve unless you override the target dataset URNs using `connect_to_platform_map`.
...
**Overriding URNs with `connect_to_platform_map`**: If the auto-detected URN does not match your database ingestion, you can force a specific platform instance:
...
**Overriding the schema**: If the connector writes to a non-default schema, set `schema.name` in the connector configuration:
