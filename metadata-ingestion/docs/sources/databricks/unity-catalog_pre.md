...
- To `include_usage_statistics` (enabled by default), your service principal must have one of the following:
  ...
  - Starting August 26, 2026, the principal must also be an account admin or a member of the account-level `databricks_pii_access` group to read unmasked query text. Create this group with the exact case-sensitive name and add the DataHub ingestion principal while retaining the permissions above. Workspace admins are not included automatically. Without this access, Databricks returns `<REDACTED>` and DataHub skips affected queries, resulting in incomplete usage statistics, operational statistics, and Query entities. Native table and column lineage from `system.access.*` remains unaffected. See [Manage account-level groups](https://docs.databricks.com/aws/en/admin/users-groups/groups).
