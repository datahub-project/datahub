...
- If the ingestion report contains **Databricks query text is redacted**, Databricks returned `<REDACTED>` instead of SQL text. Query entities, column-level usage statistics, operational statistics, and the top-SQL sample are incomplete for those queries. Table-level usage statistics are preserved only on the default system-tables path (`usage_data_source: AUTO` with `warehouse_id` set, or `SYSTEM_TABLES`) with `include_column_usage_stats: false`; on the REST API path or when `include_column_usage_stats: true`, redacted queries are dropped entirely. Create the account-level group `databricks_pii_access` and add the ingestion principal to it, while retaining its existing system-table or Query History API permissions. You can verify access by running the following query as the ingestion principal:
