...
| Volume (managed or external)              | Container (VOLUME)                | Named Unity Catalog storage location, modeled as a container nested under its Schema. Description, owner, type, and storage path are extracted. Files inside the volume are not ingested unless `include_volume_files` is enabled. |
| Volume subfolder                          | Container (FOLDER)                | Emitted only when `include_volume_files` is enabled; intermediate directories in a volume become folder containers nested under the volume.                                                                                        |
| Volume file                               | Dataset (VOLUME FILE)             | Opt-in via `include_volume_files`. Listed through the Databricks Files API and nested under its volume/folder container. Schema is inferred when `include_volume_file_schemas` is enabled.                                         |
