...
| Volume (managed or external)              | Dataset (VOLUME)                  | Named Unity Catalog storage location. Description, owner, type, and storage path are extracted. Nested under its Schema container like tables. Files inside the volume are not ingested unless `include_volume_files` is enabled. |
| Volume file                               | Dataset (VOLUME FILE)             | Opt-in via `include_volume_files`. Listed through the Databricks Files API and nested under the same Schema container as tables and views.                                                                                      |
