...
The connector builds its browse hierarchy by walking each object's MicroStrategy folder ancestry, which reflects the metadata layer's internal folder names rather than the curated grouping Strategy Web's Library shows (favorites, "My Reports" vs. "Shared Reports", hidden system folders). One specific mismatch is corrected automatically: the folder Strategy Web calls "Shared Reports" is internally named "Reports" (nested under "Public Objects"). With `use_predefined_folder_names: true` (the default), the connector resolves that folder's MicroStrategy-assigned label via `GET /api/folders/preDefined` and uses it everywhere — for the container's identity (URN), for `folder_pattern` matching, and for the displayed name — instead of the raw metadata name. Set `use_predefined_folder_names: false` to keep the raw name (for example if you already ingested that folder and want to avoid the resulting container URN change; see `docs/how/updating-datahub.md`).
