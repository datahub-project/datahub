...
Lineage is emitted from datasets to visualizations through `ChartInfo.inputs` and `ChartInfo.inputEdges` when the connector can resolve dataset bindings from static definitions or runtime visualization details. When visualization metadata exposes metric and attribute references, the connector also emits chart `InputFields` for the dataset fields used by each visualization. Direct dashboard dataset edges are disabled by default because large BI dashboards can reference many datasets and make lineage graphs noisy; set `emit_dashboard_dataset_edges: true` only when dashboard-level fallback lineage is preferred. Coarse table-level warehouse lineage from SQL-view APIs is also disabled by default; set `extract_warehouse_lineage: true` only if you want MicroStrategy dataset -> warehouse table edges before field-level metric, attribute, or fact lineage is available.
