...
Install the connector with its extra, `pip install 'acryl-datahub[microstrategy]'`, so the SQL parsing dependencies used by SQL-view lineage are present. When `extract_warehouse_lineage` or `extract_report_sql_lineage` is enabled, the connector checks for them at startup and fails the run with a message naming the extra rather than silently emitting no lineage.
