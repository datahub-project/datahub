...
- DRF lineage requires a generic table-reader service for the DRF customizing tables and assumes the replicated object keeps its name on the target platform (override casing/instance/env per system via `logical_system_to_platform`). Column-level lineage is limited to fields whose names match between MDG and the downstream dataset.
