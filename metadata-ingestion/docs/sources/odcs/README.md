...
| `quality[]` rule (library `notNull` with a column)              | `FieldAssertionInfo` + `FieldValuesAssertion` (`NOT_NULL`)                 | **Only when a physical binding resolves.** Targets the physical dataset.                                                                                          |
| `quality[]` rule (library `unique` with a column)               | `FieldAssertionInfo` + `FieldMetricAssertion` (`UNIQUE_PERCENTAGE == 100`) | Uniqueness is a column metric, not a per-row value check. Only when a physical binding resolves.                                                                  |
