...
def initialize_secret_masking(
    max_message_size: int = 5000, force: bool = False
) -> None:
    ...
    ``force`` is accepted and ignored: masking is now process-lifetime, so there
    is nothing to force. It is kept on this release line because the separately
    released executor package still passes ``force=True``.
    """
    del force
