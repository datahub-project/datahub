...
# INSERT is worse: INSERT splits on its own, leaving BULK attached to the call. Both
# predate this rule. A statement label (`retry:`) is a third gap: `:` is a continuation,
# so a label between two statements still swallows the call after it.
...
class _TokenSplitter:
    ...
    def _exec_target_follows(self, i: int) -> bool:
        ...
        Separates `EXEC proc` from the forms this rule cannot resolve to a name:
        `EXEC(@sql)`, which names no callee at all, and `EXEC @rc = proc`, which does
        call `proc` -- that one is a known gap, not a form to leave unsplit by choice,
        and sqlglot 30.12 cannot parse it even when it is split out. A column named
        `exec` needs no guard: the word is reserved in T-SQL, so it only names a column
        when bracketed, and `[exec]` tokenizes as an IDENTIFIER that never reaches here.
        """
    ...
    def _classify_new_statement(
        ...
        # continuations; a privilege list must not.
        #
        # `DENY EXECUTE ON x TO y` is safe on its own because ON is not a target token,
        # but `DENY EXECUTE TO y` has no ON and `TO` tokenizes as VAR, so without the
        # privilege-verb check it splits and sqlglot reads `EXECUTE TO y` as a call to a
        # procedure named TO -- inventing lineage, which is worse than missing it. DENY
        # is the example because it is the only one of the three this check catches:
        # GRANT/REVOKE are in _CONTINUATION_TEXT, which already fails the test below.
        if (
            tt == TokenType.EXECUTE
            and self.is_tsql
            and self._exec_target_follows(i)
            and self.prev_text not in _PRIVILEGE_VERBS
            and (
