...
class _ColumnResolver:
    ...
    # Whether to restore the schema's column casing when rendering column_logic.
    # Scoped to the dialects whose fold this change introduces; see the note in
    # render_column_logic for why the others are left alone.
    restore_logic_col_case: bool

    # Folded spelling -> the schema's real spelling, for rendering column_logic.
    # Flattened across every table in the statement, since a rendered expression
    # does not carry which table each column came from. A folded name whose tables
    # disagree on casing is left out rather than guessed at.
    _real_col_case: Dict[str, str] = dataclasses.field(init=False)

    def __post_init__(self) -> None:
        real_case: Dict[str, str] = {}
        ambiguous: Set[str] = set()
        for table_mapping in self.table_schema_normalized_mapping.values():
            for folded, real in table_mapping.items():
        ...
        self._real_col_case = real_case

    def render_column_logic(
        self, expression: sqlglot.exp.Expr, dialect: sqlglot.Dialect
    ) -> str:
        ...
        spelling while the schemaField URNs on the same edge carry the schema's real
        spelling. Restore the real spelling so an edge does not describe itself with a
        column name that differs from the URN beside it.
        ...
        """
        if not self.restore_logic_col_case or not self._real_col_case:
        ...
        for column in rendered.find_all(sqlglot.exp.Column):
            ...
            if not isinstance(identifier, sqlglot.exp.Identifier):
                continue
            real = self._real_col_case.get(identifier.name)
            if real is not None:
...
def _prepare_query_columns(
    ...
    if not is_create_ddl:
        ...
        try:
            ...
            if not isinstance(optimized, sqlglot.exp.Expression):
                raise SqlUnderstandingError(
                    f"Expected Expression from optimizer, got {type(optimized)}"
                )
            statement = optimized
            ...
            column_lineage.append(
                _ColumnLineageInfo(
                    ...
                    logic=_get_column_transformation(
                        lineage_node, dialect, column_resolver
                    ),
                )
            )
...
def _get_column_transformation(
    ...
    if not lineage_node.downstream:
        ...
        return ColumnTransformation(
            ...
            column_logic=column_resolver.render_column_logic(expression, dialect),
        )
        ...
        return ColumnTransformation(
            ...
            column_logic=column_resolver.render_column_logic(
                lineage_node.expression, dialect
            ),
        )
...
def _try_extract_select(
    ...
    if isinstance(statement, sqlglot.exp.Subquery):
        unnested = statement.unnest()
        if not isinstance(unnested, sqlglot.exp.Expression):
            raise TypeError(f"Expected Expression from unnest(), got {type(unnested)}")
        statement = unnested
...
def _sqlglot_lineage_inner(
    ...
    if not isinstance(qualified, sqlglot.exp.Expression):
        raise TypeError(f"Expected Expression from qualify(), got {type(qualified)}")
    statement = qualified
