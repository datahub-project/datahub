...
class BracketRef:
    ...
    compatibility, but resolvers should consider ``segments``: Sigma appears to
    write join-chain refs as ``[JoinElement/SourceElement/Column]``, where the
    owning element is the segment before the column rather than the first.

    That reading is INFERRED from references resolving successfully against
    real tenant data, not from Sigma documentation, which only describes the
    two-part ``[Table/Column]`` form. It also assumes Sigma escapes ``/``
    inside a name; an unescaped one would make the reading wrong for that ref.
    Resolvers must therefore treat a segment split as a candidate to validate,
    never as a fact. Only the scanner may
