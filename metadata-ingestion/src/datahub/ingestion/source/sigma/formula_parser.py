...
| [E1/E2/]                | source "E1", column "E2/" (unchanged)                     | emit; segments ('E1','E2','')           | column non-empty, so the skip rule does not fire; LAST segment empty       |
| [a//]                   | source "a", column "/" (unchanged)                        | emit; segments ('a','','')              | TWO empty segments                                                         |
| [a//b]                  | source "a", column "/b" (unchanged)                       | emit; segments ('a','','b')             | consecutive separators; MIDDLE segment empty                               |
| [a/ /b]                 | source "a", column "/b" (unchanged)                       | emit; segments ('a','','b')             | whitespace-only segment is empty after strip                               |
...
class BracketRef:
    ...
    e.g. ``[TRIP/Start Station Details/Dock Count]`` -- ``TRIP`` is the element
    the calculated column was added to and ``Start Station Details`` is a
    relationship in the data model.

    **The reference does not name the column's owner.** ``Dock Count`` lives in
    the table the relationship points AT, which appears in none of the
    segments. Mapping ``segments[0]`` to an upstream therefore emits WRONG
    lineage for the documented shape, and matching the middle segment against
    element names is unsound too, since relationships are renamed
    ...
    def __post_init__(self) -> None:
        # Coerce BEFORE testing emptiness. A list would make __hash__ raise
        # TypeError -- the failure the tuple annotation exists to prevent --
        # and mypy catches typed callers but tests and untyped code would not.
        # Order matters: an exhausted iterator is TRUTHY, so testing emptiness
        # first sends it past the default derivation and into segments[0],
        # which raises IndexError instead of deriving the default.
        ...
        # rejoin to "b/c" and both pass.
        #
        # The plain `!=` short-circuits nearly every call: string equality
        # implies normalised equality, so the regex pair only runs for the
        # whitespace-around-separator case. `column` is stripped as a whole
        # while segments are stripped one by one, so "[ a / b / c ]" gives
        # column="b / c" against segments ("a", "b", "c") -- equal only after
        # normalising. (str.strip() and regex \s agree on every codepoint, so
        # the two strip regimes really are comparable.)
        joined = "/".join(self.segments[1:])
        if (
            self.column is not None
            and self.column != joined
            and _normalize_separators(self.column) != _normalize_separators(joined)
        ):
            raise ValueError(
                f"column does not match segments for {self.raw!r}: "
                f"column={self.column!r} segments={self.segments!r}. "
                f"Pass segments= as well, or rebuild with BracketRef.from_body()."
            )

    @staticmethod
    def from_body(body: str) -> BracketRef:
        """Build a ref from a bracket BODY by running the scanner on it.

        The four-argument form derives ``segments`` from the first-slash split,
        so ``BracketRef("[a/b/c]", "a", "b/c", False)`` yields ``("a", "b/c")``
        -- a two-segment ref describing a three-segment formula. A resolver
        branching on ``len(segments)`` then takes the wrong path, and nothing
        on the object reveals it. Callers holding the original text should
        build through here, so ``segments`` always means the same thing.

        Raises ``ValueError`` for anything that is not exactly one reference.
        That includes bodies the emit/skip rule drops (``""``, ``" "``,
        ``"a/"``, ``"/a"``), which the four-argument form would happily
        construct -- callers migrating from it should expect the stricter
        contract.

        Not a ``classmethod``: it returns a ``BracketRef`` whatever it is
        called on, so binding ``cls`` would be a lie.
        """
        refs = extract_bracket_refs(f"[{body}]")
        # `raw` is the slice the scanner actually consumed, so comparing it to
        # the whole input is what catches a body that closes the bracket early:
        # "a]b" parses as one valid ref, [a], and silently discards "]b". The
        # count check alone passes there, which would make the constructor
        # advertised as the safe path the only one that fails quietly.
        if len(refs) != 1 or refs[0].raw != f"[{body}]":
            raise ValueError(f"not a single bracket reference: {body!r}")
        return refs[0]
        ...
        if state == _NORMAL:
            ...
            if ch == "]":
                ...
                if source and (column is None or column):
                    # No guard around the constructor. A malformed FORMULA is
                    # expected input and is skipped below; a scanner-built ref
                    # that fails the invariants is a BUG IN THIS FILE, and the
                    # two deserve opposite treatment. Swallowing the second
                    # into a warning would hide a regression behind a log line
                    # nobody reads, so let it raise and fail a test loudly.
                    out.append(
                        BracketRef(
