...
Extracts glossary terms from RDF graphs and creates RDF AST objects.
...
class GlossaryTermExtractor(EntityExtractor[RDFGlossaryTerm]):
    """
    ...
    def extract(
        ...
        try:
            ...
            source = self._extract_source(graph, uri)

            # Extract relationships (only broader/narrower supported)
            relationships = self._extract_relationships(graph, uri)
    ...
    def _extract_relationships(
        ...
        skos:related, skos:closeMatch, skos:exactMatch are NOT supported
        for term-to-term relationships.
        """
        relationships = []
