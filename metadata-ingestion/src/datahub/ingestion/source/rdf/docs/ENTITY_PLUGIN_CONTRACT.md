...
entities/
  your_entity/              # Folder name = entity_type (snake_case)
    ...
    converter.py          # Must implement EntityConverter
    mcp_builder.py        # Must implement EntityMCPBuilder
    ast.py               # Must define RDF* and DataHub* AST classes
...
Your `__init__.py` must export exactly these components:

1. **Extractor class**: `{EntityName}Extractor` (e.g., `GlossaryTermExtractor`)
2. **Converter class**: `{EntityName}Converter` (e.g., `GlossaryTermConverter`)
3. **MCP Builder class**: `{EntityName}MCPBuilder` (e.g., `GlossaryTermMCPBuilder`)
4. **ENTITY_METADATA**: `EntityMetadata` instance
...
- **Converter class**: `{PascalCaseEntityName}Converter` (e.g., `GlossaryTermConverter`)
...
ENTITY_METADATA = EntityMetadata(
    ...
    rdf_ast_class=RDFYourEntity,  # RDF AST class from ast.py
    ...
    processing_order=100,  # Order in which entities are processed (lower = first)
...
- **`rdf_ast_class`**: The RDF AST class that represents entities of this type before conversion
...
Must implement `EntityExtractor[RDFEntityT]`:
...
class YourEntityExtractor(EntityExtractor[RDFYourEntity]):
    ...
    def extract_all(self, graph: Graph, context: Dict[str, Any] = None) -> List[RDFYourEntity]:
        """Extract all entities of this type from the RDF graph."""
        # Implementation: find all entities and extract them
        pass
```
...
- ✅ Checks for required components (Extractor, Converter, MCPBuilder, ENTITY_METADATA)
- ✅ Validates `ENTITY_METADATA.entity_type` matches folder name
- ✅ Validates `processing_order` is an integer (defaults to 100 if not specified)
