...
class StaleEntityRemovalHandler(
    ...
    def _init_job_id(self, unique_id: Optional[str] = None) -> JobId:
        platform: Optional[str] = self.platform or "default"
    ...
    def gen_removed_entity_workunits(self) -> Iterable[MetadataWorkUnit]:
        ...
        self.report.entity_type_deletion_summary = self._build_checkpoint_comparison(
        ...
        if not copy_previous_state_and_exit and (
            ...
            self.report.failure(
                title="Skipping stateful ingestion / stale entity removal",
                message="Entity difference exceeds fail_safe_threshold; skipping stale entity removal. "
                "To update, set stateful_ingestion.fail_safe_threshold in your recipe.",
                context=f"previous_run={last_checkpoint_state.urn_count()} entities, "
                f"current_run={cur_checkpoint_state.urn_count()} entities, "
                f"change={entity_difference_percent:.1f}%, "
                f"threshold={self.stateful_ingestion_config.fail_safe_threshold}",
            )
            copy_previous_state_and_exit = True
        ...
        for urn in last_checkpoint_state.get_urns_not_in(
            ...
            if (
                ...
                self.report.report_last_state_non_deletable_entities(urn)
                continue
            if urn in self._urns_to_skip:
                ...
                continue
            self.report.entity_type_deletion_summary[
                entity_type
