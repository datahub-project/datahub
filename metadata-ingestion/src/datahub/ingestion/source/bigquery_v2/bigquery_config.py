...
class BigQueryV2Config(
    ...
    use_legacy_table_stats: bool = Field(
        default=False,
        description="Source table row count, size, and last-altered time from the deprecated "
        ...
        "`size_in_bytes` custom properties. Google deprecated `__TABLES__` in 2021 and it may stop "
        "working without notice. Leave `False` to use `PARTITIONS`, which is dataset-scoped "
        "but covers base tables only.",
    )
