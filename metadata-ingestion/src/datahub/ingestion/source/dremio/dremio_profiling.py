...
class DremioProfiler:
    def __init__(self, api_operations, config: ProfileConfig):
    ...
    def _profile_chunk(self, table_name: str, columns: List[Tuple[str, str]]) -> Dict:
        ...
        try:
            results = self.api_operations.execute_query(profile_sql, timeout=self.QUERY_TIMEOUT)
            return self._parse_profile_results(results, columns)
        except Exception as e:
            logger.error(f"Error profiling {table_name}: {str(e)}")
            raise

    def _chunk_columns(self, columns: List[Tuple[str, str]]) -> List[List[Tuple[str, str]]]:
        return [columns[i:i + self.MAX_COLUMNS_PER_QUERY] for i in range(0, len(columns), self.MAX_COLUMNS_PER_QUERY)]

    def _build_profile_sql(self, table_name: str, columns: List[Tuple[str, str]]) -> str:
        metrics = []

        if self.config.row_count:
            metrics.append(f"COUNT(*) AS row_count")
        ...
        for column_name, data_type in columns:
            try:
                ...
                logger.warning(f"Error building metrics for column {column_name}: {str(e)}")
    ...
    def _get_column_metrics(self, column_name: str, data_type: str) -> List[str]:
        ...
        if self.config.include_field_distinct_count:
            metrics.append(f"COUNT(DISTINCT {quoted_column_name}) AS {safe_column_name}_distinct_count")

        if self.config.include_field_null_count:
            metrics.append(f"SUM(CASE WHEN {quoted_column_name} IS NULL THEN 1 ELSE 0 END) AS {safe_column_name}_null_count")
        ...
        if data_type.lower() in ['int', 'bigint', 'float', 'double', 'decimal']:
            if self.config.include_field_mean_value:
                metrics.append(f"AVG(CAST({quoted_column_name} AS DOUBLE)) AS {safe_column_name}_mean")

            if self.config.include_field_stddev_value:
                metrics.append(f"STDDEV(CAST({quoted_column_name} AS DOUBLE)) AS {safe_column_name}_stdev")

            if self.config.include_field_median_value:
                metrics.append(f"MEDIAN({quoted_column_name}) AS {safe_column_name}_median")

            if self.config.include_field_quantiles:
                metrics.append(f"PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY {quoted_column_name}) AS {safe_column_name}_25th_percentile")
                metrics.append(f"PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY {quoted_column_name}) AS {safe_column_name}_75th_percentile")

        return metrics

    def _parse_profile_results(self, results: List[Dict], columns: List[Tuple[str, str]]) -> Dict:
        profile = {}
        ...
        for column_name, data_type in columns:
            ...
            column_stats = {}
            if self.config.include_field_distinct_count:
                column_stats["distinct_count"] = int(result.get(f"{safe_column_name}_distinct_count", 0))
            if self.config.include_field_null_count:
                column_stats["null_count"] = int(result.get(f"{safe_column_name}_null_count", 0))
            if self.config.include_field_min_value:
    ...
    def _combine_profile_results(self, profile_results: List[Dict]) -> Dict:
        combined_profile = {}
        combined_profile["row_count"] = sum(profile.get("row_count", 0) for profile in profile_results)
        combined_profile["column_count"] = sum(profile.get("column_count", 0) for profile in profile_results)
