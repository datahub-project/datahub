...
class S3LineageProviderConfig(ConfigModel):
    ...
    def get_s3_path(self, path: str, strip_filename: bool = True) -> Optional[str]:
        for path_spec in self.path_specs:
            if path_spec.allowed(path):
                _, table_path = path_spec.extract_table_name_and_path(path)
                return table_path

        # Fold directory prefixes before the ignore_non_path_spec_path filter so a prefix under a spec is kept.
        if not strip_filename:
            for path_spec in self.path_specs:
        ...
        if strip_filename and self.strip_urls:
    ...
    def get_s3_urn_for_lineage(
        self, path: str, env: str, strip_filename: bool = True
    ) -> Optional[str]:
        s3_path = self.get_s3_path(path, strip_filename=strip_filename)
        if s3_path is None:
            return None
