...
def _apply_databricks_proxy_fix():
    ...
    try:
        ...
        if "thrift" in str(e).lower() and "transport" in str(e).lower():
            logger.warning(
                f"Databricks-sql proxy patch skipped due to thrift version incompatibility: {e}. "
                "This is usually harmless - the SQL connection will work without the patch in most environments."
            )
        else:
            logger.error(
