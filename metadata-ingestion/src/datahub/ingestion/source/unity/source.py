...
class UnityCatalogSource(StatefulIngestionSourceBase, TestableSource):
    ...
    def __init__(self, ctx: PipelineContext, config: UnityCatalogSourceConfig):
        ...
        if self.config._forced_disable_tag_extraction:
            self.report.warning(
                message="Tag Extraction is disabled due to missing warehouse_id in config",
        ...
        if self.config._forced_disable_hive_metastore_extraction:
            self.report.warning(
                message="Hive Metastore Extraction is disabled due to missing warehouse_id in config",
                context="configuration-conflicts",
                log=False,
            )
        ...
        if (
            ...
            self.report.warning(
                message="databricks-sdk does not expose TableType.METRIC_VIEW. "
                ...
                context=f"include_metric_views has no effect with databricks-sdk {_sdk_version}",
                log=False,
            )
    ...
    def get_workunits_internal(self) -> Iterable[MetadataWorkUnit]:
        ...
        # Built once for the whole run so process_metastores (below) can look up
        # query URNs for every table's lineage as it goes. _by_edge is already
        # fully populated by this point, so Query entities can be emitted now,
        # before any aspect that could reference one: a `query` URN pointing at
        # an entity that hasn't been emitted yet renders as a ghost node in the
        # lineage graph. Emitting first also means an entity is never left
        # dangling if process_metastores/get_view_lineage fails partway through --
        # sources are generators whose workunits commit to the sink as they
        # stream, not batched at run end, so any lineage aspects already emitted
        # before such a failure would otherwise carry `query` URNs with no
        # backing entity.
        ...
        # Every lineage aspect has been generated by now, so the link count is final.
        self._warn_if_no_query_links()
    ...
    def build_service_principal_map(self) -> None:
        try:
            ...
            self.report.warning(
                message="Unable to fetch service principals",
    ...
    def _gen_query_entity_workunits(
        ...
        Called before process_metastores/get_view_lineage (see
        get_workunits_internal), so every entity exists before any lineage aspect
        can reference its URN. The usage path may emit the same URN with the same
        aspects; that upsert is idempotent, so no cross-path dedup is needed.
        """
    ...
    def _build_query_lineage_resolver(self) -> Optional["QueryLineageResolver"]:
        ...
        # usage.py always calls get_query_history_via_system_tables as the primary
        # (non-secondary) fetch when it uses the system-tables path at all, so this
        # is only truly a second read of the same window when that will happen —
        # otherwise this is the sole read and must surface its own warnings.
        usage_will_read_system_tables = (
            self.config.include_usage_statistics
            and self.config.usage_uses_system_tables(self.config.warehouse_id)
        )
        ...
        try:
            for (
                ...
                # Not "skip Operation aspects" — this narrows the *fetch*. With
                # False, usage_statement_types() allows SELECT only, which excludes
                # every lineage-producing statement (INSERT/CTAS/MERGE/UPDATE/COPY)
                # and leaves nothing to link.
                include_operational_stats=True,
                secondary_fetch=usage_will_read_system_tables,
            ):
        ...
        self.report.num_lineage_statements_seen += resolver.num_statements_seen
        self.report.num_lineage_statements_skipped += resolver.num_statements_skipped
        self.report.num_lineage_statements_unresolved_tables += (
            resolver.num_statements_unresolved
        )
        return resolver

    def _warn_if_no_query_links(self) -> None:
        """Warn when query-linked lineage ran but attached nothing.

        Without this, a fetch that returns no lineage-producing statements and a
        set of identifiers that never match the ingested tables both look exactly
        like success: the lineage aspects are still emitted, just with no query URN
        on them and so no transformation node in the graph.
        """
        if self.query_lineage_resolver is None:
            return
        if self.report.num_lineage_edges_query_linked:
            return
        self.report.warning(
            title="No lineage edge was linked to a query",
            message=(
                "Query-linked lineage is enabled but no lineage edge could be "
                "attributed to a SQL statement, so no transformation node will "
                "appear in the lineage graph. Likely causes: the ingestion window "
                "holds no warehouse-executed write statements (statements run on "
                "cluster compute carry no statement_id and can never be linked), "
                "or the table identifiers in system.access do not match the tables "
                "this recipe ingested."
            ),
            context=(
                f"statements_seen={self.report.num_lineage_statements_seen}, "
                f"statements_unresolved={self.report.num_lineage_statements_unresolved_tables}, "
                f"edges_unlinked={self.report.num_lineage_edges_query_unlinked}"
            ),
            log=False,
        )
    ...
    def _lineage_full_name_to_urn(self, full_name: str) -> Optional[str]:
        ...
        Also mirrors process_catalogs/process_schemas/process_tables' catalog/
        schema/table pattern checks, so a table outside the configured scope never
        becomes a resolver candidate and so can never reach querySubjects. The
        query-history fetch's own catalog_pattern push-down (proxy.py) is an OR
        across the statement's source and target catalog and applies no schema or
        table filtering at all, so it alone lets plenty of out-of-scope tables
        through.
        """
        ...
        if not self.config.catalog_pattern.allowed(catalog):
            return None
        if not self.config.schema_pattern.allowed(f"{catalog}.{schema}"):
            return None
        if not self.config.table_pattern.allowed(f"{catalog}.{schema}.{table}"):
            return None
    ...
    def _generate_lineage_aspect(
        ...
        for upstream_ref, downstream_to_upstream_cols in sorted(
            ...
            if query_urn is not None:
                ...
                self.report.num_lineage_edges_query_unlinked += 1
    ...
    def _load_metric_view_spec(self, table: Table) -> Optional[Dict[str, Any]]:
        if not table.view_definition:
            ...
            self.report.warning(
                message="Metric view has no YAML body: view_definition is empty. "
        ...
        try:
            ...
            self.report.warning(
                message="Metric view YAML failed to parse. "
                "Falling back to the REST table-lineage API; column-level lineage "
                "and Dimension/Measure tags will be absent for this view.",
                context=table.ref.qualified_table_name,
                exc=e,
        ...
        if not isinstance(spec, dict):
            self.report.num_metric_views_yaml_shape_invalid += 1
            self.report.warning(
                message="Metric view YAML is not a mapping. "
                "Falling back to the REST table-lineage API; column-level lineage "
                "and Dimension/Measure tags will be absent for this view.",
                context=f"{table.ref.qualified_table_name}: top-level type is "
                f"{type(spec).__name__!r}, expected dict",
                log=False,
            )
            return None
        return spec
    ...
    def _extract_metric_view_lineage(
        ...
        if shape_issues:
            self.report.num_metric_views_yaml_shape_invalid += 1
            self.report.warning(
                message="Metric view YAML has unexpected shape. "
                "Affected entries are skipped; remaining YAML continues to be processed.",
                context=f"{table.ref.qualified_table_name}: {'; '.join(shape_issues)}",
                log=False,
            )
        ...
        if unparseable and upstreams:
            ...
            self.report.warning(
                message="Metric view source(s) skipped: need 2-part (schema.table) or 3-part "
                "(catalog.schema.table) identifiers; 1-part names and SQL "
                "subqueries are not supported by the YAML lineage path",
                context=f"{table.ref.qualified_table_name}: unparseable sources="
                f"{unparseable!r}",
                log=False,
            )
        ...
        if not upstreams:
            if source_names:
                ...
                self.report.warning(
                    message="Metric view has no parseable upstream sources. "
                    "Lineage extraction needs 2-part (schema.table) or 3-part "
                    "(catalog.schema.table) identifiers; 1-part names and SQL "
                    "subqueries fall back to the REST table-lineage API.",
                    context=f"{table.ref.qualified_table_name}: sources={source_names!r}",
                    log=False,
                )
            return None
    ...
    def _extract_metric_view_column_lineage(
        ...
        if joins_skipped:
            ...
            self.report.warning(
                message="Metric view joins skipped due to malformed shape or unparseable "
                "source. Column lineage for join-qualified columns will be absent.",
                context=f"{table.ref.qualified_table_name}: {joins_skipped} join "
        ...
        if skipped_entries:
            ...
            self.report.warning(
                message="Metric view dimension/measure entries skipped due to malformed shape "
                "(non-mapping entry, missing or non-string name/expr). Column "
                "lineage for affected columns will be absent.",
                context=f"{table.ref.qualified_table_name}: {skipped_entries} "
                "dimension/measure entry/entries skipped",
                log=False,
            )
        ...
        if unresolved_qualifiers:
            ...
            self.report.warning(
                message="Metric view expression references unknown qualifier not found in "
                "source/joins. Lineage rows for affected columns were skipped.",
                context=f"{table.ref.qualified_table_name}: qualifiers="
                f"{sorted(unresolved_qualifiers)!r}",
                log=False,
            )
    ...
    def _parse_metric_view_expr(
        ...
        try:
            ...
            self.report.warning(
                message="Metric view expression failed to parse",
                context=f"{context}: expr={expr!r}",
                exc=e,
                log=False,
            )
            return [], set()
