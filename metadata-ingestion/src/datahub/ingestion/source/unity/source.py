...
class UnityCatalogSource(StatefulIngestionSourceBase, TestableSource):
    ...
    def get_workunits_internal(self) -> Iterable[MetadataWorkUnit]:
        ...
        # query URNs for every table's lineage as it goes.
        ...
        # Query entities are emitted only now, after every lineage aspect that could
        # reference one has been generated: querySubjects is built from the URNs
        # query_urn_for actually saw during that pass (see subject_urns_for), so
        # emitting any earlier would mean guessing at subjects before they're known.
        # Every resolved statement is still emitted regardless of whether a lineage
        # edge ended up referencing it, so a `query` URN is never left dangling.
    ...
    def _gen_query_entity_workunits(
        ...
        Called after every lineage aspect has been generated (see
        get_workunits_internal), so subject_urns_for reflects every edge that
        actually referenced a query URN. The usage path may emit the same URN with
        the same aspects; that upsert is idempotent, so no cross-path dedup is
        needed.
        """
