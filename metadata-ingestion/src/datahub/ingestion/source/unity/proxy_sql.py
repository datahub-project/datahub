...
class UnityCatalogSqlProxy(UnityCatalogProxyProfilingMixin):
    ...
    def check_basic_connectivity(self) -> bool:
        ...
        try:
            query = "SELECT catalog_name FROM system.information_schema.catalogs LIMIT 1"
    ...
    def _get_all_view_definitions_for_schema(self, schema: Schema) -> Dict[str, str]:
        ...
        try:
            ...
            logger.warning(f"Error getting view definitions for schema {schema.id}: {e}")
            return {}
    ...
    def _get_all_columns_for_schema(self, schema: Schema) -> Dict[str, List[Column]]:
        ...
        try:
            ...
            for row in rows:
                table_name, column_name, data_type, position, is_nullable, default, comment, precision, scale = row
    ...
    def _get_table_columns(self, table: Table) -> Iterable[Column]:
        ...
        try:
            ...
            for row in rows:
                column_name, data_type, position, is_nullable, default, comment, precision, scale = row
                if column_name:
                    column = Column(
                        name=column_name,
        ...
        try:
            ...
            for entry in lineage_entries:
                ...
                if source_type == 'TABLE':
                    ...
                    external_ref = ExternalTableReference(path=source_table)
    ...
    def get_column_lineage(self, table: Table, column_name: str) -> None:
        ...
        try:
            ...
            logger.warning(f"Error getting column lineage for {table.ref}.{column_name}: {e}")

    def _execute_sql_query(self, query: str, params: Optional[List[Any]] = None) -> List[List[str]]:
    ...
    def _get_all_tags_for_catalog(self, catalog_name: str) -> Dict[str, Dict[str, List[UnityCatalogTag]]]:
        ...
        try:
            ...
            for row in rows:
                ...
                result['schema_tags'][schema_key].append(UnityCatalogTag(key=tag_name, value=tag_value))
        except Exception as e:
        ...
        try:
            ...
            for row in rows:
                ...
                result['table_tags'][table_key].append(UnityCatalogTag(key=tag_name, value=tag_value))
        except Exception as e:
        ...
        try:
            ...
            rows = self._execute_sql_query(query)
            for row in rows:
                cat_name, schema_name, table_name, column_name, tag_name, tag_value = row
                ...
                result['column_tags'][column_key].append(UnityCatalogTag(key=tag_name, value=tag_value))
        except Exception as e:
        ...
        try:
            ...
            for row in rows:
                ...
                result['catalog_tags'][cat_name].append(UnityCatalogTag(key=tag_name, value=tag_value))
        except Exception as e:
    ...
    def _create_table_from_sql(self, schema: Schema, row: List[Any]) -> Optional[Table]:
        ...
        (table_name, table_type, owner, comment, data_source_format,
         storage_path, created, created_by,
         last_altered, last_altered_by) = row
    ...
    def _create_service_principal(self, obj) -> Optional[ServicePrincipal]:
    ...
    def _create_query_from_sql(self, row: List[Any]) -> Optional[Query]:
        ...
        (statement_id, query_text, statement_type, start_time, end_time,
          user_id, user_name, executed_as_user_id, executed_as_user_name) = row
    ...
    def _get_view_definition(self, catalog_name: str, schema_name: str, table_name: str) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(f"Could not get view definition for {catalog_name}.{schema_name}.{table_name}: {e}")
        return None

    def _map_data_source_format(self, data_source_format_str: Optional[str]) -> Optional[DataSourceFormat]:
        ...
        try:
            ...
            logger.debug(f"Unknown data source format: {data_source_format_str}, defaulting to None")
            return None

        except Exception as e:
            logger.warning(f"Error mapping data source format '{data_source_format_str}': {e}")
            return None

    def _parse_datetime(self, datetime_input: Optional[Union[str, datetime]]) -> Optional[datetime]:
