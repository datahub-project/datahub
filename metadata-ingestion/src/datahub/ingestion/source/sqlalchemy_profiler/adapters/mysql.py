...
class MySQLAdapter(PlatformAdapter):
    ...
    def setup_profiling(
        ...
        """
        Generic setup for MySQL - just create the SQL table object.

        MySQL doesn't require special temp table handling like BigQuery.
        ...
        logger.debug(
            f"MySQL setup for {context.pretty_name}: "
    ...
    def cleanup(self, context: ProfilingContext) -> None:
        """
        Cleanup - nothing to clean up for MySQL.

        Args:
            context: Profiling context
        """
        # No temp resources created in MySQL setup
        pass
    ...
    def get_estimated_row_count(
        ...
        try:
            ...
            logger.debug(f"Failed to get MySQL row count estimate: {e}")
            return None
