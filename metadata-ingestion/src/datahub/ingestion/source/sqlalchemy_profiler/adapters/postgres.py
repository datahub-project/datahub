...
class PostgresAdapter(PlatformAdapter):
    ...
    def setup_profiling(
        ...
        """
        Generic setup for PostgreSQL - just create the SQL table object.

        PostgreSQL doesn't require special temp table handling like BigQuery.
        ...
        logger.debug(
            f"PostgreSQL setup for {context.pretty_name}: "
    ...
    def cleanup(self, context: ProfilingContext) -> None:
        """
        Cleanup - nothing to clean up for PostgreSQL.

        Args:
            context: Profiling context
        """
        # No temp resources created in PostgreSQL setup
        pass
    ...
    def get_estimated_row_count(
        ...
        try:
            ...
            logger.debug(f"Failed to get PostgreSQL row count estimate: {e}")
            return None
