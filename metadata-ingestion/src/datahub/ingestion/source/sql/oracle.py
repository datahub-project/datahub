...
class OracleSource(SQLAlchemySource):
    """
    Source that extracts metadata from Oracle databases via SQLAlchemy and oracledb.

    Implementation notes:
    - Uses python-oracledb (thin mode by default, optional thick mode)
    - Queries ALL_* or DBA_* data dictionary views based on data_dictionary_mode
    - Uses SQL aggregator for lineage from stored procedures, materialized views, and views
    - Can extract query usage from V$SQL when enabled
    """
