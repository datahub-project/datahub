    ...
    def _get_snowflake_history(self) -> Iterable[SnowflakeJoinedAccessEvent]:
        ...
        for row in results:
            ...
            if not event_dict["email"]:
                if not event_dict["user_name"] or not self.config.email_domain:
                    logging.warning(
                        f"A user cannot be generated from {event_dict}. Either the user name or email domain is missing. Skipping ...."
                    )
                    continue
