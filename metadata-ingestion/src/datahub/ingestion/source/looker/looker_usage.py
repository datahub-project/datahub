...
class BaseStatGenerator(ABC):
    ...
    def __init__(self, config: StatGeneratorConfig, looker_models: List[model.Model]):
        self.config = config
    ...
    def _execute_query(self, query: LookerQuery) -> List[Dict]:
        return self.config.looker_api_wrapper.execute_query(
...
class DashboardStatGenerator(BaseStatGenerator):
    def __init__(self, config: StatGeneratorConfig, looker_dashboards: List[Dashboard]):
        super().__init__(
            config, looker_models=cast(List[model.Model], looker_dashboards)
        )
...
class LookStatGenerator(BaseStatGenerator):
    def __init__(self, config: StatGeneratorConfig, looker_looks: List[LookWithQuery]):
        super().__init__(config, looker_models=cast(List[model.Model], looker_looks))
...
) -> Callable[[List[model.Model]], BaseStatGenerator]:
