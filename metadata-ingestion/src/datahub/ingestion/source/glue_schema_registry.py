...
class GlueSchemaRegistry(KafkaSchemaRegistryBase):
    ...
    It knows how to get SchemaMetadata of a topic from GlueSchemaRegistry
    """
    ...
    def get_schema_str_replace_confluent_ref_avro(
        ...
        for schema_ref in schema.references:
            ...
            reference_schema = self.glue_client.get_latest_version(
    ...
    def _get_schema_and_fields(
        ...
        if topic_subject is not None:
            ...
            try:
                ...
                logger.info(f'registered_schema=={registered_schema}')
                data_format = registered_schema['DataFormat']
                schema_def = registered_schema['SchemaDefinition']

                schema = Schema(str(schema_def), schema_type=data_format, references=[])
    ...
    def get_schema_metadata(
        ...
        logger.info("**********Value info start*************")
        logger.info(f" {schema} {fields}")
        logger.info("**********Value info end*************")
        # Process the key schema
        key_schema, key_fields = self._get_schema_and_fields(
            topic=topic, is_key_schema=True
        )  # type:Tuple[Optional[Schema], List[SchemaField]]

        logger.info("**********Key info start*************")
        logger.info(f" {key_schema} {key_fields}")
        logger.info("**********Key info end*************")
