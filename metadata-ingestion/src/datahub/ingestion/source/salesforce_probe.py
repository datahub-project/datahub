...
SALESFORCE_PROBE = ClientProbe(
    ...
    levels=[
        # kind_for reclassifies some items to SALESFORCE_CUSTOM_OBJECT ("Custom
        # Object"), so ClientProbe._resolved resolves a field per each item's
        # own (possibly reclassified) kind rather than the level's declared
        # kind. SalesforceConfig.object_pattern declares a Filters hint for
        # both kinds (see its definition), so both resolve to it.
        ProbeLevel(
