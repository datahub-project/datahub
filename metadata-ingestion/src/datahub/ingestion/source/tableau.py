...
class TableauSource(StatefulIngestionSourceBase):
    ...
    def close(self) -> None:
        try:
            ...
            logger.warning("During graceful closing of Tableau source a signout call was tried but ended up with"
                           " a ConnectionError (%s). Continuing to close to source", err)
            self.server = None
        super().close()
