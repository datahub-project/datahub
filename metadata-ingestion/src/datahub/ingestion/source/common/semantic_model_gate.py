...
def resolve_emit_semantic_model_entities(
    ...
    if graph is None:
        ...
        logger.warning(
            "emit_semantic_model_entities auto-enable requires a DataHub graph "
            "client (managed-server detection). Without one, it stays off unless "
            "the recipe explicitly sets it to true."
        )
        return ResolvedEmitDecision(
            enabled=False,
