...
WHERE table_schema='{schema_name}' AND {extra_clause}"""
    ...
    def semantic_view_usage_statistics(
        ...
        Uses QUERY_HISTORY with pattern matching on SEMANTIC_VIEW( function calls.
        Note: Semantic views don't appear in ACCESS_HISTORY, so we must use QUERY_HISTORY.
        """
        assert (
            time_bucket_size == BucketDuration.DAY
            or time_bucket_size == BucketDuration.HOUR
        ...
        WITH semantic_view_queries AS (
            SELECT
                ...
                REGEXP_SUBSTR(query_text, 'SEMANTIC_VIEW\\\\s*\\\\(\\\\s*([A-Za-z0-9_\\\\.]+)', 1, 1, 'i', 1) AS semantic_view_name,
