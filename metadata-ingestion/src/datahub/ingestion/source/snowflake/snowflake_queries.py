...
, raw_access_history AS (
    ...
    WHERE
        query_start_time >= to_timestamp_ltz({self.start_time_millis}, 3)
        AND query_start_time < to_timestamp_ltz({self.end_time_millis}, 3)
        AND {self.users_filter or "TRUE"}
        AND query_id IN (
...
class QueryLogExtractorStrategyNoDedup(QueryLogExtractionDedupStrategy):
    ...
    WHERE
        query_history.start_time >= to_timestamp_ltz({self.start_time_millis}, 3)
        AND query_history.start_time < to_timestamp_ltz({self.end_time_millis}, 3)
