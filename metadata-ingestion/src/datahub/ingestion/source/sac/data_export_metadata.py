...
def parse_data_export_metadata(xml_text: str) -> EdmxParseResult:
    ...
    # FACT_DATA_ENTITY is a module constant, so there is no injection surface here.
    fact_entity: Optional[Element] = root.find(
        f".//edm:EntityType[@Name='{FACT_DATA_ENTITY}']", EDMX_NS
    )

    if fact_entity is None:
        return EdmxParseResult.failed(
            f"Data Export metadata has no {FACT_DATA_ENTITY} EntityType"
        )
