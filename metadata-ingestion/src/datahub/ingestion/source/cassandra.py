...
class CassandraSource(Source):
    ...
    def __init__(self, ctx: PipelineContext, config: CassandraConfig):
        super().__init__(ctx)
        self.config = config
        ...
        self._client = Cluster(ips, auth_provider=auth_provider, **kwargs)
    ...
    def get_workunits(self) -> Iterable[MetadataWorkUnit]:
        ...
        for keyspace in keyspaces:
            ...
            for table_name, table_schema in self._get_db_tables(keyspace).items():
                ...
                for wu in self.prepare_metadata_for(
                    platform, keyspace, "tables", table_name, table_schema
                ):
                    yield wu
            ...
            for view_name, view_schema in self._get_db_views(keyspace).items():
                if not self.config.view_pattern.allowed(view_name):
                    ...
                    continue
                for wu in self.prepare_metadata_for(
                    platform, keyspace, "views", view_name, view_schema
                ):
                    yield wu

    def prepare_metadata_for(
        self,
        platform: str,
        keyspace: str,
        table_or_view_type: str,
        table: str,
        table_schema: cassandra.metadata.TableMetadata,
        ...
        for column_name, column_schema in self._get_db_columns(
            table_or_view_type, keyspace, table
        ).items():
            field = SchemaField(
                fieldPath=column_name,
                nativeDataType=column_schema.cql_type,
                type=self._get_db_column_type(
                    self.report, dataset_name, column_schema.cql_type
                ),
            )
            fields.append(field)
        ...
        primary_keys = [
            column_in_primary_key.name
            for column_in_primary_key in table_schema.partition_key
            + table_schema.clustering_key
        ]
    ...
    def _get_db_tables(
        self, keyspace: str
    ) -> Dict[str, cassandra.metadata.TableMetadata]:
        return self._client.metadata.keyspaces[keyspace].tables

    def _get_db_views(
        self, keyspace: str
    ) -> Dict[str, cassandra.metadata.MaterializedViewMetadata]:
        return self._client.metadata.keyspaces[keyspace].views

    def _get_db_columns(
        self, table_or_view_type: str, keyspace: str, table: str
    ) -> Dict[str, cassandra.metadata.ColumnMetadata]:
        return getattr(self._client.metadata.keyspaces[keyspace], table_or_view_type)[
            table
        ].columns

    def _get_db_column_type(
        self, report: SourceReport, dataset_name: str, column_type: str
    ) -> SchemaFieldDataType:
        ...
        if column_type in _field_type_mapping:
            ...
            type_class_instance = MapTypeClass(keyType=key_type, valueType=value_type)
            ...
            if nested_type:
                ...
                type_class_instance = ArrayTypeClass()
    ...
    def _extract_map_set_nested_type(self, column_type: str) -> Union[str, None]:
        nested_type = None
        guessed_nested_type_search_result = re.search("<(.*)>", column_type)
        if guessed_nested_type_search_result and len(
            guessed_nested_type_search_result.groups()
        ):
            nested_type = guessed_nested_type_search_result.groups()[0]
        return nested_type

    def _extract_map_key_value_types(
        self, column_type: str
    ) -> Tuple[Optional[str], Optional[str]]:
        key_type = value_type = None
        guessed_search_result = re.search(r"<(.*),\s+(.*)>", column_type)
        if guessed_search_result and len(guessed_search_result.groups()) == 2:
            key_type, value_type = guessed_search_result.groups()
        return key_type, value_type


_field_type_mapping: Dict[str, Any] = {
