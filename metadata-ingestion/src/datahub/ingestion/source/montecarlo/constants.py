...
def get_known_data_platforms() -> Optional[Set[str]]:
    ...
    The connector registry (``autogenerated/connector_registry/datahub.json``)
    ships a per-connector entry whose ``platform_id`` is the canonical platform
    name each connector emits in its dataset URNs. That set is comprehensive but
    not exhaustive — e.g. ``spark`` has no registered connector yet the
    MonteCarlo connection-type map emits it — so the result is the union of the
    registry's ``platform_id`` values, the values in
    ``CONNECTION_TYPE_TO_PLATFORM`` (the connector's own mappings), and
    ``montecarlo`` (this connector's own platform). This guarantees the
    connector's own emitted platforms are always valid while still catching
    typos like ``snowflke``.
    ...
    """
    known: Set[str] = {"montecarlo"}
    known.update(CONNECTION_TYPE_TO_PLATFORM.values())
    try:
        import json
...
# emits in its dataset URNs. They are validated at config-load time against
# ``get_known_data_platforms()`` (the connector-registry platform_ids union
# these values and ``montecarlo``), so a typo here or in a user's
# ``connection_to_platform_map`` / ``default_platform`` fails fast with a clear
# error rather than silently producing a phantom ``urn:li:dataPlatform:...``.
#
