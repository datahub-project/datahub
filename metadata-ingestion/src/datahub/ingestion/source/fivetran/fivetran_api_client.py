...
class FivetranAPIClient:
    ...
    def extract_connector_metadata(
        ...
        try:
            ...
            # Try to extract source database information if available
            source_database = self._extract_source_database_from_config(
                api_connector, connector_service
            )
            if source_database:
                additional_properties["source_database"] = source_database
                logger.info(
                    f"Detected source database '{source_database}' for connector {connector_id}"
                )
    ...
    def _extract_source_database_from_config(
        self, api_connector: Dict, connector_service: str
    ) -> Optional[str]:
        """
        Extract source database name directly from Fivetran connector configuration.

        This method gets database information directly from Fivetran's API response
        rather than trying to parse or guess from various sources.

        Args:
            api_connector: The API connector data from Fivetran
            connector_service: The connector service type

        Returns:
            Database name if explicitly provided by Fivetran, None otherwise
        """
        try:
            # Get the connector configuration from Fivetran API
            config = self._get_connector_config(api_connector)
            if not config:
                logger.debug(
                    f"No config available for connector {api_connector.get('id', 'unknown')}"
                )
                return None

            connector_type = connector_service.lower()

            # For database connectors, check for standard database field names that Fivetran uses
            database_field_names = [
                "database",  # Most common field name
                "database_name",  # Alternative field name
                "db",  # Short form
                "initial_catalog",  # SQL Server
                "catalog",  # Some databases
                "project_id",  # BigQuery
                "project",  # BigQuery alternative
                "schema",  # Some connectors use schema as database
            ]

            # Look for database name in the configuration
            for field_name in database_field_names:
                if field_name in config:
                    value = config.get(field_name)
                    if self._is_valid_database_value(value):
                        logger.debug(
                            f"Found database '{value}' in Fivetran config field '{field_name}' for connector {api_connector.get('id')}"
                        )
                        return str(value).strip()

            # For some connectors, check nested configuration
            for key, nested_config in config.items():
                if isinstance(nested_config, dict):
                    for field_name in database_field_names:
                        if field_name in nested_config:
                            value = nested_config.get(field_name)
                            if self._is_valid_database_value(value):
                                logger.debug(
                                    f"Found database '{value}' in nested config {key}.{field_name} for connector {api_connector.get('id')}"
                                )
                                return str(value).strip()

            logger.debug(
                f"No database information found in Fivetran config for {connector_type} connector {api_connector.get('id')}"
            )
            return None

        except Exception as e:
            logger.debug(f"Error extracting source database from Fivetran config: {e}")
            return None

    def _get_connector_config(self, api_connector: Dict) -> Dict:
        """
        Get connector configuration from Fivetran API.

        Args:
            api_connector: Basic connector info from API

        Returns:
            Complete connector configuration from Fivetran
        """
        # First try to get config from the existing connector data
        config = api_connector.get("config", {})

        # If no config available, fetch detailed connector information
        if not config and "id" in api_connector:
            try:
                logger.debug(
                    f"Fetching detailed config for connector {api_connector['id']}"
                )
                connector_details = self.get_connector_details(api_connector["id"])
                config = connector_details.get("config", {})
            except Exception as e:
                logger.debug(
                    f"Could not fetch detailed config for connector {api_connector.get('id')}: {e}"
                )
                config = {}

        return config

    def _is_valid_database_value(self, value: Any) -> bool:
        """
        Check if a value represents a valid database name from Fivetran.

        Args:
            value: The value to check

        Returns:
            True if this looks like a valid database name
        """
        if not value:
            return False
        ...
        invalid_values = {"", "null", "none", "default", "undefined", "n/a"}
        return value and value.lower() not in invalid_values
