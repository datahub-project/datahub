...
class LookerUtil:
    ...
    def _get_fields_and_primary_keys(
        ...
        for field in view_fields:
            is_label_autogenerated = field.label == remove_view_from_field_name(field.name).replace("_", " ").title()
            schema_field = SchemaField(
                fieldPath=field.name,
                type=LookerUtil._get_field_type(field.type, reporter),
                nativeDataType=field.type,
                description=f"{f'{field.label}. ' if (len(field.label) > 0 and not is_label_autogenerated) else ''}{field.description}"
