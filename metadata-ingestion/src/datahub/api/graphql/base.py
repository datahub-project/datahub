...
class BaseApi:
    ...
    def __init__(
        ...
        if datahub_token is None and datahub_auth is None:
            ...
            # The emitter also resolves this (#18547), but only onto
            # session.auth; resolving here puts the declarative AuthConfig on
            # DatahubClientConfig, so anything re-deriving a client from
            # graph.config keeps the credentials. It is also the only route the
            # Airflow circuit-breaker operators have: they build their config
            # from DatahubRestHook._get_config(), which cannot express an
            # AuthConfig.
