...
from datahub.cli.semantic_model_container_migration import (
    discover_semantic_model_urns as discover_all_semantic_model_urns,
    run_migration as run_semantic_model_container_migration,
)
...
@migrate.command(name="semantic-model-container")
@click.option(
    "--platform",
    type=str,
    default=None,
    help="Optional data platform filter when discovering semanticModel entities "
    "(e.g. snowflake, dbt).",
)
@click.option(
    "--platform-instance",
    type=str,
    default=None,
    help="Optional platform instance filter when discovering semanticModel entities.",
)
@click.option(
    "--urn",
    "urns",
    type=str,
    multiple=True,
    help="Explicit semanticModel urn(s) to migrate. Can be passed multiple times. "
    "If neither --urn nor --urn-file is given, entities are discovered via search.",
)
@click.option(
    "--urn-file",
    type=click.Path(exists=True, dir_okay=False),
    default=None,
    help="Path to a file with one semanticModel urn per line. Blank lines and '#' "
    "comment lines are ignored.",
)
@click.option(
    "--include-soft-deleted/--exclude-soft-deleted",
    default=False,
    help="Include soft-deleted semanticModel / metric entities when discovering "
    "(default: false).",
)
@click.option("--dry-run", "-n", type=bool, is_flag=True, default=False)
@click.option(
...
def semantic_model_container(
    ...
    if not urns_to_process:
        urns_to_process = discover_all_semantic_model_urns(
            graph,
            platform=platform,
            platform_instance=platform_instance,
            include_soft_deleted=include_soft_deleted,
    ...
    if not urns_to_process:
        if used_discovery and not include_soft_deleted:
            soft_only = discover_all_semantic_model_urns(
                graph,
                platform=platform,
                platform_instance=platform_instance,
                only_soft_deleted=True,
            ...
            if soft_only:
                click.echo(
                    f"No live semanticModel entities found to migrate, but found "
                    f"{len(soft_only)} soft-deleted. Re-run with "
