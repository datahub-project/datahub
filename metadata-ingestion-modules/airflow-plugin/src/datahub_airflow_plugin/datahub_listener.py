...
- Airflow 2.x: Uses plugin_v2 with extractor-based lineage
- Airflow 3.x: Uses plugin_v2_airflow3 with native OpenLineage integration
...
if IS_AIRFLOW_3_OR_HIGHER:
    from datahub_airflow_plugin.plugin_v2_airflow3.datahub_listener import (  # type: ignore[assignment]
        DataHubListener,
        get_airflow_plugin_listener,
    )
else:
    from datahub_airflow_plugin.plugin_v2.datahub_listener import (  # type: ignore[assignment]
        DataHubListener,
        get_airflow_plugin_listener,
    )
