...
public class SchemaBasedMatcher implements EntityMatcher {
  /**
   * Find the entity among others that best match the original entity. If there are no entities that
   * pass the similarity score threshold, do not return result
   *
   ...
   */
  @Nullable
  public EntityMatchResult match(
      EntityDetails original, Collection<EntityDetails> others, double threshold) {
    if (original.getSchemaMetadata() == null
        || original.getSchemaMetadata().getFields().size() <= 2) {
      return null;
    }
    // Preprocess field paths for easy matching
    Set<String> fieldPaths =
        original.getSchemaMetadata().getFields().stream()
            .map(SchemaField::getFieldPath)
            .collect(Collectors.toSet());

    Map<String, String> simplifiedFieldPaths =
        fieldPaths.stream()
            .collect(
                Collectors.toMap(
                    this::processFieldPath,
                    Function.identity(),
    ...
    for (EntityDetails other : others) {
      ...
      int numDiffFields =
          Math.max(0, fieldPaths.size() - other.getSchemaMetadata().getFields().size());
