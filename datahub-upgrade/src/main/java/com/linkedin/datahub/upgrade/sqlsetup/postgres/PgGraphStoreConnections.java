...
/**
 * Opens JDBC connections for pgGraph SqlSetup. When {@code pool.url} is unset, falls back to the
 * upgrade/Ebean {@link Database}. When only the URL is overridden, blank username/password fall
 * back to the Ebean pool credentials.
 */
public final class PgGraphStoreConnections {

  private PgGraphStoreConnections() {}

  @Nonnull
  public static Connection open(
      @Nonnull PgGraphSetupOptions options,
  ...
  private static String[] ebeanCredentials(@Nonnull Database fallbackServer) throws SQLException {
    try {
      ...
      throw new SQLException("Unable to read Ebean credentials for pgGraph SqlSetup", e);
    }
  }
