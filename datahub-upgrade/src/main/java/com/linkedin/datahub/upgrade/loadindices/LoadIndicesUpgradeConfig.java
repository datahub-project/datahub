...
@ComponentScan(
    ...
    // Upgrade CLI is not an ingesting process — keep it out of the post-commit hook + retention
    // buffers and their drainers so a short-lived job never competes for the cluster-wide drain
    // lock or boots an embedded Hazelcast node. Hooks/retention (if any ingest happens here)
    // fall back to synchronous post-commit via NO_OP buffers.
    excludeFilters = {
