    ...
    return (context) -> {
      try {
        ...
        for (BaseElasticSearchComponentsFactory.BaseElasticSearchComponents cluster :
            clustersToConfigure()) {
          final String indexPrefix = cluster.getConfig().getIndex().getFinalPrefix();
          final String roleName = indexPrefix + "access";
          log.info(
              "Creating search user/role on cluster engine {} prefix={}",
