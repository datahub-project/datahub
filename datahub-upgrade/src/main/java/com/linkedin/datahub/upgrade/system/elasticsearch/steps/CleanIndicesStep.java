...
public class CleanIndicesStep implements UpgradeStep {
    ...
    public CleanIndicesStep(final RestHighLevelClient searchClient, final ElasticSearchConfiguration esConfig,
        ...
        return (context) -> {
            try {
                IndexUtils.getAllReindexConfigs(indexedServices)
                        .forEach(reindexConfig -> ESIndexBuilder.cleanIndex(searchClient, esConfig, reindexConfig));
            } catch (Exception e) {
