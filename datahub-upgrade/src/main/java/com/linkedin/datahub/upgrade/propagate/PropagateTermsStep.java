...
public class PropagateTermsStep implements UpgradeStep {
  ...
  private static final Urn PROPAGATION_ACTOR =
      UrnUtils.getUrn("urn:li:corpuser:__datahub_propagator");
  ...
  public Function<UpgradeContext, UpgradeStepResult> executable() {
    return (context) -> {
      ...
      Optional<String> thresholdStr =
          context.parsedArgs().getOrDefault("THRESHOLD", Optional.empty());
      ...
      context
          .report()
          .addLine(String.format("Found %d source entities", sourceSearchResults.getNumEntities()));
      ...
      do {
        ...
        int numAspectsProducedInBatch =
            processBatch(scrollResult, sourceEntityDetails, runId, threshold, allowedTerms);
      ...
      for (String criterion : criteriaStr) {
        ...
        if (keyValue.size() != 2) {
          throw new IllegalArgumentException(
              String.format(
                  "Invalid source filter %s. Needs to be of format key1-value1;key2-value2",
                  sourceFilter));
        }
  ...
  private void filterTerms(GlossaryTerms glossaryTerms, Optional<Set<Urn>> allowedTerms) {
    glossaryTerms
        ...
        .removeIf(
            term ->
                (term.getActor() != null && term.getActor().equals(PROPAGATION_ACTOR))
                    || (allowedTerms.isPresent() && !allowedTerms.get().contains(term.getUrn())));
  }

  private boolean validSource(EntityDetails entityDetails) {
    if (entityDetails.getSchemaMetadata() == null
        || entityDetails.getSchemaMetadata().getFields().size() <= 2) {
      return false;
    }
    boolean hasTerms =
        entityDetails.getSchemaMetadata().getFields().stream()
            .filter(SchemaField::hasGlossaryTerms)
            .flatMap(
                field ->
                    field.getGlossaryTerms().getTerms().stream()
    ...
    if (entityDetails.getEditableSchemaMetadata() == null
        || entityDetails.getEditableSchemaMetadata().getEditableSchemaFieldInfo().isEmpty()) {
      return false;
    }
    return entityDetails.getEditableSchemaMetadata().getEditableSchemaFieldInfo().stream()
        .filter(EditableSchemaFieldInfo::hasGlossaryTerms)
        .flatMap(
            field ->
                field.getGlossaryTerms().getTerms().stream().map(GlossaryTermAssociation::getUrn))
        .anyMatch(e -> true);
  }
  ...
  private int processBatch(
      ...
      @Nonnull Map<Urn, EntityDetails> sourceEntityDetails,
    ...
    log.info(
        "Among {} entities in this batch {} entities had a match, and produced {} editable schema aspects",
        batch.size(),
        numMatched,
        numProduced);
    return numProduced;
  ...
  private boolean processMatch(
      @Nonnull EntityDetails destinationEntity,
      @Nonnull EntityMatcher.EntityMatchResult entityMatchResult,
      @Nonnull String runId,
      Optional<Set<Urn>> allowedTerms) {
    log.debug(
        "Processing match for source {} destination {}",
        entityMatchResult.getMatchedEntity().getUrn(),
        destinationEntity.getUrn());
    if (entityMatchResult.getMatchingFields().isEmpty()) {
      log.debug(
          "No matching fields for source {} destination {}",
          entityMatchResult.getMatchedEntity().getUrn(),
          destinationEntity.getUrn());
      return false;
    }

    Map<String, Set<Urn>> sourceFieldGlossaryTerms =
        getGlossaryTermsForEachField(entityMatchResult.getMatchedEntity());
    Map<String, Set<Urn>> destinationFieldGlossaryTerms =
        getGlossaryTermsForEachField(destinationEntity);
    Set<String> destinationFieldsWithUserDefinedTerms =
        fieldsWithUserDefinedTerms(destinationEntity, allowedTerms);
    ...
    if (termsToPropagatePerFields.isEmpty()) {
      log.debug(
          "No terms to propagate for source {} destination {}",
          entityMatchResult.getMatchedEntity().getUrn(),
          destinationEntity.getUrn());
      return false;
    }
    ...
    AuditStamp auditStamp =
        new AuditStamp()
            .setActor(PROPAGATION_ACTOR)
            .setTime(System.currentTimeMillis())
            .setMessage(
                String.format("Propagated from %s", entityMatchResult.getMatchedEntity().getUrn()));
    EditableSchemaMetadata aspectToPush =
        buildSchemaMetadata(
            destinationEntity.getEditableSchemaMetadata(), termsToPropagatePerFields, auditStamp);
    produceEditableSchemaMetadataProposal(
        destinationEntity.getUrn(), aspectToPush, runId, auditStamp);
    return true;
  }
  ...
  private Map<String, Set<Urn>> getGlossaryTermsForEachField(EntityDetails entityDetails) {
    ...
    for (EditableSchemaFieldInfo field :
        entityDetails.getEditableSchemaMetadata().getEditableSchemaFieldInfo()) {
      if (field.hasGlossaryTerms() && !field.getGlossaryTerms().getTerms().isEmpty()) {
  ...
  private Set<String> fieldsWithUserDefinedTerms(
      EntityDetails entityDetails, Optional<Set<Urn>> allowedTerms) {
  ...
  private EditableSchemaMetadata buildSchemaMetadata(
      @Nullable EditableSchemaMetadata oldAspect,
    ...
    for (EditableSchemaFieldInfo field : originalFields) {
      if (termsToAddPerField.containsKey(field.getFieldPath())) {
        ...
        if (glossaryTerms == null) {
          ...
          termsToAdd.forEach(
              term -> glossaryTerms.getTerms().add(buildGlossaryTermAssociation(term)));
        }
    ...
    for (String fieldPath : termsToAddPerField.keySet()) {
      resultFields.add(
          buildSchemaFieldInfo(fieldPath, termsToAddPerField.get(fieldPath), auditStamp));
    }

    return new EditableSchemaMetadata()
        .setEditableSchemaFieldInfo(new EditableSchemaFieldInfoArray(resultFields));
  }

  private EditableSchemaFieldInfo buildSchemaFieldInfo(
      @Nonnull String fieldPath, @Nonnull Set<Urn> terms, @Nonnull AuditStamp auditStamp) {
    return new EditableSchemaFieldInfo()
        .setFieldPath(fieldPath)
        .setGlossaryTerms(buildGlossaryTerms(terms, auditStamp));
  }

  private GlossaryTerms buildGlossaryTerms(
      @Nonnull Set<Urn> terms, @Nonnull AuditStamp auditStamp) {
  ...
  private GlossaryTermAssociation buildGlossaryTermAssociation(Urn termUrn) {
    return new GlossaryTermAssociation()
        .setUrn(GlossaryTermUrn.createFromUrn(termUrn))
        .setActor(PROPAGATION_ACTOR);
  }

  private void produceEditableSchemaMetadataProposal(
      @Nonnull Urn urn,
