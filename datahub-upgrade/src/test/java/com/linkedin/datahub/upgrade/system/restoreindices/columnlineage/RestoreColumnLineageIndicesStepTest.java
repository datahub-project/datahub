...
public class RestoreColumnLineageIndicesStepTest {
  ...
  /**
   * The step must reindex the three aspects that back column-level lineage: dataset upstreamLineage
   * plus chart and dashboard inputFields. Verifying the scan targets guards against emitting the
   * wrong aspect/entity.
   */
  @Test
  public void testExecutableScansExpectedAspects() {
    ...
    verify(mockAspectDao, times(3)).streamAspectBatches(any(), captor.capture());
    List<RestoreIndicesArgs> scans = captor.getAllValues();
    assertTrue(
        RestoreIndicesTestHelpers.scanContains(
            scans, Constants.UPSTREAM_LINEAGE_ASPECT_NAME, "urn:li:dataset:%"));
    assertTrue(
        RestoreIndicesTestHelpers.scanContains(
            scans, Constants.INPUT_FIELDS_ASPECT_NAME, "urn:li:chart:%"));
    assertTrue(
        RestoreIndicesTestHelpers.scanContains(
            scans, Constants.INPUT_FIELDS_ASPECT_NAME, "urn:li:dashboard:%"));

    // No aspects streamed -> no MCLs produced.
    verify(mockEntityService, never())
        .alwaysProduceMCLAsync(
            any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any());
    // Completion request + result recorded.
  ...
  private RestoreColumnLineageIndicesStep newStep() {
    return new RestoreColumnLineageIndicesStep(mockEntityService, mockAspectDao);
  }
}
