...
public class FixEntityConsistencyStepTest {
  ...
  public void setup() {
    ...
    // Create checks for registry
    List<ConsistencyCheck> checks =
        List.of(
            new AssertionEntityUrnMissingCheck(mockEntityRegistry),
            new AssertionEntityTypeInvalidCheck(mockEntityRegistry),
            new AssertionEntityNotFoundCheck(mockEntityRegistry),
            new AssertionEntitySoftDeletedCheck(mockEntityRegistry),
            new AssertionMonitorMissingCheck(mockEntityRegistry),
            new MonitorAssertionsEmptyCheck(mockEntityRegistry),
            new MonitorAssertionsMultipleCheck(mockEntityRegistry),
            new MonitorEntityTypeInvalidCheck(mockEntityRegistry),
            new MonitorEntityNotFoundCheck(mockEntityRegistry),
            new MonitorEntitySoftDeletedCheck(mockEntityRegistry),
            new MonitorAssertionNotFoundCheck(mockEntityRegistry),
            new MonitorAssertionSoftDeletedCheck(mockEntityRegistry),
            new MonitorEntityUrnMismatchCheck(mockEntityRegistry));
  ...
  private void setupEntityRegistryMocks() {
    ...
    when(monitorEntitySpec.getAspectSpec(MONITOR_KEY_ASPECT_NAME)).thenReturn(monitorKeyAspectSpec);
    when(mockEntityRegistry.getEntitySpec(MONITOR_ENTITY_NAME)).thenReturn(monitorEntitySpec);
  }
  ...
  /** Test to verify monitors with multiple assertions are detected. */
  @Test
  public void testMonitorWithMultipleAssertionsDetected() throws Exception {
    // Use dryRun=true to test detection without actual changes
    FixEntityConsistencyStep step =
        new FixEntityConsistencyStep(
            mockOpContext,
            mockEntityService,
    ...
    Urn monitorUrn =
        UrnUtils.getUrn(
            "urn:li:monitor:(urn:li:dataset:(urn:li:dataPlatform:hive,test,PROD),test-id)");
    ...
    // Mock system metadata DAO scroll responses
    SearchResponse emptyResponse = createEmptySearchResponse();
    SearchResponse monitorResponse = createSearchResponseWithUrns(monitorUrn.toString());

    // First call for assertions returns empty, second for monitors returns monitor
    when(mockEsSystemMetadataDAO.scroll(
            any(BoolQueryBuilder.class), anyBoolean(), any(), any(), anyString(), anyInt()))
        .thenReturn(emptyResponse)
    ...
    // Create MonitorInfo with 2 assertions
    ...
    AssertionEvaluationSpecArray specs = new AssertionEvaluationSpecArray();
    specs.add(new AssertionEvaluationSpec().setAssertion(assertionUrn1));
    specs.add(new AssertionEvaluationSpec().setAssertion(assertionUrn2)); // Extra assertion
    assertionMonitor.setAssertions(specs);
    monitorInfo.setAssertionMonitor(assertionMonitor);

    // Create MonitorKey
    MonitorKey monitorKey = new MonitorKey();
    monitorKey.setEntity(datasetUrn);
    monitorKey.setId("test-id");

    EnvelopedAspectMap aspects = new EnvelopedAspectMap();
    aspects.put(
        MONITOR_INFO_ASPECT_NAME, new EnvelopedAspect().setValue(new Aspect(monitorInfo.data())));
    aspects.put(
        MONITOR_KEY_ASPECT_NAME, new EnvelopedAspect().setValue(new Aspect(monitorKey.data())));

    EntityResponse monitorEntityResponse = new EntityResponse();
    monitorEntityResponse.setUrn(monitorUrn);
    monitorEntityResponse.setEntityName(MONITOR_ENTITY_NAME);
    monitorEntityResponse.setAspects(aspects);
    ...
    when(mockEntityService.getEntitiesV2(
            ...
            any(Set.class),
            anyBoolean()))
        .thenReturn(Map.of(monitorUrn, monitorEntityResponse));
    ...
    when(mockEntityService.exists(any(OperationContext.class), any(Set.class), anyBoolean()))
        .thenReturn(Set.of(assertionUrn1, assertionUrn2, datasetUrn));
    when(mockEntityService.exists(any(OperationContext.class), any(Urn.class), anyBoolean()))
        .thenReturn(true);
    ...
    verify(mockEntityService, never()).deleteUrn(any(OperationContext.class), any(Urn.class));
  }
  ...
  /** Test to verify that invalid entity type detection works. */
  @Test
  public void testMonitorWithInvalidEntityTypeInKeyIsDetected() throws Exception {
    ...
    FixEntityConsistencyStep step =
        new FixEntityConsistencyStep(
            ...
            mockEntityService,
            consistencyService,
            createTestConfig(true, 10, 0, 0, false, List.of(MONITOR_ENTITY_NAME), null));
    ...
    // Create test URNs - monitor with invalid entity type (corpuser instead of dataset)
    Urn monitorUrn = UrnUtils.getUrn("urn:li:monitor:(urn:li:corpuser:test,test-id)");
    Urn invalidEntityUrn = UrnUtils.getUrn("urn:li:corpuser:test");
    Urn assertionUrn = UrnUtils.getUrn("urn:li:assertion:test-assertion");

    // Mock system metadata DAO responses
    SearchResponse emptyResponse = createEmptySearchResponse();
    SearchResponse monitorSearchResponse = createSearchResponseWithUrns(monitorUrn.toString());

    // Since we specify MONITOR_ENTITY_NAME only, first call is for monitors
    when(mockEsSystemMetadataDAO.scroll(
            any(BoolQueryBuilder.class), anyBoolean(), any(), any(), anyString(), anyInt()))
        .thenReturn(monitorSearchResponse) // First batch of monitors
    ...
    AssertionMonitor assertionMonitor = new AssertionMonitor();
    AssertionEvaluationSpecArray specs = new AssertionEvaluationSpecArray();
    specs.add(new AssertionEvaluationSpec().setAssertion(assertionUrn));
    assertionMonitor.setAssertions(specs);
    monitorInfo.setAssertionMonitor(assertionMonitor);

    // Create MonitorKey with invalid entity type (corpuser instead of dataset)
    MonitorKey monitorKey = new MonitorKey();
    monitorKey.setEntity(invalidEntityUrn);
    monitorKey.setId("test-id");
    ...
    // Return monitor response for any monitor request
    when(mockEntityService.getEntitiesV2(
            any(OperationContext.class),
            eq(MONITOR_ENTITY_NAME),
            any(Set.class),
    ...
    when(mockEntityService.exists(any(OperationContext.class), any(Set.class), anyBoolean()))
        .thenReturn(Set.of(assertionUrn));
    when(mockEntityService.exists(any(OperationContext.class), any(Urn.class), anyBoolean()))
        .thenReturn(false);
    ...
    // Verify the monitor was fetched and processed (detection happened)
  ...
  public void testAssertionWithInvalidEntityTypeIsDetected() throws Exception {
    ...
    assertionInfo.setEntityUrn(invalidEntityUrn);
