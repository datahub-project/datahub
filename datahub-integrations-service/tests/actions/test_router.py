...
def test_get_config_from_details_preserves_custom_source_config() -> None:
    ...
    with patch("datahub_integrations.actions.router.secret_stores", []):
        with patch("datahub_integrations.actions.router.DataHubSecretStore"):
            with patch("datahub_integrations.actions.router.EnvironmentSecretStore"):
                with patch(
                    ...
                    assert (
                        recipe["source"]["config"]["connection"]["schema_registry_url"]
                        == "http://custom:8081"
                    )
                    assert (
                    ...
                    assert (
                        "oauth_cb"
                        in recipe["source"]["config"]["connection"]["consumer_config"]
                    )
