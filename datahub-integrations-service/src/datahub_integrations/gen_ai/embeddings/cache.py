...
class CachedEmbeddings(BaseEmbeddings):
    ...
    def diff_stats(before: CacheStats, after: CacheStats) -> CacheStats:
        ...
        return {
            "cache_hits": int(after.get("cache_hits", 0))
            - int(before.get("cache_hits", 0)),
            "cache_misses": int(after.get("cache_misses", 0))
            - int(before.get("cache_misses", 0)),
        }
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        if texts_to_embed:
            ...
            for text, embedding, idx in zip(
                texts_to_embed, new_embeddings, text_indices, strict=False
            ):
