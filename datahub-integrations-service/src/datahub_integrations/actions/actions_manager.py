...
from datahub_integrations.actions.metric_utils import (
    ...
    get_kafka_lag_monitoring_env_vars,
...
from datahub_integrations.observability import otel_counter, otel_duration
from datahub_integrations.observability.action_metrics import (
    track_action_error,
    track_action_started,
    track_action_stopped,
...
class ActionsManager(contextlib.AbstractAsyncContextManager):
    ...
    @otel_duration(
        metric_name="actions_venv_setup_duration",
        description="Time to set up action virtual environment",
        label_extractors=make_venv_label_extractors(),
    )
    ...
    async def _run_pipeline(
        ...
        *,
        ...
        with anyio.CancelScope(shield=True) as cs:
            ...
            metadata = extract_action_metadata(action_spec)
            action_error_occurred = False
            ...
            try:
                ...
                # Track action started (LIVE stage)
                track_action_started(
                    action_urn=metadata["action_urn"],
                    action_type=metadata["action_type"],
                    stage="LIVE",
                    executor_id=action_spec.action_run.executor_id,
                )

                # TODO: Add a watchdog service that restarts the pipeline if it dies?
                # Get Kafka lag monitoring configuration
                observability_config = ObservabilityConfig()
                kafka_lag_env_vars = get_kafka_lag_monitoring_env_vars(
                ...
                # Track action stopped
                track_action_stopped(
                    action_urn=metadata["action_urn"],
                    action_type=metadata["action_type"],
                    stage="LIVE",
                    executor_id=action_spec.action_run.executor_id,
                    state="failed" if action_error_occurred else "stopped",
                )
    ...
    @otel_duration(
        metric_name="actions_execution_duration",
        description="Action execution duration",
        label_extractors=make_stage_label_extractors(),
    )
    @otel_counter(
        metric_name="actions_execution_total",
