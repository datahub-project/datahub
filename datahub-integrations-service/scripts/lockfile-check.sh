...
uv pip compile --universal --python-version 3.11 -q -o ${TMPDIR}/requirements.txt requirements-local.in
uv pip compile --universal --python-version 3.11 -q -o ${TMPDIR}/requirements-dev.txt --extra dev pyproject.toml requirements-local.in requirements.txt
