 ...
 * if any number on the left hand side is less than any number on the right side.
 */
@Slf4j
public class LessThanEvaluator extends BaseOperatorEvaluator {
  ...
  public Object evaluate(ResolvedOperands resolvedOperands) throws InvalidOperandException {
    ...
    return operand1Values.stream().anyMatch(lhs -> operand2Values.stream().anyMatch(rhs -> lessThan(lhs, rhs)));
  }
  ...
  private boolean lessThan(String lhs, String rhs) {
    try {
      ...
      log.warn("Failed to evaluate Less Than (<) Operator. Input values "
  ...
  private Set<String> toStringSet(ResolvedOperand operand) {
    if (operand.getExpression().getValue() instanceof String) {
      ...
      throw new IllegalArgumentException(String.format(
          "Failed to evaluate LessThan operator against operand of type %s. Expected string or string list.",
