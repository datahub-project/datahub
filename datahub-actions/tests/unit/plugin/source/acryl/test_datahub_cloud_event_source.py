...
# ---------------------------------------------------------------------------
# Avro JSON union wrappers: from_obj handles them natively (no helpers needed)
# ---------------------------------------------------------------------------


def test_from_obj_handles_avro_json_unions_for_mcl() -> None:
    """avro-gen's from_obj resolves Avro JSON union wrappers schema-aware.

    The Avro JsonEncoder wraps optional (union) fields as {"typeName": value}.
    from_obj (without tuples=True) matches the single-key dict against the
    union's declared branches and unwraps automatically.
    """
    avro_wrapped = {
        "entityType": "dataset",
    ...
    assert (
        event.entityUrn == "urn:li:dataset:(urn:li:dataPlatform:hive,test,PROD)"
    )


def test_handle_pe_with_avro_wrapped_event() -> None:
    """Platform events with Avro union wrappers parse correctly via PlatformEventClass.from_obj.

    PlatformEvent has no union fields (header, name, payload are all required),
    so the Avro JsonEncoder does not wrap them.  Only the inner GenericAspect
    fields (which are inside a union) get wrapped.
    """
    avro_wrapped = {
        "header": {
            "timestampMillis": 1737170481713,
        },
